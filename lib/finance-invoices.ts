...
export function invoiceKey(id: string) {
  return `${INVOICE_PREFIX}${id}`;
}

export function calculateInvoiceLines(
  lines: Array<Pick<InvoiceLine, "description" | "quantity" | "unitPrice" | "taxRate"> & { id?: string }>,
) {
  const normalized: InvoiceLine[] = lines
    ...
    .map((l) => {
      ...
      return {
        id: l.id || randomUUID(),
        description: l.description.trim().slice(0, 240),
        quantity,
        unitPrice,
        taxRate,
        lineTotal: round(base + tax),
      };
    });
  const subtotal = round(normalized.reduce((s, l) => s + round(l.quantity * l.unitPrice), 0));
  const taxTotal = round(
    normalized.reduce((s, l) => s + round((l.quantity * l.unitPrice * l.taxRate) / 100), 0),
  );
...
function parseInvoice(value: string): FinanceInvoice | null {
  try {
    ...
    if (!invoice?.id || !invoice.invoiceNumber || !invoice.clientId || !Array.isArray(invoice.lines))
      return null;
    return invoice;
  } catch {
    return null;
...
export async function invoicePaymentFacts(invoice: FinanceInvoice) {
  ...
  const confirmedPayments = rows
    ...
    .map((p) => ({
      ...
      amount: appliedAmount(p),
      currency: p.currency,
      paidAt: p.paidAt,
    }));
  const confirmed = round(
    confirmedPayments.reduce((s, p) => s + (p.currency === invoice.currency ? p.amount : 0), 0),
  );
  const pending = round(
    rows
...
export async function getAccountsReceivableSnapshot(now = new Date()) {
  ...
  for (const row of rows) {
    const cur = byCurrency.get(row.invoice.currency) || {
      current: 0,
      d1_30: 0,
      d31_60: 0,
      d61_90: 0,
