...
function collectFields(view: EditorView) {
  ...
  view.state.doc.descendants((node, pos) => {
    if (node.type.name === "junBlock" && node.attrs.kind === "field")
      fields.push({ pos, attrs: { ...node.attrs } });
  });
...
function button(label: string, title: string, click: () => void) {
  ...
  b.title = title;
  b.style.cssText =
    "border:1px solid #475569;background:#1e293b;color:#e2e8f0;border-radius:6px;padding:5px 8px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap";
...
function openCompanyVariables(view: EditorView) {
  ...
  overlay.style.cssText =
    "position:fixed;inset:0;z-index:9999;background:rgba(15,23,42,.72);display:flex;align-items:center;justify-content:center;padding:20px";
  const card = document.createElement("div");
  card.style.cssText =
    "width:min(760px,96vw);max-height:85vh;overflow:auto;background:#fff;border-radius:14px;padding:18px;box-shadow:0 24px 70px rgba(0,0,0,.35);font-family:system-ui,sans-serif";
  const heading = document.createElement("div");
  heading.innerHTML =
    '<div style="font-weight:750;color:#0f172a">Company variables</div><div style="font-size:12px;color:#64748b;margin-top:3px">Insert a variable. It is replaced with the current Settings value when an official PDF is generated.</div>';
  const grid = document.createElement("div");
  grid.style.cssText =
    "display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:8px;margin:14px 0";
  COMPANY_TOKENS.forEach(({ label, token }) => {
    const item = document.createElement("button");
    item.type = "button";
    item.style.cssText =
      "text-align:left;border:1px solid #e2e8f0;border-radius:9px;padding:9px 10px;background:#fff;cursor:pointer";
    item.innerHTML =
      '<div style="font-size:12px;font-weight:700;color:#0f172a"></div><div style="font-size:11px;color:#64748b;margin-top:2px"></div>';
    (item.children[0] as HTMLElement).textContent = label;
    (item.children[1] as HTMLElement).textContent = token;
    item.addEventListener("click", () => {
      insertCompanyToken(view, token);
      overlay.remove();
    });
    grid.append(item);
  });
  ...
  const close = button("Close", "Close company variables", () => {
    overlay.remove();
    view.focus();
  });
  footer.append(close);
...
function openFieldOrder(view: EditorView) {
  ...
  if (!source.length) {
    window.alert("No fillable fields in this document.");
    return;
  }
  ...
  const card = document.createElement("div");
  card.style.cssText =
    "width:min(720px,96vw);max-height:85vh;overflow:auto;background:#fff;border-radius:14px;padding:18px;box-shadow:0 24px 70px rgba(0,0,0,.35);font-family:system-ui,sans-serif";
  const heading = document.createElement("div");
  heading.innerHTML =
    '<div style="font-weight:750;color:#0f172a">Field order</div><div style="font-size:12px;color:#64748b;margin-top:3px">This order controls Preview / Fill wizard and keyboard navigation.</div>';
  ...
  const render = () => {
    ...
    items.forEach((item, index) => {
      ...
      row.style.cssText =
        "display:grid;grid-template-columns:36px 1fr auto;align-items:center;gap:8px;border:1px solid #e2e8f0;border-radius:8px;padding:8px 10px";
      const n = document.createElement("strong");
      n.textContent = String(index + 1);
      n.style.color = "#2563eb";
  ...
  const cancel = button("Cancel", "Close without changes", () => {
    overlay.remove();
    view.focus();
  });
  const save = button("Save order", "Apply field order", () => {
    ...
    items.forEach((item, index) => {
      tr = tr.setNodeMarkup(item.pos, undefined, { ...item.attrs, order: index + 1 });
    });
...
function buildToolbar(view: EditorView) {
  ...
  bar.style.cssText =
    "display:flex;flex-wrap:wrap;align-items:center;gap:5px;padding:8px 10px;background:#0b1220;border-left:1px solid #334155;border-right:1px solid #334155;border-bottom:1px solid #334155;color:white";
  const label = document.createElement("span");
  label.textContent = "Smart fields";
  label.style.cssText = "font-size:11px;font-weight:700;color:#94a3b8;margin-right:4px";
  bar.append(label);
  SMART_FIELDS.forEach((preset) =>
    bar.append(button(preset.label, `Add ${preset.label} smart field`, () => insertSmartField(view, preset))),
  );
  const sep = document.createElement("span");
  sep.style.cssText = "width:1px;height:22px;background:#334155;margin:0 3px";
  bar.append(sep);
  bar.append(
    button("Company variables", "Insert company information from Settings", () => openCompanyVariables(view)),
  );
