...
const BLOCK_STYLE: Record<string, string> = {
  textbox:
    "border:1px solid #94a3b8;background:#ffffff;padding:12px 14px;border-radius:6px;margin:10px 0;white-space:pre-wrap",
  sticky:
    "border:1px solid #f59e0b;background:#fffbeb;padding:12px 14px;border-radius:6px;margin:10px 0;white-space:pre-wrap",
  comment:
    "border-left:4px solid #3b82f6;background:#eff6ff;padding:10px 14px;margin:10px 0;white-space:pre-wrap",
...
function openDrawPad(view: EditorView) {
  ...
  card.style.cssText =
    "width:min(760px,96vw);background:#fff;border-radius:14px;padding:18px;box-shadow:0 24px 70px rgba(0,0,0,.35);font-family:system-ui,sans-serif";
  ...
  canvas.style.cssText =
    "width:100%;height:210px;border:1px solid #cbd5e1;border-radius:8px;background:white;touch-action:none;display:block";
  ...
  insert.onclick = () => {
    if (!hasInk) {
      window.alert("Draw something before inserting.");
      return;
    }
    const src = canvas.toDataURL("image/png");
    if (src.length > 700_000) {
      window.alert("Drawing is too large. Clear it and use fewer strokes.");
      return;
    }
...
function createEditorToolButton(label: string, title: string, onClick: () => void): HTMLButtonElement {
  ...
  button.style.cssText =
    "border:1px solid #334155;background:#0f172a;color:#e2e8f0;border-radius:6px;padding:5px 8px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap";
...
function buildFillToolbar(view: EditorView): HTMLElement {
  ...
  bar.style.cssText =
    "display:flex;flex-wrap:wrap;align-items:center;gap:5px;padding:8px 10px;background:#111827;border-left:1px solid #334155;border-right:1px solid #334155;border-bottom:1px solid #334155;color:white";
  ...
  for (const fieldType of FIELD_TYPES) {
    ...
    bar.append(
      createEditorToolButton(short, `Add ${fieldType.toLowerCase()} fillable field`, () =>
        insertField(view, fieldType),
      ),
    );
  }
  return bar;
    ...
    if (kind === "field") {
      ...
      return [
        "div",
        {
          ...
          style:
            "border:1.5px dashed #2563eb;background:#eff6ff;color:#1e3a8a;padding:10px 12px;border-radius:6px;margin:10px 0;font-family:system-ui,sans-serif;font-size:12px;font-weight:600",
        },
        display,
      ];
    }
...
export const JunImage = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    const isDraw =
      typeof HTMLAttributes.src === "string" && HTMLAttributes.src.startsWith("data:image/png;base64,");
    return [
      "img",
      mergeAttributes(HTMLAttributes, {
        "data-jun-image": "true",
