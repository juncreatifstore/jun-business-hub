...
export type ClientAccountEntry = { id:string; date:Date; type:"PAYMENT"|"REFUND"|"COMMISSION"|"WITHDRAWAL"; reference:string; description:string; status:string; currency:string; credit:number; debit:number; sourceHref:string|null };
export type ClientCurrencyBalance = { currency:string; confirmedFunds:number; commissions:number; activeRefunds:number; pendingRefunds:number; refundsPaid:number; partnerWithdrawals:number; available:number };
...
export async function getClientFinancialAccount(clientId:string){
  const [payments,refunds,commissions,withdrawals,profile]=await Promise.all([
  ...
  for(const b of balances.values())b.available=round(b.confirmedFunds+b.commissions-b.activeRefunds-b.partnerWithdrawals);
  ...
  return{profile,commissions,withdrawals,balances:[...balances.values()].sort((a,b)=>a.currency.localeCompare(b.currency)),entries:statementEntries};
}
