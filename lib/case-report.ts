...
export async function getCaseReport(caseId: string) {
  const [intel, closure] = await Promise.all([getCaseIntelligence(caseId), getCaseClosureSnapshot(caseId)]);
    ...
    prisma.case.findUnique({
      ...
      include: {
        ...
        files: {
          ...
          select: { id: true, name: true, category: true, mimeType: true, createdAt: true },
        },
        activities: {
          orderBy: { createdAt: "desc" },
          take: 100,
  ...
  const executiveSummary = [
    ...
    readiness.ready
      ? "No hard closure blocker is currently detected."
      : `${Object.values(readiness.hardBlockers).reduce((n, v) => n + v.length, 0)} hard closure blocker item(s) remain.`,
  ].join(" ");
  return {
