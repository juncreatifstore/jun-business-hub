...
export async function getCaseOperations(caseId: string): Promise<CaseOperationsState> {
  ...
  try {
    ...
    return { caseId, milestones: [] };
  }
}

export async function saveCaseOperations(state: CaseOperationsState) {
  await prisma.appSetting.upsert({
    where: { key: key(state.caseId) },
    create: { key: key(state.caseId), value: JSON.stringify(state) },
    update: { value: JSON.stringify(state) },
  });
}

export function caseOperationFacts(
  ...
  const totalTrackable =
    milestones.filter((m) => m.status !== "CANCELLED").length +
