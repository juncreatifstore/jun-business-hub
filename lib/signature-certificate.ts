...
function shortUa(ua: string | null | undefined) {
  return ua ? ua.replace(/\s+/g, " ").slice(0, 88) : "—";
}
...
export async function buildSignatureCertificate(
  ...
  const audits = await prisma.auditLog.findMany({
    where: { resourceType: "SignatureRequest", resourceId: requestId },
  ...
  const rows: [string, string][] = [
    ...
    [
      "Retention until",
      meta.retentionUntil ? fmt(meta.retentionUntil) : "Policy default — see JUN retention policy",
    ],
  ];
  let y = 642;
  ...
  for (const r of recipients) {
    ...
    const signingAudit = [...signerAudits]
      .reverse()
      .find(
        (a) => a.action === "JUN_NATIVE_SIGNATURE_COMPLETED" || a.action === "JUN_NATIVE_SIGNER_COMPLETED",
      );
    const verifiedAudit = [...signerAudits]
      .reverse()
      .find((a) => a.action === "JUN_NATIVE_SIGNER_EMAIL_VERIFIED");
    const verifiedAfter =
      verifiedAudit?.after && typeof verifiedAudit.after === "object" && !Array.isArray(verifiedAudit.after)
        ? (verifiedAudit.after as Record<string, unknown>)
        : null;
    ...
    page.drawText(`${r.email}  ·  ${r.role ?? "SIGNER"}`, {
      x: 74,
      y: y - 15,
      size: 8.5,
      font: regular,
    ...
    page.drawText(`Email verified: ${fmt(r.verifiedAt)}  ·  Signed: ${fmt(r.signedAt)}`, {
      x: 74,
      y: y - 29,
      size: 8.2,
      font: regular,
    ...
    page.drawText(`Method: ${r.signatureMethod ?? "TYPE"}  ·  Session: ${String(sessionId).slice(0, 32)}`, {
      x: 74,
      y: y - 43,
      size: 7.8,
      font: mono,
    ...
    page.drawText(`Network: ${maskIp(signingAudit?.ip)}  ·  ${shortUa(signingAudit?.userAgent)}`, {
      x: 74,
      y: y - 57,
      size: 7.2,
      font: regular,
    ...
    if (r.signatureImageHash) {
      page.drawText(`Drawn signature SHA-256: ${r.signatureImageHash.slice(0, 40)}…`, {
        x: 74,
        y: y - 71,
        size: 7.2,
        font: mono,
  ...
  page.drawText(
    "This certificate records identity verification, network audit evidence, signature method, session evidence and the cryptographic fingerprint of the archived signed PDF.",
    { x: 54, y: 74, size: 8, font: regular, color: muted, maxWidth: 500 },
  );
