...
function normalize(parsed: Partial<PaymentCoreMeta>): PaymentCoreMeta {
  return {
    expectedAmount:
      typeof parsed.expectedAmount === "number" && Number.isFinite(parsed.expectedAmount)
        ? parsed.expectedAmount
        : null,
    serviceLabel:
      typeof parsed.serviceLabel === "string" && parsed.serviceLabel.trim()
        ? parsed.serviceLabel.trim()
        : null,
    providerRef:
      typeof parsed.providerRef === "string" && parsed.providerRef.trim() ? parsed.providerRef.trim() : null,
    accountId: typeof parsed.accountId === "string" && parsed.accountId ? parsed.accountId : null,
    accountLabel:
      typeof parsed.accountLabel === "string" && parsed.accountLabel.trim()
        ? parsed.accountLabel.trim()
        : null,
    accountDescriptor:
      typeof parsed.accountDescriptor === "string" && parsed.accountDescriptor.trim()
        ? parsed.accountDescriptor.trim()
        : null,
    accountMethod:
      typeof parsed.accountMethod === "string" && parsed.accountMethod ? parsed.accountMethod : null,
    accountCurrency:
      typeof parsed.accountCurrency === "string" && parsed.accountCurrency ? parsed.accountCurrency : null,
    feeAmount:
      typeof parsed.feeAmount === "number" && Number.isFinite(parsed.feeAmount) ? parsed.feeAmount : null,
  };
}

export async function getPaymentCoreMeta(paymentId: string): Promise<PaymentCoreMeta> {
  const row = await prisma.appSetting.findUnique({
    where: { key: `${PREFIX}${paymentId}` },
    select: { value: true },
  });
  if (!row) return EMPTY_PAYMENT_META;
  try {
    return normalize(JSON.parse(row.value) as Partial<PaymentCoreMeta>);
  } catch {
...
export async function getPaymentCoreMetaMap(paymentIds: string[]) {
  ...
  for (const row of rows) {
    ...
    try {
      map.set(id, normalize(JSON.parse(row.value) as Partial<PaymentCoreMeta>));
    } catch {}
  }
  return map;
}
