...
export async function listDriveAutomationRules() {
  const rows = await prisma.appSetting.findMany({
    where: { key: { startsWith: DRIVE_AUTOMATION_RULE_PREFIX } },
    orderBy: { updatedAt: "desc" },
  });
  return rows
    .map((r) => parseJson<DriveAutomationRule>(r.value))
    .filter((r): r is DriveAutomationRule => Boolean(r));
}

export async function getDriveAutomationProposals(fileId?: string) {
  const rows = await prisma.appSetting.findMany({
    where: { key: { startsWith: DRIVE_AUTOMATION_PROPOSAL_PREFIX } },
    orderBy: { updatedAt: "desc" },
  });
  const proposals = rows
    .map((r) => parseJson<DriveAutomationProposal>(r.value))
    .filter((r): r is DriveAutomationProposal => Boolean(r));
  return fileId ? proposals.filter((p) => p.fileId === fileId) : proposals;
}

export async function upsertAutomationProposal(
  input: Omit<DriveAutomationProposal, "id" | "createdAt" | "status">,
) {
  const existing = (await getDriveAutomationProposals(input.fileId)).find(
    (p) => p.type === input.type && p.value === input.value && p.status === "PENDING",
  );
  if (existing) return existing;
  const id = crypto.randomUUID();
  const proposal: DriveAutomationProposal = {
    ...input,
    id,
    status: "PENDING",
  ...
  await prisma.appSetting.create({
    data: { key: `${DRIVE_AUTOMATION_PROPOSAL_PREFIX}${id}`, value: JSON.stringify(proposal) },
  });
  return proposal;
}

function ruleMatches(
  rule: DriveAutomationRule,
  file: { name: string; category: string; mimeType: string; folderId: string | null },
) {
  ...
  if (
    ...
    !file.name.toLowerCase().includes(rule.match.filenameContains.toLowerCase())
  )
    return false;
  if (rule.match.mimePrefix && !file.mimeType.toLowerCase().startsWith(rule.match.mimePrefix.toLowerCase()))
    return false;
...
function futureDateFromIntelligence(values?: string[]) {
  ...
  const candidates = values
    ...
    .filter((v) => Number.isFinite(v.time) && v.time > now)
...
export async function processDriveAutomation(fileId: string, actorUserId: string, suppliedBuffer?: Buffer) {
  ...
  for (const rule of matched) {
    ...
    if (a.addTags?.length) {
      ...
      await prisma.appSetting.upsert({
        ...
        create: { key, value: JSON.stringify(tags) },
      });
    }
    ...
    if (a.moveToFolderId !== undefined && a.moveToFolderId !== file.folderId) {
      const targetOk =
        ...
        Boolean(
          await prisma.folder.findFirst({
            where: { id: a.moveToFolderId, isVault: false },
            select: { id: true },
          }),
        );
      if (targetOk && a.requireApproval) {
        const targetLabel =
          a.moveToFolderId === null
            ? "My Drive root"
            : ((await prisma.folder.findUnique({ where: { id: a.moveToFolderId }, select: { name: true } }))
                ?.name ?? "folder");
        await upsertAutomationProposal({
          fileId: file.id,
          ruleId: rule.id,
          type: "MOVE",
          value: a.moveToFolderId ?? "__ROOT__",
          label: `Move file to ${targetLabel}`,
        });
        proposalCount++;
      } else if (targetOk) {
    ...
    if (
      a.suggestCategory &&
      DRIVE_CATEGORIES.includes(a.suggestCategory as (typeof DRIVE_CATEGORIES)[number]) &&
      a.suggestCategory !== file.category
    ) {
      await upsertAutomationProposal({
        fileId: file.id,
        ruleId: rule.id,
        type: "CATEGORY",
        value: a.suggestCategory,
        label: `Set category to ${a.suggestCategory}`,
      });
      proposalCount++;
    }
    ...
    if (a.createTask) {
      ...
      const duplicate = await prisma.task.findFirst({
        where: { description: { contains: marker }, status: { in: ["TODO", "IN_PROGRESS"] } },
        select: { id: true },
      });
      if (!duplicate) {
        await prisma.task.create({
          data: {
            title: `Review ${file.name}`.slice(0, 200),
            description: `${marker}\nAutomation rule: ${rule.name}`,
            caseId: file.caseId,
            clientId: file.clientId,
            assigneeId: a.taskAssigneeId || null,
    ...
    if (a.notifyUserIds?.length) {
      for (const userId of [...new Set(a.notifyUserIds)]) {
        const user = await prisma.user.findFirst({
          where: { id: userId, status: "ACTIVE", role: { not: "CLIENT" } },
          select: { id: true },
        });
        if (user)
          await prisma.notification.create({
            data: {
              userId,
              type: "DRIVE_AUTOMATION",
              title: `Drive rule: ${rule.name}`,
              body: `${file.name} matched this automation rule.`,
            },
          });
      }
    }
  ...
  if (!file.clientId) {
    ...
    const hit = clients.find(
      (c) =>
        text.includes(c.internalId.toLowerCase()) ||
        (`${c.firstName} ${c.lastName}`.trim().length >= 5 &&
          text.includes(`${c.firstName} ${c.lastName}`.toLowerCase())),
    );
    if (hit) {
      await upsertAutomationProposal({
        fileId: file.id,
        ...
        type: "LINK_CLIENT",
        value: hit.id,
        label: `Link to ${hit.firstName} ${hit.lastName} (${hit.internalId})`,
      });
      proposalCount++;
    }
  }
  if (!file.caseId) {
    ...
    if (hit) {
      await upsertAutomationProposal({
        ...
        type: "LINK_CASE",
        value: hit.id,
        label: `Link to ${hit.caseNumber} — ${hit.title}`,
      });
      proposalCount++;
    }
  }
  ...
  if (
    intelligence?.suggestedCategory &&
    DRIVE_CATEGORIES.includes(intelligence.suggestedCategory as (typeof DRIVE_CATEGORIES)[number]) &&
    intelligence.suggestedCategory !== file.category
  ) {
    await upsertAutomationProposal({
      fileId: file.id,
      ruleId: null,
      type: "CATEGORY",
      value: intelligence.suggestedCategory,
      label: `AI suggests category ${intelligence.suggestedCategory}`,
    });
    proposalCount++;
  }
  ...
  if (future) {
    await upsertAutomationProposal({
      ...
      ruleId: null,
      type: "EXPIRY",
      value: new Date(future.time).toISOString(),
      label: `Possible expiration / important date: ${future.raw}`,
    });
    proposalCount++;
  }
...
export async function getExpiringDriveFiles(days = 30) {
  ...
  const items = rows
    .map((r) => ({
      fileId: r.key.slice(DRIVE_EXPIRY_PREFIX.length),
      expiresAt: r.value,
      time: Date.parse(r.value),
    }))
    .filter((r) => Number.isFinite(r.time) && r.time <= max)
    .sort((a, b) => a.time - b.time);
  const files = items.length
    ? await prisma.file.findMany({
        where: { id: { in: items.map((i) => i.fileId) }, isVault: false, archivedAt: null },
        select: { id: true, name: true, category: true, clientId: true, caseId: true },
      })
    : [];
