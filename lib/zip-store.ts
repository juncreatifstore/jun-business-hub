...
export function buildStoredZip(entries: Array<{ name: string; data: Buffer; modifiedAt?: Date }>) {
  const localParts: Buffer[] = [];
  const centralParts: Buffer[] = [];
  let offset = 0;

  entries.forEach((entry, index) => {
    const name = Buffer.from(safeZipName(entry.name, index), "utf8");
    const data = entry.data;
    const crc = crc32(data);
    const dt = dosDateTime(entry.modifiedAt ?? new Date());
