...
export type GmailMailboxCache = {
  ...
  >;
  categoryByThreadId: Record<string, GmailCategory>;
};
function key(accountId: string) {
  return `${PREFIX}${accountId}`;
}
export async function getGmailMailboxCache(accountId: string): Promise<GmailMailboxCache | null> {
  const row = await prisma.appSetting.findUnique({ where: { key: key(accountId) }, select: { value: true } });
  if (!row) return null;
  try {
    return JSON.parse(row.value) as GmailMailboxCache;
  } catch {
    return null;
  }
}
export async function getGmailMailboxCacheMap(accountIds: string[]) {
  if (!accountIds.length) return new Map<string, GmailMailboxCache>();
  const rows = await prisma.appSetting.findMany({
    where: { key: { in: accountIds.map(key) } },
    select: { key: true, value: true },
  });
  const out = new Map<string, GmailMailboxCache>();
  for (const row of rows) {
    try {
      const v = JSON.parse(row.value) as GmailMailboxCache;
      if (v?.accountId) out.set(v.accountId, v);
    } catch {}
  }
  return out;
}
export async function refreshGmailMailboxCache(accountId: string, max = DEFAULT_LIMIT) {
  const limit = Math.max(1, Math.min(max, MAX_LIMIT));
  ...
  for (const t of threads) {
    ...
    categoryByThreadId[id] = primary.has(id)
      ? "PRIMARY"
      : promotions.has(id)
        ? "PROMOTIONS"
        : social.has(id)
  ...
  await prisma.appSetting.upsert({
    where: { key: key(accountId) },
    create: { key: key(accountId), value: JSON.stringify(value) },
