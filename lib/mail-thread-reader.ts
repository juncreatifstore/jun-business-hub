...
export type MailConversationAttachment = {
...
export type MailConversationMessage = {
  ...
  isUnread: boolean;
  labels: string[];
  attachments: MailConversationAttachment[];
};
...
function shouldCollapseUrl(raw: string) {
  ...
  try {
    ...
    return (
      /(^|\.)(clicks?|track|tracking|links?|redirect|email|mail)\./.test(host) ||
      /(?:utm_|campaign|redirect|tracking|trk=|clickid|gclid|fbclid|mc_cid|mc_eid|unsubscribe|encoded|%3a%2f%2f)/i.test(
        path,
      ) ||
      u.search.length > 90
    );
...
function compactUrls(value: string) {
  ...
  let text = value.replace(urlRe, (raw) =>
    shouldCollapseUrl(raw) ? "[link]" : raw.replace(/[),.;]+$/g, ""),
  );
  text = text
...
function cleanReadableText(value: string) {
  return normalizeWhitespace(compactUrls(value));
}
...
function looksLikeTechnicalMarkup(value: string) {
  ...
  const signals: [RegExp, number][] = [
    [/\bfont-family\s*:/gi, 3],
    [/!important\b/gi, 3],
    [/\b(?:padding|margin|display|background|border|line-height|font-size|text-decoration)\s*:/gi, 2],
    [/(?:^|\n)\s*[^\n{}]{0,100}\{[^{}]{0,500}\}/gm, 3],
    [/<(?:style|table|td|div|span|body|html)\b/gi, 2],
    [/&(?:nbsp|zwnj|zwj|#8204|#x200c);/gi, 1],
    [/\bmso-[a-z-]+\s*:/gi, 3],
    [/\b@media\b/gi, 3],
    [/\bwidth\s*:\s*\d+(?:px|%)/gi, 2],
  ];
  for (const [re, weight] of signals) {
    const matches = sample.match(re)?.length ?? 0;
...
function cleanPollutedPlainText(value: string) {
  ...
  text = text
    ...
    .replace(
      /\b(?:font-family|font-size|line-height|text-decoration|background(?:-color)?|padding|margin|display|border(?:-[a-z]+)?|color|width|height)\s*:[^;\n}]+;?/gi,
      " ",
    )
...
function textParts(part: GmailPart | undefined, mimeType: "text/plain" | "text/html", out: string[] = []) {
  ...
  if (part.mimeType === mimeType && part.body?.data && !part.filename) {
    const decoded = decodeQuotedPrintable(decode(part.body.data));
...
function bodyFrom(part?: GmailPart): { body: string; htmlBody: string | null } {
  const plainRaw = textParts(part, "text/plain")
    .map((v) => v.trim())
    .filter(Boolean);
  const htmlRaw = textParts(part, "text/html")
    .map((v) => v.trim())
    .filter(Boolean);
  ...
  let body = "";
  if (plainJoined && !looksLikeTechnicalMarkup(plainJoined))
    body = cleanReadableText(decodeEntities(plainJoined));
...
function attachments(part?: GmailPart, out: MailConversationAttachment[] = []) {
  if (!part) return out;
  if (part.filename)
    out.push({
...
export async function getGmailThreadIdsForQuery(
  accountId: string,
  query: string,
  max = 3000,
): Promise<Set<string>> {
  ...
  while (ids.size < max) {
    ...
    const url = `https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=${pageSize}&q=${encodeURIComponent(query)}${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ""}`;
    const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" });
    if (!res.ok) break;
    const data = (await res.json()) as { messages?: { threadId: string }[]; nextPageToken?: string };
...
export async function getUnreadGmailThreadIds(accountId: string, max = 10000): Promise<Set<string>> {
  return getGmailThreadIdsForQuery(accountId, "is:unread", max);
}
export async function getMailConversation(
  ...
  const res = await fetch(
    `https://gmail.googleapis.com/gmail/v1/users/me/threads/${encodeURIComponent(gmailThreadId)}?format=full`,
  ...
  return (thread.messages ?? [])
    .map((m) => {
      const rendered = bodyFrom(m.payload);
      return {
        id: m.id,
        from: header(m, "From"),
        to: emails(header(m, "To")),
        cc: emails(header(m, "Cc")),
        subject: header(m, "Subject") || "(no subject)",
        date: m.internalDate ? new Date(Number(m.internalDate)) : new Date(),
        body: rendered.body.slice(0, 50000),
