...
function decodeQuotedPrintable(value:string){if(!/(?:=[0-9A-F]{2}|=\r?\n)/i.test(value))return value;return value.replace(/=\r?\n/g,"").replace(/=([0-9A-F]{2})/gi,(_,hex)=>String.fromCharCode(parseInt(hex,16)));}
function decodeEntities(value:string){const named:Record<string,string>={nbsp:" ",amp:"&",lt:"<",gt:">",quot:'"',apos:"'",hellip:"…",middot:"·",copy:"©",reg:"®",zwnj:"",zwj:"",thinsp:" ",ensp:" ",emsp:" ",ndash:"–",mdash:"—",laquo:"«",raquo:"»"};return value.replace(/&(#x?[0-9a-f]+|[a-z]+);/gi,(all,entity:string)=>{if(entity[0]==="#"){const hex=entity[1]?.toLowerCase()==="x";const n=parseInt(entity.slice(hex?2:1),hex?16:10);return Number.isFinite(n)?String.fromCodePoint(n):all;}return named[entity.toLowerCase()]??all;});}
function normalizeWhitespace(value:string){return value.replace(/[\u200B-\u200D\uFEFF]/g,"").replace(/\r/g,"").replace(/[ \t]+\n/g,"\n").replace(/\n[ \t]+/g,"\n").replace(/[ \t]{2,}/g," ").replace(/\n{3,}/g,"\n\n").trim();}
function shouldCollapseUrl(raw:string){const value=raw.replace(/[),.;]+$/g,"");if(value.length>140)return true;try{const u=new URL(value);const host=u.hostname.toLowerCase(),path=`${u.pathname}${u.search}`.toLowerCase();return /(^|\.)(clicks?|track|tracking|links?|redirect|email|mail)\./.test(host)||/(?:utm_|campaign|redirect|tracking|trk=|clickid|gclid|fbclid|mc_cid|mc_eid|unsubscribe|encoded|%3a%2f%2f)/i.test(path)||u.search.length>90;}catch{return value.length>100;}}
function compactUrls(value:string){const urlRe=/https?:\/\/[^\s<>"']+/gi;let text=value.replace(urlRe,raw=>shouldCollapseUrl(raw)?"[link]":raw.replace(/[),.;]+$/g,""));text=text.replace(/(?:\[link\][ \t]*){2,}/g,"[link]").replace(/^\s*\[link\]\s*$/gm,"[link]").replace(/\n(?:[ \t]*\[link\][ \t]*\n){2,}/g,"\n[link]\n");return text;}
function cleanReadableText(value:string){return normalizeWhitespace(compactUrls(value));}
function htmlToText(value:string){return cleanReadableText(decodeEntities(value).replace(/<!--[\s\S]*?-->/g," ").replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi," ").replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi," ").replace(/<head\b[^>]*>[\s\S]*?<\/head>/gi," ").replace(/<(br|hr)\b[^>]*>/gi,"\n").replace(/<\/(p|div|tr|li|table|section|article|h[1-6])>/gi,"\n").replace(/<li\b[^>]*>/gi,"• ").replace(/<[^>]+>/g," "));}
...
export async function getUnreadGmailThreadIds(accountId:string,max=10000):Promise<Set<string>>{
 const {token}=await accessTokenFor(accountId);const ids=new Set<string>();let pageToken:string|undefined;
 while(ids.size<max){
  const pageSize=Math.min(500,Math.max(1,max-ids.size));
  const url=`https://gmail.googleapis.com/gmail/v1/users/me/messages?maxResults=${pageSize}&q=${encodeURIComponent("is:unread")}${pageToken?`&pageToken=${encodeURIComponent(pageToken)}`:""}`;
  const res=await fetch(url,{headers:{Authorization:`Bearer ${token}`},cache:"no-store"});if(!res.ok)break;
  const data=await res.json() as {messages?:{threadId:string}[];nextPageToken?:string};for(const m of data.messages??[])ids.add(m.threadId);
