...
export async function getMailCaseContext(threadId: string): Promise<MailCaseContext> {
  ...
  try {
    const v = JSON.parse(row.value) as MailCaseContext;
    return {
      threadId,
      caseId: v.caseId ?? null,
      updatedAt: v.updatedAt || new Date(0).toISOString(),
      updatedById: v.updatedById || "SYSTEM",
    };
  } catch {
    return { threadId, caseId: null, updatedAt: new Date(0).toISOString(), updatedById: "SYSTEM" };
  }
}
...
export async function getMailCaseContextMap(threadIds: string[]) {
  ...
  const rows = unique.length
    ? await prisma.appSetting.findMany({
        where: { key: { in: unique.map(key) } },
        select: { key: true, value: true },
      })
    : [];
  const map = new Map<string, MailCaseContext>();
  for (const id of unique)
    map.set(id, { threadId: id, caseId: null, updatedAt: new Date(0).toISOString(), updatedById: "SYSTEM" });
...
export async function saveMailCaseContext(input: MailCaseContext) {
  await prisma.appSetting.upsert({
    where: { key: key(input.threadId) },
    update: { value: JSON.stringify(input) },
    create: { key: key(input.threadId), value: JSON.stringify(input) },
  });
  return input;
}
