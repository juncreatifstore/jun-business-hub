...
export async function recordCompanyFundsEntityHistory(input: {
  ...
  const row: CompanyFundsEntityHistoryRow = {
    ...
    recordedAt,
    reason:
      String(input.reason || "STATE_CHANGE")
        .trim()
        .slice(0, 180) || "STATE_CHANGE",
    snapshot: input.snapshot,
  };
  await prisma.appSetting.create({
    data: {
      key: `${PREFIX}${row.entityType.toLowerCase()}.${row.entityId}.${row.id}`,
...
export async function listCompanyFundsEntityHistory(
  entityType?: CompanyFundsHistoricalEntityType,
  entityId?: string,
) {
  ...
  return rows
    ...
    .filter((row): row is CompanyFundsEntityHistoryRow => Boolean(row));
}

export async function getCompanyFundsEntityStateAsOf(
  entityType: CompanyFundsHistoricalEntityType,
  entityId: string,
  endExclusive: Date,
) {
