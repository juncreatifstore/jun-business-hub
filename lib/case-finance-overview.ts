...
export async function getCaseFinanceOverview(caseId: string) {
  ...
  const linkedPaymentIds = [
    ...new Set(invoices.flatMap((i) => i.payments.map((p) => p.paymentId)).filter(Boolean)),
  ];
  ...
  for (const invoice of allClientInvoices) {
    for (const link of invoice.payments) {
      ...
      if (invoice.caseId === caseId)
        caseAllocation.set(link.paymentId, round((caseAllocation.get(link.paymentId) || 0) + amount));
    }
  }
  ...
  const paymentRows = payments.map((p) => {
    ...
    return {
      ...
      unapplied,
      overallocated,
      directCasePayment: p.caseId === caseId,
  ...
  const expenseRows = expenses
    .map((e) => {
      ...
      const paidNumber = round(expensePaidTotal(e));
      const remainingNumber = round(expenseRemaining(e));
  ...
  const bucket = (currency: string) => {
    ...
    const found = byCurrency.get(key);
    if (found) return found;
    const v: CaseCurrencySummary = {
      currency: key,
      grossDirectReceived: 0,
      transferFees: 0,
      netDirectReceived: 0,
      appliedToCase: 0,
      unappliedDirectFunds: 0,
      billed: 0,
      invoicePaid: 0,
      receivable: 0,
      approvedRefunds: 0,
  ...
  for (const p of paymentRows) {
    ...
    if (p.directCasePayment) {
      b.grossDirectReceived = round(b.grossDirectReceived + p.gross);
      b.transferFees = round(b.transferFees + p.fee);
      b.netDirectReceived = round(b.netDirectReceived + p.net);
      b.unappliedDirectFunds = round(b.unappliedDirectFunds + p.unapplied);
    }
  ...
  for (const r of refundRows) {
    if (!["APPROVED", "PARTIALLY_PAID", "PAID"].includes(r.status)) continue;
    const b = bucket(r.currency);
    b.approvedRefunds = round(b.approvedRefunds + r.amountNumber);
    b.refundsPaid = round(b.refundsPaid + r.paidNumber);
  }
  for (const e of expenseRows) {
    if (["REJECTED", "CANCELLED"].includes(e.effectiveStatus)) continue;
    const b = bucket(e.currency);
    b.expensePaid = round(b.expensePaid + e.paidNumber);
    if (["APPROVED", "PARTIALLY_PAID", "PAID"].includes(e.effectiveStatus))
      b.committedCost = round(b.committedCost + e.amount);
    if (["DRAFT", "SUBMITTED"].includes(e.effectiveStatus))
      b.pendingApprovalCost = round(b.pendingApprovalCost + e.amount);
  }
