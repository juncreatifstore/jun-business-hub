...
export async function listMailApprovals(){const rows=await prisma.appSetting.findMany({where:{key:{startsWith:PREFIX}},select:{value:true}});return rows.map(r=>{try{return JSON.parse(r.value) as MailApprovalRecord}catch{return null}}).filter((v):v is MailApprovalRecord=>v!==null);}
