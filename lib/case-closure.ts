...
export async function getCaseClosureSnapshot(caseId: string): Promise<CaseClosureSnapshot | null> {
  const row = await prisma.appSetting
    .findUnique({ where: { key: key(caseId) }, select: { value: true } })
    .catch(() => null);
  if (!row) return null;
  try {
    return JSON.parse(row.value) as CaseClosureSnapshot;
  } catch {
    return null;
  }
}
...
export async function saveCaseClosureSnapshot(snapshot: CaseClosureSnapshot) {
  await prisma.appSetting.upsert({
    where: { key: key(snapshot.caseId) },
    create: { key: key(snapshot.caseId), value: JSON.stringify(snapshot) },
...
export async function getCaseClosureReadiness(caseId: string) {
  const [c, finance, ops, communications] = await Promise.all([
    prisma.case.findUnique({
      ...
      include: {
        ...
        tasks: true,
        documents: true,
        files: { where: { isVault: false, archivedAt: null } },
      },
    }),
    getCaseFinanceOverview(caseId),
    getCaseOperations(caseId),
    listCaseCommunications(caseId),
  ]);
  ...
  const openInvoices = finance.invoices.filter(
    (x) => x.invoice.status !== "CANCELLED" && x.state.balance > 0.009,
  );
  const pendingPayments = finance.payments.filter((p) => p.status === "PENDING");
  const openRefunds = finance.refunds.filter(
    (r) => !["REJECTED", "CANCELLED"].includes(r.status) && r.remainingNumber > 0.009,
  );
