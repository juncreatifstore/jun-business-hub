...
export function extractIndexableText(buf: Buffer, mimeType: string) {
  ...
  try {
    return buf.toString("utf8").replace(/\u0000/g, " ").replace(/\s+/g, " ").trim().slice(0, 60_000);
  } catch {
    return "";
  }
}
...
export async function indexDriveFile(fileId: string, suppliedBuffer?: Buffer) {
  ...
  if (!buf) {
    try { buf = await storage().download(file.storageKey); } catch { buf = undefined; }
  }
  ...
  const metadataText = [
    ...
    file.mimeType,
    file.folder?.name,
    file.client ? `${file.client.firstName} ${file.client.lastName} ${file.client.internalId}` : "",
    file.case ? `${file.case.caseNumber} ${file.case.title}` : "",
  ].filter(Boolean).join(" ");
  ...
  const intelligence: DriveIntelligence = {
    ...
    ...(previous.people ? { people: previous.people } : {}),
  ...
  if (hash) {
    ops.push(prisma.appSetting.upsert({
      where: { key: `${DRIVE_HASH_PREFIX}${file.id}` },
      update: { value: hash },
      create: { key: `${DRIVE_HASH_PREFIX}${file.id}`, value: hash },
    }));
  }
  ...
  if (hash) {
    const duplicate = await prisma.appSetting.findFirst({
      where: { key: { startsWith: DRIVE_HASH_PREFIX, not: `${DRIVE_HASH_PREFIX}${file.id}` }, value: hash },
      select: { key: true },
    });
    duplicateOf = duplicate?.key.slice(DRIVE_HASH_PREFIX.length) ?? null;
    const duplicateKey = `${DRIVE_DUPLICATE_PREFIX}${file.id}`;
    if (duplicateOf) {
      await prisma.appSetting.upsert({ where: { key: duplicateKey }, update: { value: duplicateOf }, create: { key: duplicateKey, value: duplicateOf } });
    } else {
      await prisma.appSetting.deleteMany({ where: { key: duplicateKey } });
    }
  }
...
export async function getDriveIntelligence(fileId: string) {
  ...
  let parsed: DriveIntelligence | null = null;
  try { parsed = intel ? JSON.parse(intel.value) : null; } catch {}
  let manualTags: string[] = [];
  try { manualTags = tags ? JSON.parse(tags.value) : []; } catch {}
