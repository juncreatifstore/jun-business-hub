...
export async function indexDriveFile(fileId: string, suppliedBuffer?: Buffer) {
  ...
  if (!buf) {
    try {
      buf = await storage().download(file.storageKey);
    } catch {
      buf = undefined;
    }
  }
  ...
  try {
    previous = existing ? JSON.parse(existing.value) : {};
  } catch {}
  const intelligence: DriveIntelligence = {
    ...
    ...(previous.structuredData ? { structuredData: previous.structuredData } : {}),
  ...
  if (hash) {
    ...
    if (duplicateOf)
      await prisma.appSetting.upsert({
        ...
        update: { value: duplicateOf },
        create: { key: duplicateKey, value: duplicateOf },
      });
...
export async function getDriveIntelligence(fileId: string) {
  const [intel, tags, duplicate] = await Promise.all([
    prisma.appSetting.findUnique({
      where: { key: `${DRIVE_INTEL_PREFIX}${fileId}` },
      select: { value: true },
    }),
    prisma.appSetting.findUnique({
      where: { key: `${DRIVE_TAGS_PREFIX}${fileId}` },
      select: { value: true },
    }),
    prisma.appSetting.findUnique({
      where: { key: `${DRIVE_DUPLICATE_PREFIX}${fileId}` },
      select: { value: true },
    }),
  ]);
  let parsed: DriveIntelligence | null = null;
  try {
    parsed = intel ? JSON.parse(intel.value) : null;
  } catch {}
  let manualTags: string[] = [];
  try {
    manualTags = tags ? JSON.parse(tags.value) : [];
  } catch {}
