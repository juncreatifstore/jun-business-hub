...
function configKey(caseId: string) {
  return `${CONFIG_PREFIX}${caseId}`;
}
function runKey(caseId: string, runId: string) {
  return `${RUN_PREFIX}${caseId}.${runId}`;
}
...
export async function getCaseAutomationConfig(caseId: string): Promise<CaseAutomationConfig> {
  const row = await prisma.appSetting
    .findUnique({ where: { key: configKey(caseId) }, select: { value: true } })
    .catch(() => null);
  if (row?.value) {
    try {
      return JSON.parse(row.value) as CaseAutomationConfig;
    } catch {}
  }
  return {
    caseId,
...
export async function saveCaseAutomationConfig(config: CaseAutomationConfig) {
  ...
  await prisma.appSetting.upsert({
    where: { key: configKey(config.caseId) },
    create: { key: configKey(config.caseId), value },
    update: { value },
  });
  return config;
}
...
export async function listCaseAutomationRuns(caseId: string) {
  const rows = await prisma.appSetting.findMany({
    where: { key: { startsWith: `${RUN_PREFIX}${caseId}.` } },
    take: 100,
    select: { value: true },
  });
  return rows
    .map((r) => {
      try {
        return JSON.parse(r.value) as CaseAutomationRun;
      } catch {
        return null;
      }
    })
...
export function automationTaskTitle(insight: CaseInsight) {
  return `[AUTO:${insight.id}] ${insight.action}`.slice(0, 240);
}
...
export async function getCaseAutomationPlan(
  caseId: string,
  intelligenceInput?: Awaited<ReturnType<typeof getCaseIntelligence>>,
) {
  ...
  const activeTitles = await prisma.task.findMany({
    where: { caseId, status: { notIn: ["DONE", "CANCELLED"] } },
    select: { title: true },
  });
  const existing = new Set(activeTitles.map((t) => t.title));
  const candidates = intelligence.nextActions.map((insight) => ({
    insight,
    title: automationTaskTitle(insight),
    priority: suggestedTaskPriority(insight),
    dueDate: suggestedDueDate(insight),
  }));
  return {
    intelligence,
    config,
    candidates: candidates.map((c) => ({ ...c, alreadyExists: existing.has(c.title) })),
    targetPriority: targetCasePriority(intelligence.case.priority, intelligence.riskLevel),
  };
}
