...
function verificationCode(reference: string, type: string, issuedAt: string) {
  const digest = createHash("sha256")
    .update(`${cleanReference(reference)}|${type}|${issuedAt}`)
    .digest("hex")
    .toUpperCase();
...
export async function registerFinanceDocumentVerification(input: {
  ...
  const issuedAt =
    input.issuedAt instanceof Date
      ...
      : String(input.issuedAt || new Date().toISOString());
  const record: FinanceDocumentVerificationRecord = {
    reference,
    type: String(input.type || "Financial document")
      .trim()
      .slice(0, 100),
...
export async function getFinanceDocumentVerification(
  reference: string,
): Promise<FinanceDocumentVerificationRecord | null> {
  const clean = cleanReference(reference);
  if (!clean) return null;
  const row = await prisma.appSetting
    .findUnique({ where: { key: registryKey(clean) }, select: { value: true } })
    .catch(() => null);
  if (!row?.value) return null;
  try {
    const record = JSON.parse(row.value) as FinanceDocumentVerificationRecord;
    if (!record || record.reference !== clean || !record.type || !record.issuedAt || !record.verificationCode)
      return null;
    return record;
  } catch {
    return null;
