...
function reserveMatchesScope(
  reserve: { country: string | null; currency: string; accountId: string | null },
  store: TreasuryStore,
  filters: CompanyFundsFilterParams,
) {
...
export async function getProtectedCashForecast(filters: CompanyFundsFilterParams = emptyFilters) {
  ...
  const currencies = [
    ...new Set([
      ...store.accounts.filter((a) => a.active).map((a) => a.currency),
      ...store.forecastItems.filter((f) => f.status !== "CANCELLED").map((f) => f.currency),
      ...activeReserves.map((r) => r.currency),
      ...store.loans.filter((l) => l.status === "ACTIVE").map((l) => l.currency),
      ...store.investments.filter((i) => i.status === "PLANNED").map((i) => i.currency),
    ]),
  ].sort();
  const horizons: Horizon[] = [30, 60, 90];
  const rows: ProtectedCashForecast[] = [];
  for (const currency of currencies) {
    ...
    for (const horizon of horizons) {
      ...
      const protectedAvailable = round(projectedCash - reserved);
      const reserveDeficit = Math.max(0, round(reserveTarget - projectedCash));
      const status: ProtectedCashForecast["status"] =
        projectedCash < reserved || protectedAvailable < 0
          ? "CRITICAL"
          : reserveDeficit > 0 || protectedAvailable < Math.max(1000, cashNow * 0.1)
            ? "WATCH"
            : "HEALTHY";
      rows.push({
        currency,
...
export async function getLiquidityProtectionAlerts(filters: CompanyFundsFilterParams = emptyFilters) {
  ...
  return rows
    ...
    .map((r) => ({
      id: `liquidity-${r.currency}-${r.horizon}`,
      severity: r.status === "CRITICAL" ? ("CRITICAL" as const) : ("WARNING" as const),
      title:
        r.status === "CRITICAL"
          ? `Trésorerie protégée critique à ${r.horizon} jours`
          : `Réserve sous pression à ${r.horizon} jours`,
      detail:
        r.status === "CRITICAL"
          ? `Le cash projeté ne couvre plus les réserves protégées. Disponible après réserves: ${r.protectedAvailable.toFixed(2)} ${r.currency}.`
          : `Le niveau projeté devient proche ou inférieur à l’objectif de sécurité. Déficit potentiel: ${r.reserveDeficit.toFixed(2)} ${r.currency}.`,
      currency: r.currency,
    }));
}
