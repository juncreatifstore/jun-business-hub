...
function cfg() {
  ...
  if (!serviceAccountEmail || !privateKey || !impersonateUser || !rootFolderId) {
    throw new Error(
      "Google Workspace storage requires GOOGLE_WORKSPACE_SERVICE_ACCOUNT_EMAIL, GOOGLE_WORKSPACE_PRIVATE_KEY, GOOGLE_WORKSPACE_IMPERSONATE_USER and GOOGLE_WORKSPACE_ROOT_FOLDER_ID",
    );
  }
...
async function listByParent(parentId: string, name?: string) {
  ...
  return (json.files || []).map((f): WorkspaceDriveItem => ({
    id: f.id,
    name: f.name,
    mimeType: f.mimeType,
...
export async function uploadWorkspaceFile(pathKey: string, data: Buffer, contentType: string) {
  ...
  const metadata = JSON.stringify({
    ...
    parents: [resolved.parentId],
    appProperties: { junStorageKey: pathKey },
  });
...
export async function createWorkspaceResumableUploadSession(
  pathKey: string,
  contentType: string,
  sizeBytes: number,
) {
...
export async function downloadWorkspaceFile(pathKey: string) {
  ...
  const token = await googleWorkspaceAccessToken();
  const res = await fetch(
    `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(resolved.file.id)}?alt=media&supportsAllDrives=true`,
...
export async function removeWorkspaceFile(pathKey: string) {
  ...
  const token = await googleWorkspaceAccessToken();
  const res = await fetch(
    `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(resolved.file.id)}?supportsAllDrives=true`,
