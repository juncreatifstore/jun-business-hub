...
export async function getMailAICopilotAnalysis(threadId: string): Promise<MailAICopilotAnalysis | null> {
  ...
  if (!row) return null;
  try {
    const v = JSON.parse(row.value) as MailAICopilotAnalysis;
    return v?.threadId === threadId ? v : null;
  } catch {
    return null;
  }
}
export async function saveMailAICopilotAnalysis(value: MailAICopilotAnalysis) {
  await prisma.appSetting.upsert({
    where: { key: key(value.threadId) },
    update: { value: JSON.stringify(value) },
    create: { key: key(value.threadId), value: JSON.stringify(value) },
  });
  return value;
}
