...
export async function executeRefundInstallmentPayout(
  installmentId: string,
  userId: string,
): Promise<SafeRefundPayoutResult> {
  const initial = await prisma.refundInstallment.findUnique({
    where: { id: installmentId },
    include: { refund: true },
  });
  if (!initial) throw new Error("Refund installment not found");
  if (initial.status === "CANCELLED")
    throw new Error("Cette tranche a été annulée ou remplacée. Rechargez la fiche.");
  ...
  if (initial.status === "PAID") {
    ...
    return {
      ...
      refundNumber: initial.refund.refundNumber,
      clientId: initial.refund.clientId,
      caseId: initial.refund.caseId,
      paymentId: initial.refund.paymentId,
      amount: Number(initial.amount),
      currency: initial.refund.currency,
      newStatus: initial.refund.status,
      method: meta.method || "",
      transactionRef: meta.transactionRef || "",
      proofFileId: meta.proofFileId || "",
      authorizationId: authorization?.id || "",
      paidAt: initial.paidAt?.toISOString() || new Date().toISOString(),
    };
  }
  if (!["APPROVED", "PARTIALLY_PAID"].includes(initial.refund.status))
    throw new Error("Refund must be approved before payout");

  const meta = await getRefundInstallmentMeta(installmentId);
  if (!meta.method || !meta.transactionRef || !meta.proofFileId)
    throw new Error("Method, transaction reference and proof are required before payout");
  const proof = await prisma.file.findFirst({
    where: { id: meta.proofFileId, refundId: initial.refundId, archivedAt: null },
    select: { id: true },
  });
  ...
  if (!authorization || authorization.status !== "APPROVED")
    throw new Error("Approved financial authorization is required before refund payout");
  if (
    authorization.currency !== initial.refund.currency.toUpperCase() ||
    Math.abs(authorization.amount - Number(initial.amount)) > 0.005
  )
    throw new Error("Financial authorization no longer matches this refund installment");
  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      const current = await tx.refundInstallment.findUnique({
        where: { id: installmentId },
      ...
      if (current.status === "PAID")
        return {
          duplicate: true,
          current,
          newStatus: current.refund.status as RefundStatus,
          paidAt: current.paidAt || paidAt,
        };
      if (!["APPROVED", "PARTIALLY_PAID"].includes(current.refund.status))
        throw new Error("Refund is no longer payable");

      const changed = await tx.refundInstallment.updateMany({
        where: { id: installmentId, status: current.status },
        data: { status: "PAID", paidAt },
      });
      if (changed.count !== 1) {
        const latest = await tx.refundInstallment.findUnique({
          where: { id: installmentId },
          include: { refund: true },
        });
        if (latest?.status === "PAID")
          return {
            duplicate: true,
            current: { ...current, refund: latest.refund },
            newStatus: latest.refund.status as RefundStatus,
            paidAt: latest.paidAt || paidAt,
          };
        throw new Error("Refund payout changed concurrently. Refresh and retry.");
      }

      const remaining = current.refund.installments.filter(
      ...
      if (current.refund.paymentId) {
        const payment = await tx.payment.findUnique({
          where: { id: current.refund.paymentId },
        ...
        if (payment) {
          ...
          for (const refund of payment.refunds) {
            for (const installment of refund.installments) {
              if (installment.status === "PAID" || installment.id === installmentId)
                paidTotal += Number(installment.amount);
            }
          }
          const total = round(paidTotal);
          const paymentAmount = Number(payment.amount);
          if (total > 0) {
            const paymentStatus = total >= paymentAmount - 0.005 ? "REFUNDED" : "PARTIALLY_REFUNDED";
  ...
  return {
    ...
    refundId: result.current.refundId,
    refundNumber: result.current.refund.refundNumber,
    clientId: result.current.refund.clientId,
    caseId: result.current.refund.caseId,
    paymentId: result.current.refund.paymentId,
    amount: Number(result.current.amount),
    currency: result.current.refund.currency,
