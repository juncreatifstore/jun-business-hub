...
export function paymentEditMode(status: string): EditMode {
  return status === "PENDING"
    ? "DIRECT"
    : ["CONFIRMED", "PARTIALLY_REFUNDED", "REFUNDED"].includes(status)
      ? "CORRECTION"
      : "LOCKED";
}

export function refundEditMode(status: string, paidInstallments: number): EditMode {
  if (paidInstallments > 0 || ["PAID", "PARTIALLY_PAID", "REJECTED", "CANCELLED"].includes(status))
    return "LOCKED";
...
export function expenseEditMode(status: string, paymentCount: number): EditMode {
  if (paymentCount > 0 || ["PARTIALLY_PAID", "PAID", "REJECTED", "CANCELLED"].includes(status))
    return "LOCKED";
...
export function editPolicyMessage(mode: EditMode) {
  ...
  if (mode === "CORRECTION")
    return "Financial values are locked after confirmation. You may correct descriptive/reference fields with a mandatory reason.";
