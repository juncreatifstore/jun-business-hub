...
export function clientCanAccessFile(
  file: { clientId: string | null; isVault: boolean },
  ownClientId: string,
): boolean {
  return !file.isVault && file.clientId === ownClientId;
}

export function clientCanAccessDocument(
  doc: { clientId: string | null; status: string },
  ownClientId: string,
): boolean {
