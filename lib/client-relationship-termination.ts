...
export type TerminationStatus =
  "REVIEW" | "SETTLING" | "READY_TO_SIGN" | "READY_TO_TERMINATE" | "TERMINATED" | "CANCELLED";
...
export async function getClientTermination(clientId: string): Promise<ClientTerminationRecord | null> {
  ...
  if (!row) return null;
  try {
    const r = JSON.parse(row.value) as ClientTerminationRecord;
    return r?.clientId ? r : null;
  } catch {
    return null;
  }
}
export async function saveClientTermination(record: ClientTerminationRecord) {
...
export async function getClientTerminationReadiness(clientId: string) {
  const [client, account, finance] = await Promise.all([
    prisma.client.findUnique({
      ...
      include: {
        cases: true,
        documents: { where: { status: "SIGNED" }, orderBy: { updatedAt: "desc" } },
        payments: true,
        refunds: { include: { installments: true } },
      },
    }),
  ...
  const openInvoices = finance.invoices.filter(
    (r) => !["PAID", "CANCELLED"].includes(String(r.state.effectiveStatus)),
  );
  const pendingPayments = client.payments.filter((p) => p.status === "PENDING");
  const openExpenses = finance.expenses.filter(
    (e) => !["PAID", "REJECTED", "CANCELLED"].includes(e.effectiveStatus),
  );
  const openRefunds = client.refunds.filter((r) => !["PAID", "REJECTED", "CANCELLED"].includes(r.status));
  const refundableBalances = account.balances
    .map((b) => ({
      currency: b.currency,
      amount: Math.max(0, Math.round((b.available - b.pendingRefunds) * 100) / 100),
    }))
    .filter((b) => b.amount > 0.009);
  const clientDebt = account.balances
    .filter((b) => b.available < -0.009)
    .map((b) => ({ currency: b.currency, amount: Math.abs(b.available) }));
  return {
    client,
    account,
    finance,
    ...
    refundableBalances,
    clientDebt,
    signedDocuments: client.documents,
    transactionsSettled:
