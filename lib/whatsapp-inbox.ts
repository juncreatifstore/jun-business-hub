...
function incomingText(message: any) {
  ...
  if (type === "button")
    return String(message?.button?.text || message?.button?.payload || "Button reply").trim();
  if (type === "interactive")
    return String(
...
async function notificationRecipients(client: { ownerId: string | null } | null) {
  ...
  const staff = await prisma.user.findMany({
    where: { status: "ACTIVE", role: { in: ["SUPER_ADMIN", "DIRECTOR", "ADMIN", "MANAGER"] } },
    select: { id: true },
  });
...
export async function recordIncomingWhatsAppMessage(input: { message: any; contactName?: string }) {
    ...
    await prisma.activity
      .create({
        data: {
          ...
          message: `Inbound WhatsApp blocked from +${phone} · ${messageId}`,
          clientId: client.id,
          resourceType: "Client",
  ...
  const activeCase = client
    ? await prisma.case.findFirst({
        where: {
          clientId: client.id,
          status: { in: ["OPEN", "IN_PROGRESS", "WAITING_CLIENT", "WAITING_INTERNAL"] },
        },
  ...
  const timestamp =
    timestampSeconds > 0 ? new Date(timestampSeconds * 1000).toISOString() : new Date().toISOString();
  const payload: WhatsAppInboxPayload = {
    direction: "INBOUND",
    phone,
  ...
  const activity = await prisma.activity.create({
    data: {
      ...
      message: encodeWhatsAppInboxPayload(payload),
      clientId: client?.id,
