...
function findPart(part: GmailPart | undefined, attachmentId: string): GmailPart | null {
  if (!part) return null;
  if (part.body?.attachmentId === attachmentId) return part;
  for (const child of part.parts ?? []) {
    const found = findPart(child, attachmentId);
...
export async function downloadGmailAttachment(accountId: string, messageId: string, attachmentId: string) {
  ...
  return {
    data,
    filename: part.filename.slice(0, 180),
