...
async function modifyThread(
  ...
  await gmailThreadRequest(accountId, gmailThreadId, "/modify", input);
}
...
export async function gmailTrashThread(accountId: string, gmailThreadId: string) {
  await gmailThreadRequest(accountId, gmailThreadId, "/trash");
}
export async function gmailRestoreThread(accountId: string, gmailThreadId: string) {
  await gmailThreadRequest(accountId, gmailThreadId, "/untrash");
  await modifyThread(accountId, gmailThreadId, { addLabelIds: ["INBOX"] });
}
...
export async function reconcileMailboxStatesFromGmail(accountId: string, max = 5000) {
  ...
  for (const row of rows) {
    ...
    const next = {
      ...
      archived:
        !inbox.has(row.gmailThreadId) &&
        !trash.has(row.gmailThreadId) &&
        !spam.has(row.gmailThreadId) &&
        !sent.has(row.gmailThreadId) &&
        !drafts.has(row.gmailThreadId),
      updatedAt: new Date().toISOString(),
      updatedById: null,
    };
    if (
      next.isRead !== current.isRead ||
      next.starred !== current.starred ||
      next.trashed !== current.trashed ||
