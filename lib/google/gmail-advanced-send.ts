...
export type AdvancedGmailInput = {
  ...
  attachments?: AdvancedGmailAttachment[];
};
...
function b64Lines(data: Buffer | Uint8Array) {
...
export async function gmailSendAdvanced(accountId: string, input: AdvancedGmailInput): Promise<string> {
  ...
  const headers = [
    ...
    ...(input.inReplyToGmailId
      ? [
          `In-Reply-To: <${safeHeader(input.inReplyToGmailId)}@mail.gmail.com>`,
          `References: <${safeHeader(input.inReplyToGmailId)}@mail.gmail.com>`,
        ]
      : []),
    "MIME-Version: 1.0",
  ];
