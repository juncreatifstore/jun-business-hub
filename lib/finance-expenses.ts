...
export type ExpenseStatus =
  "DRAFT" | "SUBMITTED" | "APPROVED" | "PARTIALLY_PAID" | "PAID" | "REJECTED" | "CANCELLED";
...
function parse(value: string): FinanceExpense | null {
  try {
    ...
    return {
      ...e,
      amount: round(Number(e.amount || 0)),
...
export function makeExpenseNumber() {
  return `EXP-${new Date().getFullYear()}-${randomUUID().slice(0, 8).toUpperCase()}`;
}
export async function listFinanceExpenses(limit = 500) {
  const rows = await prisma.appSetting.findMany({
    where: { key: { startsWith: EXPENSE_PREFIX } },
...
export async function getFinanceExpense(id: string) {
  const row = await prisma.appSetting.findUnique({ where: { key: key(id) }, select: { value: true } });
  return row ? parse(row.value) : null;
}
export async function saveFinanceExpense(expense: FinanceExpense) {
  const value = JSON.stringify(expense);
  await prisma.appSetting.upsert({
    where: { key: key(expense.id) },
    create: { key: key(expense.id), value },
    update: { value },
  });
  return expense;
}
export async function mutateFinanceExpense<T>(
  id: string,
  mutate: (
    current: FinanceExpense,
  ) => { expense: FinanceExpense; result: T } | Promise<{ expense: FinanceExpense; result: T }>,
): Promise<T> {
...
export function expensePaidTotal(e: FinanceExpense) {
  return round(e.payments.reduce((s, p) => s + Number(p.amount || 0), 0));
}
export function expenseRemaining(e: FinanceExpense) {
  return Math.max(0, round(e.amount - expensePaidTotal(e)));
}
export function expenseEffectiveStatus(e: FinanceExpense): ExpenseStatus {
  const paid = expensePaidTotal(e);
  if (paid >= e.amount && e.amount > 0) return "PAID";
  if (paid > 0) return "PARTIALLY_PAID";
  return e.status;
}
export function expenseIsOverdue(e: FinanceExpense, now = new Date()) {
  if (!e.dueDate || ["PAID", "REJECTED", "CANCELLED"].includes(expenseEffectiveStatus(e))) return false;
  return new Date(e.dueDate).getTime() < new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
}
export function canExpenseTransition(from: ExpenseStatus, to: ExpenseStatus) {
  const map: Record<ExpenseStatus, ExpenseStatus[]> = {
...
export function expenseStatusRank(status: ExpenseStatus) {
