...
export type TreasurySourceKind =
  "REVENUE" | "OWNER_CAPITAL" | "PARTNER_CAPITAL" | "INVESTMENT" | "LOAN" | "OTHER";
...
export type TreasuryAccount = {
  ...
  currency: string;
  balance: number;
  connectionMode: TreasuryConnectionMode;
  provider: string;
  externalRef: string;
  active: boolean;
  dailyUpdateRequired: boolean;
  lastSyncAt: string | null;
  note: string;
  createdAt: string;
  updatedAt: string;
  connectionKeyHash?: string | null;
  lastExternalSyncId?: string | null;
};
...
export type TreasuryInvestment = {
  ...
  status: TreasuryInvestmentStatus;
  counterparty: string;
  note: string;
  createdAt: string;
  updatedAt: string;
...
export type TreasuryStore = {
  ...
  accountSnapshots: TreasuryAccountSnapshot[];
  reconciliations: TreasuryReconciliation[];
  forecastItems: TreasuryForecastItem[];
  updatedAt: string;
};
...
function normalizeStore(raw: unknown): TreasuryStore {
  ...
  const accounts = Array.isArray(r.accounts)
    ? r.accounts.map((a) => ({
        ...a,
        connectionKeyHash: a.connectionKeyHash || null,
        lastExternalSyncId: a.lastExternalSyncId || null,
      }))
    : [];
  return {
    accounts,
  ...
  const saved = await prisma.$transaction(
    async (tx) => {
      ...
      if (currentRow) {
        ...
        try {
          current = normalizeStore(JSON.parse(currentRow.value));
        } catch {
          throw new Error("Treasury store is corrupted and cannot be updated safely.");
        }
        if (expectedVersion && current.updatedAt !== expectedVersion) {
          throw new Error(
            "Treasury data changed concurrently. Refresh and retry; no financial data was overwritten.",
          );
        }
      }
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (!row) store = emptyStore();
        ...
        try {
          store = normalizeStore(JSON.parse(row.value));
        } catch {
          throw new Error("Treasury store is corrupted and cannot be updated safely.");
        }
      }
...
function pushSnapshot(
  ...
  const snapshot: TreasuryAccountSnapshot = {
    id: randomUUID(),
...
export async function addTreasuryAccount(
  input: Omit<TreasuryAccount, "id" | "createdAt" | "updatedAt" | "lastSyncAt">,
) {
  return mutateTreasuryStore((store) => {
    ...
    const account: TreasuryAccount = {
      ...
      balance: asMoney(input.balance),
      connectionKeyHash: input.connectionKeyHash || null,
      lastExternalSyncId: null,
      lastSyncAt: input.connectionMode === "MANUAL" ? now : null,
      createdAt: now,
      updatedAt: now,
    };
    store.accounts.unshift(account);
    if (account.connectionMode === "MANUAL") pushSnapshot(store, account, "MANUAL", null, now);
    return account;
  });
}
export async function updateTreasuryAccountBalance(id: string, balance: number) {
  return mutateTreasuryStore((store) => {
    ...
    account.balance = asMoney(balance);
    account.lastSyncAt = new Date().toISOString();
    account.updatedAt = account.lastSyncAt;
    pushSnapshot(store, account, "MANUAL", null, account.lastSyncAt);
    return account;
  });
}
export async function setTreasuryAccountConnectionKey(id: string, apiKey: string) {
  ...
  return mutateTreasuryStore((store) => {
    const account = store.accounts.find((a) => a.id === id);
    if (!account) throw new Error("Treasury account not found");
    account.connectionMode = "CONNECTED";
    account.connectionKeyHash = sha256(apiKey.trim());
...
export async function addTreasuryPartner(input: Omit<TreasuryPartner, "id" | "createdAt" | "updatedAt">) {
  return mutateTreasuryStore((store) => {
    const now = new Date().toISOString();
    const partner: TreasuryPartner = {
      ...input,
      id: randomUUID(),
      currency: input.currency.toUpperCase(),
      capitalContributed: asMoney(input.capitalContributed),
      ownershipPercent: asMoney(input.ownershipPercent),
      profitSharePercent: asMoney(input.profitSharePercent),
      createdAt: now,
      updatedAt: now,
    };
...
export async function addTreasuryLoan(input: Omit<TreasuryLoan, "id" | "createdAt" | "updatedAt">) {
  return mutateTreasuryStore((store) => {
    const now = new Date().toISOString();
    const loan: TreasuryLoan = {
      ...input,
      id: randomUUID(),
      currency: input.currency.toUpperCase(),
      principal: asMoney(input.principal),
      outstandingBalance: asMoney(input.outstandingBalance),
      interestRate: asMoney(input.interestRate),
      createdAt: now,
      updatedAt: now,
    };
...
export async function addTreasuryInvestment(
  input: Omit<TreasuryInvestment, "id" | "createdAt" | "updatedAt">,
) {
  return mutateTreasuryStore((store) => {
    const now = new Date().toISOString();
    const investment: TreasuryInvestment = {
      ...
      currency: input.currency.toUpperCase(),
      amount: asMoney(input.amount),
      expectedReturnPercent: asMoney(input.expectedReturnPercent),
      createdAt: now,
      updatedAt: now,
    };
...
export async function addTreasuryForecastItem(
  input: Omit<TreasuryForecastItem, "id" | "createdAt" | "updatedAt">,
) {
  const due = new Date(input.dueDate);
  if (Number.isNaN(due.getTime())) throw new Error("Invalid forecast due date");
  return mutateTreasuryStore((store) => {
    const now = new Date().toISOString();
    ...
    store.forecastItems.unshift(item);
    store.forecastItems = store.forecastItems.slice(0, 5000);
    return item;
  });
}
export async function updateTreasuryForecastStatus(id: string, status: TreasuryForecastStatus) {
  return mutateTreasuryStore((store) => {
    const item = store.forecastItems.find((f) => f.id === id);
    if (!item) throw new Error("Forecast item not found");
    item.status = status;
    item.updatedAt = new Date().toISOString();
    return item;
  });
}
export async function addProjectIntegration(input: {
  code: string;
  name: string;
  country: string;
  currency: string;
  ...
  return mutateTreasuryStore((store) => {
    if (store.integrations.some((i) => i.code.toLowerCase() === input.code.toLowerCase()))
...
export async function ingestProjectCashflow(input: {
  projectCode: string;
  apiKey: string;
  externalId: string;
  direction: TreasuryDirection;
  ...
  return mutateTreasuryStore((store) => {
    const integration = store.integrations.find((i) => i.enabled && i.code === input.projectCode);
    if (!integration) throw new Error("Unknown project integration");
    if (integration.apiKeyHash !== sha256(input.apiKey)) throw new Error("Invalid project API key");
    const externalId = input.externalId.trim();
    const existing = store.projectCashflows.find(
      (t) => t.integrationId === integration.id && t.externalId === externalId,
    );
    if (existing) return { transaction: existing, duplicate: true };
    const currency = input.currency.toUpperCase();
    if (currency !== integration.currency)
      throw new Error(`Currency mismatch: expected ${integration.currency}`);
    const now = new Date().toISOString();
    const tx: ProjectCashflow = {
      id: randomUUID(),
      integrationId: integration.id,
      externalId,
      direction: input.direction,
      category: input.category.trim().slice(0, 100) || "OTHER",
      amount,
      currency,
    ...
    if (tx.accountId) {
      ...
      account.balance = round(account.balance + (tx.direction === "IN" ? tx.amount : -tx.amount));
      account.updatedAt = now;
    }
    store.projectCashflows.unshift(tx);
    integration.lastSyncAt = now;
    integration.lastExternalId = tx.externalId;
    integration.updatedAt = now;
    return { transaction: tx, duplicate: false };
  });
}

export async function ingestTreasuryAccountSync(input: {
  accountId: string;
  apiKey: string;
  externalId: string;
  balance: number;
  ...
  if (Number.isNaN(captured.getTime())) throw new Error("Invalid capturedAt");
  const reportedBalance = asMoney(input.balance);
  const externalId = input.externalId.trim();
  return mutateTreasuryStore((store) => {
    const account = store.accounts.find((a) => a.id === input.accountId && a.active);
    if (!account) throw new Error("Unknown treasury account");
    if (account.connectionMode !== "CONNECTED" || !account.connectionKeyHash)
      throw new Error("Account API connection is not configured");
    if (account.connectionKeyHash !== sha256(input.apiKey.trim())) throw new Error("Invalid account API key");
    const duplicate = store.accountSnapshots.find(
      (s) => s.accountId === account.id && s.externalId === externalId,
    );
    if (duplicate) return { snapshot: duplicate, duplicate: true, reconciliation: null };
    const currency = input.currency.toUpperCase();
    if (currency !== account.currency) throw new Error(`Currency mismatch: expected ${account.currency}`);
    const expectedBalance = account.balance;
    const difference = round(reportedBalance - expectedBalance);
    const previousSnapshot = store.accountSnapshots.find((s) => s.accountId === account.id) || null;
    const now = new Date().toISOString();
    account.balance = reportedBalance;
    account.lastSyncAt = captured.toISOString();
    account.lastExternalSyncId = externalId;
    account.updatedAt = now;
    ...
    const reconciliation: TreasuryReconciliation = {
      ...
      reportedBalance,
      difference,
      status: Math.abs(difference) < 0.01 ? "MATCHED" : "REVIEW",
      periodStart: previousSnapshot?.capturedAt || null,
      periodEnd: captured.toISOString(),
      note: String(input.note || "").slice(0, 500),
      createdAt: now,
      resolvedAt: null,
    };
    store.reconciliations.unshift(reconciliation);
    store.reconciliations = store.reconciliations.slice(0, 5000);
    return { snapshot, duplicate: false, reconciliation };
  });
}
export async function resolveTreasuryReconciliation(id: string, note: string) {
  return mutateTreasuryStore((store) => {
    const row = store.reconciliations.find((r) => r.id === id);
    if (!row) throw new Error("Reconciliation not found");
    if (row.status === "RESOLVED") return row;
    row.status = "RESOLVED";
    row.resolvedAt = new Date().toISOString();
    row.note = [row.note, note.trim()].filter(Boolean).join(" · ").slice(0, 500);
    return row;
  });
}
...
export function treasuryByCurrency(store: TreasuryStore) {
  ...
  store.accounts.forEach((a) => currencies.add(a.currency));
  store.sources.forEach((s) => currencies.add(s.currency));
  store.loans.forEach((l) => currencies.add(l.currency));
  store.investments.forEach((i) => currencies.add(i.currency));
  store.projectCashflows.forEach((t) => currencies.add(t.currency));
  store.forecastItems.forEach((f) => currencies.add(f.currency));
  return Array.from(currencies)
    .sort()
    .map((currency) => {
      const cash = round(
        store.accounts.filter((a) => a.active && a.currency === currency).reduce((s, a) => s + a.balance, 0),
      );
      const projectIn = round(
        store.projectCashflows
          .filter((t) => t.currency === currency && t.direction === "IN")
          .reduce((s, t) => s + t.amount, 0),
      );
      const projectOut = round(
        store.projectCashflows
          .filter((t) => t.currency === currency && t.direction === "OUT")
          .reduce((s, t) => s + t.amount, 0),
      );
      const projectProfit = round(projectIn - projectOut);
      const loanOutstanding = round(
        store.loans
          .filter((l) => l.currency === currency && l.status === "ACTIVE")
          .reduce((s, l) => s + l.outstandingBalance, 0),
      );
      const invested = round(
        store.investments
          .filter((i) => i.currency === currency && i.status !== "CANCELLED")
...
export function projectPerformance(store: TreasuryStore) {
  return store.integrations.map((integration) => {
    ...
    const margin = income > 0 ? round((profit / income) * 100) : null;
...
export function treasuryReconciliationSummary(store: TreasuryStore) {
  ...
  return {
    ...
    recent: store.reconciliations.slice(0, 50),
  };
}

export function companyWealthByCurrency(store: TreasuryStore) {
  return treasuryByCurrency(store).map((row) => {
    const activeInvestments = round(
      store.investments
        .filter((i) => i.currency === row.currency && i.status === "ACTIVE")
        .reduce((s, i) => s + i.amount, 0),
    );
    const partnerCapital = round(
      store.partners
        .filter((p) => p.currency === row.currency && p.status === "ACTIVE")
        .reduce((s, p) => s + p.capitalContributed, 0),
    );
    ...
    return {
      ...
      netWorth,
      partnerCapital,
      projectProfit: row.projectProfit,
      debtToAssetsPercent,
    };
  });
}
...
export function partnerEconomics(store: TreasuryStore) {
  ...
  return store.partners
    ...
    .map((partner) => {
      ...
      return {
        ...partner,
        theoreticalProfitShare,
        totalEconomicExposure: round(partner.capitalContributed + theoreticalProfitShare),
      };
    });
}
  ...
  return Array.from(currencies)
    ...
    .map((currency) => {
      ...
      const rows = horizons.map((days) => {
        ...
        const planned = store.forecastItems.filter(
          (f) =>
            f.currency === currency &&
            !["PAID", "CANCELLED"].includes(f.status) &&
            new Date(f.dueDate) >= now &&
            new Date(f.dueDate) <= end,
        );
        const plannedIn = round(
          planned.filter((f) => f.direction === "IN").reduce((s, f) => s + f.amount, 0),
        );
        const plannedOut = round(
          planned.filter((f) => f.direction === "OUT").reduce((s, f) => s + f.amount, 0),
        );
        const loanDue = round(
          store.loans
            .filter(
              (l) =>
                ...
                new Date(l.dueDate) <= end,
            )
            .reduce((s, l) => s + l.outstandingBalance, 0),
        );
        const futureInvestments = round(
          store.investments
            .filter(
              (i) =>
                i.currency === currency &&
                i.status === "PLANNED" &&
                i.investedAt &&
                new Date(i.investedAt) >= now &&
                new Date(i.investedAt) <= end,
            )
            .reduce((s, i) => s + i.amount, 0),
        );
  ...
  return Array.from(currencies)
    ...
    .map((currency) => {
      const rows = Array.from({ length: months }, (_, offset) => {
        ...
        const cashflows = store.projectCashflows.filter(
          (t) => t.currency === currency && new Date(t.occurredAt) >= d && new Date(t.occurredAt) < next,
        );
        const income = round(cashflows.filter((t) => t.direction === "IN").reduce((s, t) => s + t.amount, 0));
        const expense = round(
          cashflows.filter((t) => t.direction === "OUT").reduce((s, t) => s + t.amount, 0),
        );
        const profit = round(income - expense);
        const activeProjects = new Set(
          cashflows.map((t) => integrations.get(t.integrationId)?.id).filter((v): v is string => Boolean(v)),
        ).size;
        return {
          ...
          profit,
          activeProjects,
          margin: income > 0 ? round((profit / income) * 100) : null,
        };
      }).reverse();
...
export function profitabilityByCountry(store: TreasuryStore) {
  ...
  const bucket = new Map<
    string,
    { country: string; currency: string; income: number; expense: number; projects: Set<string> }
  >();
  for (const tx of store.projectCashflows) {
    const integration = integrationMap.get(tx.integrationId);
    if (!integration) continue;
    const key = `${integration.country || "Non défini"}::${tx.currency}`;
    const row = bucket.get(key) || {
      country: integration.country || "Non défini",
    ...
    row.projects.add(integration.id);
    bucket.set(key, row);
  }
  return Array.from(bucket.values())
    .map((row) => {
      const income = round(row.income),
...
export function accountConcentration(store: TreasuryStore) {
  ...
  for (const currency of new Set(store.accounts.filter((a) => a.active).map((a) => a.currency))) {
    const accounts = store.accounts.filter((a) => a.active && a.currency === currency);
    const total = accounts.reduce((s, a) => s + a.balance, 0);
    for (const account of accounts) {
      const sharePercent = total > 0 ? round((account.balance / total) * 100) : 0;
      rows.push({
        accountId: account.id,
        name: account.name,
        ...
        balance: account.balance,
        sharePercent,
        risk: sharePercent >= 70 ? "HIGH" : sharePercent >= 50 ? "WATCH" : "NORMAL",
      });
    }
  }
...
export function executiveTreasuryAlerts(store: TreasuryStore, now = new Date()) {
  ...
  for (const a of store.accounts.filter(
    (a) =>
      a.active &&
      a.dailyUpdateRequired &&
      (!a.lastSyncAt || now.getTime() - new Date(a.lastSyncAt).getTime() > 36 * 60 * 60 * 1000),
  ))
    alerts.push({
      id: `stale-${a.id}`,
      severity: "WARNING",
      title: "Compte non mis à jour",
      detail: `${a.name} · ${a.country} · dernière synchronisation ${a.lastSyncAt || "jamais"}`,
      currency: a.currency,
    });
  for (const r of store.reconciliations.filter((r) => r.status === "REVIEW").slice(0, 20)) {
    const a = store.accounts.find((x) => x.id === r.accountId);
    alerts.push({
      id: `rec-${r.id}`,
      severity: "CRITICAL",
      title: "Écart de réconciliation",
      detail: `${a?.name || "Compte"} · différence ${r.difference}`,
      currency: a?.currency,
    });
  }
  for (const f of cashForecastByCurrency(store, now))
    for (const h of f.horizons.filter((h) => h.risk !== "HEALTHY"))
      alerts.push({
        id: `forecast-${f.currency}-${h.days}`,
        severity: h.risk === "CRITICAL" ? "CRITICAL" : "WARNING",
        title: `Trésorerie ${h.days} jours`,
        detail: `Cash projeté ${h.projectedCash}`,
        currency: f.currency,
      });
  for (const c of accountConcentration(store).filter((c) => c.risk !== "NORMAL"))
    alerts.push({
      id: `conc-${c.accountId}`,
      severity: c.risk === "HIGH" ? "CRITICAL" : "WARNING",
      title: "Concentration bancaire",
      detail: `${c.sharePercent}% des liquidités ${c.currency} sont sur ${c.name}`,
      currency: c.currency,
    });
  for (const l of loanMaturitySchedule(store, now).filter((l) =>
    ["OVERDUE", "DUE_SOON"].includes(l.severity),
  ))
    alerts.push({
      id: `loan-${l.id}`,
      severity: l.severity === "OVERDUE" ? "CRITICAL" : "WARNING",
      title: l.severity === "OVERDUE" ? "Prêt en retard" : "Prêt à échéance proche",
      detail: `${l.lender} · solde ${l.outstandingBalance} · ${l.daysUntilDue} jour(s)`,
      currency: l.currency,
    });
  for (const p of projectPerformance(store).filter((p) => p.profit < 0))
    alerts.push({
      id: `project-${p.id}`,
      severity: "WARNING",
      title: "Projet déficitaire",
      detail: `${p.name} · profit ${p.profit}`,
      currency: p.currency,
    });
  return alerts.sort(
    (a, b) =>
