...
import { getBudgetAlerts, getBudgetScenarios, listBudgetPlans } from "@/lib/finance-budgeting";

export type FinanceRiskLevel = "LOW" | "MEDIUM" | "HIGH" | "CRITICAL";
export type FinanceAnomaly = {
  id: string;
  level: FinanceRiskLevel;
  category: "DUPLICATE" | "RECONCILIATION" | "FEE" | "PROOF" | "REFUND" | "PROVIDER" | "EXPENSE" | "BUDGET";
  title: string;
  detail: string;
  href: string;
};
...
export async function getFinanceEnterpriseIntelligence() {
  ...
  const [payments, refunds, onlineSessions, accounts, expenses, budgetPlans] = await Promise.all([
  ...
  for (const payment of settled) {
    const amount = Number(payment.amount),
      fee = Number(metaMap.get(payment.id)?.feeAmount || 0);
    if (amount > 0 && fee / amount >= 0.05)
      anomalies.push({
        id: `fee-${payment.id}`,
  ...
  for (const session of onlineSessions) {
    ...
    if (
      session.status === "PAID" &&
      !["CONFIRMED", "PARTIALLY_REFUNDED", "REFUNDED"].includes(payment.status)
    )
      anomalies.push({
    ...
    if (["FAILED", "EXPIRED"].includes(session.status) && payment.status === "CONFIRMED")
      anomalies.push({
  ...
  for (const refund of refunds)
    for (const installment of refund.installments)
      if (isInstallmentOverdue(installment.status, installment.dueDate, now))
        anomalies.push({
  ...
  for (const expense of expenses)
    if (expenseIsOverdue(expense, now))
      anomalies.push({
        id: `expense-overdue-${expense.id}`,
        level: "HIGH",
        category: "EXPENSE",
        title: "Vendor bill overdue",
        detail: `${expense.expenseNumber} for ${expense.vendorName} is overdue with ${expense.currency} ${expenseRemaining(expense).toFixed(2)} outstanding.`,
        href: `/app/finance/expenses/${expense.id}`,
      });

  const activeBudgets = budgetPlans.filter(
    (plan) => plan.year === now.getUTCFullYear() && ["APPROVED", "LOCKED"].includes(plan.status),
  );
  const budgetPlanning = await Promise.all(
    activeBudgets.map(async (plan) => ({
      planId: plan.id,
      name: plan.name,
      year: plan.year,
      currency: plan.currency,
      alerts: await getBudgetAlerts(plan, now.getUTCMonth()),
      scenarios: await getBudgetScenarios(plan, now),
    })),
  );
  for (const budget of budgetPlanning)
    for (const alert of budget.alerts) {
      const severe = alert.status === "OVER_BUDGET" || alert.status === "UNDER_TARGET";
      anomalies.push({
        id: `budget-${budget.planId}-${alert.category}`,
        level: severe ? "HIGH" : "MEDIUM",
        category: "BUDGET",
        title: `Budget ${alert.status.replaceAll("_", " ").toLowerCase()}`,
        detail: `${budget.currency} ${alert.label}: budget ${alert.budget.toFixed(2)}, actual ${alert.actual.toFixed(2)}, favorable variance ${alert.favorableVariance.toFixed(2)}.`,
        href: `/app/finance/budgeting/${budget.planId}`,
      });
    }

  const accountAnalytics = new Map<
    string,
    { label: string; method: string; currency: string; volume: number; fees: number; count: number }
  >();
  for (const payment of settled) {
  ...
  const providerAnalytics = [...accountAnalytics.values()]
    .map((row) => ({ ...row, feeRate: row.volume > 0 ? round((row.fees / row.volume) * 100) : 0 }))
    .sort((a, b) => b.fees - a.fees);

  const realizedByCurrency = new Map<
    string,
    { inflow: number; fees: number; refunds: number; expenses: number }
  >();
  for (const payment of settled) {
    const c = payment.currency.toUpperCase();
    const row = realizedByCurrency.get(c) || { inflow: 0, fees: 0, refunds: 0, expenses: 0 };
  ...
  for (const refund of refunds)
    for (const installment of refund.installments) {
      if (installment.status !== "PAID" || !installment.paidAt || installment.paidAt < trailingStart)
        continue;
  ...
  for (const expense of expenses)
    for (const payment of expense.payments) {
      ...
      if (date < trailingStart) continue;
      const c = expense.currency.toUpperCase();
      const row = realizedByCurrency.get(c) || { inflow: 0, fees: 0, refunds: 0, expenses: 0 };
      row.expenses += payment.amount;
      realizedByCurrency.set(c, row);
    }

  const scheduledRefunds30 = new Map<string, number>();
  for (const refund of refunds)
    for (const installment of refund.installments) {
      if (
        ["PAID", "CANCELLED"].includes(installment.status) ||
        installment.dueDate < now ||
        installment.dueDate > in30
      )
        continue;
      const c = refund.currency.toUpperCase();
      scheduledRefunds30.set(c, round((scheduledRefunds30.get(c) || 0) + Number(installment.amount)));
    }
  const scheduledExpenses30 = new Map<string, number>();
  for (const expense of expenses) {
    const status = expenseEffectiveStatus(expense);
    if (!["APPROVED", "PARTIALLY_PAID"].includes(status) || !expense.dueDate) continue;
    const due = new Date(expense.dueDate);
    if (due < now || due > in30) continue;
    const c = expense.currency.toUpperCase();
    scheduledExpenses30.set(c, round((scheduledExpenses30.get(c) || 0) + expenseRemaining(expense)));
  }

  const forecastCurrencies = new Set([
    ...realizedByCurrency.keys(),
    ...scheduledRefunds30.keys(),
    ...scheduledExpenses30.keys(),
  ]);
  const forecast: FinanceForecastRow[] = [...forecastCurrencies]
    .map((currency) => {
      const realized = realizedByCurrency.get(currency) || { inflow: 0, fees: 0, refunds: 0, expenses: 0 };
      const daily = round((realized.inflow - realized.fees - realized.refunds - realized.expenses) / 90);
      const scheduledRefunds30d = scheduledRefunds30.get(currency) || 0;
      const scheduledExpenses30d = scheduledExpenses30.get(currency) || 0;
      const projected30dNet = round(daily * 30 - scheduledRefunds30d - scheduledExpenses30d);
      return {
        currency,
        trailing90DailyNet: daily,
        scheduledRefunds30d,
        scheduledExpenses30d,
        projected30dNet,
  ...
  const riskScore = Math.min(
    100,
    anomalies.reduce((sum, a) => sum + riskWeight[a.level], 0),
  );
  const riskLevel: FinanceRiskLevel =
    riskScore >= 70 ? "CRITICAL" : riskScore >= 40 ? "HIGH" : riskScore >= 15 ? "MEDIUM" : "LOW";
  return {
    generatedAt: now,
    riskScore,
    riskLevel,
    ...
    overdueExpenseCount: anomalies.filter((a) => a.category === "EXPENSE").length,
    budgetAlertCount: anomalies.filter((a) => a.category === "BUDGET").length,
    budgetPlanning,
    providerAnalytics,
    forecast,
...
export function deterministicExecutiveSummary(
  data: Awaited<ReturnType<typeof getFinanceEnterpriseIntelligence>>,
) {
  ...
  const forecast = data.forecast
    .map((f) => `${f.currency}: 30d ${f.projected30dNet.toFixed(2)}, 90d ${f.projected90dNet.toFixed(2)}`)
    .join("; ");
  const budget = data.budgetPlanning
    .map((b) => {
      const base = b.scenarios.find((s) => s.scenario === "BASE");
      const worst = b.scenarios.find((s) => s.scenario === "WORST");
      return `${b.currency}: ${b.alerts.length} budget alert(s), base net ${base?.net.toFixed(2) ?? "n/a"}, worst net ${worst?.net.toFixed(2) ?? "n/a"}`;
    })
    .join("; ");
  return [
    `Finance risk is ${data.riskLevel} (${data.riskScore}/100), with ${critical} critical and ${high} high-priority exception(s).`,
    `${data.reconciliationCount} reconciliation mismatch(es), ${data.duplicateCount} possible duplicate payment(s), ${data.overdueRefundCount} overdue refund installment(s), ${data.overdueExpenseCount} overdue vendor bill(s), and ${data.budgetAlertCount} budget variance alert(s) require attention.`,
    forecast
      ? `Explainable cash projection after fees, refunds and scheduled accounts payable: ${forecast}.`
      : "There is not enough realized cash history to produce a currency forecast yet.",
    budget
      ? `Budget planning scenarios: ${budget}.`
      : "No approved current-year budget is available for scenario analysis.",
    "AI findings are advisory only; budgets and projections never modify the ledger or move money.",
  ].join(" ");
}
