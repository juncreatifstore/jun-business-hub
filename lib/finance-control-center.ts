...
import { getBudgetAlerts, listBudgetPlans } from "@/lib/finance-budgeting";
...
function add(
  map: Map<string, CurrencySnapshot>,
  currency: string,
  field: keyof Omit<CurrencySnapshot, "currency">,
  value: number,
) {
...
export async function getFinanceControlCenter() {
  ...
  const [payments, refunds, accounts, onlineSessions, manualOrders, expenses, budgetPlans] =
    await Promise.all([
      ...
      listFinanceExpenses(500),
      listBudgetPlans(100),
    ]);
  const confirmedPayments = payments.filter((p) =>
    ["CONFIRMED", "PARTIALLY_REFUNDED", "REFUNDED"].includes(p.status),
  );
  ...
  for (const r of refunds)
    for (const i of r.installments)
      if (i.status === "PAID") add(currencyMap, r.currency, "refundsPaid", Number(i.amount));
  for (const e of expenses)
    for (const p of e.payments) add(currencyMap, e.currency, "expensesPaid", p.amount);
  for (const row of currencyMap.values())
    row.netCash = round(row.collected - row.fees - row.refundsPaid - row.expensesPaid);

  const monthCurrencies = new Map<
    string,
    { collected: number; refunds: number; expenses: number; net: number }
  >();
  for (const p of confirmedPayments.filter((p) => (p.paidAt || p.createdAt) >= monthStart)) {
    const c = p.currency.toUpperCase();
    const row = monthCurrencies.get(c) || { collected: 0, refunds: 0, expenses: 0, net: 0 };
    row.collected = round(row.collected + Number(p.amount) - Number(metaMap.get(p.id)?.feeAmount || 0));
    monthCurrencies.set(c, row);
  }
  for (const r of refunds)
    for (const i of r.installments)
      if (i.status === "PAID" && i.paidAt && i.paidAt >= monthStart) {
        const c = r.currency.toUpperCase();
        const row = monthCurrencies.get(c) || { collected: 0, refunds: 0, expenses: 0, net: 0 };
        row.refunds = round(row.refunds + Number(i.amount));
        monthCurrencies.set(c, row);
      }
  for (const e of expenses)
    for (const p of e.payments)
      if (new Date(p.paidAt) >= monthStart) {
        ...
        const row = monthCurrencies.get(c) || { collected: 0, refunds: 0, expenses: 0, net: 0 };
        row.expenses = round(row.expenses + p.amount);
        monthCurrencies.set(c, row);
      }
  for (const row of monthCurrencies.values()) row.net = round(row.collected - row.refunds - row.expenses);

  const allInstallments = refunds.flatMap((refund) =>
    refund.installments.map((installment) => ({ refund, installment })),
  );
  const overdueInstallments = allInstallments.filter(({ installment }) =>
    isInstallmentOverdue(installment.status, installment.dueDate, now),
  );
  const upcomingInstallments = allInstallments
    .filter(({ installment }) => installment.status === "SCHEDULED" && iFuture(installment.dueDate, now))
  ...
  const missingPaymentProof = confirmedPayments.filter(
    (p) => p.files.length === 0 && !onlinePaymentIds.has(p.id),
  );
  ...
  const expenseOpen = expenses.filter((e) =>
    ["SUBMITTED", "APPROVED", "PARTIALLY_PAID"].includes(expenseEffectiveStatus(e)),
  );
  const expenseOverdue = expenses.filter((e) => expenseIsOverdue(e, now));
  const apByCurrency = new Map<string, { outstanding: number; overdue: number; count: number }>();
  for (const e of expenseOpen) {
    const c = e.currency.toUpperCase();
    const row = apByCurrency.get(c) || { outstanding: 0, overdue: 0, count: 0 };
    row.outstanding = round(row.outstanding + expenseRemaining(e));
    row.count++;
    if (expenseIsOverdue(e, now)) row.overdue = round(row.overdue + expenseRemaining(e));
    apByCurrency.set(c, row);
  }
  ...
  const activeBudgets = budgetPlans.filter(
    (plan) => plan.year === now.getFullYear() && ["APPROVED", "LOCKED"].includes(plan.status),
  );
  const budgetVariance = (
    await Promise.all(
      activeBudgets.map(async (plan) => ({ plan, alerts: await getBudgetAlerts(plan, now.getMonth()) })),
    )
  ).flatMap((item) => item.alerts.map((alert) => ({ plan: item.plan, alert })));
  ...
  return {
    ...
    monthCurrencies: [...monthCurrencies.entries()]
    ...
    online: {
      ...
      paid: onlineSessions.filter((s) => s.status === "PAID").length,
      pending: onlineSessions.filter((s) => ["CREATED", "PENDING"].includes(s.status)).length,
      attention: onlineAttention.length,
    },
    manual: {
      total: manualOrders.length,
      open: manualOpen.length,
      completed: manualOrders.filter((o) => o.status === "COMPLETED").length,
    },
    expenses: {
      total: expenses.length,
      open: expenseOpen.length,
      overdue: expenseOverdue.length,
      paid: expenses.filter((e) => expenseEffectiveStatus(e) === "PAID").length,
      apByCurrency: [...apByCurrency.entries()].map(([currency, v]) => ({ currency, ...v })),
    },
    budgets: { active: activeBudgets.length, varianceAlerts: budgetVariance.length },
    alerts: {
      ...
      expenseOverdue: expenseOverdue.slice(0, 8),
      budgetVariance: budgetVariance.slice(0, 8),
      counts: {
        pendingPayments: pendingPayments.length,
        missingPaymentProof: missingPaymentProof.length,
        refundsToReview: refundsToReview.length,
        overdueInstallments: overdueInstallments.length,
        onlineAttention: onlineAttention.length,
        manualOpen: manualOpen.length,
        expenseOverdue: expenseOverdue.length,
        budgetVariance: budgetVariance.length,
      },
    },
    upcomingInstallments,
  };
}
function iFuture(date: Date, now: Date) {
