...
export function filterTreasuryStore(store: TreasuryStore, filters: CompanyFundsFilterParams): TreasuryStore {
  ...
  const integrations = store.integrations.filter(
    (item) => countryOk(item.country) && currencyOk(item.currency),
  );
  ...
  const investments = store.investments.filter(
    (item) =>
      countryOk(item.country) &&
      currencyOk(item.currency) &&
      (!item.projectIntegrationId || integrationIds.has(item.projectIntegrationId)),
  );
  const sources = store.sources.filter(
    (item) =>
      countryOk(item.country) &&
      currencyOk(item.currency) &&
      inPeriod(item.receivedAt, cutoff) &&
      (!item.accountId || accountIds.has(item.accountId)) &&
      (!item.partnerId || partnerIds.has(item.partnerId)),
  );

  const projectCashflows = store.projectCashflows.filter(
    (item) =>
      integrationIds.has(item.integrationId) &&
      currencyOk(item.currency) &&
      inPeriod(item.occurredAt, cutoff) &&
      (!item.accountId || accountIds.has(item.accountId)),
  );
  const accountSnapshots = store.accountSnapshots.filter(
    (item) => accountIds.has(item.accountId) && inPeriod(item.capturedAt, cutoff),
  );
  const reconciliations = store.reconciliations.filter(
    (item) => accountIds.has(item.accountId) && inPeriod(item.resolvedAt || item.createdAt, cutoff),
  );
  const forecastItems = store.forecastItems.filter(
    (item) =>
      currencyOk(item.currency) &&
      (!item.accountId || accountIds.has(item.accountId)) &&
      (!item.projectIntegrationId || integrationIds.has(item.projectIntegrationId)),
