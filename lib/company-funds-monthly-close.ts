...
async function archiveRevision(
  ...
  await prisma.appSetting.create({
    data: {
      key: revisionKey(row.period, row.revision, event),
...
export async function verifyMonthlyCloseIntegrity(period: string): Promise<MonthlyCloseIntegrityCheck> {
  ...
  for (const revision of revisions) {
    ...
    const snapshotMatches = snapshotHash === revision.integrity.snapshotHash;
    const previousMatches = revision.integrity.previousHash === previousHash;
    const chainMatches = chainHash === revision.integrity.chainHash;

    if (snapshotMatches && previousMatches && chainMatches) {
      verified += 1;
    } else {
      broken += 1;
  ...
  const status: MonthlyCloseIntegrityCheck["status"] = broken > 0
    ? "BROKEN"
    : legacy > 0
      ? "LEGACY"
      : "VERIFIED";
...
export async function getMonthlyCloseRevisionSummary(period: string) {
  ...
  const variance = current && previousClosed && current.revision > previousClosed.revision
