...
export type MonthlyFinancialClose={
  id:string;period:string;status:MonthCloseStatus;closedAt:string;closedById:string;closeNote:string;
...
function parse(value:string):MonthlyFinancialClose|null{try{const v=JSON.parse(value) as MonthlyFinancialClose;return v?.id&&validPeriod(v.period)&&v.snapshot?v:null}catch{return null}}
...
export async function listMonthlyFinancialCloses(){const rows=await prisma.appSetting.findMany({where:{key:{startsWith:PREFIX}},orderBy:{key:"desc"},take:240,select:{value:true}});return rows.map(r=>parse(r.value)).filter((v):v is MonthlyFinancialClose=>Boolean(v))}
