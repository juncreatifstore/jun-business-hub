...
export async function sendNativeVerificationCode(token: string): Promise<void> {
  ...
  if (lastSent && nowMs - lastSent < OTP_RESEND_MS) redirect(`/sign/${encodeURIComponent(token)}?error=otp_wait`);
  ...
  if (sendCount >= OTP_MAX_SENDS_PER_WINDOW) redirect(`/sign/${encodeURIComponent(token)}?error=otp_rate_limited`);
...
export async function verifyNativeVerificationCode(token: string, formData: FormData): Promise<void> {
  ...
  const nowMs = Date.now();
  if (recipient.otpLockedUntil && new Date(recipient.otpLockedUntil).getTime() > nowMs) redirect(`/sign/${encodeURIComponent(token)}?error=otp_locked`);
  if (!recipient.otpHash || !recipient.otpExpiresAt) redirect(`/sign/${encodeURIComponent(token)}?error=otp_send_first`);
  if (new Date(recipient.otpExpiresAt).getTime() <= nowMs) redirect(`/sign/${encodeURIComponent(token)}?error=otp_expired`);
  ...
  const now = new Date();
  recipients[index] = { ...recipient, verifiedAt: now.toISOString(), otpHash: null, otpExpiresAt: null, otpAttempts: 0, otpLockedUntil: null };
  await prisma.signatureRequest.update({ where: { id: request.id }, data: { recipients: signatureRecipientsPayload(recipients, meta) as never } });
  await audit({
    userId: null,
  ...
  const verifiedToken = await createVerifiedNativeSigningToken({ requestId: payload.requestId, email: payload.email, order: payload.order, linkVersion: payload.linkVersion, sessionId: payload.sessionId }, expiry);
  redirect(`/sign/${encodeURIComponent(verifiedToken)}?verified=1`);
}

export async function verifySignerInternally(requestId: string, signerOrder: number, formData: FormData): Promise<void> {
  const user = await assertPermission("DOCUMENT_SIGN");
  const reason = String(formData.get("reason") ?? "").trim().replace(/\s+/g, " ");
  if (reason.length < 5) redirect(`/app/signatures/${requestId}?toast_error=${encodeURIComponent("Indiquez le motif de la vérification interne.")}`);

  const request = await prisma.signatureRequest.findUnique({ where: { id: requestId } });
  if (!request || request.provider !== "JUN_NATIVE") redirect(`/app/signatures/${requestId}?toast_error=${encodeURIComponent("Demande JUN Secure Sign introuvable.")}`);
  if (!["SENT", "VIEWED", "PARTIALLY_SIGNED"].includes(request.status)) redirect(`/app/signatures/${requestId}?toast_error=${encodeURIComponent("Cette demande n'accepte plus de vérification interne.")}`);

  const recipients = signatureRecipients(request.recipients).sort((a, b) => a.order - b.order);
  const meta = signatureRequestMeta(request.recipients);
  const index = recipients.findIndex((r) => r.order === signerOrder);
  if (index < 0) redirect(`/app/signatures/${requestId}?toast_error=${encodeURIComponent("Signataire introuvable.")}`);
  const recipient = recipients[index];
  if (recipient.signedAt || recipient.declinedAt) redirect(`/app/signatures/${requestId}?toast_error=${encodeURIComponent("Ce signataire a déjà terminé son action.")}`);
  ...
  redirect(`/app/signatures/${requestId}?toast=${encodeURIComponent(`Identité de ${recipient.email} vérifiée en interne`)}`);
}
