...
function mailPath(accountId: string, extra: string) {
  return `/app/mail?mailbox=${encodeURIComponent(accountId)}&${extra}`;
}
...
export async function composeDraft(formData: FormData): Promise<void> {
  ...
  try {
    ...
    redirect(
      `/app/mail?compose=1&toast_error=${encodeURIComponent("You do not have access to this mailbox")}`,
    );
  }
  ...
  if (!account)
    redirect(`/app/mail?compose=1&toast_error=${encodeURIComponent("Selected mailbox is not connected")}`);
  ...
  await audit({
    ...
    after: { subject: d.subject, aiLevel, mailbox: account.email, mailAccountId: account.id },
  });
  revalidatePath("/app/mail");
  redirect(
    mailPath(account.id, `folder=DRAFTS&thread=${thread.id}&toast=${encodeURIComponent("Draft saved")}`),
  );
}
...
export async function sendDraft(threadId: string): Promise<void> {
  ...
  if (process.env.NODE_ENV === "production")
    redirect(
      `/app/mail?thread=${threadId}&toast_error=${encodeURIComponent("Use Gmail sending in production")}`,
    );
  ...
  try {
    ...
    redirect(`/app/mail?toast_error=${encodeURIComponent("You do not have access to this mailbox")}`);
  }
  if (thread.aiLevel === "BLOCKED")
    redirect(`/app/mail?thread=${threadId}&toast_error=This draft requires manual handling`);
  ...
  await audit({
    ...
    resourceType: "MailThread",
    resourceId: threadId,
    after: { delivery: "DEV mock" },
  });
  revalidatePath("/app/mail");
  redirect(
    mailPath(
      thread.mailAccountId,
      `folder=SENT&thread=${threadId}&toast=${encodeURIComponent("Marked as sent")}`,
    ),
  );
}
