...
async function recomputePaymentRefundStatus(paymentId: string) {
  ...
  if (!payment) return;
  const paidTotal = payment.refunds.reduce(
    (sum, refund) =>
      sum + refund.installments.filter((i) => i.status === "PAID").reduce((s, i) => s + Number(i.amount), 0),
    0,
  );
...
export async function createRefundWorkflow(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  if (caseId) {
    ...
    if (linkedCase.clientId !== d.clientId)
      return { message: "Selected case belongs to a different client." };
  }
  const globalAvailable = await getClientAvailableBalance(d.clientId, requestedCurrency);
  if (d.amount > globalAvailable + 0.005)
    return {
      message: `Refund exceeds the client's available ${requestedCurrency} balance: ${requestedCurrency} ${globalAvailable.toFixed(2)} available.`,
    };
  ...
  if (paymentId) {
    try {
      refund = await prisma.$transaction(async (tx) => {
        ...
        if (original.clientId !== d.clientId)
          throw new Error("Original payment belongs to a different client.");
        if (!["CONFIRMED", "PARTIALLY_REFUNDED", "REFUNDED"].includes(original.status))
          throw new Error("Only confirmed payments can be refunded.");
        if (original.currency.toUpperCase() !== requestedCurrency)
          throw new Error(`Refund currency must match the original payment (${original.currency}).`);
        const committed = original.refunds
        ...
        if (d.amount > available + 0.005)
          throw new Error(
            `Refund exceeds the available amount on ${original.reference}: ${original.currency} ${available.toFixed(2)} remaining.`,
          );
        ...
        return tx.refund.create({
          ...
          select: { id: true, refundNumber: true, currency: true, caseId: true },
        });
      });
    } catch (error) {
  ...
  await audit({
    ...
    after: {
      ...
      firstDueDate: String(formData.get("firstDueDate") || "") || null,
      paymentId,
      refundType,
      balanceMode: paymentId ? "PAYMENT_LINKED_AND_CLIENT_BALANCE" : "CLIENT_GLOBAL_BALANCE",
    },
  });
  await logActivity({
    type: "REFUND_REQUESTED",
    message: `Refund ${refundNumber} requested (${refund.currency} ${d.amount})`,
    userId: user.id,
    clientId: d.clientId,
...
export async function updateRefundReview(refundId: string, formData: FormData) {
  ...
  await saveRefundWorkflowMeta(refundId, {
    assignedToId,
    reviewNotes,
  ...
  if (refund.status === "REQUESTED")
    await prisma.refund.update({ where: { id: refundId }, data: { status: "UNDER_REVIEW" } });
  await audit({
    userId: user.id,
    action: "REFUND_REVIEW_UPDATE",
    resourceType: "Refund",
    resourceId: refundId,
    before: { assignedToId: before.assignedToId, reviewNotes: before.reviewNotes, status: refund.status },
    after: { assignedToId, reviewNotes, status: "UNDER_REVIEW" },
  });
...
export async function decideRefund(refundId: string, formData: FormData) {
  ...
  if (target === "APPROVED") {
    const globalAvailable = await getClientAvailableBalance(refund.clientId, refund.currency);
    if (Number(refund.amount) > globalAvailable + 0.005)
      throw new Error(
        `Cannot approve: client has only ${refund.currency} ${globalAvailable.toFixed(2)} available.`,
      );
    if (refund.paymentId) {
      ...
      if (!original) throw new Error("Original payment not found.");
      const others = original.refunds
        .filter((r) => r.id !== refund.id && !["REJECTED", "CANCELLED"].includes(r.status))
        .reduce((s, r) => s + Number(r.amount), 0);
      const available = Math.max(0, Math.round((Number(original.amount) - others) * 100) / 100);
      if (Number(refund.amount) > available + 0.005)
        throw new Error(
          `Cannot approve: original payment has only ${original.currency} ${available.toFixed(2)} refundable.`,
        );
    }
  }
  await prisma.refund.update({
    where: { id: refundId },
    data: { status: target, ...(target === "APPROVED" ? { approvedById: user.id } : {}) },
  });
  await saveRefundWorkflowMeta(refundId, {
    decisionReason,
  ...
  await audit({
    userId: user.id,
    action: `REFUND_${target}`,
    resourceType: "Refund",
    resourceId: refundId,
    before: { status: refund.status },
    after: { status: target, decisionReason },
  });
  await logActivity({
    type: "REFUND_UPDATED",
    message: `Refund ${refund.refundNumber} → ${target.replaceAll("_", " ")}`,
    userId: user.id,
...
export async function rescheduleRefundInstallment(installmentId: string, formData: FormData) {
  ...
  if (
    !inst ||
    ["PAID", "CANCELLED"].includes(inst.status) ||
    ["PAID", "REJECTED", "CANCELLED"].includes(inst.refund.status)
  )
    return;
  const nextStatus =
    dueDate.getTime() < new Date(new Date().setHours(0, 0, 0, 0)).getTime() ? "LATE" : "SCHEDULED";
  ...
  await audit({
    userId: user.id,
    action: "REFUND_INSTALLMENT_RESCHEDULE",
    resourceType: "RefundInstallment",
    resourceId: installmentId,
    before: { dueDate: inst.dueDate, status: inst.status },
    after: { dueDate, status: nextStatus },
  });
...
export async function saveRefundInstallmentPayout(installmentId: string, formData: FormData) {
  ...
  await audit({
    userId: user.id,
    action: "REFUND_INSTALLMENT_PAYOUT_DETAILS",
    resourceType: "RefundInstallment",
    resourceId: installmentId,
    before: { method: before.method, transactionRef: before.transactionRef },
    after: { method, transactionRef, notes },
  });
  revalidatePath(`/app/finance/refunds/${inst.refundId}`);
}
export async function sendRefundInstallmentReminder(installmentId: string) {
  ...
  const account = await prisma.clientAccount.findFirst({
    where: { clientId: inst.refund.clientId },
    select: { userId: true },
  });
  ...
  await prisma.notification.createMany({
    data: [...recipients].map((userId) => ({ userId, type: "REFUND_INSTALLMENT_REMINDER", title, body })),
  });
  await saveRefundInstallmentMeta(installmentId, {
    reminderCount: meta.reminderCount + 1,
    lastReminderAt: new Date().toISOString(),
  });
  await audit({
    userId: user.id,
    action: "REFUND_INSTALLMENT_REMINDER",
    resourceType: "RefundInstallment",
    resourceId: installmentId,
    after: { recipients: recipients.size, reminderCount: meta.reminderCount + 1 },
  });
  revalidatePath(`/app/finance/refunds/${inst.refundId}`);
}
export async function markRefundInstallmentPaid(installmentId: string) {
  ...
  const proof = await prisma.file.findFirst({
    where: { id: meta.proofFileId, refundId: inst.refundId, archivedAt: null },
    select: { id: true },
  });
  if (!proof) return;
  await prisma.refundInstallment.update({
  ...
  const remaining = inst.refund.installments.filter(
    (i) => i.id !== installmentId && !["PAID", "CANCELLED"].includes(i.status),
  ).length;
  ...
  await audit({
    userId: user.id,
    action: "REFUND_INSTALLMENT_PAID",
    resourceType: "RefundInstallment",
    resourceId: installmentId,
    after: {
      refund: inst.refund.refundNumber,
      amount: Number(inst.amount),
      newStatus,
      method: meta.method,
      transactionRef: meta.transactionRef,
      proofFileId: meta.proofFileId,
    },
  });
  await logActivity({
    type: "REFUND_UPDATED",
    message: `Refund payment recorded on ${inst.refund.refundNumber} (${newStatus.replaceAll("_", " ")})`,
    userId: user.id,
    clientId: inst.refund.clientId,
    caseId: inst.refund.caseId,
  });
  revalidatePath(`/app/finance/refunds/${inst.refundId}`);
  revalidatePath("/app/finance/refunds");
