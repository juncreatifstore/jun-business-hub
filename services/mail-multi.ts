...
export async function syncAllMailboxes(): Promise<void> {
  ...
  if (!(await rateLimitAsync(`gmail-sync-all:${user.id}`, 2, 60_000)))
    redirect("/app/mail?mailbox=ALL&toast_error=Sync all rate limit — wait a minute");
  const allowedIds = await getAccessibleMailboxIds(user, true);
  const accounts = allowedIds.length
  ...
  for (const account of accounts) {
    try {
      const accountTotal = await syncMailboxRecent(account.id, 250);
      total += accountTotal;
  ...
  if (failures.length)
    redirect(
      `/app/mail?mailbox=ALL&toast_error=${encodeURIComponent(`Sync completed with errors for ${failures.join(", ")}. ${total} new thread(s) imported.`)}`,
    );
  redirect(
    `/app/mail?mailbox=ALL&toast=${encodeURIComponent(`All accessible mailboxes synced — ${total} new thread${total === 1 ? "" : "s"}; recent labels refreshed`)}`,
  );
}
