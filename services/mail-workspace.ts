...
async function threadOrRedirect(user:CurrentUser,threadId:string){const t=await prisma.mailThread.findUnique({where:{id:threadId},select:{id:true,gmailThreadId:true,mailAccountId:true}});if(!t)redirect("/app/mail?toast_error=Thread not found");try{await assertMailboxAccess(user,t.mailAccountId);}catch{redirect("/app/mail?toast_error=You do not have access to this mailbox");}return t;}
...
export async function markMailThreadRead(threadId:string,folder="INBOX"){
 const user=await assertPermission("EMAIL_READ");const t=await threadOrRedirect(user,threadId);const state=await getMailThreadState(threadId);await saveMailThreadState({...state,isRead:true,updatedAt:new Date().toISOString(),updatedById:user.id});
