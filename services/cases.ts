...
export async function createCase(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  if (block?.blocked) {
    await audit({
      userId: user.id,
      action: "CASE_CREATE_BLOCKED_CLIENT",
      resourceType: "Client",
      resourceId: d.clientId,
      after: { reason: block.reason },
    });
    return {
      message: `New service blocked: JUN ended the commercial relationship with this client. Reason: ${block.reason}`,
    };
  }
  ...
  if (debts.length) {
    ...
    await audit({
      userId: user.id,
      action: "CASE_CREATE_BLOCKED_NEGATIVE_CLIENT_BALANCE",
      resourceType: "Client",
      resourceId: d.clientId,
      after: { debt: debtText },
    });
    return {
      message: `New service blocked: this client has an outstanding balance of ${debtText}. The debt must be settled or regularized before a new service can be opened.`,
    };
  }
  ...
  await audit({
    ...
    resourceType: "Case",
    resourceId: c.id,
    after: { caseNumber, title: c.title },
  });
  await logActivity({
    type: "CASE_CREATED",
    message: `Case ${caseNumber} opened: ${c.title}`,
    userId: user.id,
    clientId: d.clientId,
    caseId: c.id,
  });
...
export async function updateCase(caseId: string, _prev: FormState, formData: FormData): Promise<FormState> {
  ...
  if (d.status === "COMPLETED" && before.status !== "COMPLETED")
    return { message: "Use Case Closure & Final Review to mark this Case COMPLETED." };
  const client = await prisma.client.findUnique({ where: { id: d.clientId }, select: { id: true } });
  if (!client) return { message: "Selected client does not exist." };
  const reason = String(formData.get("correctionReason") || "")
    .trim()
    .slice(0, 1000);
  ...
  await logActivity({
    type: "CASE_UPDATED",
    message: `Case ${after.caseNumber} corrected: ${reason}`,
    userId: user.id,
    clientId: after.clientId,
    caseId,
  });
...
export async function updateCaseStatus(caseId: string, formData: FormData) {
  ...
  if (status === "COMPLETED")
    redirect(
      `/app/cases/${caseId}/closure?toast_error=${encodeURIComponent("COMPLETED requires the Case Closure & Final Review workflow.")}`,
    );
  ...
  await logActivity({
    type: "CASE_UPDATED",
    message: `Case ${updated.caseNumber} moved to ${status.replaceAll("_", " ")}`,
    userId: user.id,
...
export async function addCaseNote(caseId: string, formData: FormData) {
  ...
  await logActivity({
    type: "NOTE_ADDED",
    message: `Note added to ${c.caseNumber}`,
    userId: user.id,
    caseId,
    clientId: c.clientId,
  });
...
export async function deleteCase(caseId: string, formData: FormData): Promise<void> {
  ...
  if (confirmation !== expected)
    redirect(
      `/app/cases/${caseId}?toast_error=${encodeURIComponent(`Type ${expected} to confirm deletion.`)}`,
    );
  if (!reason)
  ...
  const linkedExpenses = expenses.filter(
    (e) => e.caseId === caseId && !["REJECTED", "CANCELLED"].includes(e.status),
  );
  ...
  if (blockers.length) {
    await audit({
      userId: user.id,
      action: "CASE_DELETE_BLOCKED",
      resourceType: "Case",
      resourceId: caseId,
      after: { caseNumber: c.caseNumber, blockers, reason },
    });
    redirect(
      `/app/cases/${caseId}?toast_error=${encodeURIComponent(`This service cannot be deleted because it has ${blockers.join(", ")}. Cancel or archive it instead to preserve the financial record.`)}`,
    );
  }
  await audit({
    userId: user.id,
    ...
    resourceType: "Case",
    resourceId: caseId,
    before: { caseNumber: c.caseNumber, title: c.title, clientId: c.clientId },
    after: { reason },
  });
  ...
  redirect(
    `/app/clients/${c.clientId}/services?toast=${encodeURIComponent(`Service ${c.caseNumber} deleted`)}`,
  );
}
