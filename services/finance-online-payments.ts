...
import {
  getOnlinePaymentSession,
  makeOnlinePublicToken,
  markOnlineSessionStatus,
...
export async function createOnlinePaymentRequest(formData: FormData) {
  ...
  const currency = String(formData.get("currency") || "USD")
    ...
    .toUpperCase()
    .slice(0, 3);
  const description = String(formData.get("description") || "JUN service payment")
    .trim()
    .slice(0, 180);
  ...
  if (!clientId || !Number.isFinite(amount) || amount <= 0)
    redirect(listDest("Client and a positive amount are required", true));
  ...
  if (caseId) {
    ...
    if (linkedCase.clientId !== clientId)
      redirect(listDest("Selected case belongs to a different client", true));
  }
  ...
  try {
    ...
    await audit({
      userId: user.id,
      action: "ONLINE_PAYMENT_PROVIDER_ERROR",
      resourceType: "Payment",
      resourceId: payment.id,
      after: { provider, sessionId: id, error: message.slice(0, 500) },
    });
    redirect(
      `/app/finance/online-payments/${id}?token=${encodeURIComponent(publicToken)}&toast_error=${encodeURIComponent(message)}`,
    );
  }
  ...
  await audit({
    userId: user.id,
    action: "ONLINE_PAYMENT_CREATE",
    resourceType: "Payment",
    resourceId: payment.id,
    after: { reference, provider, amount, currency, sessionId: id, expiresAt: session.expiresAt },
  });
  await logActivity({
    type: "PAYMENT_CREATED",
    message: `Online payment ${reference} created via ${provider.replaceAll("_", " ")}`,
    userId: user.id,
    clientId,
    caseId,
  });
  ...
  redirect(
    `/app/finance/online-payments/${id}?token=${encodeURIComponent(publicToken)}&toast=${encodeURIComponent("Online payment link created")}`,
  );
}
...
export async function cancelOnlinePaymentSession(id: string) {
  ...
  await audit({
    userId: user.id,
    action: "ONLINE_PAYMENT_CANCEL",
    resourceType: "Payment",
    resourceId: session.paymentId,
    before: { status: session.status },
    after: { status: "CANCELLED", sessionId: id },
  });
