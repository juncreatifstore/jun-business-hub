...
export async function addDriveComment(
  ...
  const mentionUserIds = String(formData.get("mentionUserIds") ?? "")
    .split(",")
    .map((v) => v.trim())
    .filter(Boolean)
    .slice(0, 20);
  const validMentions = mentionUserIds.length
    ? await prisma.user.findMany({
        where: { id: { in: mentionUserIds }, status: "ACTIVE", role: { not: "CLIENT" } },
        select: { id: true },
      })
    : [];
  ...
  const comment: DriveComment = {
    ...
    authorId: user.id,
    body,
    mentionUserIds: validMentions.map((u) => u.id),
    createdAt: new Date().toISOString(),
  };
  await prisma.appSetting.create({
    data: { key: `${resourceCommentPrefix(resourceType, resourceId)}${id}`, value: JSON.stringify(comment) },
  });
  for (const target of validMentions) {
    ...
    await prisma.notification.create({
      data: {
        userId: target.id,
        type: "DRIVE_MENTION",
        title: `Mentioned on ${resourceType.toLowerCase()}: ${resource.name}`,
  ...
  await audit({
    userId: user.id,
    action: "DRIVE_COMMENT_ADD",
    resourceType,
    resourceId,
    after: { commentId: id, mentions: comment.mentionUserIds },
  });
...
export async function requestDriveApproval(
  resourceType: DriveCollaborationResource,
  resourceId: string,
  formData: FormData,
): Promise<void> {
  ...
  if (!reviewerId || reviewerId === user.id)
    redirect(toast(returnTo, "toast_error", "Choose another reviewer"));
  const reviewer = await prisma.user.findFirst({
    where: { id: reviewerId, status: "ACTIVE", role: { not: "CLIENT" } },
    select: { id: true },
  });
  if (!reviewer) redirect(toast(returnTo, "toast_error", "Reviewer unavailable"));
  const id = crypto.randomUUID();
  const approval: DriveApproval = {
    id,
    resourceType,
    resourceId,
    requesterId: user.id,
  ...
  await prisma.appSetting.create({
    data: { key: `${DRIVE_APPROVAL_PREFIX}${id}`, value: JSON.stringify(approval) },
  });
  await prisma.notification.create({
    data: {
      userId: reviewer.id,
      type: "DRIVE_APPROVAL_REQUEST",
      title: `Approval requested: ${resource.name}`,
      body: message || `Please review this ${resourceType.toLowerCase()}.`,
    },
  });
  await audit({
    userId: user.id,
    action: "DRIVE_APPROVAL_REQUEST",
    resourceType,
    resourceId,
    after: { approvalId: id, reviewerId: reviewer.id },
  });
  revalidatePath("/app/drive/collaboration");
...
export async function reviewDriveApproval(approvalId: string, formData: FormData): Promise<void> {
  ...
  try {
    approval = JSON.parse(row.value);
  } catch {
    redirect(toast(returnTo, "toast_error", "Approval request is invalid"));
  }
  if (approval.status !== "PENDING") redirect(toast(returnTo, "toast_error", "Approval already reviewed"));
  if (approval.reviewerId !== user.id)
    redirect(toast(returnTo, "toast_error", "Only the assigned reviewer can decide"));
  approval.status = decision;
  approval.reviewedAt = new Date().toISOString();
  approval.reviewNote = String(formData.get("reviewNote") ?? "")
    .trim()
    .slice(0, 2000);
  ...
  await prisma.notification.create({
    data: {
      ...
      type: decision === "APPROVED" ? "DRIVE_APPROVED" : "DRIVE_CHANGES_REQUESTED",
      title: `${decision === "APPROVED" ? "Approved" : "Changes requested"}: ${resource?.name ?? approval.resourceType}`,
      body: approval.reviewNote || undefined,
    },
  });
  await audit({
    userId: user.id,
    action: decision === "APPROVED" ? "DRIVE_APPROVAL_APPROVE" : "DRIVE_APPROVAL_CHANGES",
    resourceType: approval.resourceType,
    resourceId: approval.resourceId,
    after: { approvalId, reviewNote: approval.reviewNote },
  });
  revalidatePath("/app/drive/collaboration");
...
export async function cancelDriveApproval(approvalId: string, formData?: FormData): Promise<void> {
  ...
  if (approval.requesterId !== user.id || approval.status !== "PENDING")
    redirect(toast(returnTo, "toast_error", "This request cannot be cancelled"));
  approval.status = "CANCELLED";
  approval.reviewedAt = new Date().toISOString();
  await prisma.appSetting.update({ where: { key }, data: { value: JSON.stringify(approval) } });
  await audit({
    userId: user.id,
    action: "DRIVE_APPROVAL_CANCEL",
    resourceType: approval.resourceType,
    resourceId: approval.resourceId,
    after: { approvalId },
  });
