...
export async function syncGoogleWorkspaceDesktop(): Promise<void> {
  ...
  if ((process.env.STORAGE_DRIVER || "").toUpperCase() !== "GOOGLE_WORKSPACE")
    dest("Set STORAGE_DRIVER=GOOGLE_WORKSPACE before syncing Desktop Sync", true);
  ...
  const relevant = tree.filter(
    (item) =>
      item.id === syncRootId ||
      item.relativePath === syncName ||
      item.relativePath.startsWith(`${syncName}/`),
  );
  const folderItems = relevant
    .filter((i) => i.isFolder && i.id !== syncRootId)
  ...
  for (const item of folderItems) {
    ...
    if (!folder)
      folder = await prisma.folder.findFirst({
        where: { name: item.name, parentId: parentDbId, isVault: false },
      });
    if (!folder) {
      folder = await prisma.folder.create({
        data: { name: item.name.slice(0, 120), parentId: parentDbId, isVault: false },
      });
      createdFolders++;
    } else if (folder.name !== item.name || folder.parentId !== parentDbId) {
      folder = await prisma.folder.update({
        where: { id: folder.id },
        data: { name: item.name.slice(0, 120), parentId: parentDbId },
      });
      updatedFolders++;
    }
    dbId = folder.id;
  ...
  for (const item of relevant.filter((i) => !i.isFolder)) {
    ...
    if (!file)
      file = await prisma.file.findFirst({ where: { storageKey: item.relativePath, isVault: false } });
    if (!file) {
      ...
      file.name !== item.name ||
      file.storageKey !== item.relativePath ||
      file.mimeType !== item.mimeType ||
      file.sizeBytes !== item.sizeBytes ||
      file.folderId !== parentDbId
    ) {
      file = await prisma.file.update({
        ...
        data: {
          name: item.name.slice(0, 200),
          storageKey: item.relativePath,
          mimeType: item.mimeType || file.mimeType,
          sizeBytes: Math.max(0, item.sizeBytes),
          folderId: parentDbId,
        },
      });
  ...
  await prisma.auditLog
    .create({
      data: {
        userId: user.id,
        action: "DRIVE_WORKSPACE_DESKTOP_SYNC",
        resourceType: "Drive",
        after: {
          syncFolder: syncName,
  ...
  dest(
    `Workspace sync complete: ${createdFiles} new files, ${updatedFiles} updated files, ${createdFolders} new folders`,
  );
}
