...
export async function addCaseMilestone(caseId: string, formData: FormData) {
  ...
  const state = await getCaseOperations(caseId);
  ...
  await saveCaseOperations(state);
  await audit({
    userId: user.id,
    action: "CASE_MILESTONE_CREATE",
    resourceType: "Case",
    resourceId: caseId,
    after: { title, ownerId, dueDate },
  });
  await logActivity({
    type: "CASE_UPDATED",
    message: `Milestone added to ${c.caseNumber}: ${title}`,
    userId: user.id,
    clientId: c.clientId,
    caseId,
  });
...
export async function updateCaseMilestone(caseId: string, milestoneId: string, formData: FormData) {
  ...
  if (!c) return;
  const state = await getCaseOperations(caseId);
  const item = state.milestones.find((m) => m.id === milestoneId);
  if (!item) return;
  const before = { ...item };
  const status = String(formData.get("status") || item.status) as CaseMilestoneStatus;
  if (!ALLOWED.includes(status)) return;
  const title =
    String(formData.get("title") || item.title)
      .trim()
      .slice(0, 180) || item.title;
  ...
  const blocker = String(formData.get("blocker") ?? item.blocker)
    .trim()
    .slice(0, 1000);
  Object.assign(item, {
    title,
  ...
  await logActivity({
    type: "CASE_UPDATED",
    message: `Milestone "${item.title}" → ${status.replaceAll("_", " ")}`,
    userId: user.id,
    clientId: c.clientId,
    caseId,
  });
...
export async function deleteCaseMilestone(caseId: string, milestoneId: string) {
  ...
  await logActivity({
    type: "CASE_UPDATED",
    message: `Milestone removed from ${c.caseNumber}: ${item.title}`,
    userId: user.id,
    clientId: c.clientId,
    caseId,
  });
