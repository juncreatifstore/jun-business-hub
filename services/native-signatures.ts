...
async function stampSigner(
  ...
  for (const field of recipient.fields ?? []) {
    ...
    const width = Math.max(
      40,
      field.width ?? (field.type === "SIGNATURE" ? 150 : field.type === "NAME" ? 130 : 90),
    );
...
async function sendSigningInvitation(input: {
  ...
  const link = await nativeSigningUrl(
    input.requestId,
    input.recipient.email,
    input.recipient.order,
    input.expiresAt,
  );
...
async function deliverCompletedPackage(input: {
  ...
  for (const recipient of input.recipients) {
    try {
      ...
      await audit({
        userId: null,
        action: "JUN_NATIVE_COMPLETION_EMAIL_SENT",
        resourceType: "SignatureRequest",
        resourceId: input.requestId,
        after: { signer: recipient.email, sentAt: deliveredAt, attachments: 2 },
      }).catch(() => undefined);
    } catch (error) {
      await audit({
        userId: null,
        action: "JUN_NATIVE_COMPLETION_EMAIL_FAILED",
        resourceType: "SignatureRequest",
        resourceId: input.requestId,
        after: {
...
export async function activateJunNativeSigning(requestId: string): Promise<void> {
  ...
  if (request.status !== "READY_FOR_SIGNATURE")
    redirect(`/app/signatures/${request.id}?toast_error=Request is not ready for activation`);
  ...
  if (!recipients.length || recipients.some((r) => !(r.fields ?? []).some((f) => f.type === "SIGNATURE"))) {
    redirect(
      `/app/signatures/${request.id}/prepare?toast_error=Every signer needs a Signature field before activation`,
    );
  }
  const account = await primaryMailAccount();
  if (!account)
    redirect(`/app/signatures/${request.id}?toast_error=Connect a Gmail account before starting JUN signing`);
    ...
    await prisma.signatureRequest
      .update({
        ...
        data: {
          ...
          recipients: request.recipients as never,
        },
      })
      .catch(() => undefined);
    await audit({
      userId: user.id,
      action: "JUN_NATIVE_INVITATION_FAILED",
      resourceType: "SignatureRequest",
      resourceId: request.id,
      after: {
        signer: recipients[0].email,
        error: error instanceof Error ? error.message.slice(0, 300) : "unknown",
      },
    }).catch(() => undefined);
    redirect(
      `/app/signatures/${request.id}?toast_error=Could not send the first signing invitation. Request was not activated.`,
    );
  }
  ...
  await audit({
    userId: user.id,
    action: "JUN_NATIVE_SIGNATURE_ACTIVATED",
    resourceType: "SignatureRequest",
    resourceId: request.id,
    after: {
      documentId: request.document.documentId,
      signerCount: recipients.length,
      firstInvitation: recipients[0].email,
      expiresAt: expiresAt.toISOString(),
    },
  });
...
export async function markNativeSignatureViewed(token: string): Promise<void> {
  ...
  if (!recipients[index].viewedAt)
    recipients[index] = { ...recipients[index], viewedAt: new Date().toISOString() };
  await prisma.signatureRequest
    .update({
      where: { id: request.id },
      data: {
...
export async function declineJunNativeSignature(token: string, formData: FormData): Promise<void> {
  ...
  await audit({
    userId: null,
    action: "JUN_NATIVE_SIGNATURE_DECLINED",
    resourceType: "SignatureRequest",
    resourceId: request.id,
    after: { signer: recipient.email, order: recipient.order, declinedAt: now.toISOString(), reason },
  }).catch(() => undefined);
  await prisma.notification
    .create({
      data: {
        ...
        title: `${request.document.documentId} was declined`,
        body: `${recipient.name} declined the signature request.`,
      },
    })
    .catch(() => undefined);
  revalidatePath(`/app/signatures/${request.id}`);
...
export async function sendJunNativeReminder(requestId: string, signerEmail: string): Promise<void> {
  ...
  if (index < 0 || recipients[index].signedAt || recipients[index].declinedAt)
    redirect(`/app/signatures/${requestId}?toast_error=Signer is not awaiting signature`);
  ...
  const account = await primaryMailAccount();
  if (!account)
    redirect(`/app/signatures/${requestId}?toast_error=Connect a Gmail account before sending reminders`);
  await sendSigningInvitation({
    accountId: account.id,
    requestId: request.id,
    documentId: request.document.documentId,
  ...
  await audit({
    userId: user.id,
    action: "JUN_NATIVE_SIGNATURE_REMINDER_SENT",
    resourceType: "SignatureRequest",
    resourceId: request.id,
    after: { signer: recipients[index].email, reminderSentAt: now },
  }).catch(() => undefined);
  revalidatePath(`/app/signatures/${request.id}`);
...
export async function completeJunNativeSignature(token: string, formData: FormData): Promise<void> {
  ...
  const signatureMethod =
    String(formData.get("signatureMethod") ?? "TYPE").toUpperCase() === "DRAW"
      ? ("DRAW" as const)
      : ("TYPE" as const);
  const signatureImage =
    signatureMethod === "DRAW" ? parseDrawSignature(formData.get("signatureData")) : null;
  ...
  if (signatureMethod === "DRAW" && !signatureImage)
    redirect(`/sign/${encodeURIComponent(token)}?error=signature_draw_required`);
    ...
    if (account && nextIndex >= 0) {
      try {
        await sendSigningInvitation({
          ...
          documentId: request.document.documentId,
          documentTitle: request.document.title,
          recipient: recipients[nextIndex],
          expiresAt,
          message: meta.message,
        });
        ...
        await audit({
          userId: null,
          action: "JUN_NATIVE_NEXT_INVITATION_SENT",
          resourceType: "SignatureRequest",
          resourceId: request.id,
          after: {
            signer: recipients[nextIndex].email,
            order: recipients[nextIndex].order,
            sentAt: recipients[nextIndex].invitationSentAt,
          },
        }).catch(() => undefined);
      } catch (error) {
        await prisma.notification
          .create({
            data: {
              userId: request.createdById,
              type: "SIGNATURE_EMAIL_FAILED",
              title: `Invitation failed for ${request.document.documentId}`,
              body: `Signer ${recipients[nextIndex].name} is next, but JUN could not send the automatic invitation.`,
            },
          })
          .catch(() => undefined);
        await audit({
          userId: null,
          action: "JUN_NATIVE_NEXT_INVITATION_FAILED",
          resourceType: "SignatureRequest",
          resourceId: request.id,
          after: {
            signer: recipients[nextIndex].email,
    ...
    await prisma.notification
      .create({
        data: {
          ...
          title: `${request.document.documentId} signed in JUN`,
          body: "All signers completed. The signed PDF was archived with a SHA-256 integrity hash.",
        },
      })
      .catch(() => undefined);
    await logActivity({
      ...
      message: `Native signature completed for ${request.document.documentId}`,
      clientId: request.document.clientId ?? undefined,
      caseId: request.document.caseId ?? undefined,
    }).catch(() => undefined);

    if (account) {
      try {
        const delivery = await deliverCompletedPackage({
          accountId: account.id,
          requestId: request.id,
          documentId: request.document.documentId,
          documentTitle: request.document.title,
        ...
        for (let i = 0; i < recipients.length; i++) {
          if (delivery.delivered.has(recipients[i].email.toLowerCase()))
            recipients[i] = { ...recipients[i], completionEmailSentAt: delivery.deliveredAt };
        }
        await prisma.signatureRequest.update({
          where: { id: request.id },
          data: { recipients: signatureRecipientsPayload(recipients, meta) as never },
        ...
        await prisma.notification
          .create({
            data: {
              userId: request.createdById,
              type: "SIGNATURE_EMAIL_FAILED",
              title: `Signed package email failed for ${request.document.documentId}`,
              body: "The signature is valid and archived, but JUN could not complete automatic email delivery. Use the Signed package buttons to send the files manually.",
            },
          })
          .catch(() => undefined);
        await audit({
          userId: null,
          action: "JUN_NATIVE_COMPLETION_DELIVERY_FAILED",
          resourceType: "SignatureRequest",
          resourceId: request.id,
          after: { error: error instanceof Error ? error.message.slice(0, 300) : "unknown" },
        }).catch(() => undefined);
      }
    }
  }
