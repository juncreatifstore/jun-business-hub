...
import {
  ...
  getFinancePaymentAccount,
  type FinancePaymentAccount,
} from "@/lib/finance-payment-accounts";
...
export async function createFinancePaymentAccount(formData: FormData) {
  ...
  const receiverName = String(formData.get("receiverName") || "")
    .trim()
    .slice(0, 160);
  const accountDescriptor = String(formData.get("accountDescriptor") || "")
    .trim()
    .slice(0, 240);
  ...
  const account: FinancePaymentAccount = {
    id,
    label,
    method,
    currency,
  ...
  await prisma.appSetting.create({
    data: { key: `${PAYMENT_ACCOUNT_PREFIX}${id}`, value: JSON.stringify(account) },
  });
  await audit({
    userId: user.id,
    ...
    resourceId: id,
    after: { label, method, currency, receiverName, accountDescriptor, feePercent, feeFixed },
  });
...
export async function toggleFinancePaymentAccount(id: string) {
  ...
  await audit({
    userId: user.id,
    action: next.enabled ? "PAYMENT_ACCOUNT_ENABLE" : "PAYMENT_ACCOUNT_DISABLE",
    resourceType: "FinancePaymentAccount",
    resourceId: id,
...
export async function updateFinancePaymentAccount(id: string, formData: FormData) {
  ...
  const label = String(formData.get("label") || account.label)
    .trim()
    .slice(0, 120);
  const receiverName = String(formData.get("receiverName") || "")
    .trim()
