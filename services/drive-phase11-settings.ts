...
import {
  DRIVE_PRIVACY_FILE_PREFIX,
  DRIVE_PRIVACY_GLOBAL_KEY,
  getDrivePrivacyPolicy,
  type DrivePrivacyPolicy,
...
export async function setDriveQuotaToTwoTb(formData?: FormData): Promise<void> {
  ...
  await prisma.appSetting.upsert({
    ...
    create: { key: DRIVE_ENTERPRISE_SETTINGS_KEY, value: JSON.stringify(next) },
  });
  await audit({
    userId: user.id,
    action: "DRIVE_QUOTA_SET_2TB",
    resourceType: "Drive",
...
export async function saveGlobalDrivePrivacyPolicy(formData: FormData): Promise<void> {
  ...
  if (!title || body.length < 50)
    redirect(
      toast(
        "/app/drive/privacy",
        "Provide a title and a confidentiality text of at least 50 characters",
        true,
      ),
    );
  const previous = await getDrivePrivacyPolicy("__global_preview__");
  const policy: DrivePrivacyPolicy = {
    version: new Date().toISOString(),
    title,
    body,
    updatedAt: new Date().toISOString(),
    required: String(formData.get("required") ?? "") !== "0",
  };
  await prisma.appSetting.upsert({
    where: { key: DRIVE_PRIVACY_GLOBAL_KEY },
    update: { value: JSON.stringify(policy) },
    create: { key: DRIVE_PRIVACY_GLOBAL_KEY, value: JSON.stringify(policy) },
  });
  await audit({
    userId: user.id,
    action: "DRIVE_PRIVACY_GLOBAL_UPDATE",
    resourceType: "Drive",
  ...
  redirect(
    toast(
      "/app/drive/privacy",
      "Global confidentiality policy updated. Existing consents must be accepted again because the policy version changed.",
    ),
  );
}
...
export async function saveFileDrivePrivacyPolicy(fileId: string, formData: FormData): Promise<void> {
  ...
  if (useGlobal) {
    ...
    await audit({
      userId: user.id,
      action: "DRIVE_PRIVACY_FILE_RESET",
      resourceType: "File",
      resourceId: file.id,
    });
    revalidatePath("/app/drive/privacy");
  ...
  const body = String(formData.get("body") ?? "")
    ...
    .slice(0, 12000);
  if (!title || body.length < 50)
    redirect(toast("/app/drive/privacy", "Provide a valid per-file confidentiality policy", true));
  const policy: DrivePrivacyPolicy = {
    version: new Date().toISOString(),
    title,
    body,
  ...
  await audit({
    userId: user.id,
    action: "DRIVE_PRIVACY_FILE_UPDATE",
    resourceType: "File",
    resourceId: file.id,
    after: { version: policy.version, title: policy.title },
  });
  revalidatePath("/app/drive/privacy");
