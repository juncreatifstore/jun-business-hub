...
export async function testWhatsAppWebhookLocally() {
  ...
  if (!client) {
    redirect(
      `/app/settings/whatsapp?toast_error=${encodeURIComponent("Local webhook test requires at least one client with a WhatsApp or phone number.")}`,
    );
  }
  ...
  if (!phone) {
    redirect(
      `/app/settings/whatsapp?toast_error=${encodeURIComponent("Selected client does not have a valid WhatsApp number.")}`,
    );
  }
        ...
        changes: [
          {
            ...
            value: {
              ...
              contacts: [
                { profile: { name: `${client.firstName} ${client.lastName}`.trim() }, wa_id: phone },
              ],
              messages: [
                {
                  from: phone,
  ...
  try {
    ...
    redirect(
      `/app/settings/whatsapp?toast_error=${encodeURIComponent(`Webhook URL unreachable: ${message}`)}`,
    );
  }
  ...
  if (!response.ok) {
    ...
    redirect(
      `/app/settings/whatsapp?toast_error=${encodeURIComponent(`Webhook test failed with HTTP ${response.status}${body ? `: ${body.slice(0, 300)}` : ""}`)}`,
    );
  }
  ...
  await prisma.appSetting.upsert({
    ...
    create: {
      key: "whatsapp.webhook.last_local_test",
  ...
  if (!activity) {
    redirect(
      `/app/settings/whatsapp?toast_error=${encodeURIComponent("Webhook URL returned 200, but JUN did not store the simulated incoming message.")}`,
    );
  }
