...
import {
  ...
  deleteCaseCommunicationRecord,
  listCaseCommunications,
  type CaseCommunicationChannel,
  type CaseCommunicationDirection,
  type CaseCommunicationImportance,
...
function path(caseId: string, msg?: string, error = false) {
  return `/app/cases/${caseId}/history${msg ? `?${error ? "toast_error" : "toast"}=${encodeURIComponent(msg)}` : ""}`;
}
...
export async function addCaseCommunication(caseId: string, formData: FormData) {
  ...
  if (
    ...
    !IMPORTANCE.includes(importance) ||
    !subject ||
    !summary
  )
    redirect(path(caseId, "Channel, direction, subject and summary are required.", true));
  const occurredAt =
    occurredRaw && !Number.isNaN(new Date(occurredRaw).getTime())
      ? new Date(occurredRaw).toISOString()
      : new Date().toISOString();
  const row = await createCaseCommunication({
    caseId,
    clientId: c.clientId,
    channel: channel as CaseCommunicationChannel,
    direction: direction as CaseCommunicationDirection,
    importance: importance as CaseCommunicationImportance,
    subject,
    summary,
    contact,
    occurredAt,
  ...
  await audit({
    userId: user.id,
    action: "CASE_COMMUNICATION_LOGGED",
    resourceType: "Case",
    resourceId: caseId,
    after: { communicationId: row.id, channel, direction, importance, subject, contact, occurredAt },
  });
  await logActivity({
    userId: user.id,
    type: "CASE_UPDATED",
    message: `${channel} communication logged: ${subject}`,
    clientId: c.clientId,
    caseId,
  });
...
export async function deleteCaseCommunication(caseId: string, communicationId: string, formData: FormData) {
  ...
  await audit({
    userId: user.id,
    action: "CASE_COMMUNICATION_DELETED",
    resourceType: "Case",
    resourceId: caseId,
    before: row,
    after: { reason },
  ...
  await logActivity({
    userId: user.id,
    type: "CASE_UPDATED",
    message: `Communication record removed: ${row.subject} · ${reason}`,
