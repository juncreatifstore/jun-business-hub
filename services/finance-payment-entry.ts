...
export async function createPaymentWithAccount(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  if (block?.blocked) {
    await audit({
      userId: user.id,
      action: "PAYMENT_CREATE_BLOCKED_CLIENT",
      resourceType: "Client",
      resourceId: d.clientId,
      after: { reason: block.reason },
    });
    return {
      message: `Payment blocked: JUN ended the commercial relationship with this client. Reason: ${block.reason}`,
    };
  }
  ...
  if (account && account.method !== d.method)
    return {
      message: `Payment method must match the selected account (${account.method.replaceAll("_", " ")}).`,
    };
  if (account && account.currency !== d.currency.toUpperCase())
    return { message: `Payment currency must match the selected account (${account.currency}).` };
  ...
  await logActivity({
    ...
    message: `Payment ${reference} recorded (${payment.currency} ${d.amount})${account ? ` to ${account.label}` : ""}`,
    userId: user.id,
    clientId: d.clientId,
