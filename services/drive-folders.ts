...
export async function renameDriveFolder(folderId: string, formData: FormData): Promise<void> {
  ...
  const duplicate = await prisma.folder.findFirst({
    where: { isVault: false, parentId: folder.parentId, name, id: { not: folder.id } },
    select: { id: true },
  });
  ...
  await logActivity({
    userId: user.id,
    type: "FOLDER_RENAMED",
    message: `Renamed folder ${folder.name} to ${name}`,
    resourceType: "Folder",
    resourceId: folder.id,
  });
...
export async function moveDriveFolder(folderId: string, formData: FormData): Promise<void> {
  ...
  if (targetId === folder.id)
    redirect(toast(returnTo, "toast_error", "A folder cannot be moved into itself"));
  const descendants = await descendantIds(folder.id);
  if (targetId && descendants.includes(targetId))
    redirect(toast(returnTo, "toast_error", "A folder cannot be moved into one of its subfolders"));
  if (targetId && !(await getFolder(targetId)))
    redirect(toast(returnTo, "toast_error", "Destination folder not found"));
  const duplicate = await prisma.folder.findFirst({
    where: { isVault: false, parentId: targetId, name: folder.name, id: { not: folder.id } },
    select: { id: true },
  });
  if (duplicate)
    redirect(toast(returnTo, "toast_error", "A folder with this name already exists in the destination"));
...
export async function shareDriveFolder(folderId: string, formData: FormData): Promise<void> {
  ...
  if (!targetUserId || targetUserId === user.id)
    redirect(toast(returnTo, "toast_error", "Choose another team member"));
  ...
  if (!target || target.role === "CLIENT" || target.status !== "ACTIVE")
    redirect(toast(returnTo, "toast_error", "Team member unavailable"));
  ...
  await audit({
    userId: user.id,
    action: "FOLDER_SHARE_INTERNAL",
    resourceType: "Folder",
    resourceId: folder.id,
    after: { sharedWithId: target.id, inherited: true },
  });
  await logActivity({
    userId: user.id,
    type: "FOLDER_SHARED",
    message: `Shared folder ${folder.name} with ${target.firstName} ${target.lastName}`,
    resourceType: "Folder",
    resourceId: folder.id,
  });
  revalidatePath("/app/drive");
...
export async function unshareDriveFolder(
  folderId: string,
  targetUserId: string,
  formData?: FormData,
): Promise<void> {
  ...
  await audit({
    userId: user.id,
    action: "FOLDER_UNSHARE_INTERNAL",
    resourceType: "Folder",
    resourceId: folderId,
...
async function cleanupFileMetadata(fileId: string) {
  ...
  for (const s of settings) {
    if (s.key.startsWith(`${VERSION_PREFIX}${fileId}.`)) {
      try {
        const meta = JSON.parse(s.value) as { storageKey?: string };
        if (meta.storageKey)
          await storage()
...
export async function permanentlyDeleteDriveFolder(folderId: string, formData?: FormData): Promise<void> {
  ...
  const folder = await getFolder(folderId);
  if (!folder) redirect(toast(returnTo, "toast_error", "Folder not found"));
  const trashed = await prisma.appSetting.findUnique({
    where: { key: `${FOLDER_TRASH_PREFIX}${folder.id}` },
    select: { id: true },
  });
  if (!trashed)
    redirect(toast(returnTo, "toast_error", "Move the folder to Trash before permanent deletion"));
  const ids = await descendantIds(folder.id);
  const files = await prisma.file.findMany({
    where: { isVault: false, folderId: { in: ids } },
  ...
  for (const row of [...rows].sort((a, b) => depth(b.id) - depth(a.id)))
    await prisma.folder.delete({ where: { id: row.id } });
  await audit({
    userId: user.id,
    action: "FOLDER_DELETE_PERMANENT",
    resourceType: "Folder",
    resourceId: folder.id,
    before: { name: folder.name, subtreeFolders: ids.length, files: files.length },
  });
  revalidatePath("/app/drive");
