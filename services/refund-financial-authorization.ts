...
async function requireAuthorization(input: {
  ...
  if (authorization.status !== "APPROVED") {
    await audit({
      ...
      action: "REFUND_PAYOUT_AUTHORIZATION_REQUIRED",
      resourceType: "Refund",
...
export async function markRefundInstallmentPaidAuthorized(installmentId: string): Promise<void> {
  ...
  if (!inst) throw new Error("Refund installment not found");
  ...
  await requireAuthorization({
    resourceId: `installment:${inst.id}`,
    reference: `${inst.refund.refundNumber}-${inst.number}`,
    description: `Paiement remboursement ${inst.refund.refundNumber} · tranche ${inst.number}`,
    amount: Number(inst.amount),
    currency: inst.refund.currency,
  ...
  if (!result.duplicate) {
    await audit({
      ...
      after: {
        ...
        proofFileId: result.proofFileId,
    ...
    await logActivity({
      type: "REFUND_UPDATED",
      message: `Refund payment recorded on ${result.refundNumber} (${result.newStatus.replaceAll("_", " ")})`,
      userId: user.id,
      clientId: result.clientId,
      caseId: result.caseId,
    });
  }
  revalidatePath(`/app/finance/refunds/${result.refundId}`);
  revalidatePath("/app/finance/refunds");
  revalidatePath("/app/finance/payments");
  revalidatePath(`/app/clients/${result.clientId}`);
...
export async function confirmLegacyRefundFullyPaidAuthorized(
  refundId: string,
  formData: FormData,
): Promise<void> {
  const user = await assertPermission("REFUND_APPROVE");
  const refund = await prisma.refund.findUnique({
    where: { id: refundId },
    select: { id: true, refundNumber: true, amount: true, currency: true, status: true },
  });
  if (!refund) throw new Error("Refund not found");
  // Completed retries return before authorization or closed-period checks.
  if (refund.status === "PAID") return;
  await requireAuthorization({
    resourceId: `legacy:${refund.id}`,
    reference: refund.refundNumber,
    description: `Remboursement complet legacy ${refund.refundNumber}`,
