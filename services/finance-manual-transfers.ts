...
export async function createManualTransferReceiver(formData: FormData) {
  ...
  const receiver: ManualTransferReceiver = {
    ...
    legalName,
    receiverType:
      String(formData.get("receiverType") || "BUSINESS") === "INDIVIDUAL_BUSINESS_REPRESENTATIVE"
        ? "INDIVIDUAL_BUSINESS_REPRESENTATIVE"
        : "BUSINESS",
    ...
    iban: String(formData.get("iban") || "")
      .trim()
      .slice(0, 120),
    swiftBic: String(formData.get("swiftBic") || "")
      .trim()
      .slice(0, 80),
  ...
  await prisma.appSetting.create({
    data: { key: `${MANUAL_RECEIVER_PREFIX}${id}`, value: JSON.stringify(receiver) },
  });
  await audit({
    userId: user.id,
    action: "MANUAL_TRANSFER_RECEIVER_CREATE",
    resourceType: "ManualTransferReceiver",
    resourceId: id,
    after: { label, rail, legalName, country: receiver.country, currency },
  });
...
export async function toggleManualTransferReceiver(id: string) {
  ...
  await prisma.appSetting.update({
    where: { key: `${MANUAL_RECEIVER_PREFIX}${id}` },
    data: { value: JSON.stringify(next) },
  });
  await audit({
    userId: user.id,
    action: next.enabled ? "MANUAL_TRANSFER_RECEIVER_ENABLE" : "MANUAL_TRANSFER_RECEIVER_DISABLE",
    resourceType: "ManualTransferReceiver",
    resourceId: id,
    before: { enabled: receiver.enabled },
...
function baseInstructions(
  order: Omit<ManualTransferOrder, "instructions">,
  receiver: ManualTransferReceiver,
  language: string,
) {
        ...
        messages: [
          {
            role: "system",
            content:
              "Translate the following professional payment instructions faithfully into the requested language. Preserve all amounts, banking identifiers, legal names, reference numbers, numbered steps, and the distinction between commercial/business payment and personal/friends/family transfers. Do not add facts.",
          },
...
export async function createManualTransferOrder(formData: FormData) {
  ...
  const sendCurrency = String(formData.get("sendCurrency") || "USD")
    .trim()
    .toUpperCase()
    .slice(0, 3);
  const receiveCurrency = String(formData.get("receiveCurrency") || receiver.currency)
    .trim()
    .toUpperCase()
    .slice(0, 3);
  const exchangeRate = Math.max(
    0.0000001,
    Number(formData.get("exchangeRate") || (sendCurrency === receiveCurrency ? 1 : 0)),
  );
  if (!Number.isFinite(sendAmount) || sendAmount <= 0)
    redirect(orderDest("Amount to send must be positive", true));
  if (!exchangeRate || !Number.isFinite(exchangeRate))
    redirect(orderDest("A valid exchange rate is required when currencies differ", true));
  const { feeAmount, netAfterFees, receiveAmount } = calculateManualTransfer(
    sendAmount,
    receiver.feePercent,
    receiver.feeFixed,
    exchangeRate,
  ...
  const language = String(formData.get("language") || "French")
    .trim()
    .slice(0, 80);
  const orderBase: Omit<ManualTransferOrder, "instructions"> = {
    ...
    payerName: String(formData.get("payerName") || "")
      .trim()
      .slice(0, 180),
    purpose: String(formData.get("purpose") || "Commercial payment")
      .trim()
      .slice(0, 240),
    originCountry: String(formData.get("originCountry") || "")
      .trim()
      .slice(0, 100),
    destinationCountry: String(formData.get("destinationCountry") || receiver.country)
      .trim()
      .slice(0, 100),
    sendAmount,
    sendCurrency,
  ...
  await prisma.appSetting.create({
    data: { key: `${MANUAL_ORDER_PREFIX}${id}`, value: JSON.stringify(order) },
  });
  await audit({
    userId: user.id,
    action: "MANUAL_TRANSFER_ORDER_CREATE",
...
export async function setManualTransferOrderStatus(id: string, formData: FormData) {
  ...
  const next = {
    ...order,
    status: status as ManualTransferOrder["status"],
    updatedAt: new Date().toISOString(),
  };
  await prisma.appSetting.update({
    where: { key: `${MANUAL_ORDER_PREFIX}${id}` },
    data: { value: JSON.stringify(next) },
  });
  await audit({
    userId: user.id,
    action: `MANUAL_TRANSFER_ORDER_${status}`,
    resourceType: "ManualTransferOrder",
    resourceId: id,
