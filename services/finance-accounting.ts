...
export async function syncAccountingLedgerAction() {
  ...
  await audit({
    ...
    resourceType: "AccountingLedger",
    after: result,
  });
  revalidatePath("/app/finance/accounting");
  revalidatePath("/app/finance/accounting/statements");
  redirect(
    `/app/finance/accounting?success=${encodeURIComponent(`Ledger synchronized: ${result.created} created, ${result.skipped} already posted, ${result.closed} blocked by closed periods.`)}`,
  );
}
...
export async function closeAccountingPeriodAction(formData: FormData) {
  ...
  if (confirmation !== "CLOSE")
    redirect(
      `/app/finance/accounting/close?error=${encodeURIComponent("Type CLOSE to confirm the accounting period lock.")}`,
    );
  let errorMessage = "";
  try {
    if ((await getClosedPeriods()).some((p) => p.period === period))
      throw new Error(`Period ${period} is already closed and cannot be modified.`);
    ...
    revalidatePath("/app/finance/accounting");
    revalidatePath("/app/finance/accounting/close");
    revalidatePath("/app/finance/accounting/statements");
  } catch (error) {
    errorMessage = error instanceof Error ? error.message : "Unable to close period";
  }
