...
export async function createFolder(formData: FormData): Promise<void> {
  ...
  if (parentId) {
    const parent = await prisma.folder.findFirst({
      where: { id: parentId, isVault: false },
      select: { id: true },
    });
  ...
  const existing = await prisma.folder.findFirst({
    where: { name, parentId, isVault: false },
    select: { id: true },
  });
  if (existing)
    redirect(appendToast(driveDest(parentId), "toast_error", "A folder with this name already exists"));
...
export async function uploadFile(formData: FormData): Promise<void> {
  ...
  if (!ALLOWED_MIME.includes(mime)) {
    redirect(
      `${isVault ? "/app/vault" : "/app/drive"}?toast_error=${encodeURIComponent(`File type not allowed (${mime})`)}`,
    );
  }
  ...
  const vaultCategory =
    isVault && (VAULT_CATEGORIES as readonly string[]).includes(vaultCategoryRaw) ? vaultCategoryRaw : null;
  ...
  if (clientId && !(await prisma.client.findUnique({ where: { id: clientId }, select: { id: true } })))
    redirect("/app/drive?toast_error=Linked%20client%20not%20found");
  if (caseId && !(await prisma.case.findUnique({ where: { id: caseId }, select: { id: true } })))
    redirect("/app/drive?toast_error=Linked%20case%20not%20found");
  if (
    folderId &&
    !(await prisma.folder.findFirst({ where: { id: folderId, isVault: false }, select: { id: true } }))
  )
  ...
  if (!isVault) {
    ...
    if (!quota.allowed) {
      ...
      redirect(
        `${driveDest(folderId)}${driveDest(folderId).includes("?") ? "&" : "?"}toast_error=${encodeURIComponent(`Drive quota exceeded (${usedGb} GB used / ${limitGb} GB)`)} `,
      );
    }
  }
  ...
  await audit({
    userId: user.id,
    action: isVault ? "VAULT_UPLOAD" : "FILE_UPLOAD",
    resourceType: "File",
    resourceId: record.id,
    after: { name: record.name, sizeBytes: raw.size, category, isVault, folderId },
  });
  await logActivity({
    userId: user.id,
    type: "FILE_UPLOADED",
    message: `Uploaded ${record.name}`,
...
export async function deleteFile(fileId: string, formData?: FormData): Promise<void> {
  ...
  if (!file.isVault) {
    ...
    await audit({
      ...
      resourceType: "File",
      resourceId: file.id,
      before: { name: file.name, folderId: file.folderId },
      after: { archived: true },
    });
    revalidatePath("/app/drive");
    redirect(
      appendToast(safeDriveReturn(formData, driveDest(file.folderId)), "toast", "File moved to Trash"),
    );
  }
  ...
  await audit({
    ...
    resourceType: "File",
    resourceId: file.id,
    before: { name: file.name, isVault: true },
  });
...
export async function restoreFile(fileId: string, formData?: FormData): Promise<void> {
  ...
  const file = await prisma.file.findFirst({
    where: { id: fileId, isVault: false, archivedAt: { not: null } },
  });
  ...
  await audit({
    ...
    resourceType: "File",
    resourceId: file.id,
    after: { name: file.name, folderId: file.folderId },
  });
  revalidatePath("/app/drive");
...
export async function permanentlyDeleteFile(fileId: string, formData?: FormData): Promise<void> {
  ...
  revalidatePath("/app/drive");
  redirect(
    appendToast(safeDriveReturn(formData, "/app/drive?view=trash"), "toast", "File permanently deleted"),
  );
}
...
export async function toggleFavorite(fileId: string, formData?: FormData): Promise<void> {
  ...
  await audit({
    userId: user.id,
    action: existing ? "FILE_UNSTAR" : "FILE_STAR",
    resourceType: "File",
    resourceId: file.id,
  });
  revalidatePath("/app/drive");
...
export async function shareFile(fileId: string, formData: FormData): Promise<void> {
  ...
  if (!targetUserId || targetUserId === user.id)
    redirect(appendToast(returnTo, "toast_error", "Choose another team member"));
  ...
  if (!target || target.role === "CLIENT" || target.status !== "ACTIVE")
    redirect(appendToast(returnTo, "toast_error", "Team member unavailable"));
  ...
  await audit({
    userId: user.id,
    action: "FILE_SHARE_INTERNAL",
    resourceType: "File",
    resourceId: file.id,
    after: { sharedWithId: target.id },
  });
  await logActivity({
    userId: user.id,
    type: "FILE_SHARED",
    message: `Shared ${file.name} with ${target.firstName} ${target.lastName}`,
    resourceType: "File",
    resourceId: file.id,
  });
  revalidatePath("/app/drive");
...
export async function unshareFile(fileId: string, targetUserId: string, formData?: FormData): Promise<void> {
  ...
  await audit({
    userId: user.id,
    action: "FILE_UNSHARE_INTERNAL",
    resourceType: "File",
    resourceId: fileId,
    after: { sharedWithId: targetUserId },
  });
  revalidatePath("/app/drive");
  redirect(safeDriveReturn(formData));
}
