...
export async function confirmLegacyRefundFullyPaid(refundId: string, formData: FormData) {
  ...
  const initial = await prisma.refund.findUnique({
    where: { id: refundId },
  ...
  if (!method || !transactionRef || !proofFileId)
    throw new Error("Method, transaction reference and proof are required.");
  if (initial.status !== "APPROVED")
    throw new Error("Only an approved refund can be confirmed as fully paid.");
  if (initial.installments.length > 0)
    throw new Error(
      "This refund already has an installment schedule. Use the installment payout workflow instead.",
    );
  ...
  if (!authorization || authorization.status !== "APPROVED")
    throw new Error("Approved financial authorization is required before legacy refund payout.");
  if (
    authorization.currency !== initial.currency.toUpperCase() ||
  ...
  const created = await prisma.$transaction(
    async (tx) => {
      ...
      if (refund.status === "PAID")
        return { duplicate: true, refund, installment: refund.installments[0] ?? null };
      if (refund.status !== "APPROVED") throw new Error("Refund is no longer payable.");
      if (refund.installments.length > 0)
        throw new Error(
          "This refund now has an installment schedule. Refresh and use the installment payout workflow.",
        );
      ...
      if (refund.paymentId) {
        ...
        if (payment) {
          ...
          if (total > 0) {
            const paymentStatus: "PARTIALLY_REFUNDED" | "REFUNDED" =
              total >= Number(payment.amount) - 0.005 ? "REFUNDED" : "PARTIALLY_REFUNDED";
