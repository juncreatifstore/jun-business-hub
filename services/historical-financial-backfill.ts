...
function parsePayments(rows: PaymentRow[]): { rows?: ParsedPayment[]; error?: string } {
  if (rows.length > MAX_ROWS)
    return { error: `A maximum of ${MAX_ROWS} historical payments can be imported at once.` };
  const out: ParsedPayment[] = [];
  for (let index = 0; index < rows.length; index += 1) {
    const row = rows[index];
    const hasAny = [row.date, row.amount, row.transactionRef, row.purpose, row.notes].some((value) =>
      String(value ?? "").trim(),
    );
    if (!hasAny) continue;
...
function parseRefunds(rows: RefundRow[]): { rows?: ParsedRefund[]; error?: string } {
  if (rows.length > MAX_ROWS)
    return { error: `A maximum of ${MAX_ROWS} historical refunds can be imported at once.` };
  const out: ParsedRefund[] = [];
  for (let index = 0; index < rows.length; index += 1) {
    const row = rows[index];
    const hasAny = [row.date, row.amount, row.transactionRef, row.reason, row.notes].some((value) =>
      String(value ?? "").trim(),
    );
    if (!hasAny) continue;
    const date = parseHistoricalDate(row.date);
...
function sumByCurrency<T extends { currency: string; amount: number }>(rows: T[]) {
  ...
  for (const row of rows)
    totals.set(row.currency, Math.round(((totals.get(row.currency) ?? 0) + row.amount) * 100) / 100);
  return totals;
}

export async function importHistoricalFinancialBatch(
  _prev: HistoricalBackfillState,
  formData: FormData,
): Promise<HistoricalBackfillState> {
  ...
  if (!/^[a-zA-Z0-9-]{12,80}$/.test(batchId))
    return { message: "Historical batch identifier is invalid. Reload the page and try again." };
  if (!confirmed)
    return { message: "Confirm that these transactions already occurred and are being entered late." };
  ...
  if (existingBatch) {
    redirect(
      `/app/clients/${clientId}/account?toast=${encodeURIComponent("This historical batch was already recorded. No duplicate transactions were created.")}`,
    );
  }
  ...
  if (!paymentJson || !refundJson)
    return { message: "Historical transaction data could not be read. Reload the page and try again." };
  ...
  if (!payments.length && !refunds.length)
    return { message: "Add at least one historical payment or refund." };

  const refs = payments.map((row) => row.transactionRef).filter((value): value is string => Boolean(value));
  if (new Set(refs.map((value) => value.toLowerCase())).size !== refs.length)
    return { message: "The same payment transaction reference appears more than once in this batch." };
  if (refs.length) {
    const duplicates = await prisma.payment.findMany({
      where: { clientId, providerRef: { in: refs } },
      select: { reference: true, providerRef: true },
    });
    if (duplicates.length)
      return {
        message: `A payment with transaction reference ${duplicates[0].providerRef} already exists (${duplicates[0].reference}).`,
      };
  }
  ...
  for (const [currency, refundTotal] of refundTotals) {
    ...
    if (refundTotal > currentAvailable + incoming + 0.005) {
      return {
        message: `Historical refunds exceed available ${currency} funds after this batch: ${currency} ${(currentAvailable + incoming).toFixed(2)} available versus ${currency} ${refundTotal.toFixed(2)} refunded.`,
      };
    }
  }
  ...
  const created = await prisma.$transaction(async (tx) => {
    ...
    for (let i = 0; i < payments.length; i += 1) {
      ...
      const payment = await tx.payment.create({
        data: {
          ...
          notes: [
            `Historical late entry · batch ${batchId}`,
    ...
    for (let i = 0; i < refunds.length; i += 1) {
      ...
      createdRefunds.push({
        ...
        installmentId: refund.installments[0].id,
        row,
      });
    }
  ...
  await Promise.all(
    created.createdPayments.map(async ({ id, reference, row }) => {
      ...
      await ensureUniversalFinancialReceipt({
        ...
        description:
          row.purpose || row.notes || "Historical payment entered after the original transaction date",
        status: "CONFIRMED",
        method: row.method,
  ...
  await Promise.all(
    created.createdRefunds.map(async ({ id, refundNumber, installmentId, row }) => {
      ...
      await saveRefundWorkflowMeta(id, {
        refundType: "UNLINKED",
        decisionReason:
          "Historical transaction backfill: refund had already been completed before system entry.",
  ...
  await audit({
    ...
    after: {
      batchId,
      caseId,
      payments: created.createdPayments.map((row) => ({
        reference: row.reference,
        amount: row.row.amount,
        currency: row.row.currency,
        paidAt: row.row.date.toISOString(),
      })),
      refunds: created.createdRefunds.map((row) => ({
  ...
  redirect(
    `/app/clients/${clientId}/account?toast=${encodeURIComponent(`Historical batch recorded: ${payments.length} payment(s) confirmed and ${refunds.length} refund(s) marked paid.`)}`,
  );
}
