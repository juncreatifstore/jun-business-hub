...
export async function disconnectMailbox(accountId: string): Promise<void> {
  ...
  await prisma.mailAccount.update({
    where: { id: acc.id },
    data: { refreshTokenEnc: null, accessTokenEnc: null, tokenExpiry: null },
  });
  await audit({
    userId: user.id,
...
export async function syncMailbox(accountId: string): Promise<void> {
  ...
  try {
    ...
    redirect("/app/mail?toast_error=You do not have access to this mailbox");
  }
  if (!(await rateLimitAsync(`gmail-sync:${user.id}`, 6, 60_000)))
    redirect("/app/mail?toast_error=Sync rate limit — wait a minute");
  ...
  try {
    ...
    await audit({
      ...
      after: { newThreads: total, maxPerFolder: 250, paginated: true },
    });
    revalidatePath("/app/mail");
    revalidatePath("/app/mail/security");
    redirect(
      `/app/mail?toast=${encodeURIComponent(`Gmail sync complete — ${total} new conversation${total === 1 ? "" : "s"}; recent labels refreshed`)}`,
    );
  } catch (e) {
    ...
    await recordMailReliabilityEvent({
      ...
      accountId,
      userId: user.id,
      message: e instanceof Error ? e.message : "Gmail sync failed",
    });
    revalidatePath("/app/mail/security");
    redirect(
      `/app/mail?toast_error=${encodeURIComponent(e instanceof Error ? e.message : "Gmail sync failed")}`,
    );
  }
}
...
function automatedSender(email: string): boolean {
  ...
  return (
    /(^|[._+-])(no-?reply|do-?not-?reply|newsletter|notifications?|alerts?|mailer|updates?)($|[._+-])/i.test(
      local,
    ) || /(^|\.)(newsletter|mailer|notifications?)\./i.test(domain)
  );
}
async function generateReplyText(input: {
  subject: string;
  message: string;
  recipient: string;
}): Promise<{ text: string; mode: "MODEL" | "OFFLINE_TEMPLATE" }> {
        ...
        messages: [
          {
            role: "system",
            content:
              "You draft concise, professional email replies for JUN CREATIF AND TRAVEL LLC. Draft only; never claim an action was completed unless the incoming message proves it. Do not promise visa approvals, refunds, payments, legal outcomes, or guaranteed travel results. Return only the email body.",
          },
          {
            role: "user",
...
export async function draftReplyWithJunAI(threadId: string): Promise<void> {
  ...
  if (!(await rateLimitAsync(`jun-ai-mail-draft:${user.id}`, 12, 60_000)))
    redirect(`/app/mail?thread=${threadId}&toast_error=AI draft rate limit — wait a minute`);
  ...
  if (thread.aiDraft)
    redirect(`/app/mail?folder=DRAFTS&thread=${thread.id}&toast=This thread already has a draft`);
  ...
  if (automatedSender(recipient)) {
    await prisma.mailThread.update({
      ...
      data: {
        ...
        aiSummary: "Automated/newsletter sender detected. JUN AI did not prepare a reply.",
      },
    });
    await audit({
      userId: user.id,
      action: "AI_EMAIL_REPLY_AUTOMATED_SENDER_BLOCKED",
    ...
    revalidatePath("/app/mail");
    redirect(
      `/app/mail?folder=AI_REVIEW&thread=${thread.id}&toast_error=${encodeURIComponent("Automated or newsletter sender detected — reply drafting was blocked")}`,
    );
  }
  const { classifyEmailAILevel } = await import("@/services/ai");
  const sourceText = thread.snippet ?? "",
    level = await classifyEmailAILevel(thread.subject ?? "", sourceText);
  if (level === "BLOCKED") {
    await prisma.mailThread.update({
      where: { id: thread.id },
      data: { aiLevel: "BLOCKED", requiresAttention: true },
    });
    await audit({
      userId: user.id,
      action: "AI_EMAIL_REPLY_BLOCKED",
      resourceType: "MailThread",
      resourceId: thread.id,
      after: { reason: "Sensitive topic requires manual handling" },
    });
    revalidatePath("/app/mail");
    redirect(
      `/app/mail?folder=AI_REVIEW&thread=${thread.id}&toast_error=JUN AI did not draft this reply because the topic requires manual handling`,
    );
  }
  const generated = await generateReplyText({
    ...
    replySubject = /^re:/i.test(thread.subject ?? "")
  ...
  revalidatePath("/app/mail");
  redirect(
    `/app/mail?folder=DRAFTS&thread=${thread.id}&toast=${encodeURIComponent(generated.mode === "MODEL" ? "JUN AI reply draft created — review before sending" : "Offline reply template created — review before sending")}`,
  );
}
...
export async function updateMailDraft(threadId: string, formData: FormData): Promise<void> {
  ...
  const to = String(formData.get("to") ?? "")
    ...
    body = String(formData.get("body") ?? "")
      ...
      .slice(0, 20000);
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to))
    redirect(`/app/mail?folder=DRAFTS&thread=${threadId}&toast_error=Enter a valid recipient email`);
  if (!subject || !body)
    redirect(`/app/mail?folder=DRAFTS&thread=${threadId}&toast_error=Subject and message are required`);
  await prisma.mailThread.update({
    where: { id: threadId },
    data: { toEmails: [to], subject, aiDraft: body, requiresAttention: true },
  });
  await audit({
    userId: user.id,
    action: "EMAIL_DRAFT_EDITED",
    resourceType: "MailThread",
    resourceId: threadId,
    after: { to, subject },
  });
...
export async function sendDraftViaGmail(threadId: string): Promise<void> {
  ...
  if (thread.aiLevel !== "AUTO") {
    const approval = await getMailApproval(threadId),
      valid = await approvalIsCurrent(threadId).catch(() => false);
    if (!valid)
      redirect(
        `/app/mail?folder=DRAFTS&thread=${threadId}&toast_error=${encodeURIComponent(approval?.status === "APPROVED" ? "Draft changed after approval — submit again" : "This email requires approval before sending")}`,
      );
  }
  const to = thread.toEmails[0];
  if (!to) redirect(`/app/mail?thread=${threadId}&toast_error=No recipient on this draft`);
  if (automatedSender(to))
    redirect(
      `/app/mail?folder=DRAFTS&thread=${threadId}&toast_error=${encodeURIComponent("Sending to an automated/newsletter address is blocked. Edit the recipient if you have a valid reply address.")}`,
    );
  ...
  let lock;
  try {
    lock = await acquireMailSendLock({ threadId: thread.id, fingerprint, userId: user.id });
  } catch (e) {
    redirect(
      `/app/mail?folder=DRAFTS&thread=${threadId}&toast_error=${encodeURIComponent(e instanceof Error ? e.message : "Email send is already in progress")}`,
    );
  }
  const { gmailSend } = await import("@/lib/google/gmail");
  let gmailMessageId: string;
  try {
    gmailMessageId = await gmailSend(thread.mailAccountId, {
      to,
      subject: thread.subject ?? "(no subject)",
      text: thread.aiDraft,
    });
    ...
    if (e && typeof e === "object" && "digest" in e) throw e;
    await markMailSendFailure({
      threadId: thread.id,
      attemptId: lock.attemptId,
      error: e,
      accountId: thread.mailAccountId,
      userId: user.id,
    });
    revalidatePath("/app/mail/security");
    redirect(
      `/app/mail?thread=${thread.id}&toast_error=${encodeURIComponent(e instanceof Error ? e.message : "Gmail send failed")}`,
    );
  }
  ...
  await prisma.mailThread.update({
    where: { id: thread.id },
    data: {
      snippet: thread.aiDraft.slice(0, 500),
      aiDraft: null,
