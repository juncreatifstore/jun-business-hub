...
export async function importAdministrativeTemplateCatalog(formData: FormData) {
  ...
  if (!(upload instanceof File) || upload.size === 0) {
    redirect(
      `/app/documents/templates/import?toast_error=${encodeURIComponent("Choose a Markdown or text catalog file")}`,
    );
  }
  if (upload.size > 2_000_000) {
    redirect(
      `/app/documents/templates/import?toast_error=${encodeURIComponent("Catalog file is too large")}`,
    );
  }
  ...
  if (entries.length === 0) {
    redirect(
      `/app/documents/templates/import?toast_error=${encodeURIComponent("No numbered template entries were found")}`,
    );
  }
  ...
  await audit({
    userId: user.id,
    action: "DOCUMENT_TEMPLATE_CATALOG_IMPORT",
    resourceType: "DocumentTemplate",
    after: { parsed: entries.length, inserted },
  });
  await logActivity({
    type: "DOCUMENT_TEMPLATE_CATALOG_IMPORTED",
    message: `Administrative template catalog imported: ${inserted} new reference entries (${entries.length} parsed)`,
    userId: user.id,
    resourceType: "DocumentTemplate",
  });
  revalidatePath("/app/documents/templates");
  redirect(
    `/app/documents/templates?state=reference&toast=${encodeURIComponent(`${inserted} reference templates imported (${entries.length} parsed)`)}`,
  );
}
