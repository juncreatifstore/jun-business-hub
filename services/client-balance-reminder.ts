...
function buildBody(
  lang: ClientStatementLanguage,
  name: string,
  debts: Array<{ currency: string; debt: number; netReceived: number; committed: number }>,
) {
  const debtLines = debts
    .map(
      (d) =>
        `- ${d.currency}: ${money(d.debt, d.currency)} outstanding (net received ${money(d.netReceived, d.currency)}; committed service costs ${money(d.committed, d.currency)})`,
    )
    .join("\n");
...
export async function createClientBalanceReminderDraft(clientId: string): Promise<void> {
  ...
  const [client, finance, profile, account] = await Promise.all([
    prisma.client.findUnique({
      where: { id: clientId },
      select: { id: true, firstName: true, lastName: true, email: true, internalId: true },
    }),
  ...
  if (!client)
    redirect(`/app/clients/${clientId}/dashboard?toast_error=${encodeURIComponent("Client not found")}`);
  if (!client.email)
    redirect(
      `/app/clients/${clientId}/dashboard?toast_error=${encodeURIComponent("Add a client email address before creating a reminder")}`,
    );
  if (!account)
    redirect(
      `/app/clients/${clientId}/dashboard?toast_error=${encodeURIComponent("Connect a Gmail mailbox first")}`,
    );
  const debts = finance.summaries
    .filter((s) => s.forecastProfit < -0.009)
    .map((s) => ({
      currency: s.currency,
      debt: Math.abs(s.forecastProfit),
      netReceived: s.netReceived,
      committed: s.expenseCommitted + s.approvedRefunds,
    }));
  if (!debts.length)
    redirect(
      `/app/clients/${clientId}/dashboard?toast=${encodeURIComponent("Client has no negative committed balance")}`,
    );
  ...
  await audit({
    userId: user.id,
    action: "CLIENT_BALANCE_REMINDER_DRAFT",
    resourceType: "Client",
    resourceId: client.id,
    after: { email: client.email, debts },
  });
  await logActivity({
    type: "EMAIL_DRAFTED",
    message: `Account balance reminder drafted for ${client.internalId}`,
    userId: user.id,
    clientId: client.id,
  });
  ...
  redirect(
    `/app/mail?folder=DRAFTS&thread=${thread.id}&toast=${encodeURIComponent("Account status reminder draft created — review and send")}`,
  );
}
