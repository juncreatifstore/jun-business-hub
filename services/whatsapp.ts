...
export async function saveWhatsAppSettings(formData: FormData) {
  ...
  await saveWhatsAppConfig({
    ...
    graphVersion: String(formData.get("graphVersion") || "v23.0"),
    defaultTemplate: String(formData.get("defaultTemplate") || ""),
    languageCode: String(formData.get("languageCode") || "fr"),
  ...
  await audit({
    ...
    resourceId: "whatsapp",
    after: {
      appId: String(formData.get("appId") || "").trim() || null,
      tokenUpdated: Boolean(accessToken.trim()),
      webhookVerifyTokenUpdated: Boolean(webhookVerifyToken.trim()),
    },
  });
...
export async function sendClientWhatsApp(clientId: string, formData: FormData) {
  ...
  if (await isClientCommunicationBanned(client.id))
    redirect(
      `/app/clients/${clientId}/whatsapp?toast_error=${encodeURIComponent("Client banni — communication WhatsApp bloquée partout dans JUN")}`,
    );
  ...
  if (!to)
    redirect(
      `/app/clients/${clientId}/whatsapp?toast_error=${encodeURIComponent("Client has no WhatsApp number")}`,
    );
  let errorMessage = "";
  try {
    ...
    if (mode === "TEMPLATE" && selectedTemplate === GENERAL_DOCUMENT_TEMPLATE) {
      throw new Error(
        "jun_document_notification requires a PDF document. Open the Receipt, Invoice, Statement or Official Document you want to send and click Send by WhatsApp there.",
      );
    }
    const result =
      mode === "TEMPLATE"
    ...
    await audit({
      ...
      after: {
        ...
        to,
        messageId,
        template: mode === "TEMPLATE" ? selectedTemplate : null,
      },
    });
    await prisma.activity
      .create({
        data: {
          ...
          message: `WhatsApp ${mode === "TEMPLATE" ? "template" : "message"} accepted by Meta for ${to}${messageId ? ` · ${messageId}` : ""}`,
        },
      })
      .catch(() => null);
  ...
  if (errorMessage)
    redirect(`/app/clients/${clientId}/whatsapp?toast_error=${encodeURIComponent(errorMessage)}`);
  redirect(
    `/app/clients/${clientId}/whatsapp?toast=${encodeURIComponent("Accepted by Meta — awaiting delivery confirmation")}`,
  );
}
...
export async function sendDocumentByWhatsApp(documentId: string, formData: FormData) {
  ...
  if (!doc.client)
    redirect(
      `/app/documents/${documentId}?toast_error=${encodeURIComponent("This document is not linked to a client")}`,
    );
  if (await isClientCommunicationBanned(doc.client.id))
    redirect(
      `/app/documents/${documentId}?toast_error=${encodeURIComponent("Client banni — aucun document ne peut être envoyé par WhatsApp")}`,
    );
  const to = String(formData.get("to") || doc.client.whatsapp || doc.client.phone || "").trim();
  if (!to)
    redirect(
      `/app/documents/${documentId}?toast_error=${encodeURIComponent("Client has no WhatsApp number")}`,
    );
  if (!doc.finalPdfKey)
    redirect(
      `/app/documents/${documentId}?toast_error=${encodeURIComponent("Finalize the document first so JUN has an official PDF to send")}`,
    );
  let errorMessage = "";
  try {
    const cfg = await getWhatsAppConfig();
    ...
    const filename =
      `${doc.documentId}-${doc.title}`.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 180) + ".pdf";
    ...
    await audit({
      ...
      after: {
        ...
        messageId,
        mediaId,
        template: cfg.defaultTemplate || null,
      },
    });
    await prisma.activity
      .create({
        data: {
          ...
          type: "WHATSAPP_ACCEPTED",
          message: `Document ${doc.documentId} accepted by Meta for WhatsApp delivery to ${to}${messageId ? ` · ${messageId}` : ""}`,
          resourceType: "Document",
          resourceId: doc.id,
        },
      })
  ...
  redirect(
    `/app/documents/${documentId}?toast=${encodeURIComponent("Accepted by Meta — awaiting delivery confirmation")}`,
  );
}
...
export async function sendClientStatementByWhatsApp(clientId: string, formData: FormData) {
  ...
  if (await isClientCommunicationBanned(client.id))
    redirect(
      `/app/clients/${clientId}/statement?toast_error=${encodeURIComponent("Client banni — relevé WhatsApp bloqué")}`,
    );
  const to = String(client.whatsapp || client.phone || "").trim();
  const rawLang = String(formData.get("language") || "").toUpperCase();
  const language: ClientStatementLanguage = ["FR", "EN", "ES", "HT"].includes(rawLang)
    ? (rawLang as ClientStatementLanguage)
    : account.profile.preferredLanguage;
  if (!to)
    redirect(
      `/app/clients/${clientId}/statement?lang=${language}&toast_error=${encodeURIComponent("Client has no WhatsApp number")}`,
    );
  if (!cfg.defaultTemplate)
    redirect(
      `/app/clients/${clientId}/statement?lang=${language}&toast_error=${encodeURIComponent("Configure the approved jun_document_notification template in Settings → WhatsApp first")}`,
    );
  let errorMessage = "";
  try {
    ...
    const bytes = await renderClientStatementV2({
      ...
      balances: account.balances.map((b) => ({
        currency: b.currency,
        confirmedFunds: b.confirmedFunds,
        commissions: b.commissions,
        committedExpenses: b.committedExpenses,
        activeRefunds: b.activeRefunds,
        partnerWithdrawals: b.partnerWithdrawals,
    ...
    await audit({
      userId: user.id,
      action: "WHATSAPP_STATEMENT_SEND",
      resourceType: "Client",
      resourceId: client.id,
      after: {
        clientInternalId: client.internalId,
    ...
    await prisma.activity
      .create({
        data: {
          ...
          clientId: client.id,
          type: "WHATSAPP_ACCEPTED",
          message: `Statement ${reference} accepted by Meta for WhatsApp delivery to ${to}${messageId ? ` · ${messageId}` : ""}`,
          resourceType: "ClientStatement",
  ...
  if (errorMessage)
    redirect(
      `/app/clients/${clientId}/statement?lang=${language}&toast_error=${encodeURIComponent(errorMessage)}`,
    );
  redirect(
    `/app/clients/${clientId}/statement?lang=${language}&toast=${encodeURIComponent("Statement accepted by Meta — awaiting delivery confirmation")}`,
  );
}
