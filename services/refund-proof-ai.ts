...
function baseScore(input: {
  ...
  if (input.samePayment) {
    score += 45;
    reasons.push("linked to the original payment");
  }
  if (input.sameCase) {
    score += 20;
    reasons.push("same case/service");
  }
  if (["PAYMENT_PROOF", "RECEIPT", "REFUND"].includes(input.category)) {
    score += 20;
    reasons.push(`category ${input.category}`);
  }
  if (
    /proof|receipt|reçu|recibo|deposit|dépôt|depot|payment|paiement|transfer|transfert|bank|banque|sogebank|zelle|paypal|stripe/i.test(
      name,
    )
  ) {
    score += 15;
    reasons.push("filename suggests payment evidence");
  }
  if (input.paymentReference && name.includes(input.paymentReference.toLowerCase())) {
    score += 35;
    reasons.push("payment reference found in filename");
  }
  if (name.includes(input.refundNumber.toLowerCase())) {
    score += 30;
    reasons.push("refund reference found in filename");
  }
  const compactAmount = String(input.amount).replace(/\.00$/, "");
  if (compactAmount && name.replace(/[, ]/g, "").includes(compactAmount.replace(/[, ]/g, ""))) {
    score += 10;
    reasons.push("amount may appear in filename");
  }
  const days = daysBetween(input.createdAt, input.refundCreatedAt);
  if (days <= 3) {
    score += 12;
    reasons.push("date very close to refund request");
  } else if (days <= 14) {
    score += 7;
    reasons.push("date close to refund request");
  } else if (days <= 60) {
    score += 3;
    reasons.push("date reasonably close");
  }
  ...
  try {
        ...
        messages: [
          {
            role: "system",
            content:
              'You rank existing client-drive files as possible proof for a refund. Never invent files. Return JSON only: {"ranking":[{"id":"...","score":0-100,"reason":"short reason"}]}. Use only supplied ids.',
          },
    ...
    return candidates
      .map((c) => {
        ...
        return {
          ...c,
          score: Math.round(c.score * 0.45 + aiScore * 0.55),
          reason: String(hit.reason || c.reason).slice(0, 180),
        };
      })
...
export async function findRefundProofCandidates(
  refundId: string,
): Promise<{ candidates?: RefundProofCandidate[]; error?: string }> {
...
export async function attachExistingDriveFileToRefund(
  refundId: string,
  fileId: string,
  ...
  const refund = await prisma.refund.findUnique({
    where: { id: refundId },
    select: { id: true, clientId: true, caseId: true, refundNumber: true },
  });
  if (!refund) return { error: "Refund not found." };
  const file = await prisma.file.findUnique({
    where: { id: fileId },
    select: { id: true, name: true, clientId: true, refundId: true, archivedAt: true },
  });
  ...
  if (file.refundId && file.refundId !== refundId)
    return { error: "This file is already attached to another refund." };

  await prisma.file.update({
    where: { id: fileId },
    data: { refundId, ...(refund.caseId ? { caseId: refund.caseId } : {}) },
  });
  await audit({
    userId: user.id,
    action: "REFUND_PROOF_AI_ATTACH",
    resourceType: "Refund",
    resourceId: refundId,
    after: { fileId, fileName: file.name },
  });
  await logActivity({
    type: "REFUND_UPDATED",
    message: `Existing Drive proof attached to ${refund.refundNumber}: ${file.name}`,
    userId: user.id,
    clientId: refund.clientId,
...
export async function detachRefundProof(
  refundId: string,
  fileId: string,
  _formData?: FormData,
): Promise<void> {
  ...
  if (refund.status === "PAID")
    throw new Error("Evidence on a completed refund cannot be removed. Correct the refund first.");
  ...
  if (paidUse)
    throw new Error(
      `This proof confirms paid installment ${paidUse.number} and cannot be removed without correcting that payout.`,
    );
...
export async function attachProofAndLinkOriginalPayment(
  refundId: string,
  fileId: string,
): Promise<{ ok?: true; error?: string }> {
  const user = await assertPermission("REFUND_CREATE");
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (["REJECTED", "CANCELLED", "PAID"].includes(refund.status))
        throw new Error("This refund can no longer be relinked to an original payment.");
      ...
      if (file.refundId && file.refundId !== refundId)
        throw new Error("This file is already attached to another refund.");
      if (!file.payment) throw new Error("This proof is not linked to an original payment.");
      if (file.payment.clientId !== refund.clientId)
        throw new Error("The payment linked to this proof belongs to another client.");
      if (!["CONFIRMED", "PARTIALLY_REFUNDED", "REFUNDED"].includes(file.payment.status))
        throw new Error("Only a confirmed payment can be linked to a refund.");
      if (file.payment.currency.toUpperCase() !== refund.currency.toUpperCase())
        throw new Error(
          `Currency mismatch: refund is ${refund.currency}, payment is ${file.payment.currency}.`,
        );

      await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${file.payment.id}))`;
      const payment = await tx.payment.findUnique({
        where: { id: file.payment.id },
        include: { refunds: true },
      });
      if (!payment) throw new Error("Original payment not found.");
      const committed = payment.refunds
        .filter((r) => r.id !== refund.id && !["REJECTED", "CANCELLED"].includes(r.status))
      ...
      if (Number(refund.amount) > available + 0.005)
        throw new Error(
          `Cannot link this refund to ${payment.reference}: only ${payment.currency} ${available.toFixed(2)} remains refundable.`,
        );

      await tx.refund.update({ where: { id: refund.id }, data: { paymentId: payment.id } });
      await tx.file.update({
        where: { id: file.id },
        data: { refundId: refund.id, ...(refund.caseId ? { caseId: refund.caseId } : {}) },
      });
      const refundType: "FULL" | "PARTIAL" =
        Math.abs(Number(refund.amount) - available) < 0.005 ? "FULL" : "PARTIAL";
    ...
    await audit({
      userId: user.id,
      action: "REFUND_PROOF_LINK_PAYMENT",
      resourceType: "Refund",
      resourceId: refundId,
      after: {
    ...
    await logActivity({
      type: "REFUND_UPDATED",
      message: `Refund ${result.refund.refundNumber} linked to original payment ${result.payment.reference} using Drive proof ${result.file.name}`,
      userId: user.id,
      clientId: result.refund.clientId,
      caseId: result.refund.caseId,
    });
    revalidatePath(`/app/finance/refunds/${refundId}`);
