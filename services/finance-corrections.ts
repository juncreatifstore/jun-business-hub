...
function text(fd: FormData, key: string, max = 3000) {
...
function fail(path: string, msg: string): never {
  redirect(`${path}?toast_error=${encodeURIComponent(msg)}`);
}
function correctionReason(fd: FormData, path: string) {
  const r = text(fd, "correctionReason", 1000);
  if (!r) fail(path, "Correction reason is required");
  return r;
}
...
export async function correctPayment(id: string, fd: FormData) {
  ...
  if (mode === "LOCKED")
    fail(path, "This payment is locked. Use a reversal/replacement instead of overwriting history.");
  ...
  if (mode === "CORRECTION") {
    ...
    await audit({
      ...
      after: {
        ...
        financialFieldsLocked: true,
      },
    });
    refreshClientAccount(p.clientId);
    revalidatePath(`/app/finance/payments/${id}`);
    redirect(
      `/app/finance/payments/${id}?toast=${encodeURIComponent("Payment reference/details corrected")}`,
    );
  }
  ...
  if (
    ...
    fail(path, "Invalid payment correction data");
  ...
  if (caseId) {
    ...
    if (!c || c.clientId !== clientId) fail(path, "Case does not belong to selected client");
  }
  ...
  if (expectedAmount != null && (!Number.isFinite(expectedAmount) || expectedAmount <= 0))
    fail(path, "Invalid expected amount");
  ...
  const updated = await prisma.payment.update({
    where: { id },
    data: { clientId, caseId, amount, currency, method, providerRef, paidAt, notes },
  });
  ...
  await logActivity({
    ...
    message: `Payment ${updated.reference} corrected: ${reason}`,
    clientId,
    caseId,
  });
  refreshClientAccount(p.clientId);
...
export async function correctRefund(id: string, fd: FormData) {
  ...
  if (
    ...
    fail(path, "Invalid refund correction data");
  const client = await prisma.client.findUnique({ where: { id: clientId }, select: { id: true } });
  if (!client) fail(path, "Client not found");
  if (caseId) {
    const c = await prisma.case.findUnique({ where: { id: caseId }, select: { clientId: true } });
  ...
  if (paymentId) {
    ...
    if (!["CONFIRMED", "PARTIALLY_REFUNDED", "REFUNDED"].includes(p.status))
      fail(path, "Only confirmed payments can be refunded");
    currency = p.currency;
    const committed = p.refunds
      .filter((x) => x.id !== id && !["REJECTED", "CANCELLED"].includes(x.status))
      .reduce((s, x) => s + Number(x.amount), 0);
    const available = Math.max(0, Math.round((Number(p.amount) - committed) * 100) / 100);
    if (amount > available + 0.005)
      fail(path, `Refund exceeds available amount (${currency} ${available.toFixed(2)})`);
  ...
  if (r.status === "APPROVED") {
    ...
    const availableWithCurrentReleased = sameClientCurrency
      ? currentAvailable + Number(r.amount)
      : currentAvailable;
    if (amount > availableWithCurrentReleased + 0.005)
      fail(
        path,
        `Correction exceeds client available balance (${currency} ${availableWithCurrentReleased.toFixed(2)})`,
      );
  }
  ...
  const before = {
    ...
    reason: r.reason,
    status: r.status,
    refundType: workflow.refundType,
  };
  ...
  await prisma.$transaction(async (tx) => {
    await tx.refund.update({
      ...
      data: {
        ...
        currency,
        reason: reasonText,
        ...(r.status === "APPROVED" ? { status: "UNDER_REVIEW", approvedById: null } : {}),
      },
    });
    for (let i = 0; i < r.installments.length; i++)
      await tx.refundInstallment.update({
        where: { id: r.installments[i].id },
        data: { amount: parts[i].amount },
      });
  });
  await saveRefundWorkflowMeta(id, {
    refundType,
    ...(r.status === "APPROVED" ? { decisionReason: "", decidedAt: null, decidedById: null } : {}),
  });
  await audit({
    userId: user.id,
    ...
    after: {
      ...
      currency,
      reason: reasonText,
      status: r.status === "APPROVED" ? "UNDER_REVIEW" : r.status,
      refundType,
      correctionReason: reason,
      reapprovalRequired: r.status === "APPROVED",
    },
  });
  await logActivity({
    userId: user.id,
    type: "REFUND_UPDATED",
    message: `Refund ${r.refundNumber} corrected: ${reason}`,
    clientId,
    caseId,
  });
  ...
  redirect(
    `/app/finance/refunds/${id}?toast=${encodeURIComponent(r.status === "APPROVED" ? "Refund corrected and returned to review" : "Refund corrected")}`,
  );
}
...
export async function correctExpense(id: string, fd: FormData) {
  ...
  const reason = correctionReason(fd, path);
  const vendorName = text(fd, "vendorName", 200),
    vendorCountry = text(fd, "vendorCountry", 120),
    category = text(fd, "category", 60) as ExpenseCategory,
    description = text(fd, "description", 3000),
    invoiceNumber = text(fd, "invoiceNumber", 120),
    amount = money(fd, "amount"),
    currency = text(fd, "currency", 3).toUpperCase(),
    dueDate = text(fd, "dueDate", 20) || null,
    clientId = text(fd, "clientId", 100) || null,
    caseId = text(fd, "caseId", 100) || null;
  if (
    !vendorName ||
    !description ||
    !Number.isFinite(amount) ||
    ...
    fail(path, "Invalid expense correction data");
  if (caseId) {
    const c = await prisma.case.findUnique({ where: { id: caseId }, select: { clientId: true } });
    if (!c || (clientId && c.clientId !== clientId)) fail(path, "Case does not belong to selected client");
  }
  ...
  const next: FinanceExpense = {
    ...
    caseId,
    status,
    approvedById: e.status === "APPROVED" ? null : e.approvedById,
    decisionNote: e.status === "APPROVED" ? "" : e.decisionNote,
  ...
  await audit({
    ...
    after: {
      ...
      status,
      correctionReason: reason,
      reapprovalRequired: e.status === "APPROVED",
    },
  });
  revalidatePath(`/app/finance/expenses/${id}`);
  revalidatePath("/app/finance/expenses");
  redirect(
    `/app/finance/expenses/${id}?toast=${encodeURIComponent(e.status === "APPROVED" ? "Expense corrected and returned for approval" : "Expense corrected")}`,
  );
}

function invoiceLines(fd: FormData) {
  const descriptions = fd.getAll("lineDescription").map(String),
    quantities = fd.getAll("lineQuantity").map(Number),
    prices = fd.getAll("lineUnitPrice").map(Number),
    taxes = fd.getAll("lineTaxRate").map(Number);
...
export async function correctInvoice(id: string, fd: FormData) {
  ...
  const clientId = text(fd, "clientId", 100),
    ...
    terms = text(fd, "terms", 2000),
    dueDateRaw = text(fd, "dueDate", 20),
    calc = invoiceLines(fd);
  if (!clientId || !title || !/^[A-Z]{3}$/.test(currency) || !dueDateRaw || calc.total <= 0)
    fail(path, "Invalid invoice correction data");
  const client = await prisma.client.findUnique({ where: { id: clientId }, select: { id: true } });
  if (!client) fail(path, "Client not found");
  if (caseId) {
    const c = await prisma.case.findUnique({ where: { id: caseId }, select: { clientId: true } });
  ...
  if (Number.isNaN(due.getTime())) fail(path, "Invalid due date");
  ...
  const next = {
    ...
    ...calc,
    status: wasSent ? ("DRAFT" as const) : inv.status,
    sentAt: wasSent ? null : inv.sentAt,
    updatedAt: new Date().toISOString(),
  };
  await saveInvoice(next);
  await audit({
    userId: user.id,
    action: "INVOICE_CORRECTION",
    ...
    after: {
      ...
      status: next.status,
      correctionReason: reason,
      resendRequired: wasSent,
    },
  });
  revalidatePath(`/app/finance/invoices/${id}`);
  revalidatePath("/app/finance/invoices");
  redirect(
    `/app/finance/invoices/${id}?toast=${encodeURIComponent(wasSent ? "Invoice corrected and returned to draft for re-send" : "Invoice corrected")}`,
  );
}
