...
export async function createDriveAutomationRule(formData: FormData): Promise<void> {
  ...
  const rule: DriveAutomationRule = {
    ...
    enabled: true,
    match: {
      ...(category && DRIVE_CATEGORIES.includes(category as (typeof DRIVE_CATEGORIES)[number])
        ? { categories: [category] }
        : {}),
    ...
    actions: {
      ...
      ...(String(formData.get("createTask") ?? "") === "1"
        ? { createTask: true, taskAssigneeId, taskDueDays }
        : {}),
      ...(suggestCategory && DRIVE_CATEGORIES.includes(suggestCategory as (typeof DRIVE_CATEGORIES)[number])
        ? { suggestCategory }
        : {}),
  ...
  await prisma.appSetting.create({
    data: { key: `${DRIVE_AUTOMATION_RULE_PREFIX}${id}`, value: JSON.stringify(rule) },
  });
  await audit({
    userId: user.id,
    action: "DRIVE_AUTOMATION_RULE_CREATE",
    resourceType: "DriveAutomationRule",
    resourceId: id,
...
export async function toggleDriveAutomationRule(ruleId: string, formData?: FormData): Promise<void> {
  ...
  try {
    ...
    redirect(toast(returnTo, "toast_error", "Automation rule is invalid"));
  }
  ...
  await audit({
    userId: user.id,
    action: rule.enabled ? "DRIVE_AUTOMATION_RULE_ENABLE" : "DRIVE_AUTOMATION_RULE_DISABLE",
    resourceType: "DriveAutomationRule",
    resourceId: ruleId,
  });
  revalidatePath("/app/drive/automation");
...
export async function deleteDriveAutomationRule(ruleId: string, formData?: FormData): Promise<void> {
  ...
  await audit({
    userId: user.id,
    action: "DRIVE_AUTOMATION_RULE_DELETE",
    resourceType: "DriveAutomationRule",
    resourceId: ruleId,
  });
  revalidatePath("/app/drive/automation");
...
export async function reviewDriveAutomationProposal(proposalId: string, formData: FormData): Promise<void> {
  ...
  if (decision !== "APPROVED" && decision !== "REJECTED")
    redirect(toast(returnTo, "toast_error", "Invalid decision"));
  ...
  try {
    proposal = JSON.parse(row.value);
  } catch {
    redirect(toast(returnTo, "toast_error", "Proposal is invalid"));
  }
  if (proposal.status !== "PENDING") redirect(toast(returnTo, "toast_error", "Proposal already reviewed"));

  const file = await prisma.file.findFirst({
    where: { id: proposal.fileId, isVault: false, archivedAt: null },
  });
  ...
  if (decision === "APPROVED") {
    if (
      ...
      DRIVE_CATEGORIES.includes(proposal.value as (typeof DRIVE_CATEGORIES)[number])
    ) {
      await prisma.file.update({
        where: { id: file.id },
        data: { category: proposal.value as FileCategory },
      });
      ...
      const c = await prisma.case.findUnique({
        where: { id: proposal.value },
        select: { id: true, clientId: true },
      });
      if (!c) redirect(toast(returnTo, "toast_error", "Suggested case no longer exists"));
      await prisma.file.update({
        where: { id: file.id },
        data: { caseId: c.id, ...(file.clientId ? {} : { clientId: c.clientId }) },
      });
    } else if (proposal.type === "EXPIRY") {
      const time = Date.parse(proposal.value);
      if (!Number.isFinite(time))
        redirect(toast(returnTo, "toast_error", "Suggested expiration date is invalid"));
      const expiryKey = `${DRIVE_EXPIRY_PREFIX}${file.id}`;
      await prisma.appSetting.upsert({
        where: { key: expiryKey },
        update: { value: new Date(time).toISOString() },
        create: { key: expiryKey, value: new Date(time).toISOString() },
      });
    } else if (proposal.type === "MOVE") {
      const targetId = proposal.value === "__ROOT__" ? null : proposal.value;
      if (
        targetId &&
        !(await prisma.folder.findFirst({ where: { id: targetId, isVault: false }, select: { id: true } }))
      )
        redirect(toast(returnTo, "toast_error", "Destination folder no longer exists"));
  ...
  await audit({
    userId: user.id,
    action: decision === "APPROVED" ? "DRIVE_AUTOMATION_APPROVE" : "DRIVE_AUTOMATION_REJECT",
    resourceType: "File",
    resourceId: file.id,
    after: { proposalId, type: proposal.type, value: proposal.value },
  });
  ...
  redirect(
    toast(
      returnTo,
      "toast",
      decision === "APPROVED" ? "Automation proposal applied" : "Automation proposal rejected",
    ),
  );
}
...
export async function runDriveAutomationScan(formData?: FormData): Promise<void> {
    ...
    if (!existing) {
      await prisma.notification.create({
        data: {
          userId: user.id,
          type: "DRIVE_EXPIRY",
          title: `Document expiration: ${item.file.name}`,
          body: `Expiration / important date: ${new Date(item.expiresAt).toLocaleDateString("en-US")}`,
        },
      });
  ...
  await prisma.appSetting.upsert({
    where: { key: DRIVE_AUTOMATION_LAST_SCAN },
    update: { value: new Date().toISOString() },
    create: { key: DRIVE_AUTOMATION_LAST_SCAN, value: new Date().toISOString() },
  });
  await audit({
    userId: user.id,
    action: "DRIVE_AUTOMATION_SCAN",
    resourceType: "Drive",
    after: { files: files.length, matched, proposals, expiring: expiring.length },
  });
  revalidatePath("/app/drive/automation");
  redirect(
    toast(
      returnTo,
      "toast",
      `Scan complete: ${files.length} files, ${proposals} proposals, ${expiring.length} expiring`,
    ),
  );
}
