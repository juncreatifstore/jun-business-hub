...
export async function createPayment(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  if (providerRef) {
    ...
    if (existing) {
      const samePayment =
        Number(existing.amount) === Number(d.amount) &&
        existing.currency === currency &&
        existing.method === d.method;
      if (!samePayment)
        return {
          message: `Provider reference ${providerRef} is already attached to payment ${existing.reference} with different payment details.`,
        };
      redirect(
        `/app/finance/payments/${existing.id}?toast=${encodeURIComponent("Payment already recorded — existing transaction reopened")}`,
      );
    }
  }
  ...
  const expectedAmount =
    d.expectedAmount === "" || d.expectedAmount == null ? null : Number(d.expectedAmount);
  ...
  await logActivity({
    ...
    message: `Payment ${reference} recorded (${payment.currency} ${d.amount})`,
    userId: user.id,
    clientId: d.clientId,
    caseId: payment.caseId,
  });
  redirect(
    `/app/finance/payments/${payment.id}?toast=${encodeURIComponent("Payment recorded — pending confirmation")}`,
  );
}
...
export async function confirmPayment(paymentId: string) {
  ...
  await audit({
    ...
    after: { status: "CONFIRMED", receipt: receiptRef },
  });
  await logActivity({
    type: "PAYMENT_CONFIRMED",
    message: `Payment ${payment.reference} confirmed · receipt ${receiptRef} available`,
    userId: user.id,
...
export async function rejectPayment(paymentId: string) {
  ...
  await logActivity({
    ...
    message: `Payment ${payment.reference} rejected`,
    userId: user.id,
    clientId: payment.clientId,
    caseId: payment.caseId,
  });
...
export async function createRefund(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  if (d.paymentId) {
    ...
    if (original.clientId !== d.clientId)
      return { message: "Original payment belongs to a different client." };
    ...
    if (d.amount > available) {
      return {
        message: `Refund exceeds the available amount on ${original.reference}: ${original.currency} ${available.toFixed(2)} remaining.`,
      };
    }
  }
  ...
  await audit({
    ...
    after: { refundNumber, amount: d.amount, installments: d.installments },
  });
  await logActivity({
...
export async function setRefundStatus(refundId: string, formData: FormData) {
  ...
  const result = await prisma.refund.updateMany({
    where: { id: refundId, status: before.status },
    data: { status: status as never, ...(status === "APPROVED" ? { approvedById: user.id } : {}) },
  });
  if (result.count !== 1) return;
  ...
  await logActivity({
    type: "REFUND_UPDATED",
    message: `Refund ${refund.refundNumber} → ${status.replaceAll("_", " ")}`,
    userId: user.id,
...
export async function markInstallmentPaid(installmentId: string) {
  ...
  const changed = await prisma.$transaction(async (tx) => {
    const result = await tx.refundInstallment.updateMany({
      where: { id: installmentId, status: { not: "PAID" } },
      data: { status: "PAID", paidAt },
    });
    if (result.count !== 1) return null;
    const remaining = await tx.refundInstallment.count({
      where: {
        refundId: inst.refundId,
  ...
  await audit({
    ...
    after: { refund: inst.refund.refundNumber, newStatus: changed },
  });
  await logActivity({
    type: "REFUND_UPDATED",
    message: `Installment paid on ${inst.refund.refundNumber} (${changed.replaceAll("_", " ")})`,
    userId: user.id,
