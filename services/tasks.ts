...
export async function updateTask(taskId: string, _prev: FormState, formData: FormData): Promise<FormState> {
  ...
  if (task.assigneeId && task.assigneeId !== before.assigneeId && task.assigneeId !== user.id)
    await prisma.notification.create({
      data: {
        userId: task.assigneeId,
        type: "TASK_ASSIGNED",
  ...
  await logActivity({
    type: "TASK_UPDATED",
    message: `Task corrected: ${task.title} · ${reason}`,
    userId: user.id,
    clientId: task.clientId,
    caseId: task.caseId,
  });
...
export async function setTaskStatus(taskId: string, formData: FormData) {
  ...
  await logActivity({
    type: "TASK_UPDATED",
    message: `Task "${t.title}" → ${status.replaceAll("_", " ")}`,
    userId: user.id,
    clientId: t.clientId,
    caseId: t.caseId,
  });
