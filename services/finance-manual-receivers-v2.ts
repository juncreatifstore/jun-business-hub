...
function normalizeReceiver(
  formData: FormData,
  base: { id: string; enabled: boolean; createdAt: string },
): ManualTransferReceiver {
  const rail: ManualTransferRail =
    text(formData, "rail", 30) === "WESTERN_UNION" ? "WESTERN_UNION" : "BANK_TRANSFER";
  const firstName = text(formData, "firstName", 100),
    lastName = text(formData, "lastName", 100),
    legalNameInput = text(formData, "legalName", 180);
  ...
  return {
    ...
    receiverType:
      text(formData, "receiverType", 50) === "INDIVIDUAL_BUSINESS_REPRESENTATIVE"
...
function validateReceiver(r: ManualTransferReceiver) {
  if (!r.label || !r.legalName) return "Label and receiver name are required";
  if (r.currency.length !== 3) return "Receiver currency must have 3 letters";
  if (r.rail === "BANK_TRANSFER" && !r.bankName) return "Bank name is required for a bank transfer receiver";
  if (r.rail === "BANK_TRANSFER" && !r.accountNumber && !r.iban && !r.clabe)
    return "Add an account number, IBAN or CLABE for a bank transfer receiver";
  return null;
}
function refresh() {
...
export async function createManualTransferReceiverV2(formData: FormData) {
  ...
  const receiver = normalizeReceiver(formData, { id, enabled: true, createdAt: now });
  const error = validateReceiver(receiver);
  if (error) redirect(dest(error, true));
  await prisma.appSetting.create({
    data: { key: `${MANUAL_RECEIVER_PREFIX}${id}`, value: JSON.stringify(receiver) },
  });
  await audit({
    userId: user.id,
    action: "MANUAL_TRANSFER_RECEIVER_CREATE_V2",
...
export async function updateManualTransferReceiverV2(receiverId: string, formData: FormData) {
  ...
  const row = await prisma.appSetting.findUnique({
    where: { key: `${MANUAL_RECEIVER_PREFIX}${receiverId}` },
    select: { value: true },
  });
  if (!row) redirect(dest("Receiver not found", true));
  let before: ManualTransferReceiver;
  try {
    before = JSON.parse(row.value) as ManualTransferReceiver;
  } catch {
    redirect(dest("Receiver data is invalid", true));
  }
  const next = normalizeReceiver(formData, {
    id: receiverId,
    enabled: before!.enabled,
    createdAt: before!.createdAt || new Date().toISOString(),
  });
  const error = validateReceiver(next);
  if (error)
    redirect(
      `/app/finance/manual-transfers/receivers/${receiverId}/edit?toast_error=${encodeURIComponent(error)}`,
    );
  ...
  await audit({
    userId: user.id,
    action: "MANUAL_TRANSFER_RECEIVER_UPDATE",
    resourceType: "ManualTransferReceiver",
    ...
    after: {
      ...
      feeFixed: next.feeFixed,
    },
  });
  refresh();
  redirect(dest("Receiver updated"));
}
