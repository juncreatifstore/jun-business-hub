...
import {
  ...
  getClientFinancialAccount,
  makeClientCommission,
  makeClientWithdrawal,
  saveClientAccountProfile,
  saveClientCommission,
  saveClientWithdrawal,
  type ClientStatementLanguage,
} from "@/lib/client-financial-account";
...
export async function updateClientFinancialProfile(clientId: string, formData: FormData) {
  ...
  const preferredLanguage = text(
    ...
    2,
  ).toUpperCase() as ClientStatementLanguage;
  if (!["FR", "EN", "ES", "HT"].includes(preferredLanguage))
    throw new Error("Unsupported statement language");
  ...
  await audit({
    userId: user.id,
    action: "CLIENT_FINANCIAL_PROFILE_UPDATE",
    resourceType: "Client",
    resourceId: clientId,
    before: { preferredLanguage: before.preferredLanguage, isPartner: before.isPartner },
    after: { preferredLanguage, isPartner, partnerNote: next.partnerNote },
  });
  await logActivity({
    type: "CLIENT_ACCOUNT_UPDATED",
    message: `Financial account settings updated for ${client.firstName} ${client.lastName}`,
    userId: user.id,
    clientId,
  });
  refresh(clientId);
...
export async function addPartnerCommission(clientId: string, formData: FormData) {
  ...
  const entry = makeClientCommission({
    clientId,
    amount,
    currency,
    description,
  ...
  await ensureUniversalFinancialReceipt({
    ...
    currency,
    direction: "CREDIT",
    title: "Partner commission receipt",
    description,
    status: "CREDITED",
    transactionReference: sourceReference,
    issuedById: user.id,
  });
  await audit({
    userId: user.id,
    action: "CLIENT_COMMISSION_CREDIT",
    resourceType: "ClientCommission",
    resourceId: entry.id,
    after: { clientId, amount, currency, description, sourceReference },
  });
  await logActivity({
    type: "CLIENT_COMMISSION_CREDIT",
    message: `${currency} ${amount.toFixed(2)} commission credited to ${client.firstName} ${client.lastName}`,
    userId: user.id,
    clientId,
  });
  refresh(clientId);
...
export async function recordPartnerWithdrawal(clientId: string, formData: FormData) {
  ...
  if (!client) throw new Error("Client not found");
  if (!account.profile.isPartner) throw new Error("Partner status is required for a partner withdrawal");
  const amount = money(formData.get("amount"));
  const currency = text(formData.get("currency"), 3).toUpperCase();
  const method = text(formData.get("method"), 100);
  const transactionReference = text(formData.get("transactionReference"), 180);
  const description = text(formData.get("description"), 500) || "Partner withdrawal";
  const receiverId = text(formData.get("receiverId"), 100) || null;
  if (!Number.isFinite(amount) || amount <= 0) throw new Error("Withdrawal amount must be positive");
  if (!/^[A-Z]{3}$/.test(currency)) throw new Error("Invalid currency");
  if (!method || !transactionReference) throw new Error("Method and transaction reference are required");
  const available = account.balances.find((b) => b.currency === currency)?.available ?? 0;
  if (amount > available + 0.005)
    throw new Error(`Withdrawal exceeds available balance: ${currency} ${available.toFixed(2)} available.`);
  const entry = makeClientWithdrawal({
    clientId,
    amount,
    currency,
    method,
  ...
  await saveClientWithdrawal(entry);
  const receipt = await ensureUniversalFinancialReceipt({
    ...
    currency,
    direction: "DEBIT",
    title: "Partner withdrawal receipt",
    description,
    status: "PAID",
    method,
    transactionReference,
  ...
  await audit({
    userId: user.id,
    action: "CLIENT_PARTNER_WITHDRAWAL",
    resourceType: "ClientWithdrawal",
    resourceId: entry.id,
    after: {
      clientId,
      amount,
  ...
  await logActivity({
    type: "CLIENT_PARTNER_WITHDRAWAL",
    message: `${currency} ${amount.toFixed(2)} partner withdrawal paid to ${client.firstName} ${client.lastName}`,
    userId: user.id,
    clientId,
  });
  refresh(clientId);
...
export async function voidPartnerWithdrawal(clientId: string, withdrawalId: string, formData: FormData) {
  ...
  const current = account.withdrawals.find((w) => w.id === withdrawalId);
  if (!current || current.status !== "PAID") throw new Error("Withdrawal not found or already voided");
  ...
  await saveClientWithdrawal(next);
  await audit({
    userId: user.id,
    action: "CLIENT_PARTNER_WITHDRAWAL_VOID",
    resourceType: "ClientWithdrawal",
    resourceId: withdrawalId,
    before: { status: current.status, amount: current.amount, currency: current.currency },
