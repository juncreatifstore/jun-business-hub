...
import {
  clearMailboxAccessRule,
  recordMailReliabilityEvent,
  saveMailboxAccessRule,
} from "@/lib/mail-security";

function securityPath(message: string, error = false) {
  return `/app/mail/security?${error ? "toast_error" : "toast"}=${encodeURIComponent(message)}`;
}
...
export async function updateUserMailboxAccess(userId: string, formData: FormData) {
  ...
  const existing = requested.length
    ? await prisma.mailAccount.findMany({ where: { id: { in: requested } }, select: { id: true } })
    : [];
  if (existing.length !== requested.length)
    redirect(securityPath("One or more mailboxes no longer exist", true));
  const before = await prisma.appSetting.findUnique({
    where: { key: `mail.access.user.${userId}` },
    select: { value: true },
  });
  ...
  await audit({
    ...
    resourceType: "User",
    resourceId: userId,
    before: { rule: before?.value ?? null },
    after: { mailAccountIds: rule.accountIds, count: rule.accountIds.length },
  });
  revalidatePath("/app/mail/security");
  revalidatePath("/app/mail");
  redirect(securityPath(`Mailbox restriction saved for ${target.firstName} ${target.lastName}`));
}
...
export async function clearUserMailboxRestriction(userId: string) {
  ...
  await audit({
    userId: actor.id,
    action: "MAILBOX_ACCESS_CLEARED",
    resourceType: "User",
    resourceId: userId,
    after: { mode: "LEGACY_FULL_ACCESS" },
  });
  revalidatePath("/app/mail/security");
  revalidatePath("/app/mail");
  redirect(securityPath(`Mailbox restriction cleared for ${target.firstName} ${target.lastName}`));
}
...
export async function probeMailboxHealth(accountId: string) {
  ...
  if (!account) redirect(securityPath("Mailbox not found", true));
  try {
    ...
    await recordMailReliabilityEvent({
      ...
      accountId,
      userId: actor.id,
      message: "Mailbox credential health check succeeded",
    });
    await audit({
      userId: actor.id,
      action: "MAILBOX_HEALTH_CHECK",
      resourceType: "MailAccount",
      resourceId: accountId,
      after: { email: account.email, status: "SUCCESS" },
    });
    revalidatePath("/app/mail/security");
    redirect(securityPath(`${account.email} health check passed`));
  } catch (e) {
    ...
    await audit({
      userId: actor.id,
      action: "MAILBOX_HEALTH_CHECK_FAILED",
      resourceType: "MailAccount",
      resourceId: accountId,
      after: { email: account.email, status: "FAILED" },
    });
    revalidatePath("/app/mail/security");
    redirect(securityPath(`${account.email}: ${message}`, true));
  }
}
