...
async function downloadLegacy(key: string) {
  ...
  if (error || !data)
    throw new Error(`Unable to read legacy object ${key}: ${error?.message || "not found"}`);
...
export async function migrateLegacyStorageBatch(): Promise<void> {
  ...
  if ((process.env.STORAGE_DRIVER || "SUPABASE").toUpperCase() === "GOOGLE_WORKSPACE")
    go("Migration must be completed before switching STORAGE_DRIVER to GOOGLE_WORKSPACE", true);
  ...
  for (const file of candidates) {
    try {
      ...
      await prisma.appSetting.upsert({
        where: { key: `${FILE_MARKER}${file.id}` },
        update: { value: new Date().toISOString() },
        create: { key: `${FILE_MARKER}${file.id}`, value: new Date().toISOString() },
      });
    } catch {
      failedFiles++;
    }
  ...
  const versionRows = await prisma.appSetting.findMany({
    where: { key: { startsWith: VERSION_PREFIX } },
  ...
  for (const row of versionRows) {
    ...
    try {
      ...
      copiedVersions++;
      await prisma.appSetting.upsert({
        where: { key: `${VERSION_MARKER}${row.key}` },
        update: { value: new Date().toISOString() },
        create: { key: `${VERSION_MARKER}${row.key}`, value: new Date().toISOString() },
      });
    } catch {
      failedVersions++;
    }
  ...
  await prisma.auditLog
    .create({
      data: {
        userId: user.id,
        action: "DRIVE_WORKSPACE_MIGRATION_BATCH",
        resourceType: "Drive",
        after: {
          copiedFiles,
          existingFiles,
  ...
  const done =
    markedFiles >= totalFiles && markedVersions >= totalVersions && failedFiles === 0 && failedVersions === 0;
  go(
    done
      ? "Migration copy complete. Verify the Workspace files, then set STORAGE_DRIVER=GOOGLE_WORKSPACE."
      : `Batch complete: ${copiedFiles} files + ${copiedVersions} versions copied. Run another batch to continue.${failedFiles + failedVersions ? ` ${failedFiles + failedVersions} item(s) need retry.` : ""}`,
  );
}
