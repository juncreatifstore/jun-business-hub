...
import {
  getCaseAutomationPlan,
  saveCaseAutomationConfig,
  saveCaseAutomationRun,
} from "@/lib/case-automation";

function path(caseId: string, msg?: string, error = false) {
  return `/app/cases/${caseId}/automation${msg ? `?${error ? "toast_error" : "toast"}=${encodeURIComponent(msg)}` : ""}`;
}
      ...
      return await prisma.$transaction(
        async (tx) => {
          const existing = await tx.task.findFirst({
            where: { caseId: input.caseId, title: input.title, status: { notIn: ["DONE", "CANCELLED"] } },
            select: { id: true },
          });
...
export async function updateCaseAutomationSettings(caseId: string, formData: FormData) {
  ...
  await audit({
    userId: user.id,
    action: "CASE_AUTOMATION_SETTINGS_UPDATED",
    resourceType: "Case",
    resourceId: caseId,
    after: config,
  });
...
export async function runCaseSafeAutomation(caseId: string) {
  ...
  const c = await prisma.case.findUnique({
    where: { id: caseId },
    select: { id: true, caseNumber: true, clientId: true, ownerId: true, priority: true, status: true },
  });
  if (!c) redirect("/app/cases?toast_error=Case%20not%20found");
  if (["COMPLETED", "CANCELLED", "ARCHIVED"].includes(c.status))
    redirect(path(caseId, "Closed or archived Cases cannot run automation.", true));
  ...
  if (config.createFollowUpTasks) {
    for (const candidate of candidates) {
      ...
      const task = await createAutomationTaskSafely({
        caseId,
        clientId: c.clientId,
        ownerId: c.ownerId,
        creatorId: user.id,
      ...
      if (config.notifyOwner && c.ownerId && c.ownerId !== user.id) {
        await prisma.notification.create({
          data: {
            userId: c.ownerId,
            type: "TASK_ASSIGNED",
            title: "Case automation assigned a follow-up",
            body: task.title,
          },
        });
      }
      await logActivity({
        type: "TASK_CREATED",
        message: `Safe Automation created task: ${task.title}`,
        userId: user.id,
        clientId: c.clientId,
        caseId,
      });
  ...
  if (config.escalatePriority && targetPriority !== c.priority) {
    const updated = await prisma.case.update({
      where: { id: caseId },
      data: { priority: targetPriority as never },
    });
    priorityAfter = updated.priority;
    actions.push(`Escalated Case priority from ${c.priority} to ${updated.priority}`);
    await logActivity({
      type: "CASE_UPDATED",
      message: `Safe Automation escalated Case priority ${c.priority} → ${updated.priority}`,
      userId: user.id,
      clientId: c.clientId,
      caseId,
    });
  ...
  const run = {
    ...
    ranById: user.id,
    riskScore: intelligence.score,
    riskLevel: intelligence.riskLevel,
    tasksCreated,
    tasksSkipped,
    priorityBefore: c.priority,
    priorityAfter,
    actions,
  };
  ...
  await audit({
    userId: user.id,
    action: "CASE_SAFE_AUTOMATION_RUN",
    resourceType: "Case",
    resourceId: caseId,
    after: {
      runId: run.id,
  ...
  redirect(
    path(
      caseId,
      `Automation completed: ${tasksCreated} task(s) created, ${tasksSkipped} duplicate(s) skipped.`,
    ),
  );
}
