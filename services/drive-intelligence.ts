...
import {
  DRIVE_INTEL_PREFIX,
  DRIVE_TAGS_PREFIX,
  getDriveIntelligence,
  indexDriveFile,
  type DriveIntelligence,
} from "@/lib/drive-intelligence";
...
type AnalysisResult = { ok: true; intelligence: DriveIntelligence } | { ok: false; error: string };
...
async function richOpenAIAnalysis(input: {
  fileName: string;
  mimeType: string;
  context: string;
  buffer?: Buffer;
  ...
  if (input.buffer && input.buffer.length <= 18 * 1024 * 1024) {
    ...
    if (input.mimeType.startsWith("image/")) {
      content.push({
        type: "input_image",
        image_url: `data:${input.mimeType};base64,${b64}`,
        detail: "high",
      });
    } else if (
...
async function runAnalysis(fileId: string): Promise<AnalysisResult> {
  ...
  try {
    buffer = await storage().download(file.storageKey);
  } catch {
    buffer = undefined;
  }
  ...
  const context = [
    ...
    file.client
      ? `Linked client: ${file.client.firstName} ${file.client.lastName} (${file.client.internalId})`
      : "",
    file.case ? `Linked case: ${file.case.caseNumber} — ${file.case.title}` : "",
    excerpt
      ? `Previously extracted text:\n${excerpt.slice(0, 12000)}`
      : "Previously extracted text: unavailable or not applicable.",
  ]
    .filter(Boolean)
    .join("\n");
  ...
  try {
    ...
    const suggestedCategory = CATEGORIES.includes(
      String(parsed.suggestedCategory ?? "OTHER") as (typeof CATEGORIES)[number],
    )
...
export async function analyzeDriveFile(fileId: string, formData: FormData): Promise<void> {
  ...
  await audit({
    userId: user.id,
    action: "FILE_AI_ANALYZE",
    resourceType: "File",
    resourceId: fileId,
    after: { suggestedCategory: result.intelligence.suggestedCategory, richAnalysis: true },
  });
  await logActivity({
    userId: user.id,
    type: "FILE_AI_ANALYZED",
    message: "Rich AI file intelligence generated",
    resourceType: "File",
    resourceId: fileId,
  });
...
export async function acceptDriveSuggestedCategory(fileId: string, formData: FormData): Promise<void> {
  ...
  if (!suggested || !CATEGORIES.includes(suggested as (typeof CATEGORIES)[number]))
    redirect(toast(returnTo, "toast_error", "No valid AI category suggestion"));
  const next = suggested as FileCategory;
  if (next !== file.category) {
    await prisma.file.update({ where: { id: file.id }, data: { category: next } });
    await audit({
      userId: user.id,
      action: "FILE_AI_CATEGORY_ACCEPT",
      resourceType: "File",
      resourceId: file.id,
      before: { category: file.category },
      after: { category: next },
    });
  }
  revalidatePath("/app/drive");
