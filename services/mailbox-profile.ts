...
export async function setOtpSenderMailbox(accountId:string):Promise<void>{
 const user=await assertPermission("SETTINGS_MANAGE");
 const account=await prisma.mailAccount.findUnique({where:{id:accountId},select:{id:true,email:true,accessTokenEnc:true,refreshTokenEnc:true}});
 if(!account)redirect("/app/settings/email?toast_error=Boîte email introuvable");
 if(!account.accessTokenEnc&&!account.refreshTokenEnc)redirect(`/app/settings/email?toast_error=${encodeURIComponent(`La boîte ${account.email} est déconnectée`)}`);
 const before=await prisma.appSetting.findUnique({where:{key:OTP_SENDER_SETTING_KEY},select:{value:true}});
 await prisma.appSetting.upsert({where:{key:OTP_SENDER_SETTING_KEY},create:{key:OTP_SENDER_SETTING_KEY,value:account.id},update:{value:account.id}});
 await audit({userId:user.id,action:"OTP_MAILBOX_SELECTED",resourceType:"MailAccount",resourceId:account.id,before:{accountId:before?.value??null},after:{accountId:account.id,email:account.email}});
 revalidatePath("/app/settings/email");
 redirect(`/app/settings/email?toast=${encodeURIComponent(`${account.email} est maintenant l’expéditeur OTP par défaut`)}`);
}
