    ...
    if (target === "APPROVED") {
      ...
      if (refund && refund.status === "APPROVED")
        await ensureUniversalFinancialReceipt({
          ...
          direction: "DEBIT",
          title: "Refund / withdrawal approval receipt",
          description: refund.reason,
          status: refund.status,
          transactionReference: refund.refundNumber,
          issuedById: user.id,
        });
    }
  } catch (error) {
    const message =
      error instanceof Error && error.message ? error.message : "The refund decision could not be completed.";
