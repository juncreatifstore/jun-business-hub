...
export async function resendSigningInvitation(requestId: string, signerEmail: string): Promise<void> {
  ...
  if (
    ...
    redirect(
      `/app/signatures/${requestId}?toast_error=Only the current unsigned signer can receive an invitation`,
    );
  }
  const meta = signatureRequestMeta(request.recipients);
  const expiresAt = requestExpiry(meta, request.sentAt);
  if (expiresAt.getTime() <= Date.now())
    redirect(`/app/signatures/${requestId}?toast_error=Request has expired`);
  try {
    await sendInvitation({
      ...
      subjectPrefix: "Signing invitation resent",
    });
  } catch (error) {
    redirect(
      `/app/signatures/${requestId}?toast_error=${encodeURIComponent(error instanceof Error ? error.message : "Could not resend invitation")}`,
    );
  }
  ...
  await audit({
    userId: user.id,
    action: "JUN_NATIVE_INVITATION_RESENT",
    resourceType: "SignatureRequest",
    resourceId: requestId,
    before: { signer: recipient.email },
    after: { signer: recipient.email, sentAt },
  });
...
export async function regenerateSignerLink(requestId: string, signerEmail: string): Promise<void> {
  ...
  if (index < 0 || recipients[index].signedAt || recipients[index].declinedAt)
    redirect(`/app/signatures/${requestId}?toast_error=Signer link cannot be regenerated`);
  const current = currentUnsigned(recipients);
  if (!current || current.email.toLowerCase() !== recipients[index].email.toLowerCase())
    redirect(`/app/signatures/${requestId}?toast_error=Only the current signer link can be regenerated`);
  ...
  try {
    await sendInvitation({
      ...
      expiresAt,
      message: meta.message,
      subjectPrefix: "New secure signing link",
    });
    ...
    await audit({
      userId: user.id,
      action: "JUN_NATIVE_LINK_REGENERATED_EMAIL_FAILED",
      resourceType: "SignatureRequest",
      resourceId: requestId,
      before: { linkVersion: oldVersion },
      after: {
        signer: recipients[index].email,
        linkVersion: oldVersion + 1,
        error: error instanceof Error ? error.message.slice(0, 250) : "unknown",
      },
    }).catch(() => undefined);
    redirect(
      `/app/signatures/${requestId}?toast_error=Link was revoked but the replacement email could not be sent`,
    );
  }
  await audit({
    userId: user.id,
    action: "JUN_NATIVE_LINK_REGENERATED",
    resourceType: "SignatureRequest",
    resourceId: requestId,
    before: { signer: signerEmail, linkVersion: oldVersion },
    after: { signer: recipients[index].email, linkVersion: oldVersion + 1 },
  });
  revalidatePath(`/app/signatures/${requestId}`);
  redirect(`/app/signatures/${requestId}?toast=Old link revoked and new link sent`);
}

export async function replacePendingSigner(
  requestId: string,
  order: number,
  formData: FormData,
): Promise<void> {
  ...
  if (index < 0 || recipients[index].signedAt || recipients[index].declinedAt)
    redirect(`/app/signatures/${requestId}?toast_error=This signer can no longer be changed`);
  ...
  if (name.length < 2 || !validEmail(email))
    redirect(`/app/signatures/${requestId}?toast_error=Enter a valid signer name and email`);
  if (recipients.some((r, i) => i !== index && r.email.toLowerCase() === email))
    redirect(`/app/signatures/${requestId}?toast_error=That email already belongs to another signer`);
  ...
  if (current?.order === order) {
    try {
      await sendInvitation({
        ...
        title: request.document.title,
        recipient: recipients[index],
        expiresAt: requestExpiry(meta, request.sentAt),
        message: meta.message,
        subjectPrefix: "Updated signing invitation",
      });
      recipients[index] = { ...recipients[index], invitationSentAt: new Date().toISOString() };
      await prisma.signatureRequest.update({
        where: { id: requestId },
  ...
  await audit({
    userId: user.id,
    action: "JUN_NATIVE_SIGNER_REPLACED",
    resourceType: "SignatureRequest",
    resourceId: requestId,
    before: { name: before.name, email: before.email, role: before.role, order },
    after: { name, email, role, order, linkVersion: recipients[index].linkVersion, invitationSent: sent },
  });
  revalidatePath(`/app/signatures/${requestId}`);
  redirect(
    `/app/signatures/${requestId}?toast=${sent ? "Signer updated and new invitation sent" : "Signer updated"}`,
  );
}
...
export async function extendSignatureExpiration(requestId: string, formData: FormData): Promise<void> {
  ...
  if (!Number.isInteger(days) || days < 1 || days > 30)
    redirect(`/app/signatures/${requestId}?toast_error=Extension must be between 1 and 30 days`);
  const recipients = signatureRecipients(request.recipients).sort((a, b) => a.order - b.order);
  const meta = signatureRequestMeta(request.recipients);
  ...
  if (current) {
    try {
      await sendInvitation({
        ...
        expiresAt: newExpiry,
        message: meta.message,
        subjectPrefix: "Signing deadline extended",
      });
  ...
  await audit({
    userId: user.id,
    action: "JUN_NATIVE_EXPIRATION_EXTENDED",
    resourceType: "SignatureRequest",
    resourceId: requestId,
    before: { expiresAt: oldExpiry.toISOString() },
    after: { expiresAt: newExpiry.toISOString(), daysAdded: days, invitationSent },
  });
  revalidatePath(`/app/signatures/${requestId}`);
...
export async function cancelNativeSignatureRequest(requestId: string, formData: FormData): Promise<void> {
  ...
  const meta = signatureRequestMeta(request.recipients);
  const now = new Date();
  const revoked = recipients.map((r) =>
    r.signedAt
      ? r
      : {
          ...r,
  ...
  await prisma.signatureRequest.update({
    ...
    data: {
      status: "VOIDED",
      completedAt: now,
      recipients: signatureRecipientsPayload(revoked, {
        ...meta,
  ...
  await audit({
    userId: user.id,
    action: "JUN_NATIVE_REQUEST_CANCELLED",
    resourceType: "SignatureRequest",
    resourceId: requestId,
    ...
    after: {
      ...
      cancelledAt: now.toISOString(),
      reason,
      linksRevoked: revoked.filter((r) => !r.signedAt).length,
    },
  });
  revalidatePath(`/app/signatures/${requestId}`);
