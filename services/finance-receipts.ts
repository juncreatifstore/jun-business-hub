...
export async function voidReceipt(paymentId: string, formData: FormData) {
  ...
  if (!reason)
    redirect(
      `/app/finance/receipts/${paymentId}?toast_error=${encodeURIComponent("A void reason is required")}`,
    );
  ...
  if (!payment || !payment.paidAt)
    redirect(`/app/finance/receipts?toast_error=${encodeURIComponent("Receipt not found")}`);

  const meta = await ensureReceiptMeta(payment);
  if (meta.status === "VOID")
    redirect(`/app/finance/receipts/${paymentId}?toast=${encodeURIComponent("Receipt is already void")}`);
  ...
  await audit({
    ...
    before: { receiptStatus: meta.status },
    after: { receiptStatus: next?.status, reason },
  });
  await logActivity({
    type: "PAYMENT_UPDATED",
    message: `Receipt RCT-${payment.reference} voided`,
    userId: user.id,
