...
export async function saveDrivePublicSecurity(fileId: string, formData: FormData): Promise<void> {
  ...
  if (enterprise.publicLinkPolicy === "DISABLED")
    redirect(withToast(returnTo, "toast_error", "Public links are disabled by Enterprise policy"));
  ...
  if (expiresRaw) {
    ...
    if (Number.isNaN(parsed.getTime()))
      redirect(withToast(returnTo, "toast_error", "Invalid expiration date"));
    if (parsed.getTime() <= Date.now())
      redirect(withToast(returnTo, "toast_error", "Expiration must be in the future"));
    if (
      enterprise.maxPublicLinkDays > 0 &&
      parsed.getTime() > Date.now() + enterprise.maxPublicLinkDays * 86400000 + 60000
    ) {
      redirect(
        withToast(
          returnTo,
          "toast_error",
          `Enterprise policy limits public links to ${enterprise.maxPublicLinkDays} days`,
        ),
      );
    }
    ...
    redirect(
      withToast(
        returnTo,
        "toast_error",
        `Enterprise policy requires an expiration within ${enterprise.maxPublicLinkDays} days`,
      ),
    );
  }
  ...
  if (password && password.length < 6)
    redirect(withToast(returnTo, "toast_error", "Public-link password must contain at least 6 characters"));
  ...
  if (
    enterprise.publicLinkPolicy === "PASSWORD_REQUIRED" &&
    (clearPassword || (!password && !existingPassword))
  ) {
    redirect(
      withToast(returnTo, "toast_error", "Enterprise policy requires password protection for public links"),
    );
  }

  if (expiresIso)
    await prisma.appSetting.upsert({
      where: { key: expiresKey },
      update: { value: expiresIso },
      create: { key: expiresKey, value: expiresIso },
    });
  ...
  if (clearPassword) {
    ...
    await prisma.appSetting.upsert({
      ...
      create: { key: passwordKey, value: passwordHash },
    });
    passwordChanged = true;
  }
  ...
  await audit({
    ...
    after: {
      ...
      passwordChanged,
      passwordCleared: clearPassword,
      enterprisePolicy: enterprise.publicLinkPolicy,
    },
  });
