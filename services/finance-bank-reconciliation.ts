...
export async function importBankStatementAction(formData: FormData) {
  ...
  if (!(file instanceof File) || file.size === 0)
    go("/app/finance/reconciliation/import", "error", "Choose a CSV or OFX statement file.");
  if (file.size > 8 * 1024 * 1024)
    go("/app/finance/reconciliation/import", "error", "Statement file must be 8 MB or smaller.");
  if (!bankName || !accountLabel || !/^[A-Z]{3}$/.test(currency))
    go(
      "/app/finance/reconciliation/import",
      "error",
      "Bank, account label and a valid 3-letter currency are required.",
    );
  const lower = file.name.toLowerCase();
  const format = lower.endsWith(".ofx") || lower.endsWith(".qfx") ? "OFX" : ("CSV" as const);
  try {
    const record = await importBankStatement({
      fileName: file.name,
    ...
    go(
      `/app/finance/reconciliation/${record.id}`,
      "success",
      `Imported ${record.transactionCount} transaction(s); ${record.duplicateCount} duplicate(s) skipped.`,
    );
  } catch (error) {
    go(
      "/app/finance/reconciliation/import",
...
export async function refreshBankSuggestionsAction(importId: string) {
  ...
  await audit({
    userId: user.id,
    action: "BANK_RECON_SUGGEST",
    resourceType: "BankStatementImport",
    resourceId: importId,
    after: { suggested: count },
  });
...
export async function confirmBankMatchAction(formData: FormData) {
  ...
  if (!tx) go("/app/finance/reconciliation", "error", "Bank transaction not found.");
  try {
    const match = await confirmReconciliation(
      transactionId,
      journalEntryId,
      user.id,
      note,
      tx.suggestedEntryId === journalEntryId ? "AUTO_SUGGESTED" : "MANUAL",
    );
    await audit({
      userId: user.id,
      action: "BANK_RECON_CONFIRM",
    ...
    go(
      `/app/finance/reconciliation/${tx.importId}`,
      "error",
      error instanceof Error ? error.message : "Unable to reconcile transaction",
    );
  }
}
...
export async function ignoreBankTransactionAction(formData: FormData) {
  ...
  try {
    ...
    await audit({
      userId: user.id,
      action: "BANK_RECON_IGNORE",
      resourceType: "BankTransaction",
      resourceId: id,
      after: { status: "IGNORED" },
    });
    revalidatePath(`/app/finance/reconciliation/${tx.importId}`);
    go(`/app/finance/reconciliation/${tx.importId}`, "success", "Transaction marked ignored.");
  } catch (error) {
    go(
      `/app/finance/reconciliation/${tx.importId}`,
      "error",
      error instanceof Error ? error.message : "Unable to ignore transaction",
    );
  }
}
...
export async function closeBankReconciliationPeriodAction(formData: FormData) {
  ...
  if (confirmation !== "CLOSE")
    go("/app/finance/reconciliation/close", "error", "Type CLOSE to confirm the reconciliation lock.");
  try {
    const record = await closeBankReconciliationPeriod({
      period,
      currency,
      accountLabel,
      closedById: user.id,
    ...
    await audit({
      userId: user.id,
      action: "BANK_RECON_PERIOD_CLOSE",
      resourceType: "BankReconciliationPeriod",
      resourceId: `${period}:${currency}:${accountLabel}`,
      after: record,
    });
    revalidatePath("/app/finance/reconciliation");
    revalidatePath("/app/finance/reconciliation/close");
    go("/app/finance/reconciliation/close", "success", `Reconciliation period ${period} closed.`);
  } catch (error) {
    go(
      "/app/finance/reconciliation/close",
      "error",
      error instanceof Error ? error.message : "Unable to close reconciliation period",
    );
  }
}
