...
import {
  renderInvoicePdf,
  renderRefundPdf,
  renderFinancialMovementReceiptPdf,
} from "@/services/pdf/finance-documents";
...
async function deliverFinancePdf(input: {
  ...
  if (!cfg.defaultTemplate)
    throw new Error("Configure the approved jun_document_notification template in Settings → WhatsApp first");
  if (!cfg.tokenConfigured || !cfg.phoneNumberId)
    throw new Error("Configure Meta WhatsApp credentials in Settings → WhatsApp first");
...
export async function sendFinanceInvoiceByWhatsApp(invoiceId: string) {
  ...
  if (!client)
    redirect(`/app/finance/invoices/${invoiceId}?toast_error=${encodeURIComponent("Client not found")}`);
  const to = cleanPhone(client.whatsapp || client.phone);
  if (!to)
    redirect(
      `/app/finance/invoices/${invoiceId}?toast_error=${encodeURIComponent("Client has no WhatsApp number")}`,
    );
  ...
  try {
    ...
    await deliverFinancePdf({
      ...
      auditAction: "WHATSAPP_INVOICE_SEND",
  ...
  if (errorMessage)
    redirect(`/app/finance/invoices/${invoiceId}?toast_error=${encodeURIComponent(errorMessage)}`);
  redirect(
    `/app/finance/invoices/${invoiceId}?toast=${encodeURIComponent("Invoice accepted by Meta — awaiting delivery confirmation")}`,
  );
}
...
export async function sendFinanceRefundByWhatsApp(refundId: string) {
  ...
  const r = await prisma.refund.findUnique({
    where: { id: refundId },
    include: { client: true, case: true, payment: true, installments: { orderBy: { dueDate: "asc" } } },
  });
  if (!r) redirect("/app/finance/refunds?toast_error=Refund not found");
  const to = cleanPhone(r.client.whatsapp || r.client.phone);
  if (!to)
    redirect(
      `/app/finance/refunds/${refundId}?toast_error=${encodeURIComponent("Client has no WhatsApp number")}`,
    );
  ...
  try {
    ...
    await deliverFinancePdf({
      ...
      to,
      bytes,
      filename: `${r.refundNumber}.pdf`,
      documentLabel: "Refund / Withdrawal Statement",
      reference: r.refundNumber,
      clientName: `${r.client.firstName} ${r.client.lastName}`.trim(),
      auditAction: "WHATSAPP_REFUND_SEND",
      resourceType: "Refund",
      resourceId: r.id,
    });
  ...
  if (errorMessage)
    redirect(`/app/finance/refunds/${refundId}?toast_error=${encodeURIComponent(errorMessage)}`);
  redirect(
    `/app/finance/refunds/${refundId}?toast=${encodeURIComponent("Refund PDF accepted by Meta — awaiting delivery confirmation")}`,
  );
}
...
export async function sendPaymentReceiptByWhatsApp(paymentId: string) {
  ...
  if (!payment.paidAt || !["CONFIRMED", "PARTIALLY_REFUNDED", "REFUNDED"].includes(payment.status))
    redirect(
      `/app/finance/receipts/${payment.id}?toast_error=${encodeURIComponent("Receipt unavailable until the payment is confirmed")}`,
    );
  const meta = await ensureReceiptMeta(payment);
  if (meta.status === "VOID")
    redirect(
      `/app/finance/receipts/${payment.id}?toast_error=${encodeURIComponent("This receipt has been voided")}`,
    );
  const to = cleanPhone(payment.client.whatsapp || payment.client.phone);
  if (!to)
    redirect(
      `/app/finance/receipts/${payment.id}?toast_error=${encodeURIComponent("Client has no WhatsApp number")}`,
    );
  ...
  try {
    ...
    await deliverFinancePdf({
      ...
      to,
      bytes,
      filename: `${meta.receiptReference}.pdf`,
      documentLabel: "Official Payment Receipt",
      reference: meta.receiptReference,
      clientName: `${payment.client.firstName} ${payment.client.lastName}`.trim(),
      auditAction: "WHATSAPP_PAYMENT_RECEIPT_SEND",
  ...
  if (errorMessage)
    redirect(`/app/finance/receipts/${payment.id}?toast_error=${encodeURIComponent(errorMessage)}`);
  redirect(
    `/app/finance/receipts/${payment.id}?toast=${encodeURIComponent("Receipt accepted by Meta — awaiting delivery confirmation")}`,
  );
}
...
export async function sendUniversalFinanceReceiptByWhatsApp(receiptId: string, returnOrderId?: string) {
  ...
  const returnPath = returnOrderId
    ? `/app/finance/manual-transfers/${returnOrderId}`
    : "/app/finance/receipts";
  if (!receipt) redirect(`${returnPath}?toast_error=${encodeURIComponent("Financial receipt not found")}`);
  if (!receipt.clientId)
    redirect(
      `${returnPath}?toast_error=${encodeURIComponent("This financial receipt is not linked to a client")}`,
    );
  const client = await prisma.client.findUnique({
    where: { id: receipt.clientId },
    select: { id: true, firstName: true, lastName: true, internalId: true, phone: true, whatsapp: true },
  });
  ...
  let errorMessage = "";
  try {
    const bytes = await renderFinancialMovementReceiptPdf({
    ...
    await deliverFinancePdf({
      ...
      documentLabel: receipt.title || "Financial Receipt",
      reference: receipt.receiptNumber,
      clientName: `${client.firstName} ${client.lastName}`.trim(),
      auditAction: "WHATSAPP_FINANCIAL_RECEIPT_SEND",
      resourceType: "UniversalFinancialReceipt",
  ...
  redirect(
    `${returnPath}?toast=${encodeURIComponent("Financial receipt accepted by Meta — awaiting delivery confirmation")}`,
  );
}
...
export async function sendManualTransferOrderByWhatsApp(orderId: string) {
  ...
  if (!order.clientId)
    redirect(
      `/app/finance/manual-transfers/${orderId}?toast_error=${encodeURIComponent("This payment order is not linked to a client")}`,
    );
  const client = await prisma.client.findUnique({
  ...
  if (!client)
    redirect(
      `/app/finance/manual-transfers/${orderId}?toast_error=${encodeURIComponent("Client not found")}`,
    );
  const to = cleanPhone(client.whatsapp || client.phone);
  if (!to)
    redirect(
      `/app/finance/manual-transfers/${orderId}?toast_error=${encodeURIComponent("Client has no WhatsApp number")}`,
    );
  ...
  try {
    ...
    await deliverFinancePdf({
      ...
      bytes,
      filename,
      documentLabel: "Manual Payment Order",
      reference: order.orderNumber,
      clientName: `${client.firstName} ${client.lastName}`.trim(),
      auditAction: "WHATSAPP_MANUAL_TRANSFER_ORDER_SEND",
  ...
  if (errorMessage)
    redirect(`/app/finance/manual-transfers/${orderId}?toast_error=${encodeURIComponent(errorMessage)}`);
  redirect(
    `/app/finance/manual-transfers/${orderId}?toast=${encodeURIComponent("Payment order accepted by Meta — awaiting delivery confirmation")}`,
  );
}
