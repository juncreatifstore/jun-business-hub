...
import {
  makeOnlinePublicToken,
  saveOnlinePaymentSession,
  type OnlinePaymentProvider,
  type OnlinePaymentSession,
} from "@/lib/finance-online-payments";

function dest(id: string, msg: string, error = false) {
  return `/app/finance/invoices/${encodeURIComponent(id)}?${error ? "toast_error" : "toast"}=${encodeURIComponent(msg)}`;
}
function value(fd: FormData, key: string, max = 500) {
...
async function blockFor(clientId: string) {
  return getClientBlock(clientId);
}
...
export async function createInvoice(formData: FormData) {
  ...
  if (!clientId || currency.length !== 3 || !dueDateRaw)
    redirect(
      `/app/finance/invoices/new?toast_error=${encodeURIComponent("Client, due date and 3-letter currency are required")}`,
    );
  const [client, block] = await Promise.all([
    prisma.client.findFirst({ where: { id: clientId, archivedAt: null }, select: { id: true } }),
    blockFor(clientId),
  ]);
  if (!client) redirect(`/app/finance/invoices/new?toast_error=${encodeURIComponent("Client not found")}`);
  if (block?.blocked)
    redirect(
      `/app/finance/invoices/new?toast_error=${encodeURIComponent(`Client blocked: ${block.reason}`)}`,
    );
  if (caseId) {
    const linkedCase = await prisma.case.findUnique({ where: { id: caseId }, select: { clientId: true } });
    if (!linkedCase || linkedCase.clientId !== clientId)
      redirect(
        `/app/finance/invoices/new?toast_error=${encodeURIComponent("Selected case does not belong to client")}`,
      );
  }
  const calc = parseLines(formData);
  if (!calc.lines.length || calc.total <= 0)
    redirect(
      `/app/finance/invoices/new?toast_error=${encodeURIComponent("Add at least one positive invoice line")}`,
    );
  const now = new Date();
  const due = new Date(`${dueDateRaw}T23:59:59.999Z`);
  if (Number.isNaN(due.getTime()))
    redirect(`/app/finance/invoices/new?toast_error=${encodeURIComponent("Invalid due date")}`);
  const invoice: FinanceInvoice = {
    ...
    status: "DRAFT",
    title,
    notes: value(formData, "notes", 2000),
    terms: value(formData, "terms", 2000) || "Payment due by the stated due date.",
    ...calc,
    payments: [],
    reminders: [],
    createdById: user.id,
  ...
  await logActivity({
    ...
    message: `Invoice ${invoice.invoiceNumber} created (${invoice.currency} ${invoice.total.toFixed(2)})`,
    userId: user.id,
    clientId,
    caseId,
  });
...
export async function markInvoiceSent(id: string) {
  ...
  if (!invoice) redirect("/app/finance/invoices");
  const block = await blockFor(invoice.clientId);
  if (block?.blocked) redirect(dest(id, `Client blocked: ${block.reason}`, true));
  ...
  await audit({
    ...
    after: { status: next.status, sentAt: next.sentAt },
  });
...
export async function linkPaymentToInvoice(id: string, formData: FormData) {
  ...
  if (invoice.status === "CANCELLED" || invoice.status === "DRAFT")
    redirect(dest(id, "Send the invoice before linking payment", true));
  const paymentId = value(formData, "paymentId", 100);
  const payment = await prisma.payment.findUnique({
    where: { id: paymentId },
    select: { id: true, clientId: true, currency: true, amount: true, reference: true },
  });
  if (!payment || payment.clientId !== invoice.clientId)
    redirect(dest(id, "Payment does not belong to invoice client", true));
  if (payment.currency !== invoice.currency)
    redirect(dest(id, "Payment currency does not match invoice currency", true));
  if (invoice.payments.some((p) => p.paymentId === payment.id))
    redirect(dest(id, "Payment is already linked to this invoice", true));
  const allInvoices = await listInvoices(3000);
  const allocatedElsewhere = round(
    allInvoices
  ...
  if (availableFromPayment <= 0)
    redirect(dest(id, "This payment has already been fully allocated to invoices", true));
  const state = await invoiceFinancialState(invoice);
  ...
  const next = {
    ...
    payments: [
      ...invoice.payments,
      { paymentId: payment.id, linkedAt: new Date().toISOString(), linkedById: user.id, amountApplied },
    ],
    updatedAt: new Date().toISOString(),
  };
  await saveInvoice(next);
  ...
  await audit({
    ...
    resourceType: "FinanceInvoice",
    resourceId: id,
    after: { paymentId: payment.id, reference: payment.reference, paymentAmount, amountApplied, unallocated },
  });
  revalidatePath(`/app/finance/invoices/${id}`);
  revalidatePath("/app/finance/invoices");
  redirect(
    dest(
      id,
      `Payment ${payment.reference} linked: ${invoice.currency} ${amountApplied.toFixed(2)} applied${unallocated > 0 ? `, ${invoice.currency} ${unallocated.toFixed(2)} remains unallocated` : ""}`,
    ),
  );
}
...
export async function recordInvoiceReminder(id: string, formData: FormData) {
  ...
  const next = {
    ...invoice,
    reminders: [...invoice.reminders, { sentAt: new Date().toISOString(), sentById: user.id, channel, note }],
    updatedAt: new Date().toISOString(),
  };
  await saveInvoice(next);
  await audit({
...
export async function createInvoiceOnlinePayment(id: string, formData: FormData) {
  ...
  if (block?.blocked) redirect(dest(id, `Client blocked: ${block.reason}`, true));
  if (invoice.status === "CANCELLED" || invoice.status === "DRAFT")
    redirect(dest(id, "Send the invoice before creating a payment link", true));
  const state = await invoiceFinancialState(invoice);
  ...
  const provider: OnlinePaymentProvider =
    providerRaw === "PAYPAL" ? "PAYPAL" : providerRaw === "MERCADO_PAGO" ? "MERCADO_PAGO" : "STRIPE";
  ...
  try {
    ...
    await saveOnlinePaymentSession({
      ...session,
      status: "FAILED",
  ...
  await prisma.payment.update({
    where: { id: payment.id },
    data: { providerRef: checkout.providerSessionId },
  });
  const next = {
    ...invoice,
    payments: [
  ...
  await audit({
    userId: user.id,
    action: "INVOICE_ONLINE_PAYMENT_CREATE",
    resourceType: "FinanceInvoice",
    resourceId: id,
    after: { paymentId: payment.id, reference, provider, amount, sessionId },
  });
  revalidatePath(`/app/finance/invoices/${id}`);
  revalidatePath("/app/finance/online-payments");
  redirect(
    `/app/finance/online-payments/${sessionId}?token=${encodeURIComponent(publicToken)}&toast=${encodeURIComponent(`Payment link created for ${invoice.invoiceNumber}`)}`,
  );
}
