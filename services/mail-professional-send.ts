...
import { approvalIsCurrent, getMailApproval, markMailApprovalSent } from "@/lib/mail-approval";
...
export async function sendProfessionalDraft(threadId:string){
 ...
 revalidatePath("/app/mail");revalidatePath("/app/mail/approvals");redirect(`/app/mail?mailbox=${thread.mailAccountId}&folder=SENT&thread=${thread.id}&toast=${encodeURIComponent("Professional email sent")}`);
}
