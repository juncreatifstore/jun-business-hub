...
function footer(c: Ctx) {
  ...
  c.page.drawText(safe(`${c.company} · ${c.website}`), {
    x: PAGE.m,
    y: 22,
    size: 7.2,
    font: c.font,
  ...
  c.page.drawText(p, {
    x: PAGE.w - PAGE.m - c.font.widthOfTextAtSize(p, 7.2),
    y: 22,
    size: 7.2,
    font: c.font,
    color: MUTED,
...
function addPage(c: Ctx) {
  ...
  c.page.drawText(website, {
    x: PAGE.w - PAGE.m - c.font.widthOfTextAtSize(website, 8),
    y: c.y,
    size: 8,
    font: c.font,
    color: MUTED,
...
function field(c: Ctx, label: string, value: string, x: number, y: number, width: number) {
  ...
  lines
    .slice(0, 3)
    .forEach((line, i) =>
      c.page.drawText(safe(line), { x, y: y - 13 - i * 11, size: 8.8, font: c.font, color: INK }),
    );
}
...
function receiverName(r: ManualTransferReceiver) {
  return [r.firstName, r.lastName].filter(Boolean).join(" ") || r.legalName || "-";
}
...
export async function renderManualTransferOrderPdf(order: ManualTransferOrder) {
  ...
  const c: Ctx = {
    ...
    y: 0,
    pageNo: 0,
    reference: safe(order.orderNumber),
    company: co.name,
    website: co.website,
  };
  addPage(c);
  ...
  c.y -= 15;
  const routeLine = safe(
    `${order.receiverSnapshot.rail.replaceAll("_", " ")} · ${order.originCountry} -> ${order.destinationCountry} · ${order.language}`,
  );
  ...
  c.page.drawText("IMPORTANT - BEFORE SENDING", {
    x: PAGE.m + 10,
    y: c.y - 10,
    size: 7,
    font: bold,
