...
export async function renderFormalNoticePdf(input: {
  ...
  const addPage = (first = false) => {
    ...
    if (first) {
      if (logo) {
        const scale = Math.min(110 / logo.width, 78 / logo.height);
        page.drawImage(logo, { x: M, y: H - 119, width: logo.width * scale, height: logo.height * scale });
      } else page.drawText("JUN", { x: M, y: H - 82, size: 30, font: bold, color: NAVY });

      const infoX = M + 124;
      page.drawText(company, { x: infoX, y: H - 61, size: 13.5, font: bold, color: NAVY });
