...
vi.mock("@/services/company-funds-approvals", () => ({
  approveFinancialAuthorizationAction: vi.fn(),
  rejectFinancialAuthorizationAction: vi.fn(),
}));
import {
...
describe("authorization decision feedback", () => {
  it("shows a requester denial without crashing the route", async () => {
    vi.mocked(approveFinancialAuthorizationAction).mockRejectedValueOnce(
      new Error("Requester cannot approve their own financial authorization"),
    );
    const result = await submitFinancialAuthorizationDecision("a", state, form("APPROVE"));
    expect(result.success).toBe(false);
    expect(result.message).toContain("autre Super Admin");
  });
  it("reports a stale decision as feedback", async () => {
    vi.mocked(rejectFinancialAuthorizationAction).mockRejectedValueOnce(
      new Error("Authorization is no longer pending"),
    );
    expect((await submitFinancialAuthorizationDecision("a", state, form("REJECT"))).message).toContain(
      "déjà été traitée",
    );
  });
  ...
  it("does not expose internal database errors", async () => {
    ...
    expect((await submitFinancialAuthorizationDecision("a", state, form("APPROVE"))).message).not.toContain(
      "database",
    );
  });
  it("does not act for a non-admin", async () => {
    vi.mocked(requireUser).mockResolvedValueOnce({ role: "FINANCE" } as Awaited<
      ReturnType<typeof requireUser>
    >);
    await expect(submitFinancialAuthorizationDecision("a", state, form("APPROVE"))).rejects.toThrow(
