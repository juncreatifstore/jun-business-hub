...
describe("HTML sanitization — stored XSS prevention", () => {
  ...
  it("blocks javascript: and data: URLs in links", () => {
    const out = sanitizeDocumentHtml(
      '<a href="javascript:alert(1)">j</a><a href="data:text/html,x">d</a><a href="https://ok.example">ok</a>',
    );
  ...
  it("keeps legitimate document structure", () => {
    const src =
      "<h1>Contract</h1><h2>Scope</h2><p><strong>Bold</strong> <em>it</em> <u>u</u></p><ul><li>a</li></ul><ol><li>b</li></ol><blockquote>q</blockquote><table><tbody><tr><td>c</td></tr></tbody></table>";
  ...
  it("allows only text-align in style, strips the rest", () => {
    const out = sanitizeDocumentHtml(
      '<p style="text-align: center; position:fixed; background:url(javascript:x)">c</p>',
    );
  ...
  it("survives nested/malformed payloads", () => {
    const out = sanitizeDocumentHtml(
      "<p><scr<script>ipt>alert(1)</scr</script>ipt></p><img src=x onerror=alert(1)>",
    );
    expect(out).not.toContain("<script");
