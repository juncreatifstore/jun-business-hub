...
vi.mock("@/lib/auth", () => ({
  assertPermission: vi.fn(async () => ({ id: "finance" })),
...
vi.mock("@/services/refund-financial-authorization", () => ({
  markRefundInstallmentPaidAuthorized: vi.fn(),
}));
...
function data() {
  ...
  form.set(
    ...
    JSON.stringify([
      ...
      { amount: "2000", dueDate: "2026-11-10" },
    ]),
  );
  return form;
}
...
describe("saving a refund schedule", () => {
  it("preserves paid rows, cancels only open rows and audits the replacement", async () => {
    ...
    expect(db.refundInstallment.updateMany).toHaveBeenCalledWith({
      where: { id: { in: ["open"] }, status: { in: ["SCHEDULED", "LATE"] } },
      data: { status: "CANCELLED" },
    });
    expect(db.refundInstallment.createMany).toHaveBeenCalledWith({
      data: [
        {
          refundId: "refund",
          number: 3,
  ...
  it.each(["PENDING", "APPROVED"])("protects %s financial authorizations", async (status) => {
    db.appSetting.findMany.mockResolvedValueOnce([
      { value: JSON.stringify({ type: "REFUND", resourceId: "installment:open", status }) },
    ]);
  ...
  it("protects payout details and proof already attached", async () => {
    db.appSetting.findMany
      ...
      .mockResolvedValueOnce([{ value: JSON.stringify({ proofFileId: "proof" }) }]);
    expect((await saveRefundPaymentPlan("refund", {}, data())).success).toBe(false);
    expect(db.refundInstallment.createMany).not.toHaveBeenCalled();
  });
