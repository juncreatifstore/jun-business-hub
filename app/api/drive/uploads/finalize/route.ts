...
export async function POST(req: NextRequest) {
  ...
  if (!row)
    return NextResponse.json({ error: "Upload session expired or already finalized" }, { status: 404 });
  ...
  if (meta.refundInstallmentId) {
    const installment = await prisma.refundInstallment.findUnique({
      where: { id: meta.refundInstallmentId },
      select: { id: true, refundId: true },
    });
    if (!installment || !meta.refundId || installment.refundId !== meta.refundId)
      return NextResponse.json({ error: "Linked installment not found" }, { status: 404 });
  }
  ...
  if (meta.refundInstallmentId)
    await saveRefundInstallmentMeta(meta.refundInstallmentId, { proofFileId: record.id });

  const action = meta.refundInstallmentId
    ? "REFUND_INSTALLMENT_PROOF_UPLOAD"
    : meta.refundId
      ? "REFUND_PROOF_UPLOAD"
      : meta.paymentId
        ? "PAYMENT_PROOF_UPLOAD"
        : "FILE_UPLOAD";
  const message = meta.refundInstallmentId
    ? `Uploaded installment proof ${record.name}`
    : meta.refundId
      ? `Uploaded refund document ${record.name}`
      : meta.paymentId
        ? `Uploaded payment proof ${record.name}`
  ...
  await audit({
    ...
    after: {
      ...
      refundInstallmentId: meta.refundInstallmentId || null,
      directUpload: true,
      mode: meta.mode,
    },
  });
