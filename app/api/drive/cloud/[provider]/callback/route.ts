...
export async function GET(req: NextRequest, { params }: { params: { provider: string } }) {
  ...
  if (error)
    return NextResponse.redirect(new URL(`/app/drive/cloud?error=${encodeURIComponent(error)}`, req.url));
  const code = req.nextUrl.searchParams.get("code");
  const state = req.nextUrl.searchParams.get("state");
  if (!code || !state)
    return NextResponse.redirect(new URL("/app/drive/cloud?error=oauth_missing_code", req.url));

  let verified: { userId: string; provider: CloudProvider };
  try {
    verified = await verifyCloudOAuthState(state);
  } catch {
    return NextResponse.redirect(new URL("/app/drive/cloud?error=oauth_invalid_state", req.url));
  }
  if (verified.userId !== user.id || verified.provider !== provider)
    return NextResponse.redirect(new URL("/app/drive/cloud?error=oauth_identity_mismatch", req.url));
  ...
  if (!tokenRes.ok)
    return NextResponse.redirect(
      new URL(`/app/drive/cloud?error=${provider}_token_exchange_failed`, req.url),
    );
  ...
  await prisma.auditLog
    .create({
      data: {
        userId: user.id,
        action: "DRIVE_CLOUD_CONNECTED",
        resourceType: "CloudConnection",
        resourceId: provider,
        after: { provider, accountEmail },
      },
    })
    .catch(() => undefined);
