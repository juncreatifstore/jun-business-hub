...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  const now = new Date();
  const throughMonth =
    now.getUTCFullYear() === plan.year ? now.getUTCMonth() : now.getUTCFullYear() > plan.year ? 11 : -1;
  ...
  for (const line of plan.lines) {
    ...
    line.monthly.forEach((amount, month) =>
      rows.push([
        plan.name,
        String(plan.year),
        plan.currency,
        ...
        String(v?.budget ?? 0),
        String(v?.actual ?? 0),
        String(v?.favorableVariance ?? 0),
        v?.utilizationPercent === null ? "" : String(v?.utilizationPercent ?? 0),
        v?.status ?? "ON_TRACK",
      ]),
    );
  }
  ...
  return new NextResponse(body, {
    headers: {
      ...
      "content-disposition": `attachment; filename="jun-budget-${plan.year}-${plan.currency}.csv"`,
