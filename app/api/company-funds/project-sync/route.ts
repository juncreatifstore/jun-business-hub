  ...
  try {
    ...
    if (!projectCode || !apiKey)
      return NextResponse.json(
        { ok: false, error: "Missing project authentication headers" },
        { status: 401 },
      );
    const body = (await request.json()) as Record<string, unknown>;
    const direction = String(body.direction || "").toUpperCase() as TreasuryDirection;
    if (!(["IN", "OUT"] as string[]).includes(direction))
      return NextResponse.json({ ok: false, error: "direction must be IN or OUT" }, { status: 400 });
    ...
    return NextResponse.json(
      { ok: true, duplicate: result.duplicate, transactionId: result.transaction.id },
      { status: result.duplicate ? 200 : 201 },
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : "Project sync failed";
    const status =
      message.includes("Invalid project API key") || message.includes("Unknown project integration")
        ? 401
