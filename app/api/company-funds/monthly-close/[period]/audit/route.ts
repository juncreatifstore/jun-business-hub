...
export async function GET(_request: Request, { params }: { params: { period: string } }) {
  ...
  const rows = await prisma.auditLog.findMany({
    where: {
      ...
      OR: [
        { resourceType: { contains: "Payment", mode: "insensitive" } },
        { resourceType: { contains: "Refund", mode: "insensitive" } },
        { resourceType: { contains: "Expense", mode: "insensitive" } },
        { resourceType: { contains: "Treasury", mode: "insensitive" } },
        { resourceType: { contains: "Financial", mode: "insensitive" } },
        { action: { contains: "COMPANY_FUNDS", mode: "insensitive" } },
  ...
  return new NextResponse(lines.join("\n"), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="JUN-audit-${close.period}.csv"`,
