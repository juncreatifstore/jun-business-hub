...
export async function GET(_req: Request, { params }: { params: { id: string; versionId: string } }) {
  ...
  const setting = await prisma.appSetting.findUnique({
    where: { key: `drive.version.${file.id}.${params.versionId}` },
  });
  ...
  try {
    ...
    return NextResponse.json({ error: "Invalid version metadata" }, { status: 500 });
  }

  await audit({
    userId: user.id,
    action: "FILE_VERSION_ACCESS",
    resourceType: "File",
    resourceId: file.id,
    after: { versionId: params.versionId },
  });
