...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const file = await prisma.file.findFirst({
    ...
    select: { id: true, name: true, category: true, mimeType: true },
  });
  ...
  const context = [
    ...
    intel?.contentExcerpt
      ? `Extracted content:\n${intel.contentExcerpt.slice(0, 12000)}`
  ...
  try {
    ...
    const result = await generateText({
      ...
      system:
        "Answer questions about one internal JUN Drive file. Use only the supplied file context. If the answer is not supported by the context, say that the information is not available in the indexed content. Never invent identity, financial, legal, visa, or travel facts.",
      prompt: `${context}\n\nQuestion: ${question}`,
      temperature: 0.1,
    });
    await audit({
      userId: user.id,
      action: "FILE_AI_QUESTION",
      resourceType: "File",
      resourceId: file.id,
