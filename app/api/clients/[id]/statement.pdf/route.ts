...
export async function GET(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  if (user.role === "CLIENT") {
    const a = await prisma.clientAccount.findUnique({
      where: { userId: user.id },
      select: { clientId: true },
    });
    allowed = a?.clientId === params.id;
  }
  ...
  const q = String(req.nextUrl.searchParams.get("lang") || "").toUpperCase();
  const language: ClientStatementLanguage = ["FR", "EN", "ES", "HT"].includes(q)
    ? (q as ClientStatementLanguage)
  ...
  return new NextResponse(Buffer.from(bytes), {
    headers: {
      ...
      "Content-Disposition": `inline; filename="${client.internalId}-statement.pdf"`,
