...
export async function POST(req: NextRequest) {
  ...
  if (!verifyMercadoPagoWebhook(req.headers.get("x-signature"), req.headers.get("x-request-id"), dataId))
  ...
  if (eventId && (await hasWebhookEvent("MERCADO_PAGO", eventId)))
    ...
    if (session) {
      ...
      if (status === "approved")
        ...
        await markOnlineSessionStatus(session.id, "FAILED", {
          providerPaymentId: String(payment?.id || dataId),
          error: String(payment?.status_detail || "Mercado Pago payment rejected"),
        });
      else if (["cancelled", "refunded", "charged_back"].includes(status) && session.status !== "PAID")
        await markOnlineSessionStatus(session.id, "CANCELLED", {
          providerPaymentId: String(payment?.id || dataId),
          error: status,
        });
    }
