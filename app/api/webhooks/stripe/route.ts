...
export async function POST(req: NextRequest) {
  ...
  if (eventId && (await hasWebhookEvent("STRIPE", eventId)))
  ...
  if (sessionId) {
    if (event.type === "checkout.session.completed" && object.payment_status === "paid") {
      await markOnlineSessionStatus(sessionId, "PAID", {
        providerPaymentId: String(object.payment_intent || object.id || ""),
      });
    } else if (event.type === "checkout.session.expired") {
      await markOnlineSessionStatus(sessionId, "EXPIRED");
    } else if (event.type === "payment_intent.payment_failed") {
      await markOnlineSessionStatus(sessionId, "FAILED", {
        providerPaymentId: String(object.id || ""),
        error: String(object?.last_payment_error?.message || "Stripe payment failed"),
      });
    }
  }
