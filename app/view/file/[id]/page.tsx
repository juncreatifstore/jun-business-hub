...
export default async function PublicFileViewer({ params, searchParams }: { params: { id: string }; searchParams: { key?: string } }) {
  const suppliedToken = typeof searchParams.key === "string" ? searchParams.key : undefined;
  if (!(await canAccessPublicDriveFile(params.id, suppliedToken))) notFound();
  ...
  const rawUrl = `/public/files/${file.id}${suppliedToken ? `?key=${encodeURIComponent(suppliedToken)}` : ""}`;
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          <div className="flex min-w-0 items-center gap-4">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-500/15 text-blue-300"><FileText className="h-6 w-6" /></div>
            <div className="min-w-0">
              <div className="mb-1 flex items-center gap-2 text-xs font-medium uppercase tracking-[0.16em] text-emerald-300"><ShieldCheck className="h-4 w-4" /> JUN shared document</div>
              <h1 className="truncate text-xl font-semibold">{file.name}</h1>
              <p className="mt-1 text-sm text-white/55">{file.category.replace(/_/g, " ")} · {humanSize(file.sizeBytes)} · Shared from JUN Business Hub</p>
            </div>
          </div>
          <a href={rawUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center gap-2 rounded-lg bg-blue-500 px-4 text-sm font-semibold text-white hover:bg-blue-400"><Download className="h-4 w-4" /> Open file</a>
        </div>

        <div className="overflow-hidden rounded-2xl border border-white/10 bg-white shadow-2xl">
          {previewable ? <iframe src={rawUrl} title={file.name} className="h-[78vh] min-h-[620px] w-full bg-white" /> : (
