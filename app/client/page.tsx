...
export default async function ClientPortalPage() {
  ...
  const [client, cases, documents, payments, refunds, files] = await Promise.all([
    ...
    prisma.document.findMany({
      where: { clientId, status: { in: ["FINAL", "SIGNED"] } },
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="pt-6">
            <p className="text-xs uppercase tracking-wide text-white/50">Active cases</p>
            <p className="mt-1 font-display text-2xl">
              {cases.filter((c) => !["COMPLETED", "CANCELLED", "ARCHIVED"].includes(c.status)).length}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-xs uppercase tracking-wide text-white/50">Confirmed payments</p>
            <p className="mt-1 font-display text-2xl">{formatMoney(totalPaid)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-xs uppercase tracking-wide text-white/50">Documents available</p>
            <p className="mt-1 font-display text-2xl">{documents.length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="divide-y divide-white/5">
          {cases.length === 0 ? (
            <p className="py-2 text-sm text-white/50">No cases yet.</p>
          ) : (
            cases.map((c) => (
              <div key={c.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
                <div>
                  <p className="font-medium">{c.title}</p>
                  <p className="registry-id text-xs text-white/50">
                    {c.caseNumber} · opened {formatDate(c.createdAt)}
                  </p>
                </div>
      ...
      <Card>
        ...
        <CardContent className="divide-y divide-white/5">
          {documents.length === 0 ? (
            <p className="py-2 text-sm text-white/50">Finalized documents will appear here.</p>
          ) : (
            documents.map((d) => (
              <div key={d.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
                <div>
                  ...
                  <p className="registry-id text-xs text-white/50">
                    ...
                    ·{" "}
                    <a href={`/verify/${d.documentId}`} className="hover:text-gold">
                      verify
                    </a>
                  </p>
                </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="divide-y divide-white/5">
            {payments.length === 0 ? (
              <p className="py-2 text-sm text-white/50">No payments recorded.</p>
            ) : (
              payments.map((p) => (
                <div key={p.id} className="flex items-center justify-between gap-2 py-3">
                  <div>
                    ...
                    <p className="registry-id text-xs text-white/50">
                      {p.reference} · {formatDate(p.paidAt)} · {p.method}
                    </p>
                  </div>
          ...
          <CardContent className="divide-y divide-white/5">
            {confirmedPayments.length === 0 ? (
              <p className="py-2 text-sm text-white/50">
                Receipts are available when a payment is confirmed.
              </p>
            ) : (
                ...
                return (
                  <div key={p.id} className="flex items-center justify-between gap-2 py-3">
                    <div>
                      <p className="font-medium">{formatMoney(Number(p.amount), p.currency)}</p>
                      <p className="registry-id text-xs text-white/50">
                        {receiptReference} · issued {formatDate(p.paidAt)}
                      </p>
                    </div>
                  ...
                  <div>
                    ...
                    <p className="registry-id text-xs text-white/50">
                      ...
                      {r.installments.length > 0
                        ? ` · ${paid}/${r.installments.length} installments paid`
                        : ""}
                    </p>
                  </div>
