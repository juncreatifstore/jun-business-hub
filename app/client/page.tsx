...
export default async function ClientPortalPage() {
  ...
  const [client, cases, documents, payments, refunds, files] = await Promise.all([
  ...
  const confirmedPayments = payments.filter((p) => p.status === "CONFIRMED" && p.paidAt);
    ...
    <div className="space-y-8">
        ...
        <CardContent className="divide-y divide-white/5">
          {cases.length === 0 ? <p className="py-2 text-sm text-white/50">No cases yet.</p> : cases.map((c) => (
            <div key={c.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
              <div><p className="font-medium">{c.title}</p><p className="registry-id text-xs text-white/50">{c.caseNumber} · opened {formatDate(c.createdAt)}</p></div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
          ...
          <CardContent className="divide-y divide-white/5">
            {payments.length === 0 ? <p className="py-2 text-sm text-white/50">No payments recorded.</p> : payments.map((p) => (
              <div key={p.id} className="flex items-center justify-between gap-2 py-3">
                <div><p className="font-medium">{formatMoney(Number(p.amount), p.currency)}</p><p className="registry-id text-xs text-white/50">{p.reference} · {formatDate(p.paidAt)} · {p.method}</p></div>
        ...
        <Card>
          ...
          <CardContent className="divide-y divide-white/5">
            {confirmedPayments.length === 0 ? <p className="py-2 text-sm text-white/50">Receipts are available when a payment is confirmed.</p> : confirmedPayments.map((p) => {
              const receiptReference = `RCT-${p.reference}`;
              return (
                <div key={p.id} className="flex items-center justify-between gap-2 py-3">
                  <div><p className="font-medium">{formatMoney(Number(p.amount), p.currency)}</p><p className="registry-id text-xs text-white/50">{receiptReference} · issued {formatDate(p.paidAt)}</p></div>
                  <span className="flex gap-3 text-xs"><a href={`/api/receipts/${p.id}/pdf`} target="_blank" rel="noreferrer" className="text-gold hover:underline">PDF</a></span>
                </div>
              );
            })}
          </CardContent>
            ...
            {refunds.map((r) => {
              ...
              return (
                <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 py-3">
                  <div><p className="font-medium">{formatMoney(Number(r.amount), r.currency)}</p><p className="registry-id text-xs text-white/50">{r.refundNumber}{r.installments.length > 0 ? ` · ${paid}/${r.installments.length} installments paid` : ""}</p></div>
          ...
          <CardContent className="divide-y divide-white/5">
            {files.map((f) => (
              <div key={f.id} className="flex items-center justify-between gap-2 py-3">
                <div><a href={`/api/files/${f.id}`} target="_blank" rel="noreferrer" className="font-medium hover:text-gold">{f.name}</a><p className="text-xs text-white/50">{f.category.replace(/_/g, " ")} · {formatDate(f.createdAt)}</p></div>
              </div>
            ))}
          </CardContent>
        </Card>
