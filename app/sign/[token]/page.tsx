...
function errorMessage(code?: string) {
  ...
  if (code === "otp_locked") return "Too many incorrect attempts. Request a new verification code.";
  return null;
}
...
export default async function NativeSignPage({ params, searchParams }: { params: { token: string }; searchParams?: { error?: string; done?: string; declined?: string; otp?: string; verified?: string } }) {
  ...
  // Once the signer has verified this request, an old unverified link can be
  // upgraded server-side without asking for another OTP during the same request lifecycle.
  if (!done && !declined && !expired && signer.verifiedAt && !payload.verified) {
    const verifiedToken = await createVerifiedNativeSigningToken({ requestId: payload.requestId, email: payload.email, order: payload.order }, expiryFromRequest(meta.expiresAt, request.sentAt));
            ...
            {otpWasSent ? (
              <>
                <form action={verifyNativeVerificationCode.bind(null, token)} className="mt-5">
                  <label htmlFor="otp" className="block text-sm font-medium">6-digit verification code</label>
                  <input id="otp" name="otp" inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required autoComplete="one-time-code" placeholder="000000" className="mt-2 h-14 w-full rounded-lg border border-line px-3 text-center text-2xl tracking-[0.35em] outline-none focus:border-electric focus:ring-2 focus:ring-electric/20" />
                  <button type="submit" className="mt-3 h-11 w-full rounded-lg bg-night px-4 text-sm font-medium text-white hover:bg-night-soft"><MailCheck className="mr-2 inline h-4 w-4" />Verify & continue</button>
                </form>
                <form action={sendNativeVerificationCode.bind(null, token)} className="mt-3"><button type="submit" className="h-10 w-full rounded-lg border border-line text-sm font-medium hover:bg-surface">Send a new code</button></form>
                <p className="mt-3 text-center text-xs text-muted2">The code expires after 10 minutes. Maximum 5 verification attempts per code.</p>
              </>
            ) : (
              <form action={sendNativeVerificationCode.bind(null, token)} className="mt-5"><button type="submit" className="h-11 w-full rounded-lg bg-night px-4 text-sm font-medium text-white hover:bg-night-soft">Send verification code</button><p className="mt-3 text-center text-xs text-muted2">JUN will send a one-time code to {maskedEmail(signer.email)}.</p></form>
            )}
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            <section className="rounded-2xl border border-line bg-white p-3 shadow-sm">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-3 px-2 py-1"><div><p className="font-medium">{request.document.title}</p><p className="text-xs text-muted2">{request.document.documentId}</p></div><div className="flex items-center gap-3"><a href={pdfUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-line px-3 py-1.5 text-xs font-medium hover:bg-surface"><ExternalLink className="h-3.5 w-3.5" /> Open document</a><FileSignature className="h-5 w-5 text-electric" /></div></div>
              <iframe title="Document to sign" src={pdfUrl} className="h-[760px] w-full rounded-xl border border-line bg-surface" />
              <p className="px-2 pt-2 text-xs text-muted2">Email verified {signer.verifiedAt ? new Date(signer.verifiedAt).toLocaleString() : ""}. Review the document before signing.</p>
            </section>
            <aside className="space-y-4">
              <div className="rounded-2xl border border-line bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-muted2">Verified signer</p><h1 className="mt-2 text-xl font-semibold">{signer.name}</h1><p className="text-sm text-muted2">{signer.email}</p><p className="mt-2 text-xs text-emerald-600"><MailCheck className="mr-1 inline h-3.5 w-3.5" /> Email identity verified</p><p className="mt-1 text-xs text-muted2">Signing order {signer.order} of {recipients.length}</p></div>
              {error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div> : null}
              {!isTurn ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="font-medium">Waiting for the previous signer</p><p className="mt-2 text-sm text-muted2">Your identity is verified, but the document must be signed in routing order.</p></div> : <><form action={completeVerifiedJunNativeSignature.bind(null, token)} className="rounded-2xl border border-line bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-muted2">Electronic signature</p><label className="mt-4 block text-sm font-medium" htmlFor="signatureName">Type your full name</label><input id="signatureName" name="signatureName" required defaultValue={signer.name} maxLength={160} className="mt-2 h-12 w-full rounded-lg border border-line px-3 text-xl italic outline-none focus:border-electric focus:ring-2 focus:ring-electric/20" /><p className="mt-2 text-xs text-muted2">Your typed name will be placed in the Signature field. Name, date and initials fields are completed automatically.</p><label className="mt-5 flex items-start gap-3 rounded-lg border border-line bg-surface p-3 text-sm"><input type="checkbox" name="consent" className="mt-1" required /><span>I have reviewed this document and agree to use my typed name as my electronic signature. I understand that submitting this form records my consent, signing time, verified email identity and document integrity information.</span></label><button type="submit" className="mt-5 h-11 w-full rounded-lg bg-night px-4 text-sm font-medium text-white hover:bg-night-soft">Accept & sign document</button></form><form action={declineVerifiedJunNativeSignature.bind(null, token)} className="rounded-2xl border border-red-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-red-600">Cannot sign?</p><label className="mt-3 block text-sm font-medium" htmlFor="reason">Reason for declining</label><textarea id="reason" name="reason" required maxLength={500} rows={3} placeholder="Explain briefly what needs to be corrected." className="mt-2 w-full rounded-lg border border-line px-3 py-2 text-sm outline-none focus:border-red-400" /><button type="submit" className="mt-3 h-10 w-full rounded-lg border border-red-300 text-sm font-medium text-red-700 hover:bg-red-50">Decline to sign</button></form></>}
            </aside>
          </div>
        )}
      </div>
