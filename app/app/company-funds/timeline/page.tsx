...
function statusTone(status: string | undefined) {
  ...
  if (["APPROVED", "COMPLETED", "MATCHED", "RESOLVED"].includes(status))
...
export default async function CompanyFundsTimelinePage({ searchParams }: { searchParams?: Params }) {
  ...
  for (const item of evidence) {
    ...
    events.push({
      ...
      detail: item.transactionReference
        ? `Référence transaction : ${item.transactionReference}`
        : "Preuve d’exécution enregistrée",
  ...
  for (const transfer of transfers) {
    ...
    if (initiated)
      events.push({
        ...
        currency: transfer.fromCurrency,
        country: from?.country || transferCountry,
        status: "IN_TRANSIT",
        href: "/app/company-funds/transfers",
      });
    ...
    if (completed)
      events.push({
        ...
        country: to?.country || transferCountry,
        status: "COMPLETED",
        href: "/app/company-funds/transfers",
      });
  }
  ...
  const q = String(searchParams?.q || "")
    .trim()
    .toLowerCase();
  const kind = String(searchParams?.kind || "")
    .trim()
    .toUpperCase();
  ...
  const kinds = new Set<TimelineEvent["kind"]>([
    "AUTHORIZATION",
    "APPROVAL",
    "EVIDENCE",
    "TRANSFER",
  ...
  const filtered = events
    ...
    .filter(
      (event) =>
        !q ||
        `${event.title} ${event.detail} ${event.reference} ${event.country || ""} ${event.currency || ""} ${event.status || ""}`
          .toLowerCase()
          .includes(q),
    )
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-muted2">Super Admin · Traçabilité</p>
          <h1 className="mt-1 text-3xl font-semibold">Timeline des opérations</h1>
          <p className="mt-1 max-w-3xl text-sm text-muted2">
            Chronologie consolidée des autorisations, approbations, preuves, transferts, flux projets et
            rapprochements. Les filtres globaux Pays / Devise / Période sont appliqués réellement à cette
            chronologie.
          </p>
        </div>
      ...
      <form className="grid gap-3 rounded-2xl border border-line bg-white p-4 shadow-sm md:grid-cols-[1fr_220px_auto]">
        ...
        {periodDays ? <input type="hidden" name="period" value={String(periodDays)} /> : null}
        <label className="text-xs font-medium text-muted2">
          Recherche
          <input
            name="q"
            defaultValue={searchParams?.q || ""}
            placeholder="Référence, projet, compte, statut…"
            className="mt-1 h-10 w-full rounded-lg border border-line px-3 text-sm text-ink outline-none focus:border-electric"
          />
        </label>
        <label className="text-xs font-medium text-muted2">
          Type
          <select
            ...
            className="mt-1 h-10 w-full rounded-lg border border-line px-3 text-sm text-ink"
          >
            <option value="">Tous les événements</option>
            <option value="AUTHORIZATION">Autorisations</option>
            <option value="APPROVAL">Décisions</option>
            <option value="EVIDENCE">Preuves</option>
            <option value="TRANSFER">Transferts</option>
            <option value="PROJECT">Flux projets</option>
            <option value="RECONCILIATION">Rapprochements</option>
          </select>
        </label>
        <div className="flex items-end gap-2">
          <button className="h-10 rounded-lg bg-ink px-4 text-sm font-semibold text-white">Filtrer</button>
          {q || selectedKind ? (
            <Link
              href={resetHref}
              className="inline-flex h-10 items-center rounded-lg border border-line px-4 text-sm font-medium text-muted2 hover:bg-surface"
            >
      ...
      {country || currency || periodDays ? (
        <div className="flex flex-wrap gap-2 text-[11px] text-muted2">
          <span className="font-semibold text-ink">Filtres actifs :</span>
          {country ? <span className="rounded-full bg-surface px-2 py-1">Pays · {country}</span> : null}
          {currency ? <span className="rounded-full bg-surface px-2 py-1">Devise · {currency}</span> : null}
          {periodDays ? (
            <span className="rounded-full bg-surface px-2 py-1">Période · {periodDays} jours</span>
          ) : null}
        </div>
      ) : null}
      ...
      <div className="rounded-2xl border border-line bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-line px-5 py-4">
          <div>
            <h2 className="font-semibold text-ink">Événements financiers</h2>
            <p className="mt-0.5 text-xs text-muted2">
              {filtered.length} événement(s) affiché(s), maximum 500.
            </p>
          </div>
          <Landmark className="h-5 w-5 text-muted2" />
        </div>
        {filtered.length ? (
          <div className="divide-y divide-line">
            {filtered.map((event) => {
              ...
              return (
                <div key={event.id} className="grid gap-3 px-4 py-4 sm:grid-cols-[44px_1fr_auto] sm:px-5">
                  <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-surface text-muted2">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {event.status ? (
                        <span
                          className={`rounded-full px-2 py-1 text-[10px] font-semibold ${statusTone(event.status)}`}
                        >
                          {event.status}
                        </span>
                      ) : null}
                    </div>
                    <p className="mt-1 text-sm text-muted2">{event.detail}</p>
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted2">
                      <span>{new Date(event.at).toLocaleString("fr-FR")}</span>
                  ...
                  <div className="flex items-center sm:justify-end">
                    <Link
                      href={event.href}
                      className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-line px-3 text-xs font-semibold text-muted2 transition hover:bg-surface hover:text-ink"
                    >
          ...
          <div className="px-5 py-12 text-center text-sm text-muted2">
