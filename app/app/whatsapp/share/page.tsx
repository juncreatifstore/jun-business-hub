...
export default async function WhatsAppSharePage({
  ...
  if (type === "receipt") {
    ...
    if (p) {
      const n = p.client.whatsapp || p.client.phone || "";
      info = {
  ...
  if (type === "statement") {
    ...
    if (c) {
      ...
      info = {
        ...
        reference: `STATEMENT-${c.internalId}`,
  ...
  if (!info)
    return (
      <div>
        <PageHeader title="WhatsApp" subtitle="Document not found or not linked to a client." />
        <Link href="/app/whatsapp">
          <Button variant="outline">Back to WhatsApp</Button>
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl">
      <PageHeader
        ...
        subtitle="Send this JUN-generated document to the linked client."
      />
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-2 text-sm">
            <div>
              <p className="text-xs text-muted2">Reference</p>
              <p className="font-medium">{info.reference}</p>
            </div>
            <div>
              <p className="text-xs text-muted2">Client</p>
              <p className="font-medium">{info.clientName}</p>
            </div>
            <div>
              <p className="text-xs text-muted2">WhatsApp</p>
              <p className="font-medium">{info.number || "No WhatsApp number"}</p>
            </div>
          </div>
          {!info.number ? (
            <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
              Add a WhatsApp number to the client record before sending.
            </p>
          ) : null}
          <div className="flex flex-wrap gap-2">
            {info.canDirectPdf && info.documentId ? (
              <form action={sendDocumentByWhatsApp.bind(null, info.documentId)}>
                <input type="hidden" name="to" value={info.number} />
                <input type="hidden" name="caption" value={`JUN CREATIF AND TRAVEL LLC — ${info.title}`} />
            ...
            <Link href={composer}>
              <Button variant={info.canDirectPdf ? "outline" : "primary"} disabled={!info.number}>
                Open WhatsApp composer
              </Button>
            </Link>
          </div>
          <p className="text-xs text-muted2">
            Official finalized documents can be sent as PDF directly. Other generated documents open the JUN
            WhatsApp composer with the client and document message already prepared.
          </p>
        </CardContent>
      </Card>
    </div>
