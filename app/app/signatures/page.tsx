...
const FILTERS = [
  ...
  { key: "VIEWED", label: "Consultées", icon: Eye },
  { key: "VERIFIED", label: "Vérifiées", icon: ShieldCheck },
  { key: "PARTIALLY_SIGNED", label: "Partielles", icon: Clock3 },
  { key: "SIGNED", label: "Signées", icon: CheckCircle2 },
  { key: "DECLINED", label: "Refusées", icon: XCircle },
...
function expiryFor(request: { recipients: unknown; sentAt: Date | null }) {
  ...
  if (meta.expiresAt) { const parsed = new Date(meta.expiresAt); if (!Number.isNaN(parsed.getTime())) return parsed; }
  return request.sentAt ? new Date(request.sentAt.getTime() + 14 * 24 * 60 * 60 * 1000) : null;
}
function verifiedRequest(recipients: ReturnType<typeof signatureRecipients>) { return recipients.length > 0 && recipients.some((r) => Boolean(r.verifiedAt)); }
function attentionReason(request: { status: string; recipients: unknown; sentAt: Date | null }, now: Date) {
  if (request.status === "DECLINED") return "Signature refusée — vérification requise";
  if (request.status === "EXPIRED") return "Demande expirée — créez une nouvelle demande";
  ...
  if (expiresAt) { const remaining = expiresAt.getTime() - now.getTime(); if (remaining > 0 && remaining <= 3 * 24 * 60 * 60 * 1000) return "Expire dans moins de 3 jours"; }
  if (current && !current.invitationSentAt) return "Le signataire actuel n’a pas reçu son invitation";
  if (current?.invitationSentAt && !current.viewedAt) { const age = now.getTime() - new Date(current.invitationSentAt).getTime(); if (age >= 48 * 60 * 60 * 1000) return "Invitation non consultée depuis 48 h"; }
  return null;
}
...
export default async function SignaturesPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const enriched = requests.map((request) => {
    ...
    const verified = verifiedRequest(recipients); const expiresAt = expiryFor(request); const attention = attentionReason(request, now);
    const expiringSoon = Boolean(expiresAt && ["SENT", "VIEWED", "PARTIALLY_SIGNED"].includes(request.status) && expiresAt.getTime() > now.getTime() && expiresAt.getTime() - now.getTime() <= 3 * 24 * 60 * 60 * 1000);
    return { request, recipients, signed, verified, expiresAt, attention, expiringSoon };
  });
  const statusCounts = Object.fromEntries(FILTERS.map((filter) => [filter.key, enriched.filter((item) => filter.key === "VERIFIED" ? item.verified : item.request.status === filter.key).length]));
  const activatedStatuses = new Set(["SENT", "VIEWED", "PARTIALLY_SIGNED", "SIGNED", "DECLINED", "EXPIRED"]); const activated = enriched.filter((item) => activatedStatuses.has(item.request.status)).length;
  const completed = enriched.filter((item) => item.request.status === "SIGNED").length; const signatureRate = activated > 0 ? Math.round((completed / activated) * 100) : 0;
  const attentionCount = enriched.filter((item) => Boolean(item.attention)).length; const expiringCount = enriched.filter((item) => item.expiringSoon).length;
  ...
  return <div>
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <Card className="border-emerald-500/15 bg-emerald-500/[0.035]"><CardContent className="p-4"><p className="text-xs uppercase tracking-wider text-muted2">Taux de signature</p><div className="mt-2 flex items-end justify-between gap-3"><p className="text-3xl font-semibold">{signatureRate}%</p><CheckCircle2 className="h-5 w-5 text-emerald-400" /></div><p className="mt-1 text-xs text-muted2">{completed} signée(s) / {activated} activée(s)</p></CardContent></Card>
      <Link href={queryHref(params,{attention:attentionOnly?undefined:"1"})}><Card className={`h-full transition hover:border-amber-400/40 ${attentionOnly?"border-amber-400/50 bg-amber-500/[0.05]":""}`}><CardContent className="p-4"><p className="text-xs uppercase tracking-wider text-muted2">À traiter</p><div className="mt-2 flex items-end justify-between"><p className="text-3xl font-semibold">{attentionCount}</p><AlertTriangle className="h-5 w-5 text-amber-400" /></div><p className="mt-1 text-xs text-muted2">Exceptions et demandes bloquées</p></CardContent></Card></Link>
      <Link href={queryHref(params,{expiring:expiringOnly?undefined:"1"})}><Card className={`h-full transition hover:border-amber-400/40 ${expiringOnly?"border-amber-400/50 bg-amber-500/[0.05]":""}`}><CardContent className="p-4"><p className="text-xs uppercase tracking-wider text-muted2">Expiration proche</p><div className="mt-2 flex items-end justify-between"><p className="text-3xl font-semibold">{expiringCount}</p><Clock3 className="h-5 w-5 text-amber-400" /></div><p className="mt-1 text-xs text-muted2">Échéance dans moins de 3 jours</p></CardContent></Card></Link>
      <Card><CardContent className="p-4"><p className="text-xs uppercase tracking-wider text-muted2">Demandes totales</p><div className="mt-2 flex items-end justify-between"><p className="text-3xl font-semibold">{enriched.length}</p><FileSignature className="h-5 w-5 text-blue-400" /></div><p className="mt-1 text-xs text-muted2">500 demandes les plus récentes</p></CardContent></Card>
    </div>
