...
function attentionReason(request: { status: string; recipients: unknown; sentAt: Date | null }, now: Date) {
  ...
  if (expiresAt) {
    ...
    if (remaining > 0 && remaining <= 3 * 24 * 60 * 60 * 1000) return "Expire dans moins de 3 jours";
  }
  ...
  if (current?.invitationSentAt && !current.viewedAt) {
    ...
    if (age >= 48 * 60 * 60 * 1000) return "Invitation non consultée depuis 48 h";
  }
  return null;
}
...
export default async function SignaturesPage(props: { searchParams?: Promise<SearchParams> }) {
  ...
  const status = FILTERS.some((f) => f.key === params.status)
    ? (params.status as DashboardFilter)
    : undefined;
  ...
  const statusCounts = Object.fromEntries(
    FILTERS.map((filter) => [
      ...
      enriched.filter((item) =>
        filter.key === "VERIFIED" ? item.verified : item.request.status === filter.key,
      ).length,
    ]),
  );
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Suivez la préparation, la vérification d’identité, l’envoi, la signature et les exceptions JUN Secure Sign."
        actions={
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="border-emerald-500/15 bg-emerald-500/[0.035]">
        ...
        <Link href={queryHref(params, { attention: attentionOnly ? undefined : "1" })}>
          <Card
            className={`h-full transition hover:border-amber-400/40 ${attentionOnly ? "border-amber-400/50 bg-amber-500/[0.05]" : ""}`}
          >
            <CardContent className="p-4">
        ...
        <Link href={queryHref(params, { expiring: expiringOnly ? undefined : "1" })}>
          <Card
            className={`h-full transition hover:border-amber-400/40 ${expiringOnly ? "border-amber-400/50 bg-amber-500/[0.05]" : ""}`}
          >
            <CardContent className="p-4">
      ...
      <div className="mb-5 flex flex-wrap gap-2">
        <Link href={queryHref(params, { status: undefined })}>
          <Button size="sm" variant={!status ? "primary" : "secondary"}>
            Toutes <span className="ml-1 opacity-70">{enriched.length}</span>
          </Button>
        </Link>
        {FILTERS.map((filter) => (
          <Link
            key={filter.key}
            href={queryHref(params, { status: status === filter.key ? undefined : filter.key })}
          >
            <Button size="sm" variant={status === filter.key ? "primary" : "secondary"}>
              <filter.icon className="mr-1.5 h-3.5 w-3.5" />
              {filter.label}
              <span className="ml-1 opacity-70">{statusCounts[filter.key] ?? 0}</span>
            </Button>
          </Link>
        ))}
      </div>
      ...
      >
        ...
        {query || status || attentionOnly || expiringOnly ? (
      ...
      {pageItems.length === 0 ? (
        requests.length === 0 ? (
          <EmptyState
            ...
            description="Créez la première demande depuis un document finalisé."
            actionHref="/app/signatures/new"
        ...
        <>
          <div className="mb-3">
            <ListCount shown={pageItems.length} total={total} label="demande" />
          </div>
          <div className="hidden md:block">
            <Table>
              ...
              <tbody>
                {pageItems.map(({ request, recipients, signed, verified, expiresAt, attention }) => {
                  ...
                  return (
                    <TR key={request.id}>
                      <TD>
                        ...
                        <div className="max-w-[320px] truncate text-xs text-muted2">
                          {request.document.title}
                        </div>
                        {client ? (
                          <div className="mt-1 text-xs">
                            {client.firstName} {client.lastName}
                            {client.email ? <span className="text-muted2"> · {client.email}</span> : null}
                          </div>
                        ) : (
                          <div className="mt-1 text-xs text-muted2">Aucun client lié</div>
                        )}
                      </TD>
                      <TD>
                        <div className="text-sm font-medium">
                          {signed}/{recipients.length} signé(s)
                        </div>
                        <div className="mt-0.5 text-xs text-muted2">
                          ...
                          {current
                            ? `Actuel : ${current.name} · ${current.email}`
                            : signed === recipients.length && recipients.length > 0
                              ? "Tous les signataires ont terminé"
                              : "—"}
                        </div>
                      </TD>
                      <TD>
                      ...
                      <TD className="text-muted2">
                        {expiresAt ? (
                          <>
                            ...
                            <div className="text-xs">{formatDateTime(expiresAt)}</div>
                          </>
                        ) : (
                          "—"
                        )}
            ...
            {pageItems.map(({ request, recipients, signed, verified, expiresAt, attention }) => {
              ...
              return (
                <RecordCard
                  ...
                  subtitle={<span className="registry-id">{request.document.documentId}</span>}
                  badges={
                    <>
                      <StatusBadge status={request.status} />
                      {verified ? (
                  ...
                  footer={`Créée ${formatDate(request.createdAt)} · ${request.createdBy.firstName} ${request.createdBy.lastName}`}
                >
                  <RecordField
                    label="Client"
                    value={client ? `${client.firstName} ${client.lastName}` : "—"}
                  />
                  <RecordField label="Progression" value={`${signed}/${recipients.length} signé(s)`} />
                  <RecordField
                    label="Signataire actuel"
                    value={
                      current
                        ? current.name
                        : signed === recipients.length && recipients.length > 0
