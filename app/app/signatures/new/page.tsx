  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          docusignReady
            ? "Prepare the document, place signer fields and send securely via DocuSign."
            : "Prepare the complete signature request now. It will remain READY FOR SIGNATURE until DocuSign is configured."
        }
        actions={
          <Link href="/app/signatures">
            <Button variant="secondary">Back to Signatures</Button>
          </Link>
        }
      />
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-electric/10 text-electric">
                <FileSignature className="h-4 w-4" />
              </div>
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-electric">Step 1</p>
          ...
          <CardContent className="space-y-4">
            ...
            <p className="text-xs text-muted2">
              Only FINAL documents can enter the signature workflow. A document can only have one active
              prepared/sent request at a time.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-electric">Step 2</p>
                <CardTitle>Signers & routing order</CardTitle>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-5">
                ...
                <div key={i} className="rounded-xl border border-line bg-ink/[0.02] p-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <Field label="Name">
                      <Input
                        name={`signer${i}Name`}
                        defaultValue={agency ? `${user.firstName} ${user.lastName}` : ""}
                        placeholder={i === 1 ? "Client full name" : "Optional signer"}
                      />
                    </Field>
                    <Field label="Email">
                      <Input
                        name={`signer${i}Email`}
                        type="email"
                        defaultValue={agency ? user.email : ""}
                    ...
                    <Field label="Role">
                      <Select
                        name={`signer${i}Role`}
                        defaultValue={agency ? "AGENCY" : i === 1 ? "CLIENT" : "OTHER"}
                      >
                  ...
                  <div className="mt-5 rounded-lg border border-electric/20 bg-electric/[0.03] p-4">
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <p className="text-sm font-medium">PDF field placement</p>
                    </div>
                    <p className="mb-4 text-xs text-muted2">
                      Coordinates use DocuSign PDF points. Page 1 starts at the top-left. Signature is
                      required; the other fields are optional.
                    </p>
                    <div className="grid gap-3 sm:grid-cols-3">
                      <Field label="Signature page">
                        <Input name={`signer${i}SignaturePage`} type="number" min="1" defaultValue="1" />
                      </Field>
                      <Field label="Signature X">
                        <Input name={`signer${i}SignatureX`} type="number" min="0" defaultValue="72" />
                      </Field>
                      <Field label="Signature Y">
                        <Input
                          name={`signer${i}SignatureY`}
                          type="number"
                          min="0"
                          defaultValue={String(700 - (i - 1) * 90)}
                        />
                      </Field>
                    </div>
                    ...
                    <div className="mt-4 grid gap-4 lg:grid-cols-3">
                      <div className="rounded-lg border border-line p-3">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          <Input name={`signer${i}NamePage`} type="number" min="1" defaultValue="1" />
                          <Input name={`signer${i}NameX`} type="number" min="0" defaultValue="72" />
                          <Input
                            name={`signer${i}NameY`}
                            type="number"
                            min="0"
                            defaultValue={String(660 - (i - 1) * 90)}
                          />
                        </div>
                      </div>
                      <div className="rounded-lg border border-line p-3">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          <Input name={`signer${i}DatePage`} type="number" min="1" defaultValue="1" />
                          <Input name={`signer${i}DateX`} type="number" min="0" defaultValue="330" />
                          <Input
                            name={`signer${i}DateY`}
                            type="number"
                            min="0"
                      ...
                      <div className="rounded-lg border border-line p-3">
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          <Input name={`signer${i}InitialsPage`} type="number" min="1" defaultValue="1" />
                          <Input name={`signer${i}InitialsX`} type="number" min="0" defaultValue="470" />
                          <Input
                            name={`signer${i}InitialsY`}
                            type="number"
                            min="0"
                            defaultValue={String(700 - (i - 1) * 90)}
                          />
            ...
            <p className="text-xs text-muted2">
              Signer 1 signs first, then signer 2, and so on. Leave unused signer rows empty. The same email
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-electric">Step 3</p>
                <CardTitle>{docusignReady ? "Message & send" : "Message & prepare"}</CardTitle>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              <Button type="submit" variant="gold" disabled={!documents.length}>
                {docusignReady ? "Send signature request" : "Prepare signature request"}
              </Button>
              <Link href="/app/signatures">
