...
import { CheckCircle2, ExternalLink, MapPin, Move, Send, ShieldCheck } from "lucide-react";
    ...
    <div>
      ...
      {request.status === "READY_FOR_SIGNATURE" ? (
        <div className="mb-6 rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm">
          <span className="font-medium text-amber-700">Prepared — not sent.</span>
          <span className="text-muted2"> All signers and PDF fields are stored in JUN. You can start JUN Secure Sign now{docusignReady ? " or use DocuSign." : "."}</span>
        </div>
      ) : null}

      {request.provider === "JUN_NATIVE" && open ? (
        <div className="mb-6 rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm">
          <span className="font-medium text-emerald-700">JUN Secure Sign active.</span>
          <span className="text-muted2"> Secure signer links are available below. Routing order is enforced automatically.</span>
        </div>
      ) : null}
        ...
        <div className="space-y-6 lg:col-span-2">
                ...
                return (
                  <div key={`${s.email}-${index}`} className="py-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      {s.signedAt ? (
                        <span className="flex items-center gap-2 text-sm text-emerald-600"><CheckCircle2 className="h-4 w-4" /> Signed {formatDateTime(new Date(s.signedAt))}</span>
                        ...
                        <span className="text-sm text-muted2">{request.status === "READY_FOR_SIGNATURE" ? "Prepared" : request.provider === "DOCUSIGN" ? "Awaiting signature via DocuSign" : request.provider === "JUN_NATIVE" ? (isCurrent ? "Ready to sign" : "Waiting for routing order") : "Awaiting signature"}</span>
                      )}
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(s.fields ?? []).map((f, fieldIndex) => (
                        <span key={`${f.type}-${fieldIndex}`} className="inline-flex items-center gap-1 rounded-md border border-white/10 bg-white/[0.03] px-2 py-1 text-xs text-muted2">
                          <MapPin className="h-3 w-3" /> {f.type.replaceAll("_", " ")} · p{f.page} · x{f.x} · y{f.y}{f.width ? ` · ${f.width}×${f.height ?? "?"}` : ""}
                        </span>
                      ))}
                    </div>

                    {request.provider === "JUN_NATIVE" && signingLink ? (
                      <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-line bg-surface p-3">
                        <span className="mr-auto min-w-0 truncate text-xs text-muted2">Secure link · expires in 14 days</span>
                        <CopySigningLink url={signingLink} />
                        <Link href={signingLink} target="_blank"><Button size="sm" variant="secondary"><ExternalLink className="h-3.5 w-3.5" />Open</Button></Link>
                      </div>
                    ) : null}
                  </div>
                );
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {request.status === "READY_FOR_SIGNATURE" ? <p><span className="text-muted2">Prepared:</span> Ready to activate a signature provider</p> : null}
