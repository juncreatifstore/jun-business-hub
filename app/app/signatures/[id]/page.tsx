...
export default async function SignatureDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div>
      ...
      {request.provider === "JUN_NATIVE" && open ? <div className="mb-6 rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm"><span className="font-medium text-emerald-700">JUN Secure Sign active.</span><span className="text-muted2"> Routing order and automatic signer invitations are enforced{expiresAt ? ` · expires ${formatDateTime(expiresAt)}` : ""}.</span></div> : null}
