...
import { activateJunNativeSigning } from "@/services/native-signatures";
...
import { CheckCircle2, Download, ExternalLink, FileCheck2, MapPin, Move, Send, ShieldCheck } from "lucide-react";
...
export default async function SignatureDetailPage({ params }: { params: { id: string } }) {
  ...
  const nativeLinks = request.provider === "JUN_NATIVE"
    ? await Promise.all(recipients.map(async (r) => ({ email: r.email, url: await nativeSigningUrl(request.id, r.email, r.order) })))
    : [];
  ...
  return (
    <div>
      ...
      {request.provider === "JUN_NATIVE" && open ? <div className="mb-6 rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-4 py-3 text-sm"><span className="font-medium text-emerald-700">JUN Secure Sign active.</span><span className="text-muted2"> Secure signer links are available below. Routing order is enforced automatically.</span></div> : null}
          ...
          <Card>
            ...
            <CardContent className="divide-y divide-white/5">
              {recipients.length === 0 ? <p className="py-3 text-sm text-muted2">No recipients recorded.</p> : recipients.map((s, index) => {
                ...
                const isCurrent = request.provider === "JUN_NATIVE" && !s.signedAt && firstUnsigned?.email.toLowerCase() === s.email.toLowerCase();
                return <div key={`${s.email}-${index}`} className="py-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{s.name}</p>{s.role ? <span className="rounded bg-white/5 px-2 py-0.5 text-xs text-muted2">{s.role.replaceAll("_", " ")}</span> : null}{isCurrent ? <span className="rounded bg-emerald-100 px-2 py-0.5 text-xs text-emerald-700">CURRENT SIGNER</span> : null}</div><p className="text-sm text-muted2">{s.email} · routing order {s.order}</p></div>
                    {s.signedAt ? <span className="flex items-center gap-2 text-sm text-emerald-600"><CheckCircle2 className="h-4 w-4" /> Signed {formatDateTime(new Date(s.signedAt))}</span> : canSign && open && allowMockSign ? <form action={mockSignRecipient.bind(null, request.id, index)}><Button variant="gold" size="sm">Mock sign (dev)</Button></form> : <span className="text-sm text-muted2">{request.status === "READY_FOR_SIGNATURE" ? "Prepared" : request.provider === "DOCUSIGN" ? "Awaiting signature via DocuSign" : request.provider === "JUN_NATIVE" ? (isCurrent ? "Ready to sign" : "Waiting for routing order") : "Awaiting signature"}</span>}
                  </div>
                  ...
                  {request.provider === "JUN_NATIVE" && signingLink ? <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-line bg-surface p-3"><span className="mr-auto min-w-0 truncate text-xs text-muted2">Secure link · expires in 14 days</span><CopySigningLink url={signingLink} /><Link href={signingLink} target="_blank"><Button size="sm" variant="secondary"><ExternalLink className="h-3.5 w-3.5" />Open</Button></Link></div> : null}
                </div>;
              })}
            </CardContent>
          </Card>
