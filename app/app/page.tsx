...
function MetricCard({
  ...
  return (
    <Link
      href={href}
      className="group min-w-0 rounded-2xl border border-white/[0.07] bg-[#101827] p-3.5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-400/25 hover:bg-[#121d2f] sm:p-4"
    >
      ...
      <p className="mt-3 truncate text-[11px] font-medium text-slate-400 sm:mt-4 sm:text-xs">{label}</p>
      <p className="mt-1 truncate text-xl font-semibold tracking-tight text-white sm:text-2xl">{value}</p>
      <p className="mt-2 line-clamp-2 text-[10px] text-emerald-400 sm:text-[11px]">{detail}</p>
    </Link>
  );
}
...
export default async function DashboardPage() {
  ...
  return (
    <div className="-mx-3 -mt-3 min-h-[calc(100dvh-4rem)] overflow-x-hidden bg-[#070c14] px-3 pb-28 pt-3 text-slate-100 sm:-m-6 sm:p-6 lg:p-7">
      <div className="mx-auto max-w-[1650px]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
          <div className="min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-blue-400 sm:text-[11px] sm:tracking-[0.2em]">
          ...
          <div className="w-fit rounded-xl border border-white/[0.07] bg-[#0e1624] px-3 py-2 text-[11px] text-slate-400 sm:px-4 sm:text-xs">
        ...
        <div className="mt-3 grid gap-3 sm:mt-4 sm:gap-4 xl:grid-cols-[minmax(0,1.75fr)_minmax(320px,.75fr)]">
          <section className="min-w-0 rounded-2xl border border-white/[0.07] bg-[#0e1624] shadow-sm">
            ...
            <div className="p-3 sm:p-5">
              <div className="flex h-44 items-end gap-1.5 border-b border-white/[0.06] pb-2 sm:h-64 sm:gap-3">
                {months.map((m) => (
                  <div key={m.label} className="flex h-full min-w-0 flex-1 flex-col justify-end gap-2">
                    <div className="flex flex-1 items-end">
                      ...
                      >
                        <span className="absolute -top-5 left-1/2 hidden -translate-x-1/2 whitespace-nowrap text-[9px] text-slate-500 md:block">
                          {m.total > 0 ? formatMoney(m.total) : ""}
                        </span>
                      </div>
                    </div>
                    <span className="truncate text-center text-[9px] capitalize text-slate-500 sm:text-[11px]">
                      {m.label}
                    </span>
                  </div>
                ))}
          ...
          <section className="rounded-2xl border border-white/[0.07] bg-[#0e1624] shadow-sm">
            <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3.5 sm:px-5 sm:py-4">
              <div>
                <h2 className="text-sm font-semibold text-white">À traiter maintenant</h2>
                <p className="mt-0.5 text-[11px] text-slate-500 sm:text-xs">Files de travail prioritaires</p>
              </div>
            ...
            <div className="space-y-2.5 p-4 sm:space-y-3 sm:p-5">
              ...
              {user.role === "SUPER_ADMIN" ? (
                <Link
                  ...
                  className="flex items-center justify-between rounded-xl border border-blue-500/15 bg-blue-500/[0.06] px-3.5 py-3 text-sm text-blue-300 hover:bg-blue-500/[0.1] sm:px-4"
                >
        ...
        <div className="mt-3 grid gap-3 sm:mt-4 sm:gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,.75fr)]">
          <section className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0e1624] shadow-sm">
            <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3.5 sm:px-5 sm:py-4">
              <div>
                <h2 className="text-sm font-semibold text-white">Dossiers récents</h2>
                <p className="mt-0.5 text-[11px] text-slate-500 sm:text-xs">Dernières mises à jour CRM</p>
              </div>
              <Link href="/app/cases" className="text-xs text-blue-400 hover:text-blue-300">
                Voir tout
              </Link>
            </div>
            <div className="divide-y divide-white/[0.05] md:hidden">
              {recentCases.map((c) => (
                <Link
                  key={c.id}
                  href={`/app/cases/${c.id}`}
                  className="block px-4 py-3.5 active:bg-white/[0.03]"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium text-white">{c.caseNumber}</div>
                      <div className="mt-1 truncate text-xs text-slate-400">
                        {c.client ? `${c.client.firstName} ${c.client.lastName}` : "Client non lié"}
                      </div>
                    </div>
                    <span className="shrink-0 rounded-full bg-blue-500/10 px-2 py-1 text-[9px] font-medium text-blue-300">
                      {c.status.replaceAll("_", " ")}
                    </span>
                  </div>
                  <div className="mt-2 text-[10px] text-slate-600">
                    Mis à jour {formatDateTime(c.updatedAt)}
                  </div>
                </Link>
              ))}
              {recentCases.length === 0 ? (
                <div className="px-4 py-8 text-center text-sm text-slate-500">Aucun dossier récent.</div>
              ) : null}
            </div>
          ...
          <section className="rounded-2xl border border-white/[0.07] bg-[#0e1624] shadow-sm">
            <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3.5 sm:px-5 sm:py-4">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-slate-500 sm:text-xs">Ce qui vient de se passer</p>
              </div>
            ...
            <div className="divide-y divide-white/[0.05]">
              {recentActivity.map((a) => (
                <div key={a.id} className="flex gap-3 px-4 py-3.5 sm:px-5">
                  ...
                  <div className="min-w-0">
                    <p className="line-clamp-3 text-xs leading-5 text-slate-300">{a.message}</p>
              ...
              {recentActivity.length === 0 ? (
                <p className="px-5 py-8 text-center text-sm text-slate-500">L’activité apparaîtra ici.</p>
              ) : null}
            </div>
          </section>
        </div>
        ...
        <div className="mt-3 grid gap-3 sm:mt-4 sm:gap-4 lg:grid-cols-2">
          <section className="rounded-2xl border border-white/[0.07] bg-[#0e1624] shadow-sm">
            <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3.5 sm:px-5 sm:py-4">
              ...
              <Link href="/app/tasks" className="text-xs text-blue-400">
                Voir tout
              </Link>
            </div>
            <div className="divide-y divide-white/[0.05]">
              ...
              {overdueTasks.length === 0 ? (
                <p className="px-5 py-8 text-center text-sm text-slate-500">Aucune tâche en retard.</p>
              ) : null}
            </div>
          </section>
          <section className="rounded-2xl border border-white/[0.07] bg-[#0e1624] shadow-sm">
            <div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3.5 sm:px-5 sm:py-4">
              ...
              <Link href="/app/documents" className="text-xs text-blue-400">
                Voir tout
              </Link>
            </div>
            <div className="divide-y divide-white/[0.05]">
              {recentDocs.map((d) => (
                <div key={d.id} className="flex items-start justify-between gap-3 px-4 py-3.5 sm:px-5">
                  ...
                  <span className="shrink-0 rounded-full bg-white/[0.05] px-2 py-1 text-[9px] text-slate-400 sm:px-2.5 sm:text-[10px]">
                    {d.status.replaceAll("_", " ")}
                  </span>
                </div>
              ))}
              {recentDocs.length === 0 ? (
                <p className="px-5 py-8 text-center text-sm text-slate-500">Aucun document récent.</p>
              ) : null}
            </div>
          </section>
        </div>
...
function QuickStatus({
  ...
  return (
    <Link
      href={href}
      className={`flex items-center justify-between rounded-xl border px-3.5 py-3 text-sm transition hover:bg-white/[0.05] sm:px-4 ${cls}`}
    >
