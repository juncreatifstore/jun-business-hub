...
export default async function CasesPage({ searchParams }: { searchParams: Params }) {
  ...
  const cases = await prisma.case.findMany({ where, orderBy: caseOrder(sort), skip: (page - 1) * PAGE_SIZE, take: PAGE_SIZE, include: { client: true, owner: true } });
  ...
  return <div>
    ...
    <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      ...
      <form className="grid flex-1 gap-2 rounded-2xl border border-line bg-night-soft/45 p-3 shadow-sm md:grid-cols-[minmax(240px,1fr)_190px_180px_auto] lg:max-w-4xl">
        ...
        <Select name="sort" defaultValue={sort}><option value="RECENT">Plus récents</option><option value="DUE_ASC">Échéance proche</option><option value="PRIORITY">Priorité</option><option value="CLIENT">Client A → Z</option></Select>
        <div className="flex gap-2"><Button variant="outline">Appliquer</Button>{(q || status || sort !== "RECENT") ? <Link href="/app/cases"><Button type="button" variant="ghost">Réinitialiser</Button></Link> : null}</div>
      </form>
    </div>
    {cases.length === 0 ? <EmptyState icon={FolderKanban} title="Aucun dossier" description="Ouvrez un dossier pour suivre un engagement client de bout en bout." actionHref="/app/cases/new" actionLabel="Ouvrir un dossier"/> : <>
      <div className="mb-3"><ListCount shown={cases.length} total={total} label="dossier" /></div>
      ...
      <div className="grid gap-3 md:hidden">{cases.map((c)=><RecordCard
        key={c.id}
        href={`/app/cases/${c.id}/dashboard`}
        title={c.title}
        subtitle={<span className="registry-id">{c.caseNumber}</span>}
        badges={<><StatusBadge status={c.status}/><StatusBadge status={c.priority}/><Badge className="border border-white/[0.07] bg-white/[0.03] text-slate-400">{c.type}</Badge></>}
