...
export default async function CasesPage({ searchParams }: { searchParams: { q?: string; status?: string } }) {
  ...
  return (
    <div>
      ...
      {cases.length === 0 ? (
        <EmptyState icon={FolderKanban} title="No cases yet" description="Open a case to track a client engagement from start to finish." actionHref="/app/cases/new" actionLabel="Open case" />
      ) : (
        <Table>
          <THead><tr><TH>Case</TH><TH>Title</TH><TH>Client</TH><TH>Type</TH><TH>Status</TH><TH>Priority</TH><TH>Owner</TH><TH>Due</TH></tr></THead>
          <tbody>
            {cases.map((c) => (
              <TR key={c.id}>
