    ...
    <div>
      ...
      <form method="get" className="mb-4 flex flex-wrap gap-3">
        ...
        >
          ...
          {types.map((t) => (
            <option key={t.resourceType} value={t.resourceType}>
              {t.resourceType} ({t._count._all})
            </option>
          ))}
        </select>
        ...
        <EmptyState
          ...
          description={
            q || type
              ...
              : "Sensitive actions will be recorded here automatically."
          }
        />
      ) : (
        <Table>
              ...
              <TR key={l.id}>
                ...
                <TD>
                  {l.user ? (
                    ...
                    <span className="text-muted2">System</span>
                  )}
                </TD>
                <TD>
                  <span className="registry-id text-electric">{l.action}</span>
                </TD>
                <TD className="text-muted2">
                  {l.resourceType}
                  {l.resourceId ? <span className="registry-id block text-xs">{l.resourceId}</span> : null}
                </TD>
