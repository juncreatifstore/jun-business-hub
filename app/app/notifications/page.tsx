...
export default async function NotificationsPage({
  ...
  const filter: FilterKey = ["all", "unread", "whatsapp", "finance", "cases", "documents", "system"].includes(
    requestedFilter,
  )
    ? requestedFilter
    : "all";
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span
                                className={`rounded-full px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${meta.badgeClass}`}
                              >
                                {meta.label}
                              </span>
                              {!n.readAt ? (
                            ...
                            {n.body ? (
                              <p className="mt-1 max-w-5xl whitespace-pre-wrap text-sm leading-relaxed text-muted2">
                                {n.body}
                              </p>
                            ) : null}
...
function hrefFor(type: string) {
  ...
  if (category === "finance") return "/app/finance/payments";
...
function notificationMeta(type: string) {
  ...
  if (/SECURITY|BAN|WARNING|ALERT/.test(type.toUpperCase()))
    return {
      label: "Sécurité",
...
function groupByDay(
  items: Array<NotificationRow & { category: ReturnType<typeof categoryFor>; href: string }>,
) {
