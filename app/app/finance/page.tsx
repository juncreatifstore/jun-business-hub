...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, Banknote, CircleDollarSign, Clock3, Download, Landmark, ReceiptText, RefreshCw, WalletCards } from "lucide-react";
...
export default async function FinanceControlCenterPage() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-xs uppercase tracking-[0.18em] text-muted2">Finance operations</p><h1 className="mt-1 text-3xl font-semibold">Finance Control Center</h1><p className="mt-1 text-sm text-muted2">Cash position, collections, fees, refunds, payment rails and operational exceptions in one view.</p></div>
    ...
    <Card><CardHeader><CardTitle>Cash position by currency</CardTitle></CardHeader><CardContent>{data.currencies.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.currencies.map((row) => <div key={row.currency} className="rounded-xl border border-line bg-surface p-4"><div className="flex items-center justify-between"><span className="registry-id text-sm">{row.currency}</span><span className={`text-xs font-semibold ${row.netCash >= 0 ? "text-emerald-700" : "text-red-700"}`}>{row.netCash >= 0 ? "POSITIVE" : "NEGATIVE"}</span></div><div className="mt-3 text-2xl font-semibold">{formatMoney(row.netCash, row.currency)}</div><div className="mt-3 grid grid-cols-3 gap-2 text-xs"><Mini label="Collected" value={formatMoney(row.collected, row.currency)} /><Mini label="Fees" value={formatMoney(row.fees, row.currency)} /><Mini label="Refunded" value={formatMoney(row.refundsPaid, row.currency)} /></div></div>)}</div> : <p className="text-sm text-muted2">No confirmed financial movement yet.</p>}</CardContent></Card>

    <div className="grid gap-4 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>This month cash flow</CardTitle></CardHeader><CardContent className="space-y-2">{data.monthCurrencies.length ? data.monthCurrencies.map((row) => <div key={row.currency} className="flex items-center justify-between rounded-lg border border-line px-3 py-3"><div><div className="registry-id">{row.currency}</div><div className="mt-1 text-xs text-muted2">Net collections after fees minus refunds paid</div></div><div className="text-right"><div className={`font-semibold ${row.net >= 0 ? "text-emerald-700" : "text-red-700"}`}>{formatMoney(row.net, row.currency)}</div><div className="text-[11px] text-muted2">{formatMoney(row.collected,row.currency)} in · {formatMoney(row.refunds,row.currency)} out</div></div></div>) : <p className="text-sm text-muted2">No cash movement this month.</p>}</CardContent></Card>

      <Card><CardHeader><CardTitle>Payment methods</CardTitle></CardHeader><CardContent className="space-y-2">{data.methods.length ? data.methods.slice(0,8).map((row) => <div key={row.method} className="flex items-center justify-between rounded-lg border border-line px-3 py-2"><div><div className="text-sm font-medium">{row.method.replaceAll("_", " ")}</div><div className="text-xs text-muted2">{row.count} confirmed payment{row.count === 1 ? "" : "s"}</div></div><div className="text-right text-xs">{Object.entries(row.amountByCurrency).map(([c,a]) => <div key={c}>{formatMoney(a,c)}</div>)}</div></div>) : <p className="text-sm text-muted2">No payment method data yet.</p>}</CardContent></Card>
    </div>
...
function Metric({icon:Icon,label,value,hint}:{icon:typeof CircleDollarSign;label:string;value:string;hint:string}) { return <div className="rounded-xl border border-line bg-white p-4"><Icon className="mb-3 h-5 w-5 text-electric"/><div className="text-xs text-muted2">{label}</div><div className="mt-1 text-2xl font-semibold">{value}</div><div className="mt-1 text-xs text-muted2">{hint}</div></div>; }
function Mini({label,value}:{label:string;value:string}) { return <div><div className="text-muted2">{label}</div><div className="mt-0.5 font-medium break-words">{value}</div></div>; }
function AlertCard({title,count,icon:Icon,href,children}:{title:string;count:number;icon:typeof AlertTriangle;href:string;children:React.ReactNode}) { return <Card><CardHeader><CardTitle><span className="flex items-center justify-between gap-2"><span className="flex items-center gap-2"><Icon className="h-4 w-4"/>{title}</span><span className="rounded-full bg-surface px-2 py-0.5 text-xs">{count}</span></span></CardTitle></CardHeader><CardContent className="space-y-2">{count ? children : <p className="text-xs text-muted2">No items.</p>}<Link href={href} className="inline-block pt-1 text-xs font-medium text-electric">Open module →</Link></CardContent></Card>; }
function AlertRow({href,primary,secondary,trailing}:{href:string;primary:string;secondary:string;trailing:string}) { return <Link href={href} className="flex items-center justify-between gap-3 rounded-lg border border-line px-3 py-2 hover:bg-surface"><div className="min-w-0"><div className="truncate text-xs font-medium">{primary}</div><div className="truncate text-[11px] text-muted2">{secondary}</div></div><div className="shrink-0 text-[11px] font-medium">{trailing}</div></Link>; }
