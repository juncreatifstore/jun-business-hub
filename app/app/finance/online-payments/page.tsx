...
function effectiveStatus(s: OnlinePaymentSession) {
  return ["CREATED", "PENDING"].includes(s.status) && new Date(s.expiresAt).getTime() <= Date.now()
    ? "EXPIRED"
    : s.status;
}
...
export default async function OnlinePaymentsPage() {
  ...
  const paymentRows = sessions.length
    ? await prisma.payment.findMany({
        where: { id: { in: sessions.map((s) => s.paymentId) } },
  ...
  const providers = [
    ...
    {
      name: "Mercado Pago",
      ready: Boolean(process.env.MERCADO_PAGO_ACCESS_TOKEN && process.env.MERCADO_PAGO_WEBHOOK_SECRET),
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="mb-5 grid gap-3 md:grid-cols-3">
        ...
        <Metric label="Confirmed online" value={String(paid)} />
        <Metric label="Failed / expired" value={String(failed)} />
      </div>

      <div className="grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
        <Card>
          <CardHeader>
            <CardTitle>Create secure payment link</CardTitle>
          </CardHeader>
          <CardContent>
            <form action={createOnlinePaymentRequest} className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <Field label="Description">
                  <Input
                    name="description"
                    placeholder="Service or invoice being paid"
                    required
                    maxLength={180}
                  />
                </Field>
              ...
              <Field label="Link validity">
                <Select name="expiryMinutes" defaultValue="30">
            ...
            {providers.map((p) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-muted2">Server credentials + webhook verification</div>
                </div>
                <span
                  className={`rounded-full px-2.5 py-1 text-xs font-medium ${p.ready ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
                >
                  {p.ready ? "Ready" : "Needs configuration"}
                </span>
              </div>
            ))}
          </CardContent>
      ...
      <Card className="mt-5">
        <CardHeader>
          <CardTitle>Recent online payment requests</CardTitle>
        </CardHeader>
              ...
              {sessions.map((s) => {
                ...
                return (
                  <Link
                    ...
                    className="grid gap-2 px-5 py-4 hover:bg-surface md:grid-cols-[1.2fr_.8fr_.8fr_.8fr_1fr]"
                  >
                    <div>
                      <div className="text-sm font-medium">{refs.get(s.paymentId) || s.id}</div>
                      <div className="text-xs text-muted2">
                        {s.clientName} · {s.description}
                      </div>
                    </div>
                    <div className="text-sm">{s.provider.replaceAll("_", " ")}</div>
                    <div className="text-sm font-medium">{formatMoney(s.amount, s.currency)}</div>
                    <div>
                      <span className="rounded-full bg-surface px-2 py-1 text-xs">{status}</span>
                    </div>
                    <div className="text-xs text-muted2">{formatDateTime(new Date(s.updatedAt))}</div>
                  </Link>
                );
              })}
            </div>
          ) : (
            <div className="p-6 text-sm text-muted2">No online payment request yet.</div>
          )}
        </CardContent>
      </Card>
    </div>
