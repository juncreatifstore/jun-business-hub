...
export default async function ManualTransferOrderPage({ params }: { params: { id: string } }) {
  ...
  return <div className="max-w-5xl">
    <div className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><p className="registry-id text-muted2">{order.orderNumber}</p><h1 className="mt-1 text-2xl font-semibold">Manual payment order</h1><p className="mt-1 text-sm text-muted2">{r.rail.replaceAll("_", " ")} · {order.originCountry} → {order.destinationCountry} · {order.language}</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-surface px-3 py-1 text-xs font-medium">{order.status}</span>{receipt?<a href={`/api/finance/receipts/${receipt.id}/pdf`} target="_blank" className="rounded-lg border border-line px-3 py-2 text-xs font-medium">Receipt PDF</a>:null}</div></div>
