...
import {
  createFinancePaymentAccount,
  toggleFinancePaymentAccount,
  updateFinancePaymentAccount,
} from "@/services/finance-payment-accounts";
  ...
  return (
    <div>
        ...
        <Card className="mb-5">
          ...
          <CardContent>
            <form action={createFinancePaymentAccount} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Field label="Receiver / holder">
                <Input name="receiverName" placeholder="Company or account holder" />
              </Field>
              <div className="md:col-span-2">
                <Field label="Account descriptor">
                  <Input
                    name="accountDescriptor"
                    placeholder="Email, phone, bank + last 4 digits, cash drawer…"
                  />
                </Field>
              </div>
              <Field label="Fee %">
              ...
              <div className="md:col-span-2 xl:col-span-4">
                <Field label="Internal instructions">
                  <Textarea
                    name="instructions"
                    rows={3}
                    placeholder="How the finance team verifies this account. Do not store API keys or passwords here."
                  />
                </Field>
              </div>
              <div className="md:col-span-2 xl:col-span-4">
      ...
      <div className="space-y-3">
        {accounts.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center text-sm text-muted2">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted2">
                      {account.method.replaceAll("_", " ")} · {account.currency}
                      {account.receiverName ? ` · ${account.receiverName}` : ""}
                    </p>
                    {account.accountDescriptor ? (
                      <p className="mt-1 text-sm">{account.accountDescriptor}</p>
                    ) : null}
                    <p className="mt-2 text-xs text-muted2">
                      Fees: {account.feePercent.toFixed(2)}% + {account.currency}{" "}
                      {account.feeFixed.toFixed(2)}
                    </p>
                    {account.instructions ? (
                      <p className="mt-2 max-w-3xl whitespace-pre-wrap text-xs text-muted2">
                        {account.instructions}
                      </p>
                    ) : null}
                  </div>
                  {canManage ? (
                    <form action={toggleFinancePaymentAccount.bind(null, account.id)}>
                      <Button variant={account.enabled ? "outline" : "primary"}>
                ...
                {canManage ? (
                  <details className="mt-4 rounded-lg border border-line bg-surface p-3">
                    <summary className="cursor-pointer text-xs font-medium">
                      Edit account details & fees
                    </summary>
                    <form
                      action={updateFinancePaymentAccount.bind(null, account.id)}
                    ...
                    >
                      ...
                      <Field label="Receiver">
                        <Input name="receiverName" defaultValue={account.receiverName} />
                      </Field>
                      <div className="md:col-span-2">
                        <Field label="Account descriptor">
                          <Input name="accountDescriptor" defaultValue={account.accountDescriptor} />
                        </Field>
                      </div>
                      <Field label="Fee %">
                        <Input
                          ...
                          max="100"
                          step="0.01"
                          defaultValue={String(account.feePercent)}
                        />
                      </Field>
                      <Field label="Fixed fee">
                        <Input
                          ...
                          min="0"
                          step="0.01"
                          defaultValue={String(account.feeFixed)}
                        />
                      </Field>
                      <div className="md:col-span-2">
                        <Field label="Internal instructions">
                          <Textarea name="instructions" rows={2} defaultValue={account.instructions} />
                        </Field>
                      </div>
                      <div className="md:col-span-2 xl:col-span-4">
