...
export default async function PaymentsPage({searchParams}:{searchParams:{status?:string;method?:string;currency?:string;q?:string;sort?:string;page?:string}}){
  ...
  const orderBy:any=sort==="AMOUNT_DESC"?[{amount:"desc"},{createdAt:"desc"}]:sort==="AMOUNT_ASC"?[{amount:"asc"},{createdAt:"desc"}]:[{paidAt:"desc"},{createdAt:"desc"}];
  const [payments,pendingCount,confirmedPayments,proofCount]=await Promise.all([
    prisma.payment.findMany({where,orderBy,skip:(page-1)*PAGE_SIZE,take:PAGE_SIZE,include:{client:true,case:true,files:{where:{category:"PAYMENT_PROOF",archivedAt:null},select:{id:true}}}}),
  ...
  const paginationParams={q:q||undefined,status:status!=="ALL"?status:undefined,method:method!=="ALL"?method:undefined,currency:currency||undefined,sort:sort!=="RECENT"?sort:undefined};
  ...
  return <div>
    ...
    <form className="mb-4 grid gap-2 rounded-2xl border border-line bg-night-soft/45 p-3 shadow-sm md:grid-cols-[minmax(220px,1fr)_160px_175px_105px_165px_auto]">
      ...
      <Select name="sort" defaultValue={sort}><option value="RECENT">Plus récents</option><option value="AMOUNT_DESC">Montant décroissant</option><option value="AMOUNT_ASC">Montant croissant</option></Select>
      <div className="flex gap-2"><Button variant="outline">Appliquer</Button>{(q||status!=="ALL"||method!=="ALL"||currency||sort!=="RECENT")?<Link href="/app/finance/payments"><Button type="button" variant="ghost">Réinitialiser</Button></Link>:null}</div>
    </form>
    {payments.length===0?<EmptyState icon={CreditCard} title="Aucun paiement correspondant" description="Modifiez les filtres ou enregistrez un nouveau paiement." actionHref="/app/finance/payments/new" actionLabel="Enregistrer un paiement"/>:<>
      <div className="mb-3"><ListCount shown={payments.length} total={total} label="paiement" /></div>
      <div className="hidden md:block"><Table>
        ...
        <tbody>{payments.map(p=>{const meta=metaMap.get(p.id);const expected=meta?.expectedAmount??null;const grossAmount=Number(p.amount);const feeAmount=Number(meta?.feeAmount||0);const netAmount=netReceived(grossAmount,feeAmount);const balance=paymentBalance(netAmount,expected);return <TR key={p.id}>
          ...
          <TD><Link href={`/app/clients/${p.clientId}/dashboard`} className="font-medium hover:text-electric">{p.client.firstName} {p.client.lastName}</Link><div className="mt-1 text-[11px] text-muted2">{meta?.serviceLabel||p.case?.caseNumber||"Service non précisé"}</div></TD>
      ...
      <div className="grid gap-3 md:hidden">{payments.map(p=>{const meta=metaMap.get(p.id);const expected=meta?.expectedAmount??null;const grossAmount=Number(p.amount);const feeAmount=Number(meta?.feeAmount||0);const netAmount=netReceived(grossAmount,feeAmount);const balance=paymentBalance(netAmount,expected);return <RecordCard
        key={p.id}
        ...
        badges={<><StatusBadge status={p.status}/><Badge className="border border-white/[0.07] bg-white/[0.03] text-slate-400">{p.method.replaceAll("_"," ")}</Badge>{p.files.length?<Badge className="border border-emerald-400/20 bg-emerald-500/10 text-emerald-300">PREUVE ✓</Badge>:null}</>}
        footer={`${formatDate(p.paidAt)} · ${meta?.serviceLabel||p.case?.caseNumber||"Service non précisé"}`}
      >
        <RecordField label="Net reçu" value={formatMoney(netAmount,p.currency)} valueClassName="text-emerald-300" />
        {feeAmount>0?<RecordField label="Brut / frais" value={`${formatMoney(grossAmount,p.currency)} / ${formatMoney(feeAmount,p.currency)}`} />:null}
        <RecordField label="Attendu" value={expected==null?"—":formatMoney(expected,p.currency)} />
        {balance!=null?<RecordField label="Solde" value={balance>0?`${formatMoney(balance,p.currency)} dû`:balance<0?`${formatMoney(Math.abs(balance),p.currency)} trop-perçu`:"Payé intégralement"} valueClassName={balance>0?"text-amber-300":"text-emerald-300"}/>:null}
      </RecordCard>;})}</div>
      <Pagination basePath="/app/finance/payments" page={page} totalPages={totalPages} params={paginationParams}/>
    </>}
  </div>;
}
