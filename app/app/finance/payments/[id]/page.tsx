...
import { FileCheck2, History, ReceiptText, Scale, WalletCards } from "lucide-react";
...
export default async function PaymentDetailPage({ params }: { params: { id: string } }) {
  ...
  const [p, meta, auditRows] = await Promise.all([
    prisma.payment.findUnique({ where: { id: params.id }, include: { client: true, case: true, recordedBy: true, refunds: { orderBy: { createdAt: "desc" } }, files: { where: { archivedAt: null }, orderBy: { createdAt: "desc" }, include: { uploadedBy: { select: { firstName: true, lastName: true } } } } } }),
  ...
  const feeAmount = meta.feeAmount ?? 0;
  ...
  return <div className="max-w-5xl">
    <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
      <div><p className="registry-id text-muted2">{p.reference}</p><h1 className="mt-1 flex flex-wrap items-center gap-3 text-2xl font-semibold">{formatMoney(amount, p.currency)} <StatusBadge status={p.status} /></h1><p className="mt-1 text-sm text-muted2"><Link href={`/app/clients/${p.clientId}`} className="text-electric hover:underline">{p.client.firstName} {p.client.lastName}</Link>{p.case ? <> · <Link href={`/app/cases/${p.case.id}`} className="registry-id hover:text-electric">{p.case.caseNumber}</Link></> : null}{meta.serviceLabel ? <> · {meta.serviceLabel}</> : null}</p></div>
    ...
    <div className="mb-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      ...
      <Metric icon={WalletCards} label="Receiving account" value={meta.accountLabel || p.provider || "Unassigned"} hint={meta.accountDescriptor || "Manual / legacy payment"} />
      <Metric icon={Scale} label="Net after fees" value={formatMoney(netAmount, p.currency)} hint={feeAmount ? `${formatMoney(feeAmount, p.currency)} estimated fees` : "No configured fee"} />
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Payment details</CardTitle></CardHeader><CardContent><dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
        ...
        <div><dt className="text-xs text-muted2">Receiving account</dt><dd className="mt-0.5">{meta.accountLabel || p.provider || "—"}</dd></div>
        <div><dt className="text-xs text-muted2">Account details</dt><dd className="mt-0.5 break-words">{meta.accountDescriptor || "—"}</dd></div>
        ...
        <div><dt className="text-xs text-muted2">Expected amount</dt><dd className="mt-0.5">{meta.expectedAmount == null ? "—" : formatMoney(meta.expectedAmount, p.currency)}</dd></div>
        <div><dt className="text-xs text-muted2">Balance</dt><dd className="mt-0.5">{balance == null ? "—" : balance > 0 ? `${formatMoney(balance, p.currency)} due` : balance < 0 ? `${formatMoney(Math.abs(balance), p.currency)} overpaid` : "Paid in full"}</dd></div>
      ...
      <Card><CardHeader><CardTitle>Payment evidence</CardTitle></CardHeader><CardContent className="space-y-4">{canAttachProof ? <PaymentProofUpload paymentId={p.id} clientId={p.clientId} caseId={p.caseId} /> : null}{p.files.length ? <div className="divide-y divide-line rounded-lg border border-line">{p.files.map((file) => <div key={file.id} className="flex items-center justify-between gap-3 px-3 py-2.5"><div className="min-w-0"><div className="truncate text-sm font-medium">{file.name}</div><div className="text-[11px] text-muted2">{file.category.replaceAll("_", " ")} · {file.uploadedBy.firstName} {file.uploadedBy.lastName}</div></div><a href={`/api/files/${file.id}`} target="_blank" rel="noreferrer" className="text-xs font-medium text-electric hover:underline">Open</a></div>)}</div> : <p className="text-xs text-muted2">No proof or supporting file is linked to this payment yet.</p>}</CardContent></Card>
    </div>
    ...
    <Card className="mt-4"><CardHeader><CardTitle><span className="flex items-center gap-2"><History className="h-4 w-4" /> Finance history</span></CardTitle></CardHeader><CardContent>{auditRows.length ? <div className="space-y-2">{auditRows.map((row) => <div key={row.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-line bg-surface px-3 py-2 text-xs"><div><span className="font-medium">{row.action.replaceAll("_", " ")}</span><span className="ml-2 text-muted2">by {row.user ? `${row.user.firstName} ${row.user.lastName}` : "System"}</span></div><span className="text-muted2">{formatDateTime(row.createdAt)}</span></div>)}</div> : <p className="text-xs text-muted2">No finance audit events yet.</p>}</CardContent></Card>
  </div>;
}

function Metric({ icon: Icon, label, value, hint }: { icon: typeof ReceiptText; label: string; value: string; hint: string }) {
  return <div className="rounded-xl border border-line bg-white p-4"><Icon className="mb-3 h-5 w-5 text-electric" /><div className="text-xs text-muted2">{label}</div><div className="mt-1 truncate text-base font-semibold" title={value}>{value}</div><div className="mt-1 truncate text-xs text-muted2" title={hint}>{hint}</div></div>;
}
