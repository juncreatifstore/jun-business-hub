...
import { AlertTriangle, Bot, BrainCircuit, CircleDollarSign, Gauge, Link2, ShieldAlert, Target, TrendingUp } from "lucide-react";
...
export default async function FinanceIntelligencePage() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.18em] text-muted2">Enterprise finance</p><h1 className="mt-1 flex items-center gap-2 text-3xl font-semibold"><BrainCircuit className="h-7 w-7 text-electric" />Finance Intelligence</h1><p className="mt-1 text-sm text-muted2">Explainable controls, reconciliation, anomaly detection, forecasting, budget variance and read-only AI analysis.</p></div><Link href="/app/finance" className="rounded-lg border border-line bg-white px-3 py-2 text-xs font-medium hover:bg-surface">Back to Control Center</Link></div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <Metric icon={Target} label="Budget variance" value={String(data.budgetAlertCount)} hint={`${data.budgetPlanning.length} active budget${data.budgetPlanning.length===1?"":"s"}`} />
    ...
    <Card><CardHeader><CardTitle>Executive summary</CardTitle></CardHeader><CardContent><p className="text-sm leading-6 text-ink">{summary}</p><p className="mt-3 text-xs text-muted2">Generated from deterministic finance controls. Forecasts and budget scenarios are directional projections, not guarantees.</p></CardContent></Card>
    ...
    <Card><CardHeader><CardTitle>Budget scenario planning</CardTitle></CardHeader><CardContent>{data.budgetPlanning.length?<div className="grid gap-3 lg:grid-cols-2">{data.budgetPlanning.map(b=><Link key={b.planId} href={`/app/finance/budgeting/${b.planId}`} className="rounded-xl border border-line p-4 hover:bg-surface"><div className="flex items-center justify-between"><div><div className="font-semibold">{b.name}</div><div className="text-xs text-muted2">{b.year} · {b.currency} · {b.alerts.length} variance alert{b.alerts.length===1?"":"s"}</div></div><Target className="h-5 w-5 text-electric"/></div><div className="mt-3 grid grid-cols-3 gap-2">{b.scenarios.map(s=><Small key={s.scenario} label={s.scenario} value={formatMoney(s.net,b.currency)}/>)}</div></Link>)}</div>:<p className="text-sm text-muted2">No approved current-year budget available for scenario analysis.</p>}</CardContent></Card>
