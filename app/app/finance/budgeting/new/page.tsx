    ...
    <div className="mx-auto w-full max-w-3xl space-y-4 sm:space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted2">
          Planifiez les objectifs de revenus, dépenses et profits pour l’année budgétaire JUN.
        </p>
      </div>
      {searchParams.error && (
      ...
      <Card>
        <CardHeader className="p-4 pb-0 sm:p-5 sm:pb-0">
          <CardTitle>Identité du budget</CardTitle>
        </CardHeader>
        <CardContent className="p-4 sm:p-5">
          <form action={createBudgetPlanAction} className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <Field label="Nom du budget">
                <Input name="name" required minLength={3} placeholder={`Budget opérationnel FY${year}`} />
              </Field>
            </div>
            <Field label="Année budgétaire (année de fin)" hint="Ex. FY2027 = 01/09/2026 → 30/08/2027">
              <Input
                name="year"
                type="number"
                inputMode="numeric"
