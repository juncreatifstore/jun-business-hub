    ...
    <div className="relative mx-auto max-w-2xl bg-white p-10 print:p-0">
      {meta.status === "VOID" ? (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center overflow-hidden">
          <div className="-rotate-12 text-8xl font-black tracking-widest text-red-500/15">VOID</div>
        </div>
      ) : null}
      <div className="relative">
        <div className="flex items-start justify-between border-b-2 border-night pb-6">
          ...
          <div className="text-right">
            ...
            <p
              className={`mt-1 text-xs font-semibold ${meta.status === "VOID" ? "text-red-700" : "text-emerald-700"}`}
            >
              {meta.status}
            </p>
          </div>
        ...
        {meta.status === "VOID" ? (
          <div className="mt-6 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800">
            <strong>This receipt is void.</strong>
            {meta.voidReason ? ` Reason: ${meta.voidReason}` : ""}
          </div>
        ) : null}
