...
export default async function RefundsPage({searchParams}:{searchParams?:Params}){
  ...
  const params=searchParams??{}; const q=String(params.q||"").trim(); const status=STATUSES.includes(String(params.status))?String(params.status):"ALL"; const requestedPage=Math.max(1,Number.parseInt(params.page??"1",10)||1);
  const where:any={...(status!=="ALL"?{status}:{}),...(q?{OR:[{refundNumber:{contains:q,mode:"insensitive"}},{reason:{contains:q,mode:"insensitive"}},{client:{firstName:{contains:q,mode:"insensitive"}}},{client:{lastName:{contains:q,mode:"insensitive"}}},{client:{internalId:{contains:q,mode:"insensitive"}}},{payment:{reference:{contains:q,mode:"insensitive"}}}]}:{})};
  const [allRefunds,total]=await Promise.all([
    prisma.refund.findMany({orderBy:{createdAt:"desc"},take:500,include:{installments:{orderBy:{dueDate:"asc"}}}}),
    prisma.refund.count({where}),
  ]);
  const totalPages=Math.max(1,Math.ceil(total/PAGE_SIZE)); const page=Math.min(requestedPage,totalPages);
  const refunds=await prisma.refund.findMany({where,orderBy:{createdAt:"desc"},skip:(page-1)*PAGE_SIZE,take:PAGE_SIZE,include:{client:true,payment:{select:{reference:true}},installments:{orderBy:{dueDate:"asc"}},files:{where:{archivedAt:null},select:{id:true}}}});
  ...
  const overdue=allRefunds.reduce((sum,r)=>sum+r.installments.filter(i=>i.status==="LATE").length,0);
  const activeStatuses=new Set(["REQUESTED","UNDER_REVIEW","APPROVED","PARTIALLY_PAID"]);
  const activeByPayment=new Map<string,number>();
  for(const r of allRefunds)if(r.paymentId&&activeStatuses.has(r.status))activeByPayment.set(r.paymentId,(activeByPayment.get(r.paymentId)||0)+1);
  ...
  return <div>
    ...
    <form className="mb-4 grid gap-2 rounded-2xl border border-line bg-night-soft/45 p-3 shadow-sm md:grid-cols-[minmax(240px,1fr)_200px_auto]">
      <div className="relative"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-muted2"/><Input name="q" defaultValue={q} placeholder="Référence, client, paiement, motif…" className="pl-9"/></div>
      ...
      <div className="flex gap-2"><Button variant="outline">Appliquer</Button>{(q||status!=="ALL")?<Link href="/app/finance/refunds"><Button type="button" variant="ghost">Réinitialiser</Button></Link>:null}</div>
    </form>

    {refunds.length===0?<EmptyState icon={Undo2} title="Aucun remboursement correspondant" description="Modifiez les filtres ou créez une nouvelle demande de remboursement." actionHref="/app/finance/refunds/new" actionLabel="Créer une demande"/>:<>
      <div className="mb-3"><ListCount shown={refunds.length} total={total} label="remboursement" /></div>
      <div className="hidden md:block"><Table>
        ...
        <tbody>{refunds.map(r=>{const paid=refundPaidTotal(r.installments);const remaining=refundRemaining(r.amount,r.installments);const open=r.installments.filter(i=>!["PAID","CANCELLED"].includes(i.status));const next=open[0]||null;const late=r.installments.filter(i=>i.status==="LATE").length;const possibleDuplicate=Boolean(r.paymentId&&activeStatuses.has(r.status)&&(activeByPayment.get(r.paymentId)||0)>1);const nextLabel=r.status==="PAID"?"Terminé":r.status==="CANCELLED"?"Annulé":r.status==="REJECTED"?"Rejeté":"Aucun échéancier";return <TR key={r.id}>
          ...
          <TD><Link href={`/app/clients/${r.clientId}/dashboard`} className="font-medium hover:text-electric">{r.client.firstName} {r.client.lastName}</Link></TD>
      ...
      <div className="grid gap-3 md:hidden">{refunds.map(r=>{const paid=refundPaidTotal(r.installments);const remaining=refundRemaining(r.amount,r.installments);const open=r.installments.filter(i=>!["PAID","CANCELLED"].includes(i.status));const next=open[0]||null;const late=r.installments.filter(i=>i.status==="LATE").length;const possibleDuplicate=Boolean(r.paymentId&&activeStatuses.has(r.status)&&(activeByPayment.get(r.paymentId)||0)>1);return <RecordCard key={r.id} href={`/app/finance/refunds/${r.id}`} title={`${r.client.firstName} ${r.client.lastName}`} subtitle={<span className="registry-id">{r.refundNumber}</span>} badges={<><StatusBadge status={r.status}/>{r.payment?<Badge className="border border-white/[0.07] bg-white/[0.03] text-slate-400">PAIEMENT LIÉ</Badge>:<Badge className="border border-blue-500/20 bg-blue-500/10 text-blue-300">SOLDE GLOBAL</Badge>}{possibleDuplicate?<Badge className="border border-red-500/20 bg-red-500/10 text-red-300">DOUBLON POSSIBLE</Badge>:null}</>} footer={`Créé ${formatDate(r.createdAt)}`}>
          <RecordField label="Demandé" value={formatMoney(Number(r.amount),r.currency)}/><RecordField label="Payé" value={formatMoney(paid,r.currency)} valueClassName="text-emerald-300"/><RecordField label="Restant" value={formatMoney(remaining,r.currency)} valueClassName={remaining>0?"text-amber-300":"text-emerald-300"}/><RecordField label="Prochaine échéance" value={next?`${formatDate(next.dueDate)}${late?` · ${late} retard`:""}`:"—"} valueClassName={late?"text-amber-300":undefined}/>
        </RecordCard>})}</div>
      <Pagination basePath="/app/finance/refunds" page={page} totalPages={totalPages} params={paginationParams}/>
    </>}
  </div>;
}
