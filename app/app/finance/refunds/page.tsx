...
export default async function RefundsPage() {
  ...
  const unlinkedActive = refunds.filter((r) => !r.paymentId && activeStatuses.has(r.status)).length;
  ...
  return <div>
    ...
    {(duplicateGroups > 0 || unlinkedActive > 0) ? <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm"><div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-700"/><div><div className="font-medium text-amber-900">Refund reconciliation attention required</div><div className="mt-1 text-amber-800">{duplicateGroups > 0 ? `${duplicateGroups} original payment(s) have more than one active refund request. Review them before any payout. ` : ""}{unlinkedActive > 0 ? `${unlinkedActive} active refund request(s) are not linked to an original payment.` : ""}</div><div className="mt-1 text-xs text-amber-700">Cancelled and rejected requests do not count toward active duplicate warnings.</div></div></div></div> : null}
    {refunds.length === 0 ? <EmptyState icon={Undo2} title="No refunds" description="When a refund is requested it will move through review and approval here." actionHref="/app/finance/refunds/new" actionLabel="Create refund request" /> : <Table>
      ...
      <tbody>{refunds.map((r) => {
        ...
        const activeUnlinked = Boolean(!r.paymentId && activeStatuses.has(r.status));
        ...
        return <TR key={r.id}>
          ...
          <TD>{r.payment ? <span className="registry-id">{r.payment.reference}</span> : <div><span className="text-muted2">Unlinked</span>{activeUnlinked ? <div className="mt-1"><Badge className="border border-amber-200 bg-amber-50 text-amber-800">VERIFY LINK</Badge></div> : null}</div>}</TD>
