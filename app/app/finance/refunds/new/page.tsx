...
export default async function NewRefundPage(props: {
  ...
  const refundables = payments.map((p) => {
    ...
    return {
      ...
      available: Math.max(0, Math.round((Number(p.amount) - committed) * 100) / 100),
      currency: p.currency,
      clientId: p.clientId,
    };
  });
  return (
    <div className="min-w-0">
      <PageHeader
        eyebrow="Finance · Remboursements"
        title="Nouvelle demande de remboursement"
        subtitle="Créez une demande contrôlée et rattachez-la au paiement original chaque fois que cela est nécessaire pour le rapprochement financier."
      />
      <RefundForm
        clients={clients}
        cases={cases}
