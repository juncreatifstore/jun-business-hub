...
import { decideRefund, updateRefundReview } from "@/services/refunds";
...
export default async function RefundDetailPage({ params }: { params: { id: string } }) {
  ...
  return <div className="max-w-5xl">
    ...
    <Card className="mt-4"><CardHeader><CardTitle>Refund installments & scheduling</CardTitle></CardHeader><CardContent><RefundInstallmentSchedule refundId={r.id} clientId={r.clientId} caseId={r.caseId} currency={r.currency} refundStatus={r.status} installments={r.installments} files={r.files.map((f) => ({ id: f.id, name: f.name }))} approver={approver} /></CardContent></Card>
