...
export default async function ClientStatementPage({ params, searchParams }: { params: { id: string }; searchParams: { lang?: string } }) {
  ...
  return <div className="mx-auto max-w-6xl bg-white p-6">
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><Link href={`/app/clients/${client.id}/account`}><Button variant="outline">← Back / Retour</Button></Link><div className="flex gap-2">{(["FR","EN","ES","HT"] as ClientStatementLanguage[]).map((l) => <Link key={l} href={`/app/clients/${client.id}/statement?lang=${l}`} className={`rounded-lg border px-3 py-2 text-xs ${language===l?"border-electric text-electric":"border-line"}`}>{l}</Link>)}<a href={`/api/clients/${client.id}/statement.pdf?lang=${language}`} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-lg bg-ink px-3 py-2 text-xs font-medium text-white">Open PDF</a></div></div>

    <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">The official PDF is generated by the Finance PDF engine. It no longer uses browser printing, so navigation bars, browser URLs and clipped tables are excluded.</div>
