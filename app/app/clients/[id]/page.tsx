...
const TABS = ["overview", "cases", "documents", "contracts", "payments", "refunds", "emails", "notes", "activity"] as const;
...
export default async function ClientDetailPage({
  ...
  const totalPaid = client.payments
    .filter((p) => p.status === "CONFIRMED" || p.status === "PARTIALLY_REFUNDED")
    .reduce((s, p) => s + Number(p.amount), 0);
  const totalRefunded = client.refunds
    .filter((r) => r.status === "PAID" || r.status === "PARTIALLY_PAID")
    .reduce((s, r) => s + Number(r.amount), 0);
  const balance = totalPaid - totalRefunded;
  const contracts = client.documents.filter((d) => d.type === "CONTRACT" || d.type === "AGREEMENT" || d.type === "REFUND_AGREEMENT");
    ...
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="registry-id mt-1 text-muted2">{client.internalId} · since {formatDate(client.createdAt)}</p>
          <div className="mt-2 flex flex-wrap gap-1">
            {client.tags.map((t) => (
              <Badge key={t.id} className="border border-line bg-surface text-muted2">{t.tag}</Badge>
            ))}
          </div>
        </div>
        <div className="flex gap-2">
          {can(user, "CLIENT_UPDATE") ? (
            <Link href={`/app/clients/${client.id}/edit`}><Button variant="outline">Edit</Button></Link>
          ) : null}
          {can(user, "CLIENT_ARCHIVE") && client.status !== "ARCHIVED" ? (
            <form action={archiveAction}><Button variant="ghost" className="text-red-600">Archive</Button></form>
          ) : null}
        </div>
      </div>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {[
          ...
          { label: "Total refunded", value: formatMoney(totalRefunded) },
          ...
          <Card key={m.label}><CardContent className="p-4">
            <p className="text-xs text-muted2">{m.label}</p>
      ...
      <div className="mb-5 flex gap-1 overflow-x-auto border-b border-line">
        {TABS.map((t) => (
          <Link
            key={t}
            href={`/app/clients/${client.id}?tab=${t}`}
            className={cn(
              "whitespace-nowrap border-b-2 px-3 py-2 text-sm capitalize",
              tab === t ? "border-electric font-medium text-electric" : "border-transparent text-muted2 hover:text-ink"
            )}
          >
            {t}
          </Link>
      ...
      {tab === "overview" ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <Card>
            <CardHeader><CardTitle>Contact & identity</CardTitle></CardHeader>
            <CardContent>
              <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                {[
                  ["Email", client.email], ["Phone", client.phone], ["WhatsApp", client.whatsapp],
                  ["Country", client.country], ["Nationality", client.nationality],
                  ["Birth date", client.birthDate ? formatDate(client.birthDate) : null],
                  ["Address", client.address],
                  ["Owner", client.owner ? `${client.owner.firstName} ${client.owner.lastName}` : null],
                ].map(([k, v]) => (
                  <div key={k as string}>
                    <dt className="text-xs text-muted2">{k}</dt>
              ...
              {client.notes ? (
                <div className="mt-4 rounded-lg bg-surface p-3 text-sm">
                  <p className="text-xs text-muted2">Notes</p>
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {client.activities.length === 0 ? (
                <p className="p-5 text-sm text-muted2">No activity recorded yet for this client.</p>
              ) : (
      ...
      {tab === "cases" ? (
        client.cases.length === 0 ? (
          <p className="text-sm text-muted2">No cases for this client. <Link className="text-electric hover:underline" href={`/app/cases/new?clientId=${client.id}`}>Open the first case</Link>.</p>
        ) : (
          <Table>
            <THead><tr><TH>Case</TH><TH>Title</TH><TH>Status</TH><TH>Priority</TH><TH>Owner</TH><TH>Due</TH></tr></THead>
            <tbody>
              {client.cases.map((c) => (
                <TR key={c.id}>
      ...
      {tab === "documents" ? (
        client.files.length === 0 && client.documents.length === 0 ? (
          <p className="text-sm text-muted2">No documents or files linked to this client yet.</p>
        ) : (
          <div className="space-y-6">
            {client.documents.length > 0 ? (
              <Table>
                <THead><tr><TH>Registry ID</TH><TH>Title</TH><TH>Type</TH><TH>Status</TH><TH>Updated</TH></tr></THead>
                <tbody>
            ...
            {client.files.length > 0 ? (
              <Table>
                ...
                <tbody>
                  {client.files.map((f) => (
                    <TR key={f.id}>
                      <TD><Link href={`/api/files/${f.id}`} className="font-medium hover:text-electric">{f.name}</Link></TD>
                      <TD className="text-muted2">{f.category.replaceAll("_", " ")}</TD>
                      <TD className="text-muted2">{(f.sizeBytes / 1024).toFixed(0)} KB</TD>
      ...
      {tab === "contracts" ? (
        contracts.length === 0 ? (
          <p className="text-sm text-muted2">No contracts yet. <Link className="text-electric hover:underline" href={`/app/documents/new?clientId=${client.id}&type=CONTRACT`}>Draft one</Link>.</p>
        ) : (
          <Table>
            <THead><tr><TH>Registry ID</TH><TH>Title</TH><TH>Status</TH><TH>Updated</TH></tr></THead>
            <tbody>
              {contracts.map((d) => (
                <TR key={d.id}>
                  <TD><span className="registry-id">{d.documentId}</span></TD>
      ...
      {tab === "payments" ? (
        client.payments.length === 0 ? (
          <p className="text-sm text-muted2">No payments recorded. <Link className="text-electric hover:underline" href={`/app/finance/payments/new?clientId=${client.id}`}>Record a payment</Link>.</p>
        ) : (
          <Table>
      ...
      {tab === "refunds" ? (
        client.refunds.length === 0 ? (
          <p className="text-sm text-muted2">No refunds for this client.</p>
        ) : (
          <Table>
            <THead><tr><TH>Reference</TH><TH>Amount</TH><TH>Reason</TH><TH>Status</TH><TH>Created</TH></tr></THead>
            <tbody>
        ...
        <div className="max-w-2xl space-y-4">
          {can(user, "CLIENT_UPDATE") ? (
            <form action={noteAction} className="space-y-2">
              <Textarea name="body" placeholder="Add a note visible to the team…" rows={3} required maxLength={5000} />
      ...
      {tab === "activity" ? (
        client.activities.length === 0 ? (
          <p className="text-sm text-muted2">Nothing recorded yet.</p>
        ) : (
          <ul className="max-w-2xl divide-y divide-line rounded-xl border border-line bg-white">
