...
import { BookOpen, CircleDollarSign, FilePlus2, ReceiptText, RotateCcw, Send, Sparkles } from "lucide-react";
...
export default async function DriveIntelligenceActionsPage({ params, searchParams }: { params: { id: string }; searchParams: { success?: string; error?: string } }) {
  ...
  const ctx = await getDriveActionContext(params.id); if (!ctx) notFound();
  const [clients, cases, payments, receivers] = await Promise.all([
    prisma.client.findMany({ where: { archivedAt: null }, orderBy: [{ firstName: "asc" }, { lastName: "asc" }], take: 300, select: { id: true, firstName: true, lastName: true, internalId: true } }),
    prisma.case.findMany({ where: { status: { not: "ARCHIVED" } }, orderBy: { createdAt: "desc" }, take: 300, select: { id: true, caseNumber: true, title: true, clientId: true } }),
    prisma.payment.findMany({ where: { status: { in: ["CONFIRMED","PARTIALLY_REFUNDED","REFUNDED"] } }, orderBy: { createdAt: "desc" }, take: 200, select: { id: true, reference: true, clientId: true, amount: true, currency: true, status: true } }),
    getManualTransferReceivers(true),
  ]);
  const s = ctx.structured;
  const extractedAmount = Number(s.transactionAmount || s.totalAmount || 0);
  const currency = String(s.currency || ctx.file.payment?.currency || "USD").toUpperCase();
  ...
  return <div className="space-y-5">
    ...
    <Card><CardHeader><CardTitle>Extracted data</CardTitle></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4 text-sm">
