    ...
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted2">
          Create a new DRAFT from multiple JUN documents. Source documents stay unchanged.
        </p>
      </div>
      ...
      <form action={combineDocuments} className="max-w-6xl">
        ...
        <div className="mb-4 rounded-xl border border-line bg-white p-4">
          ...
          <p className="mt-2 text-xs text-muted2">
            Documents are combined in the order shown below. You can reorganize individual pages afterward.
          </p>
        </div>

        <div className="overflow-hidden rounded-xl border border-line bg-white">
          <div className="grid grid-cols-[44px_minmax(0,1fr)_120px_120px_110px] border-b border-line bg-surface px-3 py-3 text-xs font-semibold text-muted2">
            <span />
            <span>Document</span>
            <span>Pages</span>
            <span>Status</span>
          ...
          {docs.map((doc) => {
            ...
            return (
              <label
                key={doc.id}
                className="grid cursor-pointer grid-cols-[44px_minmax(0,1fr)_120px_120px_110px] items-center border-b border-line px-3 py-3 last:border-0 hover:bg-surface/70"
              >
                <span className="flex items-center gap-2">
                  <input type="checkbox" name="sourceIds" value={doc.id} className="h-4 w-4" />
                  <GripVertical className="h-4 w-4 text-muted2" />
                </span>
                <span className="min-w-0">
                  <span className="block truncate text-sm font-semibold">{doc.title}</span>
                  <span className="registry-id text-[10px] text-muted2">{doc.documentId}</span>
                </span>
