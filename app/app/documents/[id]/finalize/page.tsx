...
export default async function FinalizeDocumentPage({ params, searchParams }: { params: { id: string }; searchParams?: { error?: string } }) {
  ...
  return (
    <div className="space-y-5">
      <div><Link href={`/app/documents/${doc.id}`} className="inline-flex items-center gap-2 text-sm font-medium text-muted2 hover:text-electric"><ArrowLeft className="h-4 w-4" /> Back to document</Link></div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">Version {latest.version} · content SHA-256 {contentHash.slice(0, 16)}…</div>
      </div>

      {searchParams?.error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{searchParams.error}</div> : null}

      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <Card>
          <CardHeader className="flex-row items-center justify-between gap-4">
            <div><CardTitle>PDF preview</CardTitle><p className="mt-1 text-xs text-muted2">This preview is generated from the current draft version.</p></div>
            <a href={`/api/documents/${doc.id}/pdf`} target="_blank" rel="noreferrer"><Button variant="outline">Open PDF</Button></a>
          </CardHeader>
          <CardContent><iframe title="Finalization preview" src={`/api/documents/${doc.id}/pdf`} className="h-[760px] w-full rounded-xl border border-line bg-surface" /></CardContent>
        </Card>
        ...
        <aside className="space-y-4">
          ...
          <Card className="border-amber-200">
            ...
            <CardContent className="space-y-3 text-sm text-muted2">
              ...
              <p className="flex gap-2"><FileCheck2 className="mt-0.5 h-4 w-4 shrink-0" /> Editing a FINAL document requires a new revision. The sealed PDF remains preserved in storage and audit history.</p>
            </CardContent>
          </Card>
