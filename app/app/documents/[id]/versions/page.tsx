...
export default async function DocumentVersionsPage({
  ...
  const left =
    versions.find((v) => v.id === searchParams?.left) ??
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted2">
            Compare immutable snapshots without changing document history.
          </p>
        </div>
      </div>

      <form
        className="grid gap-3 rounded-xl border border-line bg-white p-4 sm:grid-cols-[1fr_1fr_auto]"
        method="get"
      >
        <label className="text-sm font-medium">
          ...
          <select
            ...
            className="mt-1 block h-10 w-full rounded-lg border border-line bg-white px-3 text-sm"
          >
            {versions.map((v) => (
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <div className="rounded-xl border border-line bg-white p-4">
          <p className="text-xs uppercase tracking-wider text-muted2">Left words</p>
          <p className="mt-1 text-2xl font-semibold">{leftWords.length}</p>
        </div>
        <div className="rounded-xl border border-line bg-white p-4">
          <p className="text-xs uppercase tracking-wider text-muted2">Right words</p>
          <p className="mt-1 text-2xl font-semibold">{rightWords.length}</p>
        </div>
        <div className="rounded-xl border border-line bg-white p-4">
          <p className="text-xs uppercase tracking-wider text-muted2">Approx. added</p>
          <p className="mt-1 text-2xl font-semibold text-emerald-600">+{added}</p>
        </div>
        <div className="rounded-xl border border-line bg-white p-4">
          <p className="text-xs uppercase tracking-wider text-muted2">Approx. removed</p>
          <p className="mt-1 text-2xl font-semibold text-red-600">-{removed}</p>
        </div>
      </div>
          ...
          <section key={version.id} className="rounded-xl border border-line bg-white">
            <header className="border-b border-line px-4 py-3">
              <p className="font-semibold">
                {index === 0 ? "Left" : "Right"}: Version {version.version}
              </p>
              <p className="text-xs text-muted2">
                {version.author.firstName} {version.author.lastName} · {version.createdAt.toLocaleString()} ·{" "}
                {version.changeNote ?? "No change note"}
              </p>
            </header>
            <div
              className="doc-prose max-h-[700px] overflow-auto p-5"
