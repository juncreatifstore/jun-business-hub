...
export default async function DocumentDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  const canRefreshFromData =
    can(user, "DOCUMENT_EDIT") && Boolean(doc.client) && ["DRAFT", "FINAL"].includes(doc.status);
  ...
  return (
    <div>
      <DocumentWorkspaceHeader
        ...
        client={
          doc.client ? { id: doc.client.id, name: `${doc.client.firstName} ${doc.client.lastName}` } : null
        }
        ...
        actions={
          <>
            ...
            {canRefreshFromData ? (
              <DocumentRefreshControl documentId={doc.id} variant="outline" showResult={true} />
            ) : null}
            {canEditDraft ? (
              <Link href={`/app/documents/${doc.id}/finalize`}>
                <Button variant="gold">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_350px]">
        <div className="min-w-0 space-y-4">
          {sealed ? (
            <div className="rounded-2xl border border-emerald-400/15 bg-emerald-500/[0.045] p-4 text-sm text-emerald-200">
              <p className="flex items-center gap-2 font-semibold">
                <ShieldCheck className="h-4 w-4" />
                Document officiel scellé
              </p>
              <p className="mt-1 text-xs leading-5 text-emerald-200/70">
                Le PDF officiel et ses empreintes d’intégrité sont conservés. Toute modification nécessite une
                nouvelle révision afin de préserver l’historique vérifiable.
              </p>
            </div>
          ) : null}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted2">
                    Version {latest?.version ?? 1} · {canEditDraft ? "modifiable" : "lecture seule"}
                  </p>
                </div>
                <Link href={`/app/documents/${doc.id}/fill`}>
        ...
        <aside className="space-y-4 xl:sticky xl:top-[92px] xl:self-start">
            ...
            <Card className="border-blue-400/15 bg-blue-500/[0.035]">
              ...
              <CardContent>
                <p className="mb-3 text-xs leading-5 text-muted2">
                  Après l’enregistrement tardif d’un paiement, remboursement ou autre correction, recréez une
                  version depuis les données autoritaires les plus récentes.
                </p>
                <DocumentRefreshControl
                  documentId={doc.id}
                  variant="gold"
                  fullWidth={true}
                  showResult={true}
          ...
          {canCreateRevision ? (
            <Card>
              <CardHeader>
                <CardTitle>Créer une révision</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-xs leading-5 text-muted2">
                  Le PDF FINAL actuel reste intact. Une nouvelle version DRAFT est créée pour les
                  modifications.
                </p>
                <form action={createDocumentRevision.bind(null, doc.id)} className="mt-3 space-y-2">
                  <Input name="reason" required maxLength={300} placeholder="Motif de la révision" />
          ...
          <Card>
            ...
            <CardContent className="grid gap-2">
              ...
              {canRequestSignature ? (
                <form action={sendClientSignatureViaWhatsApp.bind(null, doc.id)}>
                  ...
                  >
                    ...
                    Signature client via WhatsApp
                  </Button>
                </form>
              ) : null}
              {canRequestSignature ? (
                <form action={createSignatureRequest.bind(null, doc.id)}>
                  <Button className="w-full justify-start" variant="primary" type="submit">
                    Autre méthode de signature
                  </Button>
                </form>
              ) : null}
              <Link href={`/app/documents/${doc.id}/pages`}>
                <Button className="w-full justify-start" variant="outline">
              ...
              {can(user, "DOCUMENT_CREATE") ? (
                <Link href={`/app/documents/${doc.id}/split`}>
                  <Button className="w-full justify-start" variant="outline">
              ...
              <Link href={`/app/documents/${doc.id}/versions`}>
                <Button className="w-full justify-start" variant="outline">
                  <GitCompareArrows className="mr-2 h-4 w-4" />
                  Versions & comparaison
                </Button>
              </Link>
              <Link href={`/app/documents/${doc.id}/print`} target="_blank">
                <Button className="w-full justify-start" variant="outline">
          ...
          <Card className={sealed ? "border-emerald-400/15" : undefined}>
            ...
            <CardContent className="space-y-3 text-xs">
              ...
              <div className="flex items-start gap-2 text-muted2">
                ...
                <span>
                  {doc.finalHash
                    ? `Contenu SHA-256 ${shortHash(doc.finalHash)}`
                    : "Empreinte de contenu créée lors de la finalisation"}
                </span>
              </div>
              <div className="flex items-start gap-2 text-muted2">
                ...
                <span>
                  {doc.finalPdfHash
                    ? `PDF SHA-256 ${shortHash(doc.finalPdfHash)}`
                    : "Empreinte PDF créée lors de la finalisation"}
                </span>
              </div>
              <p className="leading-5 text-muted2">
                Le QR code, l’identifiant du document et l’empreinte en ligne servent à authentifier le
                document JUN sans exiger la signature manuscrite du responsable de l’entreprise.
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between gap-2">
                ...
                <Link
                  href={`/app/documents/${doc.id}/versions`}
              ...
              <ul className="max-h-[360px] divide-y divide-line overflow-y-auto">
                {doc.versions.map((v, index) => (
                  <li key={v.id} className="px-4 py-3">
                    ...
                    <p className="mt-1 text-xs text-muted2">
                      {v.author.firstName} {v.author.lastName} · {formatDateTime(v.createdAt)}
                    </p>
                    {v.changeNote ? (
                      <p className="mt-1 text-xs leading-5 text-muted2">{v.changeNote}</p>
                    ) : null}
                    <p className="registry-id mt-1 text-[10px] text-muted2">{shortHash(v.hash)}</p>
                    {canEditDraft && index > 0 ? (
                      <form action={restoreDocumentVersion.bind(null, doc.id, v.id)} className="mt-2">
                ...
                <ul className="divide-y divide-line">
                  {doc.signatures.map((s) => (
                    <li key={s.id} className="px-4 py-3">
                      ...
                      <p className="mt-1 text-xs text-muted2">
                        {recipientCount(s.recipients)} signataire(s) · {formatDateTime(s.createdAt)}
                      </p>
                    </li>
                  ))}
                </ul>
