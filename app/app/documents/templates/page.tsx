...
export default async function TemplateLibraryPage({
  ...
  const filtered = rows.filter((t) => {
    if (
      q &&
      !`${t.name} ${t.description ?? ""} ${t.type} ${categoryLabels.get(t.category) ?? t.category}`
        .toLowerCase()
        .includes(q)
    )
      return false;
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Reusable JUN document templates, dynamic variables, and the administrative reference catalog."
      />
      <div className="mb-5 flex flex-wrap gap-2">
        <Link href="/app/documents">
          <Button variant="outline">Back to Documents</Button>
        </Link>
        <Link href="/app/documents/templates/new">
          <Button variant="primary">New template</Button>
        </Link>
        <Link href="/app/documents/templates/import">
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          <CardContent className="flex items-center gap-3 p-4">
            <Library className="h-5 w-5 text-electric" />
            <div>
              <p className="text-2xl font-semibold">{active}</p>
              <p className="text-xs text-muted2">Active templates</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="flex items-center gap-3 p-4">
            <FileStack className="h-5 w-5 text-electric" />
            <div>
              <p className="text-2xl font-semibold">{editable}</p>
              <p className="text-xs text-muted2">Editable templates</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted2">Reference catalog</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-2xl font-semibold">{variableTemplates}</p>
              <p className="text-xs text-muted2">With variables</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <form className="mb-5 grid gap-3 rounded-xl border border-line bg-white p-4 lg:grid-cols-[minmax(260px,1fr)_260px_140px_160px_auto]">
        <div className="relative">
          ...
          <input
            name="q"
            defaultValue={q}
            placeholder="Search template, type, category…"
            className="h-10 w-full rounded-lg border border-line pl-9 pr-3 text-sm outline-none focus:border-electric"
          />
        </div>
        <select
          name="category"
        ...
        <select name="state" defaultValue={state} className="h-10 rounded-lg border border-line px-3 text-sm">
      ...
      <div className="mb-3 flex flex-wrap gap-2 text-xs">
        {[
          ...
          <Link
            key={key}
            href={hrefFor({ state: key })}
            className={`rounded-full border px-3 py-1.5 ${state === key ? "border-electric bg-electric/10 text-electric" : "border-line text-muted2 hover:bg-surface"}`}
          >
            {label}
          </Link>
        ))}
      ...
      <Table>
        ...
        <tbody>
          {filtered.map((t) => (
            <TR key={t.id}>
              <TD>
                <Link href={`/app/documents/templates/${t.id}`} className="font-medium hover:text-electric">
                  {t.name}
                </Link>
                {t.description ? (
                  <p className="mt-0.5 max-w-md truncate text-xs text-muted2">{t.description}</p>
                ) : null}
              </TD>
              ...
              <TD>
                {t.isReference ? (
                  <span className="rounded-full bg-amber-50 px-2 py-1 text-xs text-amber-700">REFERENCE</span>
                ) : t.isActive ? (
