...
export default async function NewDocumentPage({
  ...
  const [clients, cases, templates] = await Promise.all([
    ...
    prisma.documentTemplate.findMany({ orderBy: { name: "asc" }, select: { id: true, name: true, type: true } }),
  ]);
  return (
    <div>
      <PageHeader title="New document" subtitle="Start blank, from a template, or write it with JUN AI. A registry ID is assigned on creation." />
      <DocumentCreateForm
        clients={clients}
        cases={cases}
        templates={templates}
