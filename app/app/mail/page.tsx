...
import { GmailStyleMailCenterV2 } from "@/components/mail/gmail-style-mail-center-v2";
...
export default async function MailPage({searchParams}:{searchParams:{folder?:string;thread?:string;compose?:string;q?:string;mailbox?:string;source?:string;mode?:string;category?:string}}){
 ...
 return <div className="space-y-4">
  <GmailStyleMailCenterV2 searchParams={{...searchParams,mailbox}}/>
  ...
  {active?<MailOperationsPanel threadId={active.id}/>:null}{active?<MailIntelligencePanel threadId={active.id}/>:null}{active?<MailAICopilotPanel threadId={active.id} hasDraft={Boolean(active.aiDraft)}/>:null}{active?<MailContextPanel threadId={active.id}/>:null}
 </div>;
}
