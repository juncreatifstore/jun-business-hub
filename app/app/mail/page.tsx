...
import { GmailStyleMailCenter } from "@/components/mail/gmail-style-mail-center";
...
export default async function MailPage({searchParams}:{searchParams:{folder?:string;thread?:string;compose?:string;q?:string;mailbox?:string;source?:string;mode?:string}}){
 ...
 if(searchParams.compose==="1"){
  ...
  if(searchParams.mailbox&&accessibleIds.includes(searchParams.mailbox))p.set("mailbox",searchParams.mailbox);
 ...
 if(searchParams.folder==="DRAFTS"&&searchParams.thread){
  ...
  if(meta){const t=await prisma.mailThread.findFirst({where:{id:searchParams.thread,mailAccountId:{in:accessibleIds}},select:{mailAccountId:true,aiDraft:true}});if(t?.aiDraft)redirect(`/app/mail/compose?mailbox=${encodeURIComponent(t.mailAccountId)}&draft=${searchParams.thread}`);}
 }
 ...
 return <div className="space-y-4">
  <GmailStyleMailCenter searchParams={{...searchParams,mailbox}}/>
  <div className="flex flex-wrap items-center gap-2 rounded-xl border border-line bg-white px-3 py-2">
   <span className="mr-1 text-xs font-medium uppercase tracking-wide text-muted2">JUN tools</span>
   <Link href="/app/mail/search"><Button size="sm" variant="ghost">Search</Button></Link>
   <Link href="/app/mail/analytics"><Button size="sm" variant="ghost">Analytics</Button></Link>
   <Link href="/app/mail/operations"><Button size="sm" variant="ghost">Operations & SLA</Button></Link>
   <Link href="/app/mail/intelligence"><Button size="sm" variant="ghost">Intelligence</Button></Link>
   <Link href="/app/mail/approvals"><Button size="sm" variant="ghost">AI approvals</Button></Link>
   {can(user,"EMAIL_MANAGE")?<Link href="/app/mail/security"><Button size="sm" variant="ghost">Security</Button></Link>:null}
  </div>
