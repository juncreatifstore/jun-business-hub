...
import { GmailStyleMailCenterV6 } from "@/components/mail/gmail-style-mail-center-v6";
...
export default async function MailPage({searchParams}:{searchParams:{folder?:string;thread?:string;compose?:string;q?:string;mailbox?:string;source?:string;mode?:string;category?:string}}){
 ...
 return <div className="space-y-3">
  ...
  <div className="flex flex-wrap items-center gap-2 rounded-xl border border-white/[0.07] bg-[#0f1929] px-3 py-2"><span className="mr-1 text-xs font-medium uppercase tracking-wide text-slate-500">JUN tools</span><Link href="/app/mail/search"><Button size="sm" variant="ghost">Search</Button></Link><Link href="/app/mail/analytics"><Button size="sm" variant="ghost">Analytics</Button></Link><Link href="/app/mail/operations"><Button size="sm" variant="ghost">Operations & SLA</Button></Link><Link href="/app/mail/intelligence"><Button size="sm" variant="ghost">Intelligence</Button></Link><Link href="/app/mail/approvals"><Button size="sm" variant="ghost">AI approvals</Button></Link>{can(user,"EMAIL_MANAGE")?<Link href="/app/mail/security"><Button size="sm" variant="ghost">Security</Button></Link>:null}</div>
 </div>;
}
