...
export default async function SecurityPage(props: { searchParams: Promise<{ setup?: string }> }) {
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Smartphone className="h-4 w-4" /> Authentification à deux facteurs (TOTP)
            </CardTitle>
          </CardHeader>
            ...
            {row?.mfaEnabled ? (
              ...
              <>
                <p className="text-sm text-muted2">
                  Scannez ce QR avec Google Authenticator, 1Password ou Authy, puis saisissez le code à 6
                  chiffres.
                </p>
              ...
              <>
                <p className="text-sm text-muted2">
                  Protégez votre compte avec un code temporaire généré par une application d’authentification.
                </p>
              ...
              {sessions.map((s) => (
                <li key={s.id} className="grid gap-3 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 break-words text-xs leading-5 text-muted2">
                      {s.ip ?? "—"} · début {formatDateTime(s.createdAt)} · expire{" "}
                      {formatDateTime(s.expiresAt)}
                      {s.tokenHash === currentHash ? " · cet appareil" : ""}
                    </p>
                  </div>
