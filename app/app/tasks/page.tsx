...
import { CheckSquare, Pencil, Clock3, UsersRound } from "lucide-react";
...
export default async function TasksPage(props: {
  ...
  for (const t of tasks) {
    const key = t.assignee ? `${t.assignee.firstName} ${t.assignee.lastName}` : "Non assignée";
  ...
  const activeCount = tasks.filter((t) => ["TODO", "IN_PROGRESS", "WAITING"].includes(t.status)).length;
  return (
    <div>
      <PageHeader
        title="Tâches"
      ...
      >
        <Link href={`/app/tasks?view=${grouped ? "list" : "assignee"}`}>
          <Button variant="outline">{grouped ? "Vue liste" : "Par responsable"}</Button>
        </Link>
      </PageHeader>
      <div className="mb-5 grid gap-3 sm:grid-cols-3">
        <TaskMetric icon={CheckSquare} label="Résultats affichés" value={tasks.length} />
      ...
      {tasks.length === 0 ? (
        ...
        <TaskList tasks={tasks} focus={searchParams.focus} canEdit={canEdit} />
      )}
    </div>
  );
}
              ...
              <p className="mt-1 text-xs text-muted2">
                ...
                {t.client ? (
                  <>
                    {" "}
                    ·{" "}
                    <Link className="hover:text-electric" href={`/app/clients/${t.client.id}`}>
