...
export function CompanyFundsMobileNav({ workQueue }: { workQueue: WorkQueue }) {
  ...
  return (
    <>
      ...
      >
        <div className="grid grid-cols-5 gap-1">
          {primary.map((item) => {
            ...
            const isCurrent = active(pathname, item.href);
            const count = countFor(item.href, workQueue);
            return (
              <Link
                key={item.href}
                href={item.href}
              ...
              >
                ...
                {count > 0 ? (
                  <span
                    className={cn(
                      "absolute right-2 top-1 inline-flex min-w-4 items-center justify-center rounded-full px-1 text-[9px] font-bold",
                      isCurrent ? "bg-white text-ink" : "bg-red-100 text-red-700",
                    )}
                  >
                    {count > 9 ? "9+" : count}
                  </span>
          ...
          <button
            ...
            aria-expanded={open}
            aria-controls="company-funds-mobile-menu"
          ...
          >
            ...
            {workQueue.total > 0 ? (
              <span
                className={cn(
                  "absolute right-2 top-1 inline-flex min-w-4 items-center justify-center rounded-full px-1 text-[9px] font-bold",
                  moreActive || open ? "bg-white text-ink" : "bg-amber-100 text-amber-800",
                )}
              >
                {workQueue.total > 9 ? "9+" : workQueue.total}
              </span>
            ) : null}
          </button>
        </div>
      ...
      {open ? (
        <div className="fixed inset-0 z-[90] bg-black/35 md:hidden" onMouseDown={() => closeMenu()}>
          <div
            ref={menuRef}
            id="company-funds-mobile-menu"
            role="dialog"
            aria-modal="true"
            aria-labelledby="company-funds-mobile-menu-title"
            aria-describedby="company-funds-mobile-menu-description"
            className="absolute inset-x-0 bottom-0 max-h-[82vh] overflow-hidden rounded-t-3xl bg-white shadow-2xl"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-line px-4 py-4">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted2">
                  Fonds de l’entreprise
                </p>
                <h2 id="company-funds-mobile-menu-title" className="mt-1 text-lg font-semibold text-ink">
                  Toutes les options
                </h2>
                <p id="company-funds-mobile-menu-description" className="sr-only">
                  Recherchez ou choisissez une section financière. Utilisez Échap pour fermer le menu.
                </p>
              </div>
              <button
                type="button"
                onClick={() => closeMenu()}
                className="rounded-xl border border-line p-2 text-muted2"
                aria-label="Fermer le menu Fonds de l’entreprise"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="border-b border-line px-3 py-3">
              <label className="relative block">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-line bg-surface/40 pl-9 pr-9 text-sm outline-none focus:border-electric"
                />
                {query ? (
                  <button
                    type="button"
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-muted2"
            ...
            <div className="overflow-y-auto px-3 py-3" style={{ maxHeight: "calc(82vh - 151px)" }}>
              {(["Pilotage", "Contrôle", "Opérations", "Sécurité"] as const).map((group) => {
                ...
                return (
                  <div key={group} className="mb-4">
                    <p className="px-2 pb-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted2">
                      {group}
                    </p>
              ...
              {!filteredItems.length ? (
                <div className="px-4 py-10 text-center text-sm text-muted2">Aucune option trouvée.</div>
              ) : null}
            </div>
          </div>
        </div>
