...
function xhrUpload(
  url: string,
  file: File,
  mode: "supabase-signed" | "google-resumable",
  mimeType: string,
...
export function FileUploadForm({
    ...
    try {
      ...
      setMessage(
        mode === "google-resumable"
          ? "Uploading directly to Google Workspace…"
          : "Uploading directly to secure storage…",
      );
      await xhrUpload(
        String(initBody.uploadUrl),
        file,
        mode,
        String(initBody.mimeType || file.type || "application/octet-stream"),
        (bytes, eventTotal) => {
          const elapsed = Math.max(0.25, (performance.now() - started) / 1000);
          setLoaded(bytes);
          setTotal(file.size || eventTotal);
          setProgress(Math.min(99, Math.round((bytes / Math.max(1, eventTotal)) * 100)));
      ...
      setMessage("Upload complete. Registering file in JUN Drive…");
      const finalize = await fetch("/api/drive/uploads/finalize", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      void fetch("/api/drive/uploads/process", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <form
      action={isVault ? action : undefined}
      onSubmit={isVault ? undefined : directSubmit}
    ...
    >
      ...
      {!isVault && status !== "idle" ? (
        <div className="md:col-span-2 lg:col-span-4 rounded-xl border border-line bg-white p-3">
          <div className="mb-2 flex items-center justify-between gap-3 text-xs">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className={status === "error" ? "truncate text-red-700" : "truncate text-ink"}>
                {message}
              </span>
            </div>
          ...
          <div className="h-2.5 overflow-hidden rounded-full bg-surface">
            <div
              className={`h-full rounded-full transition-[width] duration-200 ${status === "error" ? "bg-red-500" : "bg-electric"}`}
              style={{ width: `${progress}%` }}
            />
          </div>
          <div className="mt-2 flex flex-wrap justify-between gap-2 text-[11px] text-muted2">
            <span>
              {humanBytes(loaded)} / {humanBytes(total)}
            </span>
            <span>{speed > 0 ? `${humanBytes(speed)}/s` : "Preparing…"}</span>
          </div>
        </div>
      ) : null}
    </form>
