...
export function FinanceWorkspaceNav() {
  ...
  return <div className="relative z-30 mb-4 rounded-2xl border border-line bg-white/95 p-2 shadow-sm backdrop-blur sm:mb-5">
    <div className="flex min-w-0 items-center gap-2">
      <div className="min-w-0 flex-1 overflow-x-auto overscroll-x-contain [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="flex w-max items-center gap-1 sm:gap-2">
          {primary.map(({ href, label, icon: Icon }) => {
            ...
            return <Link key={href} href={href} className={`inline-flex shrink-0 items-center gap-1.5 rounded-xl px-2.5 py-2 text-[11px] font-semibold transition sm:gap-2 sm:px-3 sm:text-xs ${active ? "bg-electric text-white shadow-sm" : "text-muted2 hover:bg-surface hover:text-ink"}`}>
      ...
      <details className="group relative shrink-0">
        <summary className={`inline-flex cursor-pointer list-none items-center rounded-xl px-2.5 py-2 text-[11px] font-semibold sm:px-3 sm:text-xs ${secondaryActive ? "bg-electric/[0.08] text-electric" : "text-muted2 hover:bg-surface hover:text-ink"}`}>Plus</summary>
      ...
      <div className="hidden shrink-0 sm:block"><FinanceWhatsAppAction /></div>
    </div>
    <div className="mt-2 sm:hidden"><FinanceWhatsAppAction /></div>
  </div>;
}
