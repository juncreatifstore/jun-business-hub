      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <Link
            ...
            className="rounded-md p-2 text-muted2 hover:bg-surface hover:text-night"
          >
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div>
            <p className="truncate text-sm font-semibold">Manage pages</p>
      ...
      <div className="grid min-h-[720px] overflow-hidden rounded-xl border border-line bg-white lg:grid-cols-[250px_minmax(0,1fr)_230px]">
        <aside className="border-r border-line bg-white p-3">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted2">Pages</p>
            {!readOnly ? (
              <button
                type="button"
                onClick={addAfter}
                className="rounded-md p-1.5 text-muted2 hover:bg-surface"
                title="Add page"
              >
            ...
            {pages.map((page, index) => (
              ...
              >
                ...
                <div className="relative aspect-[0.72] overflow-hidden rounded border border-line bg-white shadow-sm">
                  ...
                  />
                  {page.rotation ? (
                    <span className="absolute bottom-1 right-1 rounded bg-slate-900/75 px-1.5 py-0.5 text-[9px] text-white">
                      {page.rotation}°
                    </span>
                  ) : null}
                </div>
        ...
        <main className="overflow-auto bg-surface p-5 sm:p-8">
          <div className="mx-auto flex min-h-[900px] max-w-[850px] items-start justify-center rounded-md border border-line bg-white shadow-lg">
            {current ? (
              <div
                className="doc-prose w-full origin-top p-12 text-[15px] text-slate-900"
        ...
        <aside className="border-l border-line bg-white p-4">
          ...
          <div className="mt-4 grid gap-2">
            ...
            <Button
              type="button"
              variant="outline"
              disabled={readOnly || selected >= pages.length - 1}
              onClick={() => move(selected, selected + 1)}
            >
              <ChevronDown className="mr-1.5 h-4 w-4" />
              Move down
            </Button>
            <Button type="button" variant="outline" disabled={readOnly} onClick={rotate}>
              <RotateCw className="mr-1.5 h-4 w-4" />
              Rotate 90°
            </Button>
            <Button type="button" variant="outline" disabled={readOnly} onClick={duplicate}>
              <Copy className="mr-1.5 h-4 w-4" />
              Duplicate page
            </Button>
            <Button type="button" variant="outline" disabled={readOnly} onClick={addAfter}>
          ...
          <div className="mt-5 rounded-lg border border-line bg-surface p-3 text-xs leading-5 text-muted2">
            Drag thumbnails to reorder. Saving creates a new document version so the previous layout remains
            recoverable.
          </div>
        </aside>
      </div>
