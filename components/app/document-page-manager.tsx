...
export function DocumentPageManager({
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <div className="sticky top-0 z-30 flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-2.5">
        <div className="flex min-w-0 items-center gap-3">
          <Link href={`/app/editor/${documentId}`} className="rounded-md p-2 text-slate-500 hover:bg-slate-100"><ArrowLeft className="h-4 w-4" /></Link>
          <div><p className="truncate text-sm font-semibold text-slate-900">Manage pages</p><p className="truncate text-xs text-slate-500">{title} · {pages.length} page{pages.length === 1 ? "" : "s"}</p></div>
        </div>
      ...
      <div className="grid min-h-[calc(100vh-57px)] lg:grid-cols-[250px_minmax(0,1fr)_230px]">
        <aside className="border-r border-slate-200 bg-white p-3">
          <div className="mb-3 flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Pages</p>{!readOnly ? <button type="button" onClick={addAfter} className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100" title="Add page"><Plus className="h-4 w-4" /></button> : null}</div>
          <div className="space-y-3">
            {pages.map((page, index) => (
              <div
                ...
                className={`group cursor-pointer rounded-lg border-2 p-2 transition ${selected === index ? "border-electric bg-blue-50" : "border-transparent hover:border-slate-200"}`}
              >
                <div className="mb-1 flex items-center justify-between text-[10px] text-slate-400"><span>#{index + 1}</span><GripVertical className="h-3.5 w-3.5 opacity-50" /></div>
                <div className="relative aspect-[0.72] overflow-hidden rounded border border-slate-200 bg-white shadow-sm">
        ...
        <main className="overflow-auto p-5 sm:p-8">
          <div className="mx-auto flex min-h-[900px] max-w-[850px] items-start justify-center rounded-md border border-slate-300 bg-white shadow-lg">
        ...
        <aside className="border-l border-slate-200 bg-white p-4">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Page actions</p>
          <p className="mt-2 text-sm font-semibold text-slate-800">Page {selected + 1}</p>
          <p className="text-xs text-slate-500">Rotation: {current?.rotation ?? 0}°</p>
          ...
          <div className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs leading-5 text-slate-500">Drag thumbnails to reorder. Saving creates a new document version so the previous page layout remains recoverable.</div>
        </aside>
      </div>
    </div>
  );
