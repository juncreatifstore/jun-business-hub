...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-[72px] items-center gap-2.5 border-b border-white/[0.06] bg-[#09111f]/92 px-3 text-white shadow-[0_8px_30px_rgba(0,0,0,.12)] backdrop-blur-xl sm:px-6">
      <button className="rounded-xl p-2.5 text-slate-400 transition hover:bg-white/[0.06] hover:text-white lg:hidden" onClick={onMenu} aria-label="Ouvrir le menu">
      ...
      >
        ...
        <input
          ...
          className="h-10 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] pl-10 pr-12 text-sm text-slate-100 outline-none placeholder:text-slate-600 transition focus:border-blue-500/40 focus:bg-white/[0.055] focus:ring-2 focus:ring-blue-500/10 sm:pr-16"
        />
        ...
        {createOpen ? (
          <div className="absolute right-0 mt-2 w-60 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#101827]/98 py-1.5 shadow-2xl shadow-black/45 backdrop-blur-xl">
            <div className="px-4 pb-2 pt-2">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-600">Créer rapidement</p>
            </div>
            {quickCreate.map((item) => (
              <Link key={item.href} href={item.href} onClick={() => setCreateOpen(false)} className="block px-4 py-2.5 text-sm text-slate-300 transition hover:bg-white/[0.05] hover:text-white">
                {item.label}
              </Link>
            ))}
          </div>
      ...
      <Link href="/app/notifications" className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/[0.07] bg-white/[0.025] text-slate-400 transition hover:bg-white/[0.055] hover:text-white" aria-label="Notifications">
          ...
          <div className="absolute right-0 mt-2 w-60 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#101827]/98 py-2 shadow-2xl shadow-black/45 backdrop-blur-xl">
            <div className="border-b border-white/[0.07] px-4 pb-3 pt-1">
              <div className="flex items-center gap-3">
                <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/[0.05] text-slate-300"><UserRound className="h-4 w-4" /></span>
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-slate-100">{user.firstName} {user.lastName}</p>
                  <p className="mt-0.5 truncate text-[10px] uppercase tracking-[0.13em] text-slate-500">{user.role.replaceAll("_", " ")}</p>
                </div>
              </div>
            </div>

            <Link href="/app/settings" onClick={() => setProfileOpen(false)} className="mt-1 flex items-center gap-2 px-4 py-2.5 text-sm text-slate-300 transition hover:bg-white/[0.05] hover:text-white">
