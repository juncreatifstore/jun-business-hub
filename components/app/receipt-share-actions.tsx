...
export function ReceiptShareActions({
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {active ? (
        <Button type="button" variant="outline" onClick={() => copy("pdf", pdfUrl)}>
          {copied === "pdf" ? "PDF link copied" : "Copy PDF link"}
        </Button>
      ) : null}
      <Button type="button" variant="outline" onClick={() => copy("verify", verifyUrl)}>
        {copied === "verify" ? "Verify link copied" : "Copy verification link"}
      </Button>
      <Button type="button" variant="outline" onClick={() => copy("message", message)}>
