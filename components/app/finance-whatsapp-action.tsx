...
function match(pathname: string) {
  const patterns = [
    {
      regex: /^\/app\/finance\/invoices\/([^/]+)$/,
      label: "Send Invoice by WhatsApp",
      action: sendFinanceInvoiceByWhatsApp,
    },
    {
      regex: /^\/app\/finance\/refunds\/([^/]+)$/,
      label: "Send Refund PDF by WhatsApp",
      action: sendFinanceRefundByWhatsApp,
    },
    {
      regex: /^\/app\/finance\/receipts\/([^/]+)$/,
      label: "Send Receipt by WhatsApp",
      action: sendPaymentReceiptByWhatsApp,
    },
    {
      regex: /^\/app\/finance\/manual-transfers\/([^/]+)$/,
      label: "Send Payment Order by WhatsApp",
      action: sendManualTransferOrderByWhatsApp,
    },
  ] as const;
