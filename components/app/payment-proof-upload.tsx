...
export function PaymentProofUpload({ paymentId, clientId, caseId }: { paymentId: string; clientId: string; caseId?: string | null }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setBusy(true); setState("uploading"); setProgress(0); setLoaded(0); setTotal(file.size); setMessage("Preparing proof upload…");
    try {
      const init = await fetch("/api/drive/uploads/init", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: file.name, sizeBytes: file.size, mimeType: file.type, category: "PAYMENT_PROOF", clientId, caseId: caseId || null, paymentId }),
      });
      const initBody = await init.json();
      if (!init.ok) throw new Error(String(initBody.error || "Unable to initialize proof upload"));
      setMessage("Uploading proof directly to secure storage…");
      ...
      setProgress(100); setLoaded(file.size); setState("done"); setMessage("Payment proof attached.");
  ...
  return <form onSubmit={submit} className="space-y-3">
    <Field label="Attach payment proof">
      <Input type="file" name="file" required accept="image/*,.pdf" />
    </Field>
    <div className="flex items-center gap-3">
      <Button type="submit" variant="outline" disabled={busy}>{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <UploadCloud className="mr-2 h-4 w-4" />}Upload proof</Button>
      <span className="text-xs text-muted2">Image or PDF · stored in JUN Drive</span>
    </div>
    {state !== "idle" ? <div className="rounded-lg border border-line bg-surface p-3">
      <div className="mb-2 flex items-center justify-between gap-3 text-xs"><span className={`flex items-center gap-2 ${state === "error" ? "text-red-700" : "text-ink"}`}>{state === "done" ? <CheckCircle2 className="h-4 w-4 text-emerald-600" /> : state === "error" ? <XCircle className="h-4 w-4" /> : <Loader2 className="h-4 w-4 animate-spin text-electric" />}{message}</span><strong>{progress}%</strong></div>
      <div className="h-2 overflow-hidden rounded-full bg-white"><div className={`h-full rounded-full ${state === "error" ? "bg-red-500" : "bg-electric"}`} style={{ width: `${progress}%` }} /></div>
      <div className="mt-2 text-[11px] text-muted2">{human(loaded)} / {human(total)}</div>
    </div> : null}
  </form>;
}
