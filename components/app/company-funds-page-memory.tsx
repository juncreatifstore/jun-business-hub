...
export function CompanyFundsPageMemory({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (previous !== pathname) {
      try {
        sessionStorage.setItem(`${STORAGE_PREFIX}${previous}`, String(window.scrollY));
      } catch {}
  ...
  useEffect(() => {
    const save = () => {
      ...
      try {
        sessionStorage.setItem(`${STORAGE_PREFIX}${previousPath.current}`, String(window.scrollY));
      } catch {}
    };
  ...
  return (
    <div className={cn("transition-opacity duration-150", visible ? "opacity-100" : "opacity-70")}>
      {children}
    </div>
  );
}
