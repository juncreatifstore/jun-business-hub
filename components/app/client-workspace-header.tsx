...
function activeTab(pathname: string, clientId: string) {
  ...
  const match = tabs.find(
    (tab) => tab.path && (pathname === `${base}/${tab.path}` || pathname.startsWith(`${base}/${tab.path}/`)),
  );
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div className="flex min-w-0 items-start gap-4">
            <div className="relative shrink-0">
              ...
              <span
                className={cn(
                  "absolute -bottom-1 -right-1 h-4 w-4 rounded-full border-[3px] border-[#0f1929]",
          ...
          <div className="flex flex-wrap items-center gap-2 xl:justify-end">
            ...
            {paymentsAllowed ? (
              ...
              >
                <WalletCards className="h-4 w-4" /> Paiement
              </Link>
            ) : null}
            ...
            <div className="relative">
              <button
                ...
                className={cn(buttonVariants({ variant: "outline" }), "h-10 rounded-xl px-3")}
              ...
              {moreOpen ? (
                <>
                  ...
                  <div className="absolute right-0 z-50 mt-2 w-64 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#111b2b]/98 py-1.5 shadow-2xl shadow-black/50 backdrop-blur-xl">
                    ...
                    />
                    <Action
                      href={`${base}/relationship`}
                      icon={ShieldAlert}
                      label="Gestion de la relation"
                      onClick={() => setMoreOpen(false)}
                    />
                    {canUpdate ? (
                      <>
