    ...
    <form action={action} className="grid w-full min-w-0 max-w-3xl gap-4 sm:grid-cols-2 sm:gap-5">
      ...
      <div className="min-w-0">
        <Field
          ...
          hint="Choisissez un paiement précis seulement si le remboursement doit être rapproché de ce paiement."
        >
          <Select name="paymentId" value={paymentId} onChange={(e) => setPaymentId(e.target.value)}>
            <option value="">Solde global du client — aucun paiement lié</option>
            {clientPayments.map((p) => (
              <option key={p.id} value={p.id}>
                {p.reference} — {p.currency} {p.available.toFixed(2)} disponible
              </option>
            ))}
          </Select>
        </Field>
      ...
      <div className="min-w-0">
        <Field
          ...
          hint={
            selectedPayment
              ? `Maximum disponible sur ce paiement : ${selectedPayment.currency} ${selectedPayment.available.toFixed(2)}`
              : "Validé contre le solde global disponible du client"
          }
        >
          <Input
            name="amount"
      ...
      <div className="min-w-0">
        <Field
          ...
          hint={
            selectedPayment
              ? "Verrouillée sur la devise du paiement original"
              : "Devise du solde global remboursé"
          }
        >
          <Input
            name="currency"
            value={selectedPayment?.currency ?? undefined}
            defaultValue={selectedPayment ? undefined : "USD"}
      ...
      <div className="min-w-0">
        <Field label="Versements" hint="Jusqu’à 24 décaissements programmés">
          <Input
            name="installments"
            type="number"
            inputMode="numeric"
      ...
      <div className="min-w-0">
        <Field label="Première échéance" hint="Les versements suivants sont programmés mensuellement">
