          ...
          <div className="flex min-w-0 items-start gap-3 sm:gap-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex min-w-0 flex-wrap items-center gap-1.5 sm:gap-2">
                ...
                <Badge className="max-w-full truncate border border-white/[0.07] bg-white/[0.03] text-slate-400">{type.replaceAll("_", " ")}</Badge>
              ...
              <div className="mt-2 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1.5 text-[11px] text-slate-500 sm:gap-x-4 sm:gap-y-2 sm:text-xs">
                ...
                {client ? <Link href={`/app/clients/${client.id}/dashboard`} className="inline-flex min-w-0 items-center gap-1.5 transition hover:text-blue-300"><UserRound className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{client.name}</span></Link> : null}
                {caseInfo ? <Link href={`/app/cases/${caseInfo.id}/dashboard`} className="inline-flex min-w-0 items-center gap-1.5 transition hover:text-blue-300"><FolderKanban className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{caseInfo.number}</span></Link> : null}
                {finalizedAt ? <span className="max-w-full break-words">{sealed ? "Scellé" : "Mis à jour"} · {finalizedAt}</span> : null}
              </div>

              {(contentHash || pdfHash) ? <div className="mt-3 flex min-w-0 flex-wrap gap-1.5 sm:gap-2">
                {contentHash ? <span className="inline-flex max-w-full items-center gap-1.5 rounded-lg border border-emerald-400/10 bg-emerald-500/[0.05] px-2 py-1.5 text-[9px] text-emerald-300 sm:px-2.5 sm:text-[10px]"><Hash className="h-3 w-3 shrink-0" /><span className="truncate">Contenu {contentHash}</span></span> : null}
                {pdfHash ? <span className="inline-flex max-w-full items-center gap-1.5 rounded-lg border border-emerald-400/10 bg-emerald-500/[0.05] px-2 py-1.5 text-[9px] text-emerald-300 sm:px-2.5 sm:text-[10px]"><ShieldCheck className="h-3 w-3 shrink-0" /><span className="truncate">PDF {pdfHash}</span></span> : null}
              </div> : null}
            </div>
          </div>
