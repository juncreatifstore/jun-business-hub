      ...
      <aside className="absolute inset-y-0 right-0 w-full max-w-xl overflow-y-auto border-l border-line bg-surface p-5 text-ink shadow-2xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-lg font-semibold">
              <FolderOpen className="h-5 w-5 text-electric" /> Manage folder
            </h2>
            <p className="mt-1 text-xs text-muted2">Folder lifecycle, location and inherited team sharing.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <section className={sectionClass}>
          ...
          <form action={renameDriveFolder.bind(null, folder.id)} className="flex gap-2">
            ...
            />
            <button className="rounded-lg border border-line bg-white px-3 text-sm hover:bg-surface">
              Rename
            </button>
          </form>
        </section>
        ...
        {!folder.trashed ? (
          <section className={sectionClass}>
            <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold">
              <Move className="h-4 w-4" /> Move folder
            </h3>
        ...
        {!folder.trashed ? (
          <section className={sectionClass}>
            <h3 className="mb-1 flex items-center gap-2 text-sm font-semibold">
              <Share2 className="h-4 w-4" /> Team sharing
            </h3>
            <p className="mb-3 text-xs text-muted2">
              Access is inherited by every subfolder and file inside this folder.
            </p>
            <form action={shareDriveFolder.bind(null, folder.id)} className="flex gap-2">
              ...
              <button className="rounded-lg bg-electric px-3 text-sm font-medium text-white hover:opacity-90">
                Share
              </button>
            </form>
            <div className="mt-4 space-y-2">
              {folder.sharedWith.length ? (
                folder.sharedWith.map((s) => (
                  ...
                  >
                    <span>{s.label}</span>
                    <form action={unshareDriveFolder.bind(null, folder.id, s.userId)}>
                      <input type="hidden" name="returnTo" value={returnTo} />
                      <button className="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs text-red-700 hover:bg-red-50">
                ...
                <p className="text-xs text-muted2">Not shared directly with anyone yet.</p>
              )}
            </div>
          </section>
        ) : null}
        ...
        {canDelete ? (
          <section className="rounded-xl border border-red-200 bg-white p-4">
            <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold text-red-700">
              <Trash2 className="h-4 w-4" /> Folder lifecycle
            </h3>
            {!folder.trashed ? (
              <form action={trashDriveFolder.bind(null, folder.id)}>
                <input type="hidden" name="returnTo" value={returnTo} />
                <button className="inline-flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 hover:bg-red-100">
                  <Trash2 className="h-4 w-4" /> Move folder to Trash
                </button>
              </form>
            ) : (
              <div className="flex flex-wrap gap-2">
                <form action={restoreDriveFolder.bind(null, folder.id)}>
                  <input type="hidden" name="returnTo" value={returnTo} />
                  <button className="inline-flex items-center gap-2 rounded-lg border border-line bg-white px-3 py-2 text-sm hover:bg-surface">
                    <RotateCcw className="h-4 w-4" /> Restore
                  </button>
                </form>
                <form
                  action={permanentlyDeleteDriveFolder.bind(null, folder.id)}
                  onSubmit={(e) => {
                    if (
                      !window.confirm(
                        "Permanently delete this folder, all subfolders and all files inside it? This cannot be undone.",
                      )
                    )
                      e.preventDefault();
                  }}
                >
                  <input type="hidden" name="returnTo" value={returnTo} />
                  <button className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-3 py-2 text-sm font-medium text-white hover:bg-red-700">
            ...
            <p className="mt-3 text-xs text-muted2">
              Trash applies to the whole subtree. Permanent deletion removes every contained file and previous
              stored version.
            </p>
          </section>
        ) : null}
