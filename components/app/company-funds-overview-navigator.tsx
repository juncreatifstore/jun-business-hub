...
function resolveSectionCard(heading: HTMLHeadingElement | null) {
  ...
  const content = Array.from(card.children).find(
    (child) => child instanceof HTMLElement && child.dataset.slot === "card-content",
  );
...
export function CompanyFundsOverviewNavigator() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const headings = Array.from(
        root.querySelectorAll<HTMLHeadingElement>("h3[data-slot='card-title']"),
      ...
      try {
        ...
        if (Array.isArray(saved))
          savedCollapsed = saved.filter(
            (id): id is string => typeof id === "string" && next.some((section) => section.id === id),
          );
      } catch {}
      ...
      if (hashSection) {
        ...
        window.setTimeout(
          () => document.getElementById(hashSection.id)?.scrollIntoView({ behavior: "auto", block: "start" }),
          20,
        );
      } else {
  ...
  useEffect(() => {
    ...
    for (const section of sections) {
      ...
      const children = Array.from(card.children) as HTMLElement[];
      children.slice(1).forEach((child) => {
        child.style.display = collapsedSet.has(section.id) ? "none" : "";
      });
  ...
  const activeLabel = useMemo(
    () => sections.find((section) => section.id === activeId)?.label || "Sur cette page",
    [activeId, sections],
  );
  const focusLabel = useMemo(
    () => sections.find((section) => section.id === focusId)?.label || "",
    [focusId, sections],
  );
  ...
  const filteredSections = useMemo(() => {
    ...
    return sections.filter(
      (section) => section.label.toLowerCase().includes(q) || section.id.toLowerCase().includes(q),
    );
  ...
  function setOneCollapsed(id: string, value: boolean) {
    setCollapsed((previous) =>
      value ? (previous.includes(id) ? previous : [...previous, id]) : previous.filter((item) => item !== id),
    );
  }
  ...
  function collapseAll() {
    setCollapsed(sections.map((section) => section.id));
  }
  function expandAll() {
    setCollapsed([]);
  }
  ...
  return (
    <>
      <div className="sticky top-[116px] z-20 -mt-2 rounded-xl border border-line bg-white/95 shadow-sm backdrop-blur md:top-[126px]">
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted2">
              {focusId ? "Mode focus" : "Sur cette page"}
            </div>
            <div className="truncate text-sm font-semibold text-ink">
              {focusId ? focusLabel : activeLabel}
            </div>
          </div>
          ...
          <button
            ...
            className="hidden h-8 items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 text-xs font-medium text-muted2 transition hover:bg-surface hover:text-ink sm:inline-flex"
            title="Rechercher une section — Cmd/Ctrl + J"
          >
            <Search className="h-3.5 w-3.5" />
            <span className="hidden lg:inline">Rechercher</span>
            <kbd className="rounded border border-line bg-surface px-1.5 py-0.5 text-[10px]">⌘/Ctrl J</kbd>
          </button>

          <button
            type="button"
            onClick={copyActiveLink}
            className="hidden h-8 items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 text-xs font-medium text-muted2 transition hover:bg-surface hover:text-ink md:inline-flex"
            title="Copier le lien direct vers cette section"
          >
          ...
          {focusId ? (
            <button
              type="button"
              onClick={exitFocus}
              className="hidden h-8 items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 text-xs font-medium text-emerald-800 transition hover:bg-emerald-100 sm:inline-flex"
            ...
            <button
              ...
              onClick={() => enterFocus()}
              className="hidden h-8 items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 text-xs font-medium text-muted2 transition hover:bg-surface hover:text-ink sm:inline-flex"
              title="Afficher uniquement cette section"
            >
          ...
          <button
            type="button"
            onClick={() => setOneCollapsed(activeId, !activeCollapsed)}
            className="hidden h-8 items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 text-xs font-medium text-muted2 transition hover:bg-surface hover:text-ink xl:inline-flex"
            title={activeCollapsed ? "Développer cette section" : "Réduire cette section"}
          >
            {activeCollapsed ? (
          ...
          <button
            ...
            className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-line bg-white px-2.5 text-xs font-medium text-muted2 transition hover:bg-surface hover:text-ink"
            aria-expanded={open}
          >
        ...
        {focusId ? (
          <div className="border-t border-emerald-100 bg-emerald-50/60 px-3 py-2 text-[11px] text-emerald-800 sm:hidden">
            <div className="flex items-center justify-between gap-3">
              <span>Seule la section « {focusLabel} » est affichée.</span>
              <button
                type="button"
                onClick={exitFocus}
                className="shrink-0 rounded-lg border border-emerald-200 bg-white px-2 py-1 font-semibold"
              >
                Tout afficher
              </button>
            </div>
        ...
        <div className="hidden overflow-x-auto border-t border-line px-2 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:block">
          <div className="flex min-w-max items-center gap-1">
            ...
            <button
              type="button"
              onClick={() => setFinderOpen(true)}
              className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[11px] font-medium text-muted2 hover:bg-surface hover:text-ink"
            >
            ...
            {focusId ? (
              <button
                type="button"
                onClick={exitFocus}
                className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-[11px] font-medium text-emerald-800 hover:bg-emerald-100"
              >
          ...
          <div className="border-t border-line p-2 md:hidden">
            <div className="mb-2 grid grid-cols-2 gap-2">
              <button
                ...
                className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg border border-line bg-white text-xs font-medium text-muted2"
              >
                <Search className="h-3.5 w-3.5" />
                Rechercher
              </button>
              {focusId ? (
                <button
                  type="button"
                  onClick={exitFocus}
                  className="inline-flex h-8 items-center justify-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 text-xs font-medium text-emerald-800"
                >
                  <LayoutList className="h-3.5 w-3.5" />
                  Tout afficher
                </button>
            ...
            <div className="max-h-[42vh] overflow-y-auto rounded-lg bg-surface/50 p-1">
            ...
            <button
              type="button"
              onClick={collapseAll}
              className="mt-2 inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-line bg-white text-xs font-medium text-muted2"
            >
              <PanelTopClose className="h-3.5 w-3.5" />
              Tout réduire
            </button>
      ...
      {finderOpen ? (
        <div
          className="fixed inset-0 z-[95] flex items-start justify-center bg-black/30 px-4 pt-[14vh] backdrop-blur-sm"
          ...
          >
            <div className="flex items-center gap-3 border-b border-line px-4 py-3">
              ...
              <input
                ...
                placeholder="Rechercher une section de la Vue générale…"
            ...
            <div className="max-h-[55vh] overflow-y-auto p-2">
              {filteredSections.map((section, index) => {
                ...
                return (
                  <button
                    key={section.id}
                    type="button"
                    onMouseEnter={() => setFinderIndex(index)}
                    onClick={() => go(section.id)}
              ...
              {!filteredSections.length ? (
                <div className="px-4 py-8 text-center text-sm text-muted2">Aucune section trouvée.</div>
              ) : null}
            </div>
            <div className="flex flex-wrap items-center justify-between gap-2 border-t border-line bg-surface/50 px-4 py-2 text-[10px] text-muted2">
              <span className="inline-flex items-center gap-2">
                ...
                <ArrowDown className="h-3 w-3" /> naviguer · Entrée ouvrir
              </span>
