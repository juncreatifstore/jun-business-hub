...
export function Sidebar({
  ...
  return (
    <>
      {open ? <div className="fixed inset-0 z-40 bg-slate-950/75 backdrop-blur-sm lg:hidden" onClick={onClose} /> : null}
      <aside
        className={cn(
      ...
      >
        <div className={cn("flex h-[72px] items-center border-b border-white/[0.06] px-4", collapsed ? "lg:justify-center lg:px-3" : "justify-between")}>
          <Link href="/app" className={cn("group flex min-w-0 items-center gap-3", collapsed && "lg:justify-center")} onClick={onClose} title="JUN Business Hub">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 text-lg font-bold shadow-lg shadow-blue-950/45 ring-1 ring-white/10">J</span>
          ...
          <button className="rounded-lg p-2 text-slate-400 hover:bg-white/[0.06] hover:text-white lg:hidden" onClick={onClose} aria-label="Fermer le menu">
            <X className="h-5 w-5" />
          </button>
        </div>

        <nav className={cn("flex-1 overflow-y-auto py-4 [scrollbar-width:thin] [scrollbar-color:#243044_transparent]", collapsed ? "lg:px-2" : "px-3")}>
          {sections.map((section, i) => {
            ...
            return (
              <div key={i} className="mt-5 first:mt-0">
                {section.label ? (
                  <p className={cn("px-3 pb-1.5 text-[9px] font-semibold uppercase tracking-[0.22em] text-slate-600", collapsed && "lg:hidden")}>{section.label}</p>
                ) : null}
                {collapsed && section.label ? <div className="mx-auto mb-2 hidden h-px w-7 bg-white/[0.06] lg:block" /> : null}
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition", active ? "bg-blue-500/15 text-blue-400" : "text-slate-500 group-hover:text-slate-300")}>
                          <item.icon className="h-[17px] w-[17px]" />
                        </span>
                        <span className={cn("min-w-0 flex-1", collapsed && "lg:hidden")}>
                          <span className="block truncate">{item.label}</span>
                          {isCompanyFundsRoot && !insideCompanyFunds && lastCompanyFundsHref !== "/app/company-funds" ? (
                            <span className="mt-0.5 block truncate text-[9px] font-normal text-slate-600">Reprendre · {companyFundsLabels[lastCompanyFundsHref] || "dernière section"}</span>
                          ) : null}
                        </span>
                      </Link>
                    );
        ...
        <div className="border-t border-white/[0.06] p-3">
          <button
            ...
            className={cn("mb-2 hidden w-full items-center rounded-xl border border-white/[0.06] bg-white/[0.025] text-slate-400 transition hover:bg-white/[0.055] hover:text-white lg:flex", collapsed ? "justify-center p-2.5" : "gap-3 px-3 py-2.5")}
            title={collapsed ? "Déployer la navigation" : "Réduire la navigation"}
          >
            {collapsed ? <PanelLeftOpen className="h-4 w-4" /> : <><PanelLeftClose className="h-4 w-4" /><span className="text-xs font-medium">Réduire le menu</span></>}
          </button>

          <div className={cn("flex items-center rounded-xl border border-white/[0.06] bg-white/[0.025]", collapsed ? "lg:justify-center lg:p-2.5" : "gap-3 p-3")} title="JUN Créatif & Travel">
