  ...
  return (
    <form action={action} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]">
      <div className="space-y-5">
        <section className="grid gap-4 rounded-xl border border-line bg-white p-5 sm:grid-cols-2">
          ...
          <Field label="Document type">
            <Select name="type" defaultValue={initial?.type ?? "CUSTOM"}>
              {TYPES.map((t) => (
          ...
          <Field label="Language">
            <Select name="language" defaultValue={initial?.language ?? "FR"}>
              <option>FR</option>
              <option>EN</option>
              <option>ES</option>
              <option>HT</option>
          ...
          <div className="sm:col-span-2">
            <Field label="Category">
              <Select name="category" defaultValue={initial?.category ?? "GENERAL"}>
          ...
          <label className="sm:col-span-2 flex items-center gap-3 rounded-lg border border-line bg-surface p-3 text-sm">
            <input type="checkbox" name="isActive" defaultChecked={initial?.isActive ?? false} />
            <span>
              <strong>Active template</strong>
              <br />
              <span className="text-xs text-muted2">Active templates appear in New document.</span>
            </span>
          </label>
        </section>
        ...
        <section className="rounded-xl border border-line bg-white p-5">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="font-semibold">Template content</p>
              <p className="text-xs text-muted2">Use clean HTML and insert variables with double braces.</p>
            </div>
          ...
          {preview ? (
            <div
              className="doc-prose min-h-[420px] rounded-xl border border-line bg-white p-6"
      ...
      <aside className="space-y-4 xl:sticky xl:top-6 xl:self-start">
        <section className="rounded-xl border border-electric/30 bg-electric/5 p-4">
          <p className="font-semibold">Dynamic variables</p>
          <p className="mt-1 text-xs text-muted2">
            Click to append a variable token. Automatic variables are filled from the selected client, case or
            current date.
          </p>
          <div className="mt-3 max-h-[360px] space-y-1 overflow-auto">
            {BUILTIN_TEMPLATE_VARIABLES.map((v) => (
              <button
                type="button"
                key={v.key}
              ...
              >
                <span>
                  ...
                  <br />
                  <code className="text-[10px] text-muted2">{`{{${v.key}}}`}</code>
                </span>
                <PlusCircle className="h-4 w-4 text-electric" />
              </button>
            ))}
          </div>
        </section>
        <section className="rounded-xl border border-line bg-white p-4">
          <p className="font-semibold">Detected variables</p>
          {variables.length ? (
            <ul className="mt-3 space-y-2">
              {variables.map((v) => (
                <li key={v.key} className="rounded-lg bg-surface p-2 text-xs">
                  <code>{`{{${v.key}}}`}</code>
                  <p className="mt-1 text-muted2">
                    {v.automatic ? "Automatic" : v.required ? "Required input" : "Optional input"}
                  </p>
                </li>
              ))}
            </ul>
          ) : (
            <p className="mt-2 text-xs text-muted2">No variables detected.</p>
          )}
        </section>
      </aside>
    </form>
