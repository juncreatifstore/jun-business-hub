...
export function DriveCollaborationPanel({
  resourceType,
  resourceId,
  returnTo,
}: {
    ...
    <section className={box}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted2">Internal comments, mentions and approval requests.</p>
        </div>
        <button
          type="button"
          onClick={() => window.location.reload()}
          className="rounded-md p-2 text-muted2 hover:bg-surface"
          title="Refresh"
        >
      ...
      <form action={addDriveComment.bind(null, resourceType, resourceId)} className="space-y-2">
        ...
        {others.length ? (
          <div>
            <div className="mb-1 flex items-center gap-1 text-[11px] text-muted2">
              <AtSign className="h-3.5 w-3.5" /> Mention team members
            </div>
            <div className="flex flex-wrap gap-1.5">
              {others.map((u) => {
                const active = mentionIds.includes(u.id);
                return (
                  <button
                    key={u.id}
                    type="button"
                    onClick={() =>
                      setMentionIds((prev) => (active ? prev.filter((id) => id !== u.id) : [...prev, u.id]))
                    }
                    className={`rounded-full border px-2 py-1 text-[11px] ${active ? "border-blue-300 bg-blue-50 text-blue-800" : "border-line bg-white text-muted2"}`}
                  >
                    @{u.firstName}
                  </button>
                );
        ...
        <button className="rounded-lg bg-electric px-3 py-2 text-sm font-medium text-white">
          Add comment
        </button>
      </form>
          ...
          data.comments.map((c) => (
            <div key={c.id} className="rounded-lg border border-line bg-surface p-3">
              <div className="flex items-center justify-between gap-2 text-xs">
                ...
                <span className="text-muted2">{new Date(c.createdAt).toLocaleString()}</span>
              </div>
              <p className="mt-2 whitespace-pre-wrap text-sm text-ink">{c.body}</p>
              {c.mentionedUsers.length ? (
                <div className="mt-2 text-[11px] text-blue-700">
                  Mentions: {c.mentionedUsers.map((u) => `@${u.firstName}`).join(", ")}
                </div>
              ) : null}
            </div>
          ))
        ) : (
          <p className="text-xs text-muted2">No comments yet.</p>
        )}
      </div>

      <div className="my-4 border-t border-line" />
      <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold">
        <UserCheck className="h-4 w-4" /> Request approval
      </h4>
      <form action={requestDriveApproval.bind(null, resourceType, resourceId)} className="space-y-2">
        <input type="hidden" name="returnTo" value={returnTo} />
        <select name="reviewerId" required defaultValue="" className={`h-10 w-full px-3 text-sm ${input}`}>
        ...
        <textarea
          ...
          className={`w-full p-3 text-sm ${input}`}
        />
        <button className="rounded-lg border border-line bg-white px-3 py-2 text-sm hover:bg-surface">
          Send for approval
        </button>
      </form>
      ...
      <div className="mt-4 space-y-2">
        {data?.approvals.length
          ? data.approvals.map((a) => (
              <div key={a.id} className="rounded-lg border border-line bg-surface p-3 text-xs">
                  ...
                  <span className="font-medium text-ink">
                    {a.status === "APPROVED" ? (
                      ...
                      <span className="inline-flex items-center gap-1 text-amber-700">
                        <Clock3 className="h-3.5 w-3.5" /> {a.status.replace(/_/g, " ")}
                      </span>
                    )}
                  </span>
                  <span className="text-muted2">{new Date(a.createdAt).toLocaleString()}</span>
                </div>
                {a.message ? <p className="mt-2 text-muted2">{a.message}</p> : null}
                {a.reviewNote ? (
                  <p className="mt-2 rounded-md bg-white p-2 text-ink">Review: {a.reviewNote}</p>
                ) : null}
              </div>
            ))
          : null}
