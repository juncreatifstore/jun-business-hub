...
function Menu({
  ...
  return (
    <details className="group relative">
      ...
      <div
        className={`absolute left-0 top-full z-50 mt-1 max-h-[440px] overflow-auto rounded-lg border border-slate-700 bg-slate-950 p-1.5 shadow-2xl ${wide ? "w-[520px]" : "w-56"}`}
      >
        {children}
      </div>
    </details>
...
export function DocumentEditor({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (draftContent && draftContent !== normalizedInitial)
        setRecoveredDraft({ ...draft, content: draftContent });
    } catch {}
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      const draft: LocalDraft = {
        content: html,
        baseContent: lastServerContent.current,
  ...
  const basicFields = [
    ["Text", "Add text fillable field"],
    ["Number", "Add number fillable field"],
    ["Date", "Add date fillable field"],
    ["Dropdown", "Add dropdown fillable field"],
    ["Checkbox", "Add checkbox fillable field"],
    ["Signature", "Add signature fillable field"],
    ["Initials", "Add initials fillable field"],
    ["Image", "Add image fillable field"],
    ["Radio", "Add radio fillable field"],
    ["Formula", "Add formula fillable field"],
  ] as const;
  const smartFields = [
    ["Name", "Add Name smart field"],
    ["Email", "Add Email smart field"],
    ["Company", "Add Company smart field"],
    ["Title", "Add Title smart field"],
    ["US Phone", "Add US Phone smart field"],
    ["ZIP", "Add ZIP smart field"],
    ["USD", "Add USD smart field"],
    ["EUR", "Add EUR smart field"],
    ["Age", "Add Age smart field"],
    ["SSN", "Add SSN smart field"],
    ["EIN", "Add EIN smart field"],
    ["Card", "Add Card smart field"],
    ["US State", "Add US State smart field"],
    ["Gender", "Add Gender smart field"],
  ] as const;
    ...
    <form action={save}>
      ...
      {!readOnly && recoveredDraft ? (
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm">
          <div>
            <p className="font-medium text-amber-900">Unsaved draft recovered</p>
            <p className="text-xs text-amber-700">
              Autosaved {new Date(recoveredDraft.savedAt).toLocaleString()}.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <Button type="button" variant="primary" onClick={restoreLocalDraft}>
            ...
            <div className="flex flex-wrap items-center gap-0.5">
              ...
              <Menu label="Insert" icon={<Plus className="h-4 w-4" />} wide>
                <div className="grid grid-cols-2 gap-x-1">
                  <div>
                    ...
                    {basicFields.map(([label, title]) => (
                      <MenuItem key={title} label={label} onClick={() => clickLegacyTool(title)} />
                    ))}
                  </div>
                  <div>
                    <SectionLabel>Smart fields</SectionLabel>
                    {smartFields.map(([label, title]) => (
                      <MenuItem key={title} label={label} onClick={() => clickLegacyTool(title)} />
                    ))}
                  </div>
                </div>
              ...
              <Menu label="Variables">
                <MenuItem
                  ...
                  hint="Name, address, phone, EIN and other Settings values"
                  onClick={() => clickLegacyTool("Insert company information from Settings")}
                />
              </Menu>
              ...
              <Menu label="More" icon={<MoreHorizontal className="h-4 w-4" />}>
                ...
                <MenuItem
                  ...
                  onClick={() => editor.chain().focus().toggleJunHighlight().run()}
                />
                <MenuItem
                  label="Redact / blackout"
                  icon={<EyeOff className="h-4 w-4" />}
                  onClick={() => editor.chain().focus().toggleJunBlackout().run()}
                />
                <MenuItem
                  label="Blockquote"
                ...
                <MenuItem
                  label="Field order"
                  hint="Reorder form fields and tab sequence"
                  onClick={() => clickLegacyTool("Reorder Wizard / Tab sequence")}
                />
                <MenuItem
      ...
      {!readOnly ? (
        <div className="mt-4 flex flex-wrap items-end gap-3">
          <div className="min-w-64 flex-1">
            <Input
              name="changeNote"
              placeholder="Change note (what changed in this version?)"
              maxLength={300}
            />
            <p className="mt-1 text-xs text-muted2">
              {dirty && lastDraftAt
                ? `Local autosave: ${lastDraftAt.toLocaleTimeString()}`
                : "Official history changes only when you save a new version."}
            </p>
          </div>
