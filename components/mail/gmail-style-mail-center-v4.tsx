...
export async function GmailStyleMailCenterV4({searchParams}:{searchParams:Params}){
 ...
 if(activeThread&&activeState){return <div className="overflow-hidden rounded-2xl border border-line bg-white">
  ...
  <div className="px-5 py-5 md:px-8"><h1 className="mb-6 text-xl font-medium text-ink md:text-2xl">{activeThread.subject||"(no subject)"}</h1>
   {conversation.length?<div className="space-y-6">{conversation.map((m,i)=><article key={m.id} className="overflow-hidden rounded-xl border border-line bg-white">
    <div className="flex items-start justify-between gap-4 border-b border-line px-4 py-4"><div><p className="font-semibold text-ink">{senderLabel(m.from)} <span className="font-normal text-muted2">&lt;{senderEmail(m.from)}&gt;</span></p><p className="mt-1 text-xs text-muted2">to {m.to.join(", ")||"me"}{m.cc.length?` · cc ${m.cc.join(", ")}`:""}</p></div><span className="shrink-0 text-xs text-muted2">{m.date.toLocaleString()}</span></div>
    <div className="bg-white p-3 md:p-5">{m.htmlBody?<EmailHtmlFrame html={m.htmlBody} title={m.subject}/>:<div className="mx-auto max-w-5xl whitespace-pre-wrap break-words text-[15px] leading-7 text-ink">{m.body||m.snippet||"No readable body."}</div>}</div>
    {m.attachments.length?<div className="flex flex-wrap gap-2 border-t border-line px-4 py-3">{m.attachments.map((a,n)=><span key={`${a.filename}-${n}`} className="rounded-lg border border-line px-3 py-2 text-sm">📎 {a.filename}</span>)}</div>:null}
   ...
   </article>)}</div>:<div className="py-12 text-sm text-muted2">Gmail conversation could not be loaded. Run Sync and try again.</div>}
  </div>
 </div>}
 ...
 return <div className="grid min-h-[72vh] gap-0 overflow-hidden rounded-2xl border border-line bg-white lg:grid-cols-[210px_minmax(0,1fr)]">
  ...
  <section className="min-w-0">
   ...
   {threads.length===0?<div className="p-8"><EmptyState icon={Mail} title="No conversations" description="Try another folder or run Sync."/></div>:<div className="divide-y divide-line">{threads.map(t=>{const s=stateMap.get(t.id)!,date=t.lastMessageAt??t.updatedAt;return <div key={t.id} className={`grid min-h-11 grid-cols-[30px_minmax(130px,190px)_minmax(0,1fr)_88px] items-center gap-2 px-3 text-sm hover:bg-slate-50 ${s.isRead?"bg-[#f6f8fc]":"bg-white font-semibold"}`}>
    <form action={toggleMailThreadStar.bind(null,t.id,folder)}><button type="submit" aria-label={s.starred?"Unstar":"Star"} className="flex h-7 w-7 items-center justify-center"><Star className={`h-4 w-4 ${s.starred?"fill-amber-400 text-amber-500":"text-slate-400"}`}/></button></form>
