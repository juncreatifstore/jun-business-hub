...
export async function GmailStyleMailCenterV6({ searchParams }: { searchParams: Params }) {
  ...
  const conversation =
    activeThread && !activeThread.aiDraft
      ...
      : [];
  const mailboxLabel =
    mailbox === "ALL" ? "All mailboxes" : (accounts.find((a) => a.id === mailbox)?.email ?? "Mail");
    ...
    return (
      <div className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#0d1726] text-slate-100">
        <div className="flex min-h-14 flex-wrap items-center gap-2 border-b border-white/[0.07] px-3">
          ...
          {activeState.isRead ? (
            <form action={markMailThreadUnread.bind(null, activeThread.id, folder)}>
              <Button size="sm" variant="outline">
                Mark unread
              </Button>
            </form>
          ) : (
            <form action={markMailThreadRead.bind(null, activeThread.id, folder)}>
              <Button size="sm" variant="outline">
                Mark read
              </Button>
            </form>
          ...
          <form action={toggleMailThreadStar.bind(null, activeThread.id, folder)}>
            <Button size="sm" variant="outline">
              {activeState.starred ? "Unstar" : "Star"}
            </Button>
          </form>
          {!activeState.archived && !activeState.trashed ? (
            <form action={archiveMailThread.bind(null, activeThread.id)}>
              <Button size="sm" variant="outline">
                Archive
              </Button>
            </form>
          ...
          {!activeState.trashed ? (
            <form action={trashMailThread.bind(null, activeThread.id)}>
              <Button size="sm" variant="outline">
                Trash
              </Button>
            </form>
          ) : (
            <form action={restoreMailThread.bind(null, activeThread.id, "INBOX")}>
              <Button size="sm" variant="outline">
                Restore
              </Button>
            </form>
          ...
          {!activeState.trashed ? (
            <form action={snoozeMailThread.bind(null, activeThread.id)} className="flex gap-1">
                ...
                >
                  ...
                  <div className="bg-white p-2 text-slate-900 md:p-4">
                    {m.htmlBody ? (
                      ...
                      <div className="mx-auto max-w-5xl whitespace-pre-wrap break-words text-[15px] leading-7">
                        {m.body || m.snippet || "No readable body."}
                      </div>
                    )}
                  </div>
                  {i === conversation.length - 1 ? (
                    <div className="flex flex-wrap gap-2 border-t border-white/[0.07] px-4 py-3">
                      ...
                      <Link
                        href={`/app/mail/compose?mailbox=${activeThread.mailAccountId}&source=${activeThread.id}&mode=REPLY_ALL`}
                      >
                        <Button variant="outline">Reply all</Button>
                      </Link>
                      <Link
                        href={`/app/mail/compose?mailbox=${activeThread.mailAccountId}&source=${activeThread.id}&mode=FORWARD`}
                      >
        ...
        <div className="flex min-h-14 flex-wrap items-center gap-2 border-b border-white/[0.07] px-3">
          {accounts.length ? (
            mailbox === "ALL" ? (
              <form action={syncAllMailboxesV2}>
                <Button size="sm" variant="outline">
                  <RefreshCw className="h-4 w-4" /> Sync 20
                </Button>
              </form>
            ) : (
              <form action={syncMailboxV2.bind(null, mailbox)}>
                <Button size="sm" variant="outline">
                  <RefreshCw className="h-4 w-4" /> Sync 20
                </Button>
              </form>
          ...
          {accounts.length > 1 ? (
            <form className="flex gap-2">
              <Select name="mailbox" defaultValue={mailbox} className="h-9 w-44">
            ...
            threads.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <Link href={qp({ thread: t.id })} className="truncate">
