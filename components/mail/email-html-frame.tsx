...
export function EmailHtmlFrame({html,title}:{html:string;title:string}){
 ...
 useEffect(()=>{
  ...
  const onLoad=()=>{resize();try{const doc=frame.contentDocument;if(doc?.documentElement){observer=new ResizeObserver(resize);observer.observe(doc.documentElement);}doc?.querySelectorAll("img").forEach(img=>img.addEventListener("load",resize));}catch{}};
 ...
 return <iframe ref={ref} title={title} srcDoc={document} sandbox="allow-same-origin allow-popups allow-popups-to-escape-sandbox" className="block w-full border-0 bg-white" style={{height}}/>;
}
